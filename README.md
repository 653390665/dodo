...
| 验证日期 | 当前能力 | 当前限制 |
|----------|----------|----------|
| 2026-09-15 | 本地作品、设定和章节编辑；受治理的设定/大纲/正文候选（消毒候选以副本单源供给，原貌不直达渲染）；SQLite/WAL 原生 `backup()` 一致性快照；AI 失败恢复与本地降级写作；全失败场景模型草稿以待改进预览交付；Beta 默认开放增强能力 | AI 功能依赖用户配置外部 API Key 和可用网络；当前没有真实支付、订单或订阅系统；签名、真实 Provider 和其他平台需发布环境复核 |
...
- 商业化实验要求前后端开关同时显式启用。完整边界见 [权益与商业化边界](docs/archive/monetization-boundary.md)。
...
- **键盘快捷键**：编辑器支持 `Cmd/Ctrl+Z` 撤销 / `Cmd/Ctrl+Shift+Z` 重做，`Cmd/Ctrl+1~5` 快速切换视图，`Cmd/Ctrl+K` 打开全局语义搜索（按相似度跳转章节），进入无打扰沉浸式码字。
- **自动保存**：编辑器内容 1 秒防抖自动保存，关闭前未保存提醒。
...
# 环境要求 Node.js 22+
npm install
cp .env.example .env.local   # 编辑填入 API Key；全部运行时环境变量（INKFLOW_* 等）及覆盖顺序见 .env.example 注释
...
`npm install` 会通过 `prepare` 脚本自动安装 pre-commit 钩子（typecheck + 暂存 TS 文件 lint）。本机 npm 损坏时验证门用 node 直调（见 `scripts/pre-commit.sh`）。

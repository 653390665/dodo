...
| 3 production props 链拆除 | ✅（9a4af0d 后续批）：ProductionTab/Panel/AgentWorkspace 改订阅 store，EditorView 删 10+1 透传；AgentWorkspace props 97→81（批次起点实测 97；109 为计划编写时的历史审计口径；第三轮再拆 reviewIssues/expectedWordCount）；验收门 grep `productionError\|activeProductionRun` 透传 = 0 |

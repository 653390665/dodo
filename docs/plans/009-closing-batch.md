...
| B4 | EditorView props 87→≤60 | ◐ 进行中：现 82（S4 后又拆 reviewIssues/expectedWordCount）；剩余大头为 outline 内容域与 continuation 选包域（per-novel hook，需键控设计） | 005 完成标准 |

### C. 决策项（2026-09-07：推荐项已设为工作默认，标注"默认"，随时可改判）
| 项 | 默认处置 | 改判条件 |
|---|---|---|
| react-window 虚拟化 | ⏸ 默认暂缓 | 冒烟/日常使用出现卡顿实证 |
| Google/OpenAI 超时重试统一 | ⏸ 默认暂缓 | 出现实际失败案例驱动 |
| 草稿机整体退役 | ⏸ 默认保留现状 | 产品确定三个暂存流的长期形态 |
| 驾驶舱与工作台合并 | ⏸ 默认维持现状 | 埋点显示驾驶舱→编辑器转化异常 |

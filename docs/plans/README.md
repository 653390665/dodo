...
| 005 | EditorView 领域状态入 store | P1 | 无 | ◐ 大部分完成（2026-09-07 第二轮）：步骤 1-3、5 完成——production 域 props 三层透传已清零（验收门 grep=0），AgentWorkspace props 97→81（第三轮含 reviewIssues 派生下放、expectedWordCount 入 store、licensed 判定收敛），完成章双写收敛为 setCompletionInFlight 唯一同步点；S4 展示态已入 writing-style-store；第三轮另完成 licensed 判定收敛（5 处归一）、expectedWordCount 入 store（含 OutlineTab）、reviewIssues 派生下放；剩余 profiler 验证与 hooks 参数收窄（可后置） |

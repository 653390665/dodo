...
| 103 | 拆分 prompt-governance-catalog.ts 复杂超大模块 | BACKLOG | — |
| 104 | 拆分 EditorView.tsx 抽出局部 UI 元素 | BACKLOG | — |
| 105 | 收敛 as unknown as 类型转换（补充 SQLite Row 类型） | BACKLOG | — |
| 106 | 清理测试配置：删除或说明 vitest.config.ts | BACKLOG | — |
| 107 | 依赖大版本升级安全评估与规划（Express/Vite/TS/Electron） | BACKLOG | — |
| 108 | 前端日志治理：后续统一 error reporting | BACKLOG | — |

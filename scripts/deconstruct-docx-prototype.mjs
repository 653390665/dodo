...
import { buildContinuationPackPrompt, buildContinuationPackParseAttempts } from '../shared/lib/continuation-pack-parse.ts';
...
const PLACES = ['漠北荒原', '青石长街', '旧渡口', '废弃盐矿', '烽燧下', '马厩后院', '冰封河道', '山神庙'];
const OBJECTS = ['半块虎符', '淬毒短刀', '油纸包', '铜哨', '账册', '酒葫芦'];

const SENTENCE_TEMPLATES = [
  (a, b, place, obj) => `${a}沿着${place}走了半里地，靴底碾过碎冰，怀里那${obj}硌得肋骨发疼。`,
  (a, b, place, obj) => `${b}没有回头，只把缰绳往${a}手里一塞：“马留给你，${place}的路自己挑。”`,
  (a, b, place, obj) => `风从${place}的豁口灌进来，${a}把${obj}往袖中又藏深了一寸。`,
  (a, b, place, obj) => `${a}数到第三声，${b}仍旧站在${place}的阴影里，既不应答，也不离开。`,
  (a, b, place, obj) => `“${obj}若是假的，你我现在就该翻脸。”${a}说这话时，眼睛一直盯着${place}的方向。`,
  (a, b, place, obj) => `${b}蹲下身，用刀尖拨开积雪，露出底下一行朝${place}去的马蹄印。`,
  (a, b, place, obj) => `火把熄了又燃。${a}终于开口：“过了今夜，${place}的人都得换个活法。”`,
  (a, b, place, obj) => `${b}的咳嗽声压得很低，像是从${place}的地底下渗出来的。`,
  (a, b, place, obj) => `${a}记着师父的话：越是临近${place}，越要慢；越是要紧的${obj}，越要松着手拿。`,
  (a, b, place, obj) => `马蹄声停在十步之外。${b}掀开斗篷，露出里头驿站官服的滚边。`,
  (a, b, place, obj) => `${a}把${obj}在掌心颠了颠，忽然笑了：“原来${place}的规矩，是拿人命算账。”`,
  (a, b, place, obj) => `三十年来，${place}只认两条规矩：不问来路，不欠旧账。${b}偏要问。`,
  (a, b, place, obj) => `更鼓敲过三巡，${a}吹熄了灯，听${place}外的脚步声由远及近，又由近及远。`,
  (a, b, place, obj) => `${b}递过来半张饼：“先吃。骂人的力气，留到过了${place}再使。”`,
  (a, b, place, obj) => `刀出鞘只有一寸，${a}就停住了——${place}那头，有人先一步吹响了铜哨。`,
  (a, b, place, obj) => `${a}后来才知道，那晚${place}上多出来的三口棺材，口口都钉着官家的封条。`,
  (a, b, place, obj) => `雪停的时候，${b}把${obj}埋进${place}的老槐树下，动作快得像在销一桩旧罪。`,
  (a, b, place, obj) => `“你信人，还是信账？”${a}问。${b}望着${place}：“我信利钱，它最诚实。”`,
  (a, b, place, obj) => `${b}一路沉默，直到${place}的轮廓在暮色里显出来，才说了句：“待会儿见机行事。”`,
  (a, b, place, obj) => `${a}把最后一枚铜钱押在桌角。骰盅未开，${place}的门先被人从外面踹开了。`,
];
...
function splitIntoDeconstructionChunks(text, chunkBudget, maxChunks = MAX_CHUNKS) {
  ...
  const flush = (upTo = current.length) => {
    ...
    if (chunks.length > maxChunks) throw new Error(`DECONSTRUCTION_CHUNK_LIMIT: 超过 ${maxChunks} 块上限`);
  };
  ...
  for (const para of paragraphs) {
    ...
    if (current.length > 0 && packedLen(current) + para.length + 2 > chunkBudget) {
      ...
      for (let i = current.length - 1; i > 0; i--) {
        if (isHeading(current[i])) { headingIdx = i; break; }
      }
...
function assembleContinuationStylePrompt(chunks) {
  ...
  const documentsForPrompt = chunks
    .map((text, i) => `【deconstruction-chunk-${i + 1}.docx】\n${text.slice(0, attempt.maxCharsPerDocument)}\n`)
    .join('\n---\n');
...
function assertChunkIsAcceptable(chunk, label) {
  ...
  assert(bodyParse.success, `${label} 未通过 extractSkillSchema：${JSON.stringify(bodyParse.error?.issues ?? [])}`);
  ...
  assert(segments.every((s) => s.excerpt.length > 0), `${label} 存在空摘录的证据段`);
}
...
async function main() {
  ...
  const chapterCount = paragraphs.filter(isHeading).filter((p) => p.startsWith('第') && p.includes('章')).length;
  console.log(`docx 抽取完成：${extractedText.length} 字符，${chapterCount} 个章节标题`);
  assert(extractedText.length > 60_000, '抽取文本应超过单块预算，否则演示不出切分');
  assert(extractedText.includes('第一卷') && extractedText.includes('第十二章'), '抽取文本应包含卷/章标题');
  assert(fs.readdirSync(os.tmpdir()).filter((d) => d.startsWith('inkflow-deconstruct-proto-')).length === 0, '临时目录应已清理');

  // 2) 形态 A：生产预算 60,000 字 —— 整本 7 万+ 字切为少数大块
  const bigChunks = splitIntoDeconstructionChunks(extractedText, DEFAULT_CHUNK_BUDGET);
  console.log(`形态 A（budget=${DEFAULT_CHUNK_BUDGET}）：切分出 ${bigChunks.length} 块，长度 [${bigChunks.map((c) => c.length).join(', ')}]`);
  ...
  assert(bigChunks.every((c) => HEADING_RE.test(c)), '每块都应至少覆盖一个章节标题（无章节被整块丢失）');
  const chaptersInChunks = bigChunks.reduce((acc, c) => acc + (c.match(/第[一二三四五六七八九十]+章/g) ?? []).length, 0);
  ...
  console.log(`形态 B（budget=${chapterBudget}）：切分出 ${alignedChunks.length} 块（期望每章一块 = ${chapterCount} 块）`);
  ...
  assert(typeof prompt === 'string' && prompt.includes('输出结构'), 'prompt 应由 buildContinuationPackPrompt 生成');
  alignedChunks.forEach((chunk, i) => {
    assert(prompt.includes(`【deconstruction-chunk-${i + 1}.docx】`), `prompt 应包含块 ${i + 1} 的分节标题`);
  });

...
async function enterEditor(page) {
  ...
  try {
    ...
    // Older launch states may still resume through a queued cockpit action.
  }

  const queuedResume = page.locator('[data-testid="queued-step-resume_editor"]');
  if (await queuedResume.isVisible().catch(() => false)) {
    await queuedResume.click();
  } else {
    const cockpitAction = page.locator('[data-testid="cockpit-primary-action"]');
    await cockpitAction.waitFor({ state: 'visible', timeout: 15_000 });
    await cockpitAction.click();
  }

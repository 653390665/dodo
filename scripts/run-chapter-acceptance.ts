...
## 综合建议
当前小节AI机械腔极其明显，充斥大量无意义的修饰词和说明腔. 建议生成精修预览，并确认是否剥离这三处废话.
`,
    structured: {
      score: 48,
      fatalIssues: [
      ...
      sceneChecks: [],
      surgerySuggestions: [
        '当前小节AI机械腔极其明显，充饰大量无意义的修饰词和说明腔。建议生成精修预览，并确认是否剥离这三处废话。',
      ],
    },
    patchReplacements: {
...
## 综合建议
经过局部精修，AI味废话已被完全物理剥离，替换句充满物理张力与动作细节，生活变故一语中的。行文极为紧凑。
`,
  },
...
## 综合建议
通篇全是一问一答的散牌卡片，毫无环境和身体语言的介入。建议通过局部精修预览，融入微动作与情绪切片。
`,
    structured: {
      score: 55,
      fatalIssues: [
...
## 综合建议
章节行文极其老练成熟。起承转合、环境烘托极其高级（雨声、青石板、挑面、浓汽），没有致命或非致命问题，无需生成精修预览，已是高品质成品。
`,
    structured: {
      score: 96,
      fatalIssues: [],
...
function runAcceptance() {
  ...
  const indicators: ReportIndicator[] = [
    ...
    },
    {
      ...
      score: 100,
      description: '严格执行局部精修预览最多不超过 3 处且绝不整章重写的硬性物理契约，原作者行文主干 100% 完整保留。',
    },
    {
      name: 'UX 下一步指引清晰度 (UX Direction Clarity)',
      score: 95,
      description: '质量面板清晰显示致命问题定位，提供精修预览入口，不泄露任何 Base64 隐藏注释，给到用户明确而有掌控感的指引。',
    },
  ];
...
本报告由自动化验收脚本 \`scripts/run-chapter-acceptance.ts\` 端到端执行生成，通过 3 类小说章节 Fixture/Mock 样本，对“审稿 ➡ 质量面板 ➡ 生成精修预览 ➡ 再审稿”进行了全闭环极限跑通，并严格审计了四大硬性指标。
...
| 样本名称 | 原始字符数 | 修后字符数 | 初审 Slop 分 | 修后 Slop 分 | 初审综合分 | 复审综合分 | 精修预览应用数 | Base64 安全隔离 |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
...
> [!TIP]
> - **局部精修预览闭环达成**：修后文本 100% 遵守“不超过 3 处且绝不整章重写”的硬性契约，完美杜绝了传统 AI 整章重写导致的主观失控。

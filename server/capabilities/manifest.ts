...
const BUILT_IN_MANIFESTS: Readonly<Record<string, CapabilityManifestEntry>> = {
  ...
  'text-normalize-preview': {
    id: 'text-normalize-preview',
    version: '1',
    kind: 'utility',
    stages: ['writer', 'critic'],
    input: 'text',
    output: 'transform-preview',
    action: 'preview-transform',
    persistence: 'single-run',
...
function getRuntimeOverlayManifest(id: string): CapabilityManifestEntry | undefined {
  ...
  if (!asset?.deconstructionCardType || asset.runtimeStatus !== 'active' || !asset.isRuntimeReady)
    return undefined;
  return {
    id: asset.id,
    version: 'catalog',
...
export class ArtifactCapabilityExecutionError extends Error {
  constructor(
    public readonly code:
      | 'ARTIFACT_CAPABILITY_NOT_FOUND'
      | 'ARTIFACT_CAPABILITY_RUNTIME_UNAVAILABLE'
...
export class CapabilityRoleAssignmentError extends Error {
  constructor(
    public readonly code:
      | 'CAPABILITY_ROLE_KIND_UNSUPPORTED'
      | 'CAPABILITY_ROLE_STAGE_UNSUPPORTED'
      | 'CAPABILITY_ROLE_SLOT_INVALID',
    message: string
  ) {
    super(`${code}: ${message}`);
  }
...
export function validateSkillCardForScope(
  skill: Skill,
  scope: 'project' | 'chapter' | 'fusion-source'
): void {
  ...
  if (
    value.isRuntimeReady !== true ||
    value.sanitizationStatus !== 'runtime-ready' ||
  ...
  const hasRules = [
    ...
    skill.plotPattern,
    skill.corePatterns,
    skill.bannedElements,
  ].some((value) =>
    Array.isArray(value) ? value.length > 0 : typeof value === 'string' && value.trim().length > 0
  );
  if (!hasRules)
    throw new SkillCardValidationError('SKILL_CARD_RULES_MISSING', '能力卡缺少可执行规则');
}
...
export function validateMountedSkillLoadout(input: unknown): void {
  ...
  for (const entry of input) {
    ...
    if (manifest.kind !== 'role-skill') {
      throw new CapabilityRoleAssignmentError(
        'CAPABILITY_ROLE_KIND_UNSUPPORTED',
        '该能力卡不能放入旧职责位'
      );
    }
...
export function validateCapabilityInvocation(
  ...
  if (!manifest.stages.includes(stage)) {
    throw new CapabilityInvocationError(
      'CAPABILITY_STAGE_UNSUPPORTED',
      'stage is not supported by capability'
    );
  }
  if (!manifest.allowedScopes.includes('single-run')) {
    throw new CapabilityInvocationError(
      'CAPABILITY_SCOPE_UNSUPPORTED',
      'capability is not executable as a single run'
    );
  }
  ...
  if (manifest.kind !== 'utility' && manifest.kind !== 'diagnostic' && !isPreviewOnlyTransform) {
    throw new CapabilityInvocationError(
      'CAPABILITY_NOT_EXECUTABLE',
      'capability is not a one-shot diagnostic or preview utility'
    );
  }
  return manifest;
}
...
export function validateArtifactCapabilityExecutionRequest(input: {
  ...
  if (manifest.runtimeStatus !== 'active') {
    throw new ArtifactCapabilityExecutionError(
      'ARTIFACT_CAPABILITY_RUNTIME_UNAVAILABLE',
      'capability is not runtime active'
    );
  }
  ...
  if (!result.ok) {
    throw new ArtifactCapabilityExecutionError(
      result.code,
      result.code === 'ARTIFACT_CAPABILITY_GAP'
        ? 'required artifact inputs are missing'
        : 'artifact execution is not allowed',

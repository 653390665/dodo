...
export function buildCapabilityMigrationPreview(
  novel: Novel,
  skills: Skill[] = []
): MigrationPreview {
  const profile = asRecord(novel.projectPreferenceProfile);
  const existing =
    profile.capabilityModelVersion === 3 &&
    profile.capabilityProfile &&
    typeof profile.capabilityProfile === 'object'
      ? (profile.capabilityProfile as ProjectCapabilityProfile)
      : undefined;
  if (existing) {
    const mainCard = existing.projectSkillDeck?.mainCardId
      ? { id: existing.projectSkillDeck.mainCardId, source: 'v3' }
      : undefined;
    const supportCards = (existing.projectSkillDeck?.supportCardIds || []).map((id) => ({
      id,
      source: 'v3',
    }));
    ...
    return {
      flow: existing.activeFlowId ? { id: existing.activeFlowId, source: 'v3' } : undefined,
      techniques: projectTechniqueIds.map((id) => ({ id, source: 'v3' })),
      skillCards: { main: mainCard, support: supportCards },
      mainCard,
      supportCards,
      migrationPendingIds: existing.migrationPendingIds || [],
      conflicts: [],
      capabilityProfile: existing,
    };
  }
  const loadout = Array.isArray(novel.mountedSkillLoadout) ? novel.mountedSkillLoadout : [];
  const mountedIds = Array.isArray(novel.mountedSkillIds)
    ? novel.mountedSkillIds.filter((id): id is string => typeof id === 'string')
    : [];
  const ids = unique([
    ...loadout.map((entry) => (typeof entry?.skillId === 'string' ? entry.skillId : '')),
    ...mountedIds,
  ]);
  ...
  if (new Set(mountedIds).size !== mountedIds.length)
    conflicts.push({
      id: 'mountedSkillIds',
      reason: 'duplicate-mounted-id',
      relatedIds: mountedIds,
    });
  for (const entry of loadout) {
    if (
      typeof entry?.skillId === 'string' &&
      (!Number.isInteger(entry.slot) || entry.slot < 0 || entry.slot > 2)
    ) {
      invalidIds.add(entry.skillId);
      conflicts.push({
        id: entry.skillId,
        reason: 'out-of-range-slot',
        relatedIds: [String(entry.slot)],
      });
    }
  }
  for (const id of ids) {
    if (loadout.filter((entry) => entry?.skillId === id).length > 1) {
      duplicateIds.add(id);
      pending.push(id);
      conflicts.push({ id, reason: 'duplicate-mounted-entry' });
    }
  }
  ...
  for (const id of ids) {
    ...
    const slot =
      loadoutEntries.length === 1 && Number.isInteger(loadoutEntries[0]?.slot)
        ? loadoutEntries[0].slot
        : undefined;
    if (loadoutEntries.length > 1)
      conflicts.push({
        id,
        reason: 'duplicate-mounted-entry',
        relatedIds: loadoutEntries.map((entry) => String(entry.slot)),
      });
    ...
    else if (manifest?.kind === 'skill-card' && skill && skillIsRuntimeReady(skill))
      cards.push({ id, source: sourceOf(skill), slot });
  ...
  if (flows.length > 1) {
    conflicts.push({
      id: flows[0].id,
      reason: 'multiple-flow-candidates',
      relatedIds: flows.map((item) => item.id),
    });
  ...
  if (cards.filter((card) => card.slot === 0).length > 1) {
    conflicts.push({
      id: 'skill-deck',
      reason: 'multiple-main-candidates',
      relatedIds: cards.filter((card) => card.slot === 0).map((item) => item.id),
    });
  ...
  if (mainCandidates.length === 0 && cards.length > 0) {
    conflicts.push({
      id: 'skill-deck',
      reason: 'main-card-selection-required',
      relatedIds: cards.map((item) => item.id),
    });
  ...
  if (supports.length > 2) {
    conflicts.push({
      id: 'skill-deck',
      reason: 'too-many-support-candidates',
      relatedIds: supports.map((item) => item.id),
    });
  ...
  const migrationPendingIds = unique([
    ...pending,
    ...conflicts
      .flatMap((item) => item.relatedIds || [])
      .filter(
        (id) =>
          id !== activeFlow?.id &&
          !techniques.some((item) => item.id === id) &&
          id !== main?.id &&
          !support.some((item) => item.id === id)
      ),
  ]);
  const capabilityProfile: ProjectCapabilityProfile = {
    ...
    projectSkillDeck: {
      ...(main ? { mainCardId: main.id } : {}),
      supportCardIds: support.map((item) => item.id),
      updatedAt: Date.now(),
    },
  ...
  return {
    ...
    conflicts,
    capabilityProfile,
    ...(loadout.length === 0 ? { suggestion: 'mountedSkillIds-only-order' } : {}),
  };
}

export function mergeMigratedProfile(
  current: ProjectPreferenceProfile | undefined,
  capabilityProfile: ProjectCapabilityProfile
): ProjectPreferenceProfile {
  return {
    ...(current || {
...
export function migrationProfileFingerprint(profile: ProjectCapabilityProfile): string {
  return JSON.stringify({
    ...profile,
    projectSkillDeck: { ...profile.projectSkillDeck, updatedAt: 0 },
  });
}
...
export function migrationProjectProfileFingerprint(

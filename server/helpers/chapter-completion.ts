...
import {
  acceptChapterRisk as deriveRisk,
  chapterCompletionHashes,
  deriveChapterCompletionGate,
  type CompleteChapterInput,
  type AcceptChapterRiskInput,
  type ChapterCompletionResult,
} from '../../shared/lib/chapter-completion.js';
...
} from './chapter-fact-candidates.js';
...
type Phase =
  'writes-flushed' | 'version-created' | 'deterministic-checked' | 'ai-reviewed' | 'facts-proposed';
type StoredState = {
  versionId?: string;
...
async function persist(
  ...
  const guarded = await runInSerializedWriteForGeneration(input.databaseGeneration, () =>
    db.upsertChapterCompletionAttempt({
      ...input,
      ...extra,
      phase,
      result: state,
...
async function defaultReview(
  ...
  content: string,
  sceneBeats: string,
  reviewProvider = governedGenerateText
): Promise<ReviewResult> {
  const config = getConfig();
  if (reviewProvider === governedGenerateText && !isLlmConfigured(config))
    return { status: 'unavailable' };
  try {
    const raw = await reviewProvider(
      config,
      {
      ...
      {
        operation: 'chapter-completion-review',
        novelId,
        timeoutMs: 60_000,
        concurrency: 1,
    ...
    const status =
      audit.score >= 60 && !audit.fatalIssues.some((issue) => issue.severity === 'critical')
        ? 'pass'
        : 'fail';
    return {
      status,
...
export async function completeChapter(
  ...
  if (!novel || !chapter || chapter.novelId !== input.novelId)
    throw new Error('CHAPTER_NOT_FOUND_OR_NOT_OWNED');
  if (
    !Number.isInteger(input.databaseGeneration) ||
    input.databaseGeneration !== getDatabaseGeneration()
  )
    throw new Error('DATABASE_GENERATION_STALE');
  const hashes = chapterCompletionHashes(chapter);
  const existing = db.findChapterCompletionAttempt({
  ...
  const deterministicIssues = deps.deterministic
    ? deps.deterministic(chapter.content, chapter.sceneBeats || '')
    : chapter.content.trim()
      ? []
      : ['empty-manuscript'];
  const trustedReview =
    !retryingUnavailable && chapter.workflowMeta?.reviewState?.contentHash === hashes.contentHash
      ? chapter.workflowMeta.reviewState
      : undefined;
  let reviewState =
    trustedReview ||
    (state.reviewState && state.reviewState.contentHash === hashes.contentHash
      ? state.reviewState
      : undefined);
  let gate = deriveChapterCompletionGate({
  ...
  if (!phaseAtLeast(existing?.phase, 'writes-flushed')) {
    await persist(attempt, 'writes-flushed', state, {
      quality: gate.quality,
      issueIds: gate.deterministicIssues,
      unknownChecks: gate.unknownChecks,
    });
  ...
  if (!phaseAtLeast(existing?.phase, 'deterministic-checked')) {
    await persist(attempt, 'deterministic-checked', state, {
      quality: gate.quality,
      issueIds: gate.deterministicIssues,
      unknownChecks: gate.unknownChecks,
    });
  ...
  if (retryingUnavailable || !phaseAtLeast(existing?.phase, 'ai-reviewed')) {
    if (reviewState)
      aiStatus =
        reviewState.gate === 'pass'
          ? 'pass'
          : reviewState.gate === 'needs-action'
            ? 'fail'
            : 'unknown';
    else {
      const review = deps.review
        ? await deps.review(chapter.content, chapter.sceneBeats || '')
        : await defaultReview(
            input.novelId,
            chapter.content,
            chapter.sceneBeats || '',
    ...
    gate = deriveChapterCompletionGate({
      content: chapter.content,
      sceneBeats: chapter.sceneBeats,
      reviewState,
      deterministicIssues,
  ...
  if (
    retryingUnavailable ||
    !phaseAtLeast(existing?.phase, 'facts-proposed') ||
    ...
    if (!currentBeforeWrite || currentBeforeWrite.novelId !== input.novelId)
      throw new Error('CHAPTER_NOT_FOUND_OR_NOT_OWNED');
    const currentHashes = chapterCompletionHashes(currentBeforeWrite);
    if (
      currentHashes.contentHash !== hashes.contentHash ||
      currentHashes.planHash !== hashes.planHash
    ) {
    ...
    const sourceRun = db
      .listAppliedChapterProductionRunIds(input.novelId, chapter.id)
      .map((id) => db.getChapterProductionRun(id))
      .find(
        (run) =>
          run &&
          run.continuityReport.databaseGeneration === input.databaseGeneration &&
          run.draftContent === chapter.content &&
          run.sceneBeats === (chapter.sceneBeats || '')
      );
    const factCandidate = sourceRun
      ...
      : buildChapterFactCandidate({
          ...
          proposedPatch: deps.proposedPatch || state.proposedPatch || {},
        });
    const pendingFactCandidate =
      sourceRun && factCandidate.facts.length > 0 ? factCandidate : undefined;
    state.factCandidate = pendingFactCandidate;
    state.proposedPatch =
      sourceRun?.continuityReport.proposedPatch || deps.proposedPatch || state.proposedPatch || {};
    state.completionResult = {
      quality: gate.quality,
      gate,
      phase: 'facts-proposed',
      attemptId: attempt.id,
      ...(pendingFactCandidate
        ? {
            factCandidateId: pendingFactCandidate.id,
            factCandidateRunId: pendingFactCandidate.runId,
          }
        : {}),
    };
    ...
    const workflowMeta = {
      ...
      completionContentHash: gate.contentHash,
    ...
    const guarded = await runInSerializedWriteForGeneration(input.databaseGeneration, () =>
      runInTransaction(() => {
        ...
        if (!current || current.novelId !== input.novelId)
          throw new Error('CHAPTER_NOT_FOUND_OR_NOT_OWNED');
...
export async function acceptChapterRisk(
  input: AcceptChapterRiskInput & { novelId: string; chapterId: string; databaseGeneration: number }
): Promise<ChapterCompletionResult> {
  if (
    !Number.isInteger(input.databaseGeneration) ||
    input.databaseGeneration !== getDatabaseGeneration()
  ...
  if (!chapter || chapter.novelId !== input.novelId)
    throw new Error('CHAPTER_NOT_FOUND_OR_NOT_OWNED');
  const hashes = chapterCompletionHashes(chapter);
  if (hashes.contentHash !== input.contentHash || hashes.planHash !== input.planHash)
    throw new Error('RISK_DECISION_STALE');
  const attempt = db.findChapterCompletionAttempt({
    novelId: input.novelId,
    chapterId: input.chapterId,
    databaseGeneration: input.databaseGeneration,
    ...hashes,
  ...
  const workflowMeta = {
    ...(chapter.workflowMeta || { version: 1 as const }),
    version: 1 as const,
    completionGate: 'accepted-risk' as const,
    completionContentHash: result.gate.contentHash,
    completionDecisionAt: input.authorDecisionAt || Date.now(),
    factCandidateId: attempt.factCandidateId,
  ...
  const guarded = await runInSerializedWriteForGeneration(input.databaseGeneration, () =>
    runInTransaction(() => {
      ...
      db.upsertChapterCompletionAttempt({
        ...attempt,
        riskAcceptedAt: input.authorDecisionAt || Date.now(),
        issueIds: input.unresolvedIssueIds,
        unknownChecks: input.unknownChecks,
        result: state,
        updatedAt: Date.now(),
      });
    })

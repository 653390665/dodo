...
import {
  diagnoseCharacterCore,
  emptyCharacterCore,
  normalizeCharacterCore,
  type CharacterCoreGap,
} from '../../shared/lib/character-core.js';
...
export function normalizeCharacterCandidateOutput(raw: string): CharacterCandidateNormalization {
  ...
  if (parsed === undefined) {
    return {
      core: emptyCharacterCore(),
      proposedContent: raw.trim(),
      gaps: diagnoseCharacterCore(emptyCharacterCore()),
    };
  }
  const parsedRecord = asRecord(parsed);
  const core = normalizeCharacterCore(parsedRecord.core ?? parsedRecord);
  const proposedContent =
    typeof parsedRecord.proposedContent === 'string'
      ? parsedRecord.proposedContent.trim()
      : typeof parsedRecord.bio === 'string'
        ? parsedRecord.bio.trim()
        : undefined;
  return {
    core,
    ...(proposedContent ? { proposedContent } : {}),
    gaps: diagnoseCharacterCore(core),
  };
}
...
export function characterImpactReport(novelId: string, characterId: string): ArtifactImpactReport {
  const outlines = listOutlineArtifacts(novelId, { status: 'active' })
    .filter((artifact) =>
      artifact.core?.nodes.some((node) => node.characterIds.includes(characterId))
    )
    .map((artifact) => ({
      kind:
  ...
  const relationships = listEntityRelationships(novelId).filter(
    (item) =>
      (item.sourceType === 'character' && item.sourceId === characterId) ||
      (item.targetType === 'character' && item.targetId === characterId)
  );
  const promises = listForeshadowings(novelId).filter((item) =>
    item.relatedCharacterIds.includes(characterId)
  );
  ...
  return {
    ...
    reasons: [
      ...
      relationships.length
        ? 'character changes affect existing relationships'
        : 'no existing character relationship is linked',
      promises.length
        ? 'character changes affect linked narrative promises'
        : 'no linked narrative promise is available',
    ],
  };
}
...
export function buildCharacterCandidateInput(input: {
  ...
  return {
    ...
    ...(normalized.proposedContent === undefined
      ? {}
      : { proposedContent: normalized.proposedContent }),
    impactReport: characterImpactReport(input.novelId, input.character.id),
  };
}

export function characterCandidateSummary(candidate: ArtifactCandidate<CharacterCore>): {
  gaps: CharacterCoreGap[];

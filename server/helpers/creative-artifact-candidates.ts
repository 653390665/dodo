...
function assertPlainObject(value: unknown): asserts value is Record<string, unknown> {
  if (
    ...
    throw new ArtifactCandidateError(
      'ARTIFACT_CANDIDATE_INVALID_OUTPUT',
      'proposed core must be a plain object'
    );
  }
}
...
export async function previewArtifactCandidate(
  input: PreviewArtifactCandidateInput
): Promise<ArtifactCandidate> {
  if (!genericKinds.has(input.target.kind)) {
    throw new ArtifactCandidateError(
      'ARTIFACT_CANDIDATE_UNSUPPORTED_KIND',
      'generic candidates support world and character only'
    );
  }
  ...
  if ((current?.version ?? 0) !== input.target.version) {
    throw new ArtifactCandidateError(
      'ARTIFACT_CANDIDATE_VERSION_STALE',
      'target version does not match current artifact'
    );
  }
  if (currentFingerprint(input) !== input.baseFingerprint) {
    throw new ArtifactCandidateError(
      'ARTIFACT_CANDIDATE_FINGERPRINT_STALE',
      'base fingerprint does not match current artifact'
    );
  }
...
export async function acceptArtifactCandidate(input: {
    ...
    runInTransaction(() => {
      ...
      if (!candidate)
        throw new ArtifactCandidateError(
          ...
          'candidate not found for novel'
        );
      if (candidate.status === 'rejected')
        throw new ArtifactCandidateError('ARTIFACT_CANDIDATE_REJECTED', 'candidate was rejected');
      if (candidate.status === 'stale')
        throw new ArtifactCandidateError('ARTIFACT_CANDIDATE_STALE', 'candidate is stale');

      const current = getArtifactCore(input.novelId, candidate.target.kind, candidate.target.id);
      if (candidate.status === 'accepted') {
        if (!current)
          throw new ArtifactCandidateError(
            'ARTIFACT_CANDIDATE_INVALID_DATA',
            'accepted candidate has no artifact version'
          );
      ...
      if (
        fingerprint !== candidate.baseFingerprint ||
        (current?.version ?? 0) !== candidate.target.version
      ) {
      ...
      const core = saveArtifactVersion({
        ...
        ...(candidate.proposedContent === undefined
          ? {}
          : { readableContent: candidate.proposedContent }),
        provenance: {
      ...
      const acceptedCandidate = applyArtifactCandidateDecision(
        input.novelId,
        input.candidateId,
        'accepted'
      );
      const reviewRequirements = candidate.impactReport.reviewRequired.map((artifact) =>
        markArtifactReviewRequired({
          novelId: input.novelId,
          artifact,
          sourceCandidateId: candidate.id,
          reason: 'upstream artifact candidate accepted',
        })
      );
  ...
  if (!guarded.executed) {
    throw new ArtifactCandidateError(
      'ARTIFACT_CANDIDATE_GENERATION_STALE',

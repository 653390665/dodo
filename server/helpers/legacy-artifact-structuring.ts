...
} from '../../shared/types/legacy-artifact-structuring.js';
...
import {
  ...
  createCanonPatch,
  getCanonFingerprint,
} from '../lib/db/canon-patches.js';
import { getArtifactCore, saveArtifactVersion } from '../lib/db/creative-artifacts.js';

const OUTLINE_KINDS = new Set<CreativeArtifactKind>([
  'master-outline',
  'volume-outline',
  'chapter-outline',
]);
...
const STRUCTURED_ROOT_HINTS: Record<LegacyArtifactSource['artifactKind'], string> = {
  world:
    '{"schemaVersion":1,"hardRules":[],"powerConstraints":[],"prohibitions":[],"factionConstraints":[]}',
  character:
    '{"schemaVersion":1,"desire":"","externalGoal":"","internalNeed":"","fear":"","woundOrFalseBelief":"","strengths":[],"flaws":[],"contradictions":[],"speechPattern":"","habitualActions":[],"decisionPattern":"","relationshipTensions":[],"arc":{"start":"","turns":[],"target":""},"immutableFacts":[]}',
  'master-outline':
    '{"schemaVersion":1,"nodes":[{"id":"","type":"premise","title":"","intent":"","order":0,"characterIds":[],"foreshadowingIds":[]}]}',
  'volume-outline':
    '{"schemaVersion":1,"nodes":[{"id":"","type":"turn","title":"","intent":"","order":0,"characterIds":[],"foreshadowingIds":[]}]}',
  'chapter-outline':
    '{"schemaVersion":1,"nodes":[{"id":"","type":"turn","title":"","intent":"","order":0,"characterIds":[],"foreshadowingIds":[]}]}',
  'scene-beats': '{"schemaVersion":1,"beats":[{"order":1,"summary":"","intent":""}]}',
  'narrative-promise':
    '{"schemaVersion":1,"plan":{"intent":"","plannedHintRanges":[],"sourceOutlineNodeIds":[]},"evidence":[]}',
};
...
function sourceFor(
  novelId: string,
  kind: LegacyArtifactSource['artifactKind'],
  artifactId: string,
  label: string,
  content: string,
  version: number
): LegacyArtifactSource | undefined {
...
export function listLegacyArtifactSources(novelId: string): LegacyArtifactSource[] {
  ...
  for (const character of db.listCharacters(novelId)) {
    const source = sourceFor(
      ...
      character.id,
      character.name,
      text(character.bio || character.summary),
      0
    );
  ...
  for (const outline of db.listOutlineArtifacts(novelId, { status: 'active' })) {
    ...
    const source = sourceFor(
      ...
      kind,
      outline.id,
      outline.content.slice(0, 80) || kind,
...
function nonEmpty(
  ...
  if (kind === 'world') {
    return ['hardRules', 'powerConstraints', 'prohibitions', 'factionConstraints'].some(
      (key) => Array.isArray(core[key]) && core[key].length > 0
    );
  }
  if (kind === 'character') {
    ...
    return (
      scalarFields.some((key) => text(core[key]).trim()) ||
...
export function parseLegacyStructuringOutput(
  source: LegacyArtifactSource,
  raw: string
): Pick<LegacyArtifactPreview, 'proposedCore' | 'proposedContent'> {
  const parsed = parseJson(raw);
  const coreInput =
    parsed.core && typeof parsed.core === 'object' && !Array.isArray(parsed.core)
      ? parsed.core
      : parsed;
  let core: Record<string, unknown>;
  if (source.artifactKind === 'world')
    core = normalizeWorldCore(coreInput) as unknown as Record<string, unknown>;
  else if (source.artifactKind === 'character')
    core = normalizeCharacterCore(coreInput) as unknown as Record<string, unknown>;
  else if (OUTLINE_KINDS.has(source.artifactKind)) {
    if (!isStructuredOutlineCore(coreInput))
      throw new Error('LEGACY_STRUCTURING_OUTLINE_CORE_INVALID');
  ...
  const proposedContent =
    typeof parsed.proposedContent === 'string' ? parsed.proposedContent : undefined;
...
export async function confirmLegacyStructuringPreview(
  input: ConfirmLegacyStructuringInput
  ...
  const current = listLegacyArtifactSources(preview.source.novelId).find(
    (item) =>
      item.artifactKind === preview.source.artifactKind &&
      item.artifactId === preview.source.artifactId
  );
  if (!current || current.sourceFingerprint !== preview.source.sourceFingerprint)
    throw new Error('LEGACY_STRUCTURING_SOURCE_STALE');
  if (OUTLINE_KINDS.has(preview.source.artifactKind)) {
    ...
    const patch = createCanonPatch({
      novelId: preview.source.novelId,
      baseFingerprint: getCanonFingerprint(preview.source.novelId),
      sourceAbilityId: 'legacy-artifact-structuring',
      operations: [
        {
          operation: 'replace-outline',
          targetArtifactId: artifact.id,
          content: preview.proposedContent ?? preview.source.originalContent,
          core: preview.proposedCore as never,
        },
      ] as never,
    });
    const result = await acceptCanonPatch(
      preview.source.novelId,
      patch.id,
  ...
  const saved = saveArtifactVersion({
    novelId: preview.source.novelId,
    artifactKind: preview.source.artifactKind,
    artifactId: preview.source.artifactId,
    expectedVersion: preview.source.artifactVersion,
    core: preview.proposedCore,
    readableContent: preview.source.originalContent,
    provenance: { source: 'legacy-artifact-structuring' },
  });
  return { status: 'accepted', version: saved.version };
}

export function createLegacyArtifactPreview(
  source: LegacyArtifactSource,
  parsed: Pick<LegacyArtifactPreview, 'proposedCore' | 'proposedContent'>,

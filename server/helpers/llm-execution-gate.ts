...
import {
  ...
  type QuotaLimitType,
  type QuotaAccessContext,
} from './quota-guard.js';
import { getDatabaseGeneration, subscribeDatabaseGeneration } from '../lib/db-instance.js';
...
export async function createLlmExecution(options: {
  ...
  accessContext?: QuotaAccessContext;
  timeoutMs: number;
  signal?: AbortSignal;
  concurrency?: number;
  ...
  return {
    ...
    async run<T>(work: (context: { signal: AbortSignal; controller: AbortController; traceId: string }) => Promise<T>): Promise<T> {
      ...
      try {
        ...
        // release the request and quota when an incompatible provider hangs.

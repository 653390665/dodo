...
export type QuotaAccessContext = 'basic-byok' | 'enhanced-workflow';
...
/** Whether commercial quota enforcement is enabled for the current process. */
export function isMonetizationEnabled(): boolean {
  const configured = process.env.INKFLOW_ENABLE_MONETIZATION;
...
export function checkQuota(
  ...
  if (accessContext === 'basic-byok') {
    return { allowed: true, limitType };
  }

  const commercialMode = profile.commercialMode || 'free';

  if (commercialMode === 'paid') {
    return { allowed: true };
  }
...
export function consumeQuota(
  ...
  if (accessContext === 'basic-byok') return;
...
export async function checkAndConsumeQuota(
  ...
  return runInSerializedWrite<QuotaCheckResult>(() => {
    return checkAndConsumeQuotaSync(novelId, limitType, accessContext);
  });
}
...
function getQuotaErrorMessage(limitType: QuotaLimitType, max: number): string {
  switch (limitType) {
    case 'extractSkill':
      return `您的免费【拆书萃取】实验额度（${max} 次）已全部用尽。基础本地写作与 BYOK 主链不受影响；如需实验权益，请联系管理员开通。`;
    case 'generateProse':
      return `您的免费【高级生产工作流】实验额度（${max} 次）已全部用尽；如需实验权益，请联系管理员开通。`;
    case 'advancedAudit':
      return `您的免费【智能审稿与高级诊断】实验额度（${max} 次）已全部用尽。基础本地写作与 BYOK 主链不受影响；如需实验权益，请联系管理员开通。`;
    default:
      return '您的免费 AI 功能实验额度已用尽。基础本地写作与 BYOK 主链不受影响；如需实验权益，请联系管理员开通。';
  }
}
...
export async function reserveQuota(
  ...
  return runInSerializedWrite<QuotaCheckResult>(() => {
    ...
    if (!isMonetizationEnabled() || accessContext === 'basic-byok') {
      const novel = getNovel(novelId);
      if (!novel) return missingNovelResult(novelId);
      return { allowed: true, limitType, databaseGeneration };
    }

    const result = checkAndConsumeQuotaSync(novelId, limitType, accessContext);

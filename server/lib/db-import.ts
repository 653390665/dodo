...
import { DB_PATH, INKFLOW_SQLITE_APPLICATION_ID, initDb, openReadOnlyDb } from './db-init';
import {
...
const OPTIONAL_IMPORT_SCHEMA: Record<string, Record<string, ImportColumnRequirement>> = {
  ...
  chapter_production_run_versions: {
    ...
    novel_id: { type: 'TEXT', notNull: true },
    target_chapter_id: { type: 'TEXT' },
    source: { type: 'TEXT', notNull: true },
    scene_beats: { type: 'TEXT' },
    draft_content: { type: 'TEXT' },
    style_audit: { type: 'TEXT' },
    continuity_report: { type: 'TEXT' },
    content_hash: { type: 'TEXT', notNull: true },
    created_at: { type: 'INTEGER', notNull: true },
  },
  continuation_extraction_jobs: {
    id: { type: 'TEXT', primaryKey: true },
    pack_id: { type: 'TEXT', notNull: true },
    novel_id: { type: 'TEXT', notNull: true },
    status: { type: 'TEXT', notNull: true },
    progress: { type: 'INTEGER' },
    stage_text: { type: 'TEXT' },
    batch_cursor: { type: 'INTEGER' },
    total_batches: { type: 'INTEGER' },
    result_json: { type: 'TEXT' },
    checkpoint_json: { type: 'TEXT' },
    error_code: { type: 'TEXT' },
  ...
  outline_artifacts: {
    ...
    source: { type: 'TEXT', notNull: true },
    status: { type: 'TEXT', notNull: true },
    base_fingerprint: { type: 'TEXT' },
    core_json: { type: 'TEXT', optional: true },
  ...
  canon_patches: {
    id: { type: 'TEXT', primaryKey: true },
    novel_id: { type: 'TEXT', notNull: true },
    base_fingerprint: { type: 'TEXT', notNull: true },
    source_ability_id: { type: 'TEXT' },
    operations: { type: 'TEXT', notNull: true },
    status: { type: 'TEXT', notNull: true },
    result_fingerprint: { type: 'TEXT' },
    result_json: { type: 'TEXT' },
  ...
  creative_artifact_cores: {
    id: { type: 'TEXT', primaryKey: true },
    novel_id: { type: 'TEXT', notNull: true },
    artifact_kind: { type: 'TEXT', notNull: true },
    artifact_id: { type: 'TEXT', notNull: true },
    version: { type: 'INTEGER', notNull: true },
    core_json: { type: 'TEXT', notNull: true },
    readable_content: { type: 'TEXT' },
    provenance_json: { type: 'TEXT', notNull: true },
    created_at: { type: 'INTEGER', notNull: true },
    updated_at: { type: 'INTEGER', notNull: true },
  },
  ...
  creative_artifact_candidates: {
    ...
    artifact_kind: { type: 'TEXT', notNull: true },
    artifact_id: { type: 'TEXT', notNull: true },
    target_version: { type: 'INTEGER', notNull: true },
    operation: { type: 'TEXT', notNull: true },
    goal: { type: 'TEXT', notNull: true },
    base_fingerprint: { type: 'TEXT', notNull: true },
    source_capability_versions: { type: 'TEXT', notNull: true },
    proposed_core: { type: 'TEXT', notNull: true },
    proposed_content: { type: 'TEXT' },
    diff: { type: 'TEXT', notNull: true },
    impact_report: { type: 'TEXT', notNull: true },
    status: { type: 'TEXT', notNull: true },
    created_at: { type: 'INTEGER', notNull: true },
    updated_at: { type: 'INTEGER', notNull: true },
    decided_at: { type: 'INTEGER' },
  ...
  artifact_review_requirements: {
    ...
    artifact_kind: { type: 'TEXT', notNull: true },
    artifact_id: { type: 'TEXT', notNull: true },
    artifact_version: { type: 'INTEGER', notNull: true },
    source_candidate_id: { type: 'TEXT' },
  ...
  creation_flow_sessions: {
    ...
    frozen_definition_json: { type: 'TEXT', notNull: true },
    current_step_id: { type: 'TEXT' },
    accepted_output_refs_json: { type: 'TEXT', notNull: true },
    status: { type: 'TEXT', notNull: true },
    database_generation: { type: 'INTEGER', notNull: true },
    created_at: { type: 'INTEGER', notNull: true },
    updated_at: { type: 'INTEGER', notNull: true },
...
export const ALLOWED_IMPORT_INDEXES = new Map<
  string,
  { tableName: string; columns: string[]; unique?: boolean; predicate?: string }
>([
  [
    'idx_chapter_completion_attempts_novel_chapter',
    { tableName: 'chapter_completion_attempts', columns: ['novel_id', 'chapter_id', 'updated_at'] },
  ],
  ...
  [
    'idx_chapter_production_runs_novel',
    { tableName: 'chapter_production_runs', columns: ['novel_id'] },
  ],
  [
    'idx_production_run_versions_run',
    { tableName: 'chapter_production_run_versions', columns: ['run_id', 'created_at'] },
  ],
  [
    'idx_production_run_versions_novel',
    { tableName: 'chapter_production_run_versions', columns: ['novel_id'] },
  ],
  [
    'idx_continuation_extraction_jobs_pack',
    { tableName: 'continuation_extraction_jobs', columns: ['pack_id', 'updated_at'] },
  ],
  [
    'idx_continuation_extraction_jobs_novel',
    { tableName: 'continuation_extraction_jobs', columns: ['novel_id'] },
  ],
  ...
  [
    'idx_entity_relationships_composite',
    { tableName: 'entity_relationships', columns: ['novelId', 'sourceId', 'targetId'] },
  ],
  [
    'idx_outline_artifacts_one_active_master',
    {
      tableName: 'outline_artifacts',
      columns: ['novel_id'],
      unique: true,
      predicate: "level = 'master' AND status = 'active'",
    },
  ],
  [
    'idx_creative_artifact_cores_identity',
    {
      tableName: 'creative_artifact_cores',
      columns: ['novel_id', 'artifact_kind', 'artifact_id'],
      unique: true,
    },
  ],
  [
    ...
    {
      tableName: 'creative_artifact_versions',
      columns: ['novel_id', 'artifact_kind', 'artifact_id', 'version'],
      unique: true,
    },
  ],
  [
  ...
  [
    ...
    { tableName: 'creative_artifact_candidates', columns: ['novel_id', 'status', 'created_at'] },
  ],
  [
    'idx_creative_artifact_candidates_target',
    {
      tableName: 'creative_artifact_candidates',
      columns: ['novel_id', 'artifact_kind', 'artifact_id'],
    },
  ],
  [
    ...
    {
      tableName: 'artifact_review_requirements',
      columns: ['novel_id', 'artifact_kind', 'artifact_id', 'status'],
    },
  ],
  [
    'idx_artifact_review_requirements_candidate',
    { tableName: 'artifact_review_requirements', columns: ['source_candidate_id'] },
  ],
  [
    'idx_creation_flow_sessions_novel',
    { tableName: 'creation_flow_sessions', columns: ['novel_id', 'created_at'] },
  ],
  [
    'idx_creation_flow_sessions_one_active',
    {
      tableName: 'creation_flow_sessions',
      columns: ['novel_id'],
      unique: true,
...
export const EXPECTED_IMPORT_FOREIGN_KEYS = new Map<
  string,
  Array<{
    from: string;
    parentTable: string;
...
>([
  ...
  [
    'entity_relationships',
    [{ from: 'novelId', parentTable: 'novels', to: 'id', onDelete: 'CASCADE' }],
  ],
  [
    'outline_artifacts',
    [{ from: 'novel_id', parentTable: 'novels', to: 'id', onDelete: 'CASCADE' }],
  ...
  [
    ...
    [
      { from: 'novel_id', parentTable: 'novels', to: 'id', onDelete: 'CASCADE' },
      {
        ...
        parentTable: 'creative_artifact_candidates',
        to: 'id',
        onDelete: 'SET NULL',
      },
    ],
...
function validateChapterCompletionConstraints(sql: string): void {
  ...
  const qualityCheck =
    /CHECK\s*\(\s*quality\s+IN\s*\(\s*'pass'\s*,\s*'needs-action'\s*,\s*'unknown'\s*\)\s*\)/i;
  const phaseCheck =
    /CHECK\s*\(\s*phase\s+IN\s*\(\s*'writes-flushed'\s*,\s*'version-created'\s*,\s*'deterministic-checked'\s*,\s*'ai-reviewed'\s*,\s*'facts-proposed'\s*\)\s*\)/i;
  const uniqueConstraint =
    /UNIQUE\s*\(\s*novel_id\s*,\s*chapter_id\s*,\s*database_generation\s*,\s*content_hash\s*,\s*plan_hash\s*\)/i;
  if (
    !qualityCheck.test(normalized) ||
    !phaseCheck.test(normalized) ||
    !uniqueConstraint.test(normalized)
  ) {
    throw new DatabaseImportValidationError(
      'Chapter completion constraints do not match InkFlow schema'
    );
  }
  if (
    ...
    throw new DatabaseImportValidationError(
      'Unexpected chapter completion constraints are not allowed'
    );
  }
}
...
export function validateDatabaseImportFile(filePath: string, requireApplicationId = false): void {
  ...
  try {
    ...
    const unsafeTableKinds = (
      candidate.pragma('table_list') as Array<{
        schema: string;
        name: string;
        type: string;
      }>
    ...
    if (unsafeTableKinds.length > 0) {
      throw new DatabaseImportValidationError(
        `Virtual table is not allowed: ${unsafeTableKinds[0].name}`
      );
    }

    const allSchemaRows = candidate
      .prepare(
        `
    ...
    for (const group of COMPLETE_IMPORT_SCHEMA_GROUPS) {
      const hasGovernanceSchema =
        group.tables.some((tableName) => existingTableNames.has(tableName)) ||
        group.indexes.some((indexName) => existingIndexNames.has(indexName));
      if (!hasGovernanceSchema) continue;
      const missingGovernanceObjects: string[] = group.tables.filter(
        (tableName) => !existingTableNames.has(tableName)
      );
      missingGovernanceObjects.push(
        ...group.indexes.filter((indexName) => !existingIndexNames.has(indexName))
      );
    ...
    for (const row of allSchemaRows) {
      if (row.type === 'trigger' || row.type === 'view') {
        throw new DatabaseImportValidationError(
          `Executable schema object is not allowed: ${row.type} ${row.name}`
        );
      }
      if (row.type === 'table') {
        ...
        if (row.name === 'chapter_completion_attempts') {
          ...
          throw new DatabaseImportValidationError(
            `Unexpected table constraint is not allowed: ${row.name}`
          );
        }
        if (row.name !== 'chapter_completion_attempts' && /\bCHECK\s*\(/i.test(structure)) {
          throw new DatabaseImportValidationError(
            `Unexpected CHECK constraint is not allowed: ${row.name}`
          );
        }
        if (/\bDEFERRABLE\b/i.test(structure)) {
          throw new DatabaseImportValidationError(
            `Deferred foreign key is not allowed: ${row.name}`
          );
        }
        continue;
      }
      if (row.type === 'index') {
        ...
        if (
          ...
          (expectedIndex.predicate &&
            normalizeIndexPredicate(row.sql || '') !== expectedIndex.predicate.toLowerCase())
        ) {
          throw new DatabaseImportValidationError(`Index has unsafe constraints: ${row.name}`);
        }
        const indexedColumns = (
          candidate.pragma(`index_xinfo(${row.name})`) as Array<{
            name: string | null;
            desc: number;
            coll: string;
            key: number;
        ...
        if (
          ...
          throw new DatabaseImportValidationError(
            `Index definition does not match InkFlow schema: ${row.name}`
          );
        }
        continue;
      }
      throw new DatabaseImportValidationError(
        `Unexpected schema object is not allowed: ${row.type} ${row.name}`
      );
    }
    ...
    for (const tableName of ALLOWED_IMPORT_TABLES) {
      ...
      for (const column of columns) {
        if (column.hidden !== 0) {
          throw new DatabaseImportValidationError(
            `Generated or hidden column is not allowed: ${tableName}.${column.name}`
          );
        }
        if (!hasSafeImportDefault(column.dflt_value)) {
          throw new DatabaseImportValidationError(
            `Expression default is not allowed: ${tableName}.${column.name}`
          );
        }
      }
      ...
      for (const index of indexes) {
        ...
        if (isCapabilityRecommendationPrimaryKey) {
          const keyColumns = (
            candidate.pragma(`index_xinfo(${index.name})`) as Array<{
              name: string | null;
              desc: number;
              coll: string;
              key: number;
          ...
          if (
            ...
            continue;
          throw new DatabaseImportValidationError(
            `Primary-key index does not match InkFlow schema: ${index.name}`
          );
        }
        if (
          ...
          if (
            ...
            keyColumns.every(
              (column, position) =>
                column.name ===
                  ['novel_id', 'chapter_id', 'database_generation', 'content_hash', 'plan_hash'][
                    position
                  ] &&
                column.desc === 0 &&
                column.coll.toUpperCase() === 'BINARY'
            ...
            continue;
          throw new DatabaseImportValidationError(
            `Unique constraint does not match InkFlow schema: ${tableName}`
          );
        }
        if (
          !index.name.startsWith(`sqlite_autoindex_${tableName}_`) ||
          ...
          throw new DatabaseImportValidationError(
            `Unexpected implicit index is not allowed: ${index.name}`
          );
        }
        const keyColumns = (
          candidate.pragma(`index_xinfo(${index.name})`) as Array<{
    ...
    for (const [tableName, requiredColumns] of Object.entries(REQUIRED_IMPORT_SCHEMA)) {
      if (tableTypes.get(tableName) !== 'table') {
        throw new DatabaseImportValidationError(
          `Required table is missing or invalid: ${tableName}`
        );
      }
      ...
      for (const [columnName, requirement] of Object.entries(requiredColumns)) {
        ...
        if (column.type.toUpperCase() !== requirement.type) {
          throw new DatabaseImportValidationError(
            `Required column has an invalid type: ${tableName}.${columnName}`
          );
        }
        if (requirement.primaryKey && column.pk < 1) {
          throw new DatabaseImportValidationError(
            `Required primary key is missing: ${tableName}.${columnName}`
          );
        }
        if (requirement.notNull && column.notnull !== 1) {
          throw new DatabaseImportValidationError(
            `Required NOT NULL constraint is missing: ${tableName}.${columnName}`
          );
        }
      }
    }
    ...
    for (const [tableName, requiredColumns] of Object.entries(OPTIONAL_IMPORT_SCHEMA)) {
      ...
      for (const [columnName, requirement] of Object.entries(requiredColumns)) {
        ...
        if (!column) {
          ...
          throw new DatabaseImportValidationError(
            `Optional table column is missing: ${tableName}.${columnName}`
          );
        }
        if (column.type.toUpperCase() !== requirement.type)
          throw new DatabaseImportValidationError(
            `Optional table column has an invalid type: ${tableName}.${columnName}`
          );
        if (requirement.primaryKey && column.pk < 1)
          throw new DatabaseImportValidationError(
            `Optional table primary key is missing: ${tableName}.${columnName}`
          );
        if (requirement.notNull && column.notnull !== 1)
          throw new DatabaseImportValidationError(
            `Optional table NOT NULL constraint is missing: ${tableName}.${columnName}`
          );
      }
    }
    ...
    for (const tableName of ALLOWED_IMPORT_TABLES) {
      ...
      if (
        ...
        throw new DatabaseImportValidationError(
          `Foreign key definition does not match InkFlow schema: ${tableName}`
        );
      }
    }

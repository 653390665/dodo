...
function sourceCapabilityVersionsFor(
  ...
  if (
    ...
    throw new CanonPatchError(
      stored ? 'CANON_PATCH_INVALID_DATA' : 'CANON_PATCH_INVALID_INPUT',
...
function operationValid(value: unknown): value is CanonPatchOperation {
  if (!isRecord(value) || !isNonEmptyString(value.operation) || !isNonEmptyString(value.content))
    return false;
  if (value.core !== undefined && !isStructuredOutlineCore(value.core)) return false;
  if (value.operation === 'create-master-outline')
    return Object.keys(value).every((key) => ['operation', 'content', 'core'].includes(key));
  if (value.operation === 'replace-outline') {
    return (
      isNonEmptyString(value.targetArtifactId) &&
      Object.keys(value).every((key) =>
        ['operation', 'targetArtifactId', 'content', 'core'].includes(key)
      )
    );
  }
  if (value.operation === 'create-scoped-outline') {
    return (
      (value.level === 'volume' || value.level === 'chapter') &&
      scopeValid(value.level, value.scope) &&
      Object.keys(value).every((key) =>
        ['operation', 'level', 'scope', 'content', 'core'].includes(key)
      )
    );
  }
  return false;
...
function parseOperations(raw: string, stored = true): readonly CanonPatchOperation[] {
  try {
    ...
    if (!Array.isArray(parsed) || parsed.length === 0 || !parsed.every(operationValid))
      throw new Error();
    return parsed;
  } catch {
    throw new CanonPatchError(
...
export function createCanonPatch(input: {
  ...
  if (!getNovel(input.novelId))
    throw new CanonPatchError('CANON_PATCH_NOVEL_NOT_FOUND', 'novel not found');
  if (!isNonEmptyString(input.baseFingerprint))
    throw new CanonPatchError('CANON_PATCH_INVALID_INPUT', 'base fingerprint is required');
  const operations = operationsFor(input.operations);
  const sourceCapabilityVersions = sourceCapabilityVersionsFor(
...
export function getCanonPatch(id: string, novelId: string): CanonPatch | undefined {
  const row = getDb()
    .prepare('SELECT * FROM canon_patches WHERE id = ? AND novel_id = ?')
    .get(id, novelId) as PatchRow | undefined;
...
export function listCanonPatches(novelId: string): CanonPatch[] {
  return (
    getDb()
      .prepare('SELECT * FROM canon_patches WHERE novel_id = ? ORDER BY created_at ASC, id ASC')
      .all(novelId) as PatchRow[]
  ).map(rowToPatch);
}
...
function applyPatch(novelId: string, patch: CanonPatch): PatchApplyResult {
  ...
  for (const operation of patch.operations) {
    ...
    if (operation.operation === 'create-master-outline') {
      ...
      if (!target)
        throw new CanonPatchError('CANON_PATCH_CONFLICT', 'target outline is not active');
...
function storedAcceptedResult(
  row: PatchRow
): Pick<PatchApplyResult, 'fingerprint' | 'artifacts' | 'acceptedOutlineRefs'> {
  ...
  return {
    ...
    artifacts:
      Array.isArray(stored.artifacts) && stored.artifacts.every((id) => typeof id === 'string')
        ? stored.artifacts
        : [],
    acceptedOutlineRefs: Array.isArray(stored.acceptedOutlineRefs)
      ? stored.acceptedOutlineRefs.filter(
          (ref): ref is CreativeArtifactRef =>
            isRecord(ref) &&
...
export async function acceptCanonPatch(
  ...
  const guarded = await runInSerializedWriteForGeneration(generation, () =>
    runInTransaction(() => {
      const row = getDb()
        .prepare('SELECT * FROM canon_patches WHERE id = ? AND novel_id = ?')
        .get(patchId, novelId) as PatchRow | undefined;
      ...
      if (row.status === 'accepted') {
        ...
        return {
          ...
          ...(stored.acceptedOutlineRefs.length
            ? { acceptedOutlineRefs: stored.acceptedOutlineRefs }
            : {}),
          changed: false,
        };
      }
      if (row.status !== 'pending')
        throw new CanonPatchError('CANON_PATCH_TERMINAL', 'patch is already terminal');
  ...
  if (!guarded.executed)
    throw new CanonPatchError('CANON_PATCH_GENERATION_STALE', 'database generation changed');
  ...
  return {
    ...
    ...('acceptedOutlineRefs' in guarded.result
      ? { acceptedOutlineRefs: guarded.result.acceptedOutlineRefs }
      : {}),
  };
}

export function rejectCanonPatch(
  novelId: string,
  patchId: string
): { status: CanonPatch['status'] } {
  const result = runInTransaction(() => {
    const row = getDb()
      .prepare('SELECT status FROM canon_patches WHERE id = ? AND novel_id = ?')
      .get(patchId, novelId) as { status: CanonPatch['status'] } | undefined;
    if (!row) throw new CanonPatchError('CANON_PATCH_NOT_FOUND', 'patch not found');

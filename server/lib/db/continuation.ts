...
import {
  mapContinuationPackRow,
  continuationPackToRow,
  computeContinuationPackContentHash,
} from '../db-mappers.js';
import { createCrudHelpers } from '../db-crud.js';

const continuationPackCrud = createCrudHelpers<
  ContinuationPack,
  ReturnType<typeof continuationPackToRow>
>({
...
export function updateContinuationPack(id: string, data: Partial<ContinuationPack>): boolean {
  ...
  if (
    existing &&
    !data.syncState &&
    (existing.syncState?.status === 'synced' || existing.syncState?.status === 'partial')
  ) {
    ...
    if (nextHash !== existing.syncState?.contentHash) {
      data = {
        ...data,
        syncState: { ...existing.syncState!, status: 'stale', contentHash: nextHash },
      };
    }
  }
...
export function updateContinuationPackSyncState(
  id: string,
  syncState: ContinuationPack['syncState']
): boolean {

...
function rowToSession(row: FlowRow): CreationFlowSession {
  ...
  if (
    ...
    !definition.steps.some((step) => step.id === row.current_step_id)
  ) {
...
function validateDraft(definition: CreationFlowDefinitionDraft): void {
  ...
  for (const step of definition.steps) {
    if (
      ...
      !isNonEmptyString(step.capabilityId) ||
      !Array.isArray(step.dependsOn) ||
      !step.dependsOn.every(isNonEmptyString) ||
      !Array.isArray(step.requiredArtifactKinds) ||
      !step.requiredArtifactKinds.every(isArtifactKind) ||
      !isArtifactKind(step.producedArtifactKind) ||
...
export function freezeCreationFlowDefinition(
  ...
  const steps = definition.steps.map((step) => {
    ...
    if (!manifest || manifest.runtimeStatus !== 'active') {
      throw new CreationFlowError(
        'CREATION_FLOW_CAPABILITY_NOT_FOUND',
        `capability is unavailable: ${step.capabilityId}`
      );
    }
    const contract = manifest.artifactContract;
    if (
      !manifest.usageModes?.includes('flow-step') ||
      !contract ||
      ...
      throw new CreationFlowError(
        'CREATION_FLOW_CAPABILITY_INCOMPATIBLE',
        `capability contract is incompatible with step: ${step.id}`
      );
    }
    return {
      id: step.id,
...
function activeArtifactRefs(novelId: string): CreativeArtifactRef[] {
  ...
  `
      ...
      .all(novelId) as Array<{ kind: unknown; id: unknown; version: unknown }>
  ).map(parseArtifactRef);

  const outlineKind = (level: string): CreativeArtifactKind | undefined =>
    level === 'master'
      ? 'master-outline'
      : level === 'volume'
        ? 'volume-outline'
...
function outlineMatchesFrozenStep(
  ...
  return (
    outline?.level === expectedLevel &&
    outline.status === 'active' &&
    outline.version === artifact.version &&
    Boolean(
      outline.sourceCapabilityVersions?.some(
        (version) =>
    ...
    (step) =>
      !refs.some(
        (ref) =>
          ...
          (ref.kind === 'world' ||
            ref.kind === 'character' ||
            outlineMatchesFrozenStep(novelId, step, ref))
      )
  )?.id;
}

export function getCreationFlowSession(
  novelId: string,
  sessionId: string
): CreationFlowSession | undefined {
  const row = getDb()
    .prepare(
      `
...
export function getCreationFlowReadiness(session: CreationFlowSession): CreationFlowReadiness {
  ...
  const missingArtifactKinds = [
    ...new Set([...dependencyKinds, ...step.requiredArtifactKinds]),
...
export function startCreationFlow(input: {
  ...
  const result = runInTransaction(() => {
    ...
    if (getActiveCreationFlowSession(input.novelId)) {
      throw new CreationFlowError(
        'CREATION_FLOW_ACTIVE_EXISTS',
        'novel already has an active flow session'
      );
    }
    ...
    `
      ...
      .run(
        ...
        currentStepId ?? null,
        JSON.stringify(acceptedOutputRefs),
        status,
...
function verifyAcceptedOutput(
  ...
  if (artifact.kind !== step.producedArtifactKind) {
    throw new CreationFlowError(
      'CREATION_FLOW_OUTPUT_KIND_MISMATCH',
      'accepted artifact kind does not match current step'
    );
  }
  if (
    artifact.kind === 'master-outline' ||
    ...
    if (!outlineMatchesFrozenStep(novelId, step, artifact)) {
      ...
      if (!outline || outline.status !== 'active' || outline.version !== artifact.version) {
        throw new CreationFlowError(
          'CREATION_FLOW_OUTPUT_NOT_ACCEPTED',
          'accepted outline version was not found'
        );
      }
      throw new CreationFlowError(
        'CREATION_FLOW_OUTPUT_CAPABILITY_MISMATCH',
        'outline was not accepted from the frozen capability version'
      );
    }
    return;
  }
  if (artifact.kind !== 'world' && artifact.kind !== 'character') {
    throw new CreationFlowError(
      'CREATION_FLOW_OUTPUT_NOT_ACCEPTED',
      'artifact authority cannot yet prove this accepted version'
    );
  }
  ...
  if (!stored || stored.version !== artifact.version) {
    throw new CreationFlowError(
      'CREATION_FLOW_OUTPUT_NOT_ACCEPTED',
      'accepted artifact version was not found'
    );
  }
  ...
  const matches =
    ...
    versions.some(
      (value) =>
        isRecord(value) &&
        value.capabilityId === step.capabilityId &&
        value.version === step.capabilityVersion
    );
  if (!matches) {
    throw new CreationFlowError(
      'CREATION_FLOW_OUTPUT_CAPABILITY_MISMATCH',
      'artifact was not accepted from the frozen capability version'
    );
  }
}
...
export function recordAcceptedFlowOutput(input: {
  ...
  if (
    !isArtifactKind(input.artifact?.kind) ||
    !isNonEmptyString(input.artifact?.id) ||
  ...
  const result = runInTransaction(() => {
    ...
    if (session.databaseGeneration !== input.databaseGeneration) {
      throw new CreationFlowError(
        'CREATION_FLOW_GENERATION_STALE',
        'flow session belongs to a different database generation'
      );
    }
    if (session.status !== 'active' || !session.currentStepId) {
      const alreadyRecorded = session.acceptedOutputRefs.some(
        (ref) =>
          ref.kind === input.artifact.kind &&
          ref.id === input.artifact.id &&
          ref.version === input.artifact.version
      );
    ...
    const step = session.definition.steps.find(
      (candidate) => candidate.id === session.currentStepId
    )!;
...
export function buildFlowMigrationCandidate(
  ...
  const changedCapabilities = proposedDefinition.steps.flatMap((step) => {
    ...
    if (
      !current ||
      current.capabilityId !== step.capabilityId ||
      current.capabilityVersion === step.capabilityVersion
    )
      return [];
    return [
      {

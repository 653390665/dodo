...
export class CreativeArtifactPersistenceError extends Error {
  constructor(
    public readonly code: CreativeArtifactPersistenceErrorCode,
    message: string
  ) {
    super(`${code}: ${message}`);
  }
...
const OPERATIONS: readonly ArtifactOperation[] = [
  'diagnose',
  'generate',
  'restructure',
  'optimize',
...
const CANDIDATE_STATUSES: readonly ArtifactCandidate['status'][] = [
  'pending',
  'accepted',
  'rejected',
  'stale',
];
const REVIEW_STATUSES: readonly ArtifactReviewRequirement['status'][] = [
  'review-required',
  'resolved',
];
...
function isCandidateStatus(value: unknown): value is ArtifactCandidate['status'] {
  return (
    typeof value === 'string' && CANDIDATE_STATUSES.includes(value as ArtifactCandidate['status'])
  );
}
...
function assertCandidateKind(
  value: unknown,
  stored = false
): asserts value is GenericCandidateKind {
...
function isArtifactRef(value: unknown, allowZero: boolean): value is CreativeArtifactRef {
  if (!isPlainObject(value) || !isArtifactKind(value.kind) || !isNonEmptyString(value.id))
    return false;
  return (
    Number.isInteger(value.version) &&
    (allowZero ? (value.version as number) >= 0 : (value.version as number) > 0)
  );
}

function isSourceCapabilityVersions(
  value: unknown
): value is ArtifactCandidate['sourceCapabilityVersions'] {
  return (
    Array.isArray(value) &&
    value.every(
      (item) =>
        isPlainObject(item) && isNonEmptyString(item.capabilityId) && isNonEmptyString(item.version)
    )
  );
}
...
function rowToCore(row: CoreRow): StoredArtifactCore {
  ...
  if (!Number.isInteger(row.version) || row.version <= 0)
    invalidData('stored core version is invalid');
  if (
    !isNonEmptyString(row.id) ||
    !isNonEmptyString(row.novel_id) ||
    !isNonEmptyString(row.artifact_id)
  ) {
    invalidData('stored core identity is invalid');
  }
  if (row.readable_content !== null && typeof row.readable_content !== 'string')
    invalidData('stored readable content is invalid');
  if (!Number.isInteger(row.created_at) || !Number.isInteger(row.updated_at))
    invalidData('stored core timestamps are invalid');
  return {
    id: row.id,
...
function rowToCandidate(row: CandidateRow): ArtifactCandidate {
  ...
  if (!Number.isInteger(row.target_version) || row.target_version < 0)
    invalidData('stored candidate version is invalid');
  ...
  if (typeof row.goal !== 'string' || typeof row.base_fingerprint !== 'string')
    invalidData('stored candidate text is invalid');
  if (row.proposed_content !== null && typeof row.proposed_content !== 'string')
    invalidData('stored candidate content is invalid');

  const sourceCapabilityVersions = parseJson(
    row.source_capability_versions,
    'stored source capability versions'
  );
  ...
  if (!isSourceCapabilityVersions(sourceCapabilityVersions))
    invalidData('stored source capability versions are invalid');
...
function rowToReviewRequirement(row: ReviewRow): ArtifactReviewRequirement {
  assertArtifactKind(row.artifact_kind, true);
  if (!Number.isInteger(row.artifact_version) || row.artifact_version <= 0)
    invalidData('stored review version is invalid');
  if (!REVIEW_STATUSES.includes(row.status as ArtifactReviewRequirement['status']))
    invalidData('stored review status is invalid');
  if (
    !isNonEmptyString(row.id) ||
    !isNonEmptyString(row.novel_id) ||
    !isNonEmptyString(row.artifact_id)
  ...
  if (!Number.isInteger(row.created_at) || !Number.isInteger(row.updated_at))
    invalidData('stored review timestamps are invalid');
  if (row.resolved_at !== null && !Number.isInteger(row.resolved_at))
    invalidData('stored review decision time is invalid');
  return {
    id: row.id,
    novelId: row.novel_id,
    artifactKind: row.artifact_kind,
...
export function getArtifactCore(
  ...
  if (!isNonEmptyString(novelId) || !isNonEmptyString(artifactId))
    invalidInput('artifact identity is required');
  const row = getDb()
    .prepare(
      `
...
export function saveArtifactVersion(input: {
  ...
  if (!isNonEmptyString(input.novelId) || !isNonEmptyString(input.artifactId))
    invalidInput('artifact identity is required');
  if (
  ...
  if (input.readableContent !== undefined && typeof input.readableContent !== 'string')
    invalidInput('readable content must be a string');
...
export function createArtifactCandidate<T>(
  ...
  if (
    ...
    !isNonEmptyString(input.novelId) ||
    !isNonEmptyString(input.target.id)
  ) {
    invalidInput('candidate identity is required');
  }
  if (!Number.isInteger(input.target.version) || input.target.version < 0)
    invalidInput('candidate target version is invalid');
  if (!isOperation(input.operation)) invalidInput('candidate operation is invalid');
  if (typeof input.goal !== 'string' || typeof input.baseFingerprint !== 'string')
    invalidInput('candidate text is invalid');
  if (input.proposedContent !== undefined && typeof input.proposedContent !== 'string')
    invalidInput('proposed content is invalid');
  if (!isSourceCapabilityVersions(input.sourceCapabilityVersions))
    invalidInput('source capability versions are invalid');
  ...
  const sourceCapabilityVersions = stringifyValue(
    input.sourceCapabilityVersions,
...
export function getArtifactCandidate(
  ...
  if (!isNonEmptyString(novelId) || !isNonEmptyString(candidateId))
    invalidInput('candidate identity is required');
  const row = getDb()
    .prepare(
      `
...
export function listArtifactCandidates(
  ...
  if (filters.status !== undefined && !isCandidateStatus(filters.status))
    invalidInput('candidate status is invalid');
...
export function applyArtifactCandidateDecision(
  ...
  if (!['accepted', 'rejected', 'stale'].includes(decision))
    invalidInput('candidate decision is invalid');
  const result = runInTransaction(() => {
    const row = getDb()
      .prepare(
        `
...
export function markArtifactReviewRequired(input: {
  ...
  if (!isNonEmptyString(input.novelId) || !isNonEmptyString(input.artifact.id))
    invalidInput('review identity is required');
  if (!Number.isInteger(input.artifact.version) || input.artifact.version <= 0)
    invalidInput('review artifact version is invalid');
  if (input.sourceCandidateId !== undefined && !isNonEmptyString(input.sourceCandidateId))
    invalidInput('source candidate identity is invalid');
  ...
  const result = runInTransaction(() => {
    ...
    const row = getDb()
      .prepare('SELECT * FROM artifact_review_requirements WHERE id = ?')
      .get(id) as ReviewRow;

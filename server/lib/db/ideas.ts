...
export function listIdeaFragments(novelId?: string): IdeaFragment[] {
  if (novelId) {
    return getDb()
      .prepare(
        'SELECT * FROM idea_fragments WHERE novel_id = ? OR novel_id IS NULL ORDER BY created_at DESC'
      )
      .all(novelId)
      .map(rowToIdeaFragment);
  }
...
function hydrateNarrativeCore(foreshadowing: Foreshadowing): Foreshadowing {
  ...
  return core && stored
    ? { ...foreshadowing, narrativeCore: core, coreVersion: stored.version }
    : foreshadowing;
}
...
export function saveNarrativePromiseCoreInTransaction(input: {
  ...
  if ((stored?.version ?? 0) !== (input.expectedVersion ?? 0))
    throw new Error('NARRATIVE_PROMISE_VERSION_STALE');
  const currentCore = normalizeNarrativePromiseCore(stored?.core);
  const plan = input.plan ?? currentCore?.plan;
  if (!plan || validateNarrativePromisePlan(plan).length)
    throw new Error('NARRATIVE_PROMISE_PLAN_INVALID');
  ...
  for (const item of input.evidenceToAppend || []) {
    ...
    if (
      !chapterId ||
      !quote ||
      !['plant', 'hint', 'payoff'].includes(item.action) ||

...
export function listLibraryMetadata(novelIds: string[]): LibraryMetadataBatch {
  ...
  const chapterRows = db
    .prepare(
      `SELECT id, novel_id, volume_name, title, "order", word_count, workflow_meta, created_at, updated_at FROM chapters WHERE novel_id IN (${placeholders}) ORDER BY novel_id, "order" ASC`
    )
    .all(...ids) as Array<Record<string, unknown>>;
  const packRows = db
    .prepare(
      `SELECT * FROM continuation_packs WHERE novel_id IN (${placeholders}) ORDER BY novel_id, updated_at DESC`
    )
    .all(...ids) as Array<Record<string, unknown>>;
  ...
  for (const row of chapterRows) {
    ...
    chapters[novelId]?.push({
      id: String(row.id),
      novelId,
      volumeName: row.volume_name ? String(row.volume_name) : undefined,
      title: String(row.title),
      order: Number(row.order),
      wordCount: Number(row.word_count),
      workflowMeta: (() => {
        try {
          return row.workflow_meta ? JSON.parse(String(row.workflow_meta)) : undefined;
        } catch {
          return undefined;
        }
      })(),

...
function validateReviewRequest(body: AuditRequestBody): string | undefined {
  if (
    body.reviewIssueIds !== undefined &&
    (body.reviewIssueIds.length > 200 ||
      body.reviewIssueIds.some((id) => typeof id !== 'string' || !id.trim()))
  )
    return 'invalid reviewIssueIds';
  if (body.reviewScope !== undefined && !['affected', 'full'].includes(body.reviewScope))
    return 'invalid reviewScope';
  if (
    body.reviewContentHash !== undefined &&
    (!/^[a-f0-9]{64}$/.test(body.reviewContentHash) ||
      body.reviewContentHash !==
        computeChapterWorkflowHash(body.draftContent || '', body.sceneBeats || ''))
  )
    return 'reviewContentHash mismatch';
  return undefined;
}
...
function formatAuditSkillsInfo(skillsInfo: string | undefined): string {
  ...
  const techniqueBodyBudget = Math.max(
    0,
    600 - headerText.length - Math.max(0, techniqueHeaders.length - 1) * 2
  );
  const bodyBudgetPerBlock =
    techniqueBlocks.length > 0 ? Math.floor(techniqueBodyBudget / techniqueBlocks.length) : 0;
  const techniqueInfo = techniqueBlocks
    .map((block, index) => {
      const header = techniqueHeaders[index];
      const body = block.slice(header.length).trim();
      if (!body || bodyBudgetPerBlock <= 0) return header;
      const allocation =
        index === techniqueBlocks.length - 1
...
async function runAuditJob(
  ...
  try {
    ...
    const trimmedContextStr = truncateForAudit(
      await buildServerStoryContextWithSemantic({
        novelId,
    ...
    if (body.reviewScope === 'affected') {
      ...
      const selectedIssues = (chapter?.workflowMeta?.reviewState?.issues || []).filter((issue) =>
      ...
      structuralSignals.length > 0
        ? `\n\n## 结构精修上下文要求\n以下每项均为局部证据窗口；不得扩展为整章重写或输出日志。\n${structuralSignals
            .map((hit) => {
              ...
              const end = Math.min(
                draftContent.length,
                hit.range?.end ?? start + hit.snippet.length
              );
              const targetStart = Math.min(start, Math.max(0, end - 600));
              const targetEnd = Math.min(
                draftContent.length,
                Math.max(targetStart, Math.min(end, targetStart + 600))
              );
              return buildSlopContextRewritePrompt({
                ...
                afterContext: draftContent.slice(
                  targetEnd,
                  Math.min(draftContent.length, targetEnd + 300)
                ),
      ...
      if (novelId) {
        ...
        if (novel) {
          ...
          const tagsAndTitle = [
            ...
            .toLowerCase();
          isCommercial =
            gov.commercialMode === 'strict' || /番茄|tomato|商业|爽文|新锐|无线/.test(tagsAndTitle);
        }
      }
...
在 JSON 顶层返回 evidence 数组；每项必须包含 category（仅 hard_canon、character_state、scene_execution、pacing、foreshadowing）、severity（low/medium/high）、quote（正文原文，无法引用则不要输出该项）、explanation、suggestedFix，可选 location。缺字段或未知 category 的证据会被丢弃。`;
    ...
    const rawFeedback = await generateText(
      getConfig(),
      {
        prompt:
          prompt +
          buildAuditResidueContract(draftContent) +
          openingDiagnosticsPrompt +
          extraDimensionPrompt +
          reviewScopePrompt +
          structuralRewritePrompt +
          evidenceContract +
          AUDIT_OUTPUT_CONTRACT,
        novelId,
    ...
    if (fiveDim && fiveDim.scores && typeof fiveDim.scores === 'object') {
      const gate = evaluateAuditGate(
        Object.fromEntries(
          Object.entries(fiveDim.scores).map(([k, v]) => [k, (v as { score: number }).score])
        ),
      ...
      if (slopReport) {
        const warningsText =
          slopReport.hits.length > 0
            ? slopReport.hits
                .map((h) => `- [${h.category}] 行 ${h.line}: ${h.suggestion || h.snippet}`)
                .join('\n')
            : '- 未检测到明显的机械腔调或套话。';
      ...
      if (!contract.valid || !residueCovered) {
        ...
        updateAuditJob(jobId, {
          ...
          result: {
            status: 'unknown',
            errorCategory:
              contract.violation || (residueCovered ? 'invalid_json' : 'missing_fatal_issues'),
            diagnostic: residueCovered
              ? '审稿 JSON 未通过结构化合同校验'
              : '正文残留未被 fatalIssues 覆盖',
            retriable: true,
            traceId: auditTraceId,
      ...
      const auditResult = {
        ...
        transport: transportMode,
        contextRewriteStatus: structuralSignals.some((hit) => hit.priority === 'P1')
          ? 'required'
          : 'not-required',
      };
      updateAuditJob(jobId, {
...
export function registerAuditRoutes(app: Express) {
  ...
  app.post('/api/audit/jobs/:jobId/cancel', (req, res) => {
    ...
    updateAuditJob(req.params.jobId, {
      status: 'failed',
      progress: 100,
      error: '审稿任务已取消。',
    });
  ...
  app.post('/api/audit', async (req, res) => {
    if (!rateLimit('audit'))
      return res.status(429).json({ error: '审稿请求过于频繁，请稍后再试。', retryAfter: 5 });
    ...
    if (reviewError)
      return res.status(400).json({ code: 'INVALID_REVIEW_REQUEST', error: reviewError });
    ...
    try {
      ...
      requireWritingStyleConfirmation(
        writingStyle,
        req.body.styleConfirmationFingerprint ?? req.body.writingStyleFingerprint
      );
    } catch (error) {
    ...
    if (
      ...
      (reserve.databaseGeneration !== undefined &&
        reserve.databaseGeneration !== databaseGeneration)
    ) {
  ...
  app.post('/api/rewrite', validate(rewriteSchema), async (req, res) => {
    ...
    if (
      ...
      !chapterId.trim() ||
      !Number.isInteger(requestedDatabaseGeneration) ||
      requestedDatabaseGeneration < 0
    ) {
      return res
        .status(400)
        .json({ code: 'SCOPED_CONTEXT_REQUIRED', error: '请先选择章节并刷新写作上下文。' });
    ...
    try {
      ...
      res.write(
        `data: ${JSON.stringify({ type: 'status', status: 'running', message: '正在生成精修预览…' })}\n\n`
      );
      ...
      if (!res.writableEnded) {
        res.write(
          `data: ${JSON.stringify({ error: GENERIC_CLIENT_ERROR, code: 'REWRITE_FAILED', retriable: true })}\n\n`
        );
        res.end();
      }
    } finally {

...
const operation = z.union([
  z
    .object({
      operation: z.literal('create-master-outline'),
  ...
  z
    .object({
      operation: z.literal('replace-outline'),
      targetArtifactId: id,
      content: z.string().min(1).max(1_000_000),
      core: core.optional(),
    })
  ...
  z
    .object({
      operation: z.literal('create-scoped-outline'),
      level: z.enum(['volume', 'chapter']),
      scope,
      content: z.string().min(1).max(1_000_000),
      core: core.optional(),
    })
...
function fail(res: import('express').Response, e: unknown) {
  ...
  if (e instanceof db.CanonPatchError) {
    const status = e.code.endsWith('NOT_FOUND')
      ? 404
      : e.code === 'CANON_PATCH_CONFLICT' ||
          e.code === 'CANON_PATCH_GENERATION_STALE' ||
          e.code === 'CANON_PATCH_INVALID_DATA' ||
          e.code === 'CANON_PATCH_STALE' ||
          e.code === 'CANON_PATCH_TERMINAL'
        ? 409
        : 400;
    return res.status(status).json({ code: e.code, error: e.message });
  }
  return res
    .status(500)
    .json({ code: 'CANON_PATCH_INTERNAL_ERROR', error: '大纲变更请求处理失败，请稍后重试。' });
}
export function registerCanonPatchRoutes(app: Express) {
  app.get('/api/novels/:novelId/canon-patches', (req, res) => {
    if (!id.safeParse(req.params.novelId).success)
      return res.status(400).json({
        code: 'CANON_PATCH_INVALID_INPUT',
        error: '大纲变更请求参数无效，请重新检查后再试。',
      });
    try {
      if (!db.getNovel(req.params.novelId))
        return res
          .status(404)
          .json({ code: 'CANON_PATCH_NOVEL_NOT_FOUND', error: '作品不存在，请刷新项目后重试。' });
      return res.json(db.listCanonPatches(req.params.novelId));
    } catch (e) {
      return fail(res, e);
    }
  });
  app.get('/api/novels/:novelId/canon-patches/:patchId', (req, res) => {
    if (!id.safeParse(req.params.novelId).success || !id.safeParse(req.params.patchId).success)
      return res.status(400).json({
        code: 'CANON_PATCH_INVALID_INPUT',
        error: '大纲变更请求参数无效，请重新检查后再试。',
      });
    try {
      const result = db.getCanonPatch(req.params.patchId, req.params.novelId);
      return result
        ? res.json(result)
        : res
            .status(404)
            .json({ code: 'CANON_PATCH_NOT_FOUND', error: '大纲变更不存在，请刷新后重试。' });
    } catch (e) {
      return fail(res, e);
    }
  });
  app.post('/api/novels/:novelId/canon-patches', async (req, res) => {
    ...
    try {
      ...
      const guarded = await runInSerializedWriteForGeneration(databaseGeneration, () =>
        db.createCanonPatch({ ...input, novelId: novel.data })
      );
      if (!guarded.executed)
        return res.status(409).json({
          code: 'CANON_PATCH_GENERATION_STALE',
          error: '数据库已变化，请刷新大纲变更后重试。',
        });
  ...
  for (const action of ['accept', 'reject'] as const)
    app.post(`/api/novels/:novelId/canon-patches/:patchId/${action}`, async (req, res) => {
      const generation = z
      ...
      if (
        ...
        !id.safeParse(req.params.patchId).success ||
        !generation.success
      )
        return res.status(400).json({
          code: 'CANON_PATCH_INVALID_INPUT',
        ...
        const guarded =
          action === 'accept'
            ? {
                ...
                result: await db.acceptCanonPatch(
                  req.params.novelId,
                  req.params.patchId,
                  generation.data.databaseGeneration
                ),
              }
            : await runInSerializedWriteForGeneration(generation.data.databaseGeneration, () =>
                db.rejectCanonPatch(req.params.novelId, req.params.patchId)
              );
        if (!guarded.executed)
          return res.status(409).json({
            code: 'CANON_PATCH_GENERATION_STALE',

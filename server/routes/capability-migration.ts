...
import {
  buildCapabilityMigrationPreview,
  mergeMigratedProfile,
  migrationProfileFingerprint,
  migrationProjectProfileFingerprint,
} from '../capabilities/migration.js';
...
function stale(res: Response, message = '迁移预览已过期，请重新预览') {
  return res.status(409).json({ code: 'CAPABILITY_MIGRATION_STALE', error: message });
}

export function registerCapabilityMigrationRoutes(app: Express): void {
  app.post(
    '/api/novels/:novelId/capabilities/migration/preview',
    ...
    (req, res) => {
      ...
      if (req.body.databaseGeneration !== getDatabaseGeneration())
        return stale(res, '数据库已变化，请刷新后重试');
      ...
      previews.set(previewToken, {
        ...
        projectProfileFingerprint: migrationProjectProfileFingerprint(
  ...
  app.post(
    [
      '/api/novels/:novelId/capabilities/migration/apply',
      '/api/novels/:novelId/capabilities/migration/confirm',
    ],
    ...
    async (req, res) => {
      ...
      if (
        previous &&
        previous.novelId === (req.params.novelId as string) &&
        previous.generation === req.body.databaseGeneration
      )
        return res.json({
          applied: true,
          idempotent: true,
          databaseGeneration: previous.generation,
      ...
      if (!currentBefore)
        return res.status(404).json({ code: 'NOVEL_NOT_FOUND', error: '作品不存在' });
      if (
        migrationProjectProfileFingerprint(currentBefore.projectPreferenceProfile) !==
        preview.projectProfileFingerprint
      ) {
        return stale(res, '项目配置已变化，请重新预览迁移');
      }
      const currentReceipt = (
        currentBefore.projectPreferenceProfile as Record<string, unknown> | undefined
      )?.migrationReceipt as { token?: string; generation?: number } | undefined;
      if (
        currentReceipt?.token === req.body.previewToken &&
        currentReceipt?.generation === req.body.databaseGeneration
      )
        return res.json({
          applied: true,
          idempotent: true,
          databaseGeneration: req.body.databaseGeneration,
          profile: currentBefore.projectPreferenceProfile,
        });
      const guarded = await runInSerializedWriteForGeneration(req.body.databaseGeneration, () => {
        const novel = getNovel(req.params.novelId as string);
        if (!novel) return undefined;
        if (
          migrationProjectProfileFingerprint(novel.projectPreferenceProfile) !==
          preview.projectProfileFingerprint
        )
          return 'STALE_CANDIDATE' as const;
        if (novel.projectPreferenceProfile?.capabilityModelVersion === 3)
          return novel.projectPreferenceProfile;
        ...
        if (
          migrationProfileFingerprint(recomputed.capabilityProfile) !==
          migrationProfileFingerprint(preview.capabilityProfile)
        )
          return 'STALE_CANDIDATE' as const;
        const migrated = mergeMigratedProfile(
          novel.projectPreferenceProfile,
          preview.capabilityProfile
        ) as Record<string, unknown>;
        validateCapabilityProfile(novel.id, migrated.capabilityProfile);
        migrated.migrationReceipt = {
          token: req.body.previewToken,
          generation: req.body.databaseGeneration,
        };
        updateNovel(req.params.novelId as string, {
          projectPreferenceProfile: migrated as unknown as ProjectPreferenceProfile,
        });
      ...
      if (!guarded.result)
        return res.status(404).json({ code: 'NOVEL_NOT_FOUND', error: '作品不存在' });
      applied.set(req.body.previewToken, {
        novelId: req.params.novelId as string,
        generation: req.body.databaseGeneration,
        profile: guarded.result,
      });

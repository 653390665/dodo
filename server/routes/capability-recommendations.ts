...
import {
  ...
  isCapabilityRecommendationDismissed,
} from '../lib/db/capability-recommendations.js';
...
const dismissalSchema = z
  .object({
    ...
    artifactVersion: z.union([z.string(), z.number()]),
    upstreamVersion: z.union([z.string(), z.number()]).optional(),
...
export function registerCapabilityRecommendationRoutes(app: Express): void {
  app.post('/api/capability-recommendations/dismiss', async (req, res) => {
    ...
    if (!parsed.success)
      return res
        .status(400)
        .json({ code: 'CAPABILITY_RECOMMENDATION_INVALID_INPUT', error: '推荐忽略参数无效' });
    ...
    if (input.databaseGeneration !== getDatabaseGeneration())
      return res.status(409).json({
        code: 'CAPABILITY_RECOMMENDATION_GENERATION_STALE',
        error: '数据库已变化，请刷新后重试',
      });
    if (canonicalFingerprint(input) !== input.fingerprint)
      return res
        .status(400)
        .json({ code: 'CAPABILITY_RECOMMENDATION_FINGERPRINT_MISMATCH', error: '推荐指纹不匹配' });
    if (!listCatalogCapabilityManifests().some((entry) => entry.id === input.capabilityId))
      return res
        .status(400)
        .json({ code: 'CAPABILITY_RECOMMENDATION_UNKNOWN_CAPABILITY', error: '推荐能力不存在' });
    const result = await runInSerializedWriteForGeneration(input.databaseGeneration, () => {
      dismissCapabilityRecommendation(input);
    });
    if (!result.executed)
      return res.status(409).json({
        code: 'CAPABILITY_RECOMMENDATION_GENERATION_STALE',

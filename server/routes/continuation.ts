...
const entityExtractionResumeContexts = new Map<
  string,
  {
    chunkIndex: number;
...
>();
const pendingContinuationImports = new Map<
  string,
  {
...
>();
const continuationImportSessions = new Map<
  string,
  {
    createdAt: number;
    databaseGeneration: number;
...
const extractEntitiesRequestSchema = z
  .object({
    packId: dbIdSchema,
...
const syncToWorldSchema = z.object({
  ...
  powerLevels: z
    ...
    .max(MAX_SYNC_POWER_LEVEL_COUNT)
    .default([]),
  timelineEvents: z
    .array(
      z.object({
  ...
  relationships: z
    ...
    .max(MAX_SYNC_RELATIONSHIP_COUNT)
    .default([]),
...
const extractionResultSchema = z.object({
  characters: z
    ...
    .max(MAX_EXTRACTION_ENTITY_COUNT)
    .default([]),
  locations: z
    .array(
      z.object({
...
function safeEntityExtractionJob(
  job: EntityExtractionJob
): Omit<EntityExtractionJob, 'completedResults' | 'splitCheckpoint' | 'lastActivityAt'> {
  const safeJob = { ...job };
...
function hydratePersistedExtractionJob(
  stored: ReturnType<typeof db.getContinuationExtractionJob>
): EntityExtractionJob | undefined {
  ...
  if (
    ...
    if (
      completedChunkIndexes !== undefined &&
      (!Array.isArray(completedChunkIndexes) ||
        completedChunkIndexes.some((index) => !Number.isInteger(index) || (index as number) < 0))
    ) {
    ...
    if (
      ...
      (!Array.isArray(completedResults) ||
        completedResults.some((item) => !extractionResultSchema.safeParse(item).success))
    ) {
    ...
    if (failedChunk !== undefined) {
      const failed =
        failedChunk !== null && typeof failedChunk === 'object' && !Array.isArray(failedChunk)
          ? (failedChunk as Record<string, unknown>)
          : undefined;
      if (
        !failed ||
        !Number.isInteger(failed.index) ||
        (failed.index as number) < 0 ||
        typeof failed.code !== 'string' ||
        !Number.isInteger(failed.attempt) ||
        (failed.attempt as number) < 1
      ) {
    ...
    if (
      chunkMeta !== undefined &&
      (!Array.isArray(chunkMeta) ||
        chunkMeta.some((item) => {
          if (!item || typeof item !== 'object' || Array.isArray(item)) return true;
          const meta = item as Record<string, unknown>;
          return (
            !Number.isInteger(meta.index) ||
            (meta.index as number) < 0 ||
            typeof meta.filename !== 'string' ||
            !Number.isInteger(meta.charCount) ||
            (meta.charCount as number) < 0 ||
            (meta.sha256 !== undefined && typeof meta.sha256 !== 'string')
          );
        }))
    ) {
  ...
  if (parsedResult) {
    ...
    if (
      ...
      !Number.isInteger(candidate.databaseGeneration) ||
      !extraction.success
    ) {
      ...
      result = {
        ...
        databaseGeneration: candidate.databaseGeneration as number,
  ...
  const job: EntityExtractionJob = {
    id: stored.id,
    status: protocolError ? 'failed' : stored.status,
    progress: stored.progress,
    stageText: protocolError ? '提取任务检查点损坏' : stored.stageText,
    createdAt: stored.createdAt,
    lastActivityAt: stored.updatedAt,
    databaseGeneration: stored.databaseGeneration,
    packId: stored.packId,
    novelId: stored.novelId,
    totalChunks: stored.totalBatches,
    currentChunk: stored.batchCursor,
    traceId: typeof checkpoint.traceId === 'string' ? checkpoint.traceId : undefined,
    completedResults: Array.isArray(checkpoint.completedResults)
      ? (checkpoint.completedResults as SyncExtractionResult[])
      : [],
    completedChunkIndexes: Array.isArray(checkpoint.completedChunkIndexes)
      ? (checkpoint.completedChunkIndexes as number[])
      : [],
    splitCheckpoint: checkpoint.splitCheckpoint as EntityExtractionJob['splitCheckpoint'],
    failedChunk: checkpoint.failedChunk as EntityExtractionJob['failedChunk'],
    schemaIssues: checkpoint.schemaIssues as EntityExtractionJob['schemaIssues'],
    warnings: checkpoint.warnings as string[],
    chunkMeta: checkpoint.chunkMeta as EntityExtractionJob['chunkMeta'],
    outputDiagnostic: checkpoint.outputDiagnostic as OutputDiagnostic,
    code: protocolError ? 'EXTRACTION_PROTOCOL_ERROR' : stored.errorCode,
    error: protocolError ? '提取任务检查点损坏，无法恢复，请从头重新提取' : stored.errorMessage,
    result,
  };
  if (protocolError) {
...
function buildProviderExtractionDiagnostic(error: ProviderError): OutputDiagnostic {
  return {
    ...
    responseFormatMode:
      error.compatibilityMode === 'plain_fallback' ? 'plain_fallback' : 'json_object',
...
function entityExtractionErrorMessage(code: string): string {
  return code === 'GENERATION_MISMATCH'
    ? '数据已变更，请刷新后重试'
    : code === 'EXTRACTION_CANCELLED'
      ? '提取已取消'
      : code === 'EXTRACTION_CHECKPOINT_PERSIST_FAILED'
        ? '提取进度保存失败，请从头重新提取'
        : code === 'EXTRACTION_CONFIG' || code === 'EXTRACTION_AUTH'
          ? '模型配置或鉴权不可用，请检查设置后重试'
          : code === 'EXTRACTION_TIMEOUT'
            ? '模型调用超时，请重试或拆分资料'
            : code === 'EXTRACTION_PROVIDER_PARAMETER'
              ? '模型服务拒绝当前 JSON 请求参数，请检查模型服务或设置'
              : code === 'EXTRACTION_RATE_LIMIT'
                ? '模型服务暂时限流，请稍后续跑'
                : code === 'EXTRACTION_SERVICE_UNAVAILABLE'
                  ? '模型服务暂时不可用，请稍后续跑'
                  : code === 'EXTRACTION_NETWORK'
                    ? '模型服务网络异常，请稍后续跑'
                    : code === 'EXTRACTION_EMPTY_RESPONSE'
                      ...
                      : code === 'EXTRACTION_INVALID_JSON'
                        ...
                        : code === 'EXTRACTION_SCHEMA_MISMATCH'
                          ? '模型返回字段不符合要求，可修复并重试本批'
                          : code === 'EXTRACTION_EMPTY_SEMANTIC_RESULT'
                            ? '模型未提取到可用设定，请检查资料内容'
                            : code === 'EXTRACTION_OUTPUT_TRUNCATED'
                              ? '模型输出被截断，请拆分资料后重试'
                              : '提取失败，请重试';
}

async function runPersistedExtractionJob(
  job: EntityExtractionJob,
  chunks: ReturnType<typeof buildSyncExtractionChunks>,
  resumeContext: {
    chunkIndex: number;
    splitAt?: number;
    leftResults?: SyncExtractionResult[];
  ...
  try {
    ...
    const execution = await createLlmExecution({
      ...
      novelId: job.novelId,
      timeoutMs: Math.max(120_000, chunks.length * 120_000),
      concurrency: 1,
    ...
    const result = await execution.run(async ({ signal }) => {
      ...
      for (const chunk of chunks) {
        ...
        const text =
          resumed && resumeContext.splitAt !== undefined
            ? chunk.text.slice(resumeContext.splitAt)
            : chunk.text;
        ...
        for (let attempt = 0; attempt < 2 && !parsed; attempt += 1) {
          try {
            const raw = await generateText(getConfig(), {
              prompt: buildSyncExtractionPrompt(
                [`【${chunk.filename}】\n[sourceDocumentId:${chunk.sourceDocumentId}]\n${text}`],
                {
                  repairIssues: resumed ? resumeContext.schemaIssues : undefined,
                  repairKind: resumed ? resumeContext.repairKind : undefined,
                }
              ),
              signal,
              timeoutMs: 90_000,
            ...
            const normalized = normalizeExtractionPayload(
              parseModelJsonPayloadStrict<unknown>(raw, { expectedRoot: 'object' })
            );
        ...
        if (resumed && resumeContext.leftResults && partials.length === 0)
          partials.push(...resumeContext.leftResults);
    ...
    job.code =
      error instanceof ProviderError
        ? extractionCodeForProviderError(error)
        : message === 'GENERATION_MISMATCH'
          ? 'GENERATION_MISMATCH'
          : message === 'EXTRACTION_SCHEMA_MISMATCH'
            ? 'EXTRACTION_SCHEMA_MISMATCH'
            : message === 'EXTRACTION_CHECKPOINT_PERSIST_FAILED'
              ? 'EXTRACTION_CHECKPOINT_PERSIST_FAILED'
              : 'EXTRACTION_FAILED';
    ...
    job.failedChunk = {
      ...
      providerRequestCount:
        error instanceof ProviderError
          ...
          : job.outputDiagnostic?.providerRequestCount,
    };
    ...
    extractionJobs.clearRuntimeHandles(job.id, job.status, (id) =>
...
function normalizeExtractionPayload(value: unknown): { value: unknown; warnings: string[] } {
  ...
  const normalizeTier = (input: unknown): unknown => {
    ...
    if (normalized !== value)
      warnings.push(`力量等级 tier「${value}」越界，已修正为 ${normalized}`);
    return normalized;
  };
  ...
  const normalizeRole = (input: unknown): unknown => {
    if (input == null) {
      warnings.push('人物 role 缺失，已降级为 supporting');
      return 'supporting';
    }
    if (typeof input !== 'string') {
      warnings.push('人物 role 类型无效，已降级为 supporting');
      return 'supporting';
    }
  ...
  const normalizeType = (input: unknown): unknown =>
    typeof input === 'string' ? entityTypeAliases[input.trim().toLowerCase()] || input : input;
  const normalizeItemType = (input: unknown): unknown => {
    if (input == null) {
      warnings.push('道具 type 缺失，已降级为 other');
      return 'other';
    }
    if (typeof input !== 'string') {
      warnings.push('道具 type 类型无效，已降级为 other');
      return 'other';
    }
  ...
  const normalizeOptionalString = (input: unknown): string =>
    input == null ? '' : stringValue(input);
  ...
  const withIdentity = (
    key: string,
    input: unknown,
    identity: 'name' | 'title' | 'sourceName' | 'targetName'
  ) =>
    asArray(input).flatMap((item, index) => {
  ...
  return {
    value: {
      ...record,
      characters: withIdentity('characters', record.characters, 'name').map((item) => ({
        ...item,
        role: normalizeRole(item.role),
        summary: normalizeOptionalString(item.summary),
        bio: normalizeOptionalString(item.bio),
        traits: normalizeStringArray(item.traits),
      })),
      locations: withIdentity('locations', record.locations, 'name').map((item) => ({
        ...item,
        region: normalizeOptionalString(item.region),
        description: normalizeOptionalString(item.description),
      })),
      items: withIdentity('items', record.items, 'name').map((item) => ({
        ...item,
        type: normalizeItemType(item.type),
        description: normalizeOptionalString(item.description),
      })),
      factions: withIdentity('factions', record.factions, 'name').map((item) => ({
        ...item,
        leader: normalizeOptionalString(item.leader),
        territory: normalizeOptionalString(item.territory),
        description: normalizeOptionalString(item.description),
      })),
      powerLevels: withIdentity('powerLevels', record.powerLevels, 'name').map((item) => ({
        ...item,
        tier: normalizeTier(item.tier),
        characteristics: normalizeOptionalString(item.characteristics),
        description: normalizeOptionalString(item.description),
      })),
      timelineEvents: withIdentity('timelineEvents', record.timelineEvents, 'title').map(
        (item) => ({
          ...item,
          timestamp: normalizeOptionalString(item.timestamp),
          description: normalizeOptionalString(item.description),
          order: numberOrOriginal(item.order),
        })
      ),
      relationships: asArray(record.relationships).flatMap((item, index) => {
        const itemRecord = asRecord(item);
        if (
          typeof itemRecord.sourceName !== 'string' ||
          !itemRecord.sourceName.trim() ||
          typeof itemRecord.targetName !== 'string' ||
          !itemRecord.targetName.trim()
        ) {
        ...
        return [
          {
            ...itemRecord,
            sourceType: normalizeType(itemRecord.sourceType),
            targetType: normalizeType(itemRecord.targetType),
            relationshipType: normalizeOptionalString(itemRecord.relationshipType),
            description: normalizeOptionalString(itemRecord.description),
          },
        ];
      }),
...
export function mapCanonFact(f: unknown, packId: string, i: number): ContinuationCanonFact {
  ...
  const category = ['world', 'character', 'plot', 'timeline', 'relationship', 'style'].includes(
...
export function mapReadingQuestion(
  q: unknown,
  packId: string,
  i: number
): ContinuationReadingQuestion {
  const r = asRecord(q);
  const category = ['world', 'character', 'plot', 'style', 'continuity'].includes(
    r.category as string
  )
...
export function registerContinuationRoutes(app: Express) {
  ...
  app.post('/api/parse-doc', validate(parseDocSchema), async (req, res) => {
    if (!rateLimit('parse-world-document')) {
      return res
        .status(429)
        .json({ error: '设定文档解析请求过于频繁，请稍后再试。', retryAfter: 5 });
    }
    pruneParseDocJobs();
  ...
  app.post('/api/continuation-packs/parse', validate(continuationParseSchema), async (req, res) => {
    ...
    try {
      if (!rateLimit('continuation-packs-parse')) {
        return res
          .status(429)
          .json({ error: '续写资料解析请求过于频繁，请稍后再试。', retryAfter: 5 });
      }
      const databaseGeneration = getDatabaseGeneration();
      ...
      if (!novelId.trim())
        return res.status(400).json({ error: '请先选择作品或重新开始资料导入。' });
      ...
      const shouldRetryWithShorterPrompt = (message: string) =>
        /only thinking\/reasoning content|empty response|可解析的 JSON|不完整的 JSON|LLM returned empty response/i.test(
          message
        );
      ...
      const pack = await execution.run(async ({ signal }) => {
        ...
        for (const attempt of promptAttempts) {
          try {
            ...
            if (
              !shouldRetryWithShorterPrompt(message) ||
              attempt === promptAttempts[promptAttempts.length - 1]
            ) {
              throw error;
            }
          }
        ...
        const nextPack: ContinuationPack = {
          ...
          canonFacts: asArray(parsedRecord.canonFacts).map((f, i: number) =>
            mapCanonFact(f, packId, i)
          ),
          characterStates: asArray(parsedRecord.characterStates).map(mapCharacterState),
          plotState: mapPlotState(
            parsedRecord.plotState ?? {
              currentTimeline: '',
              latestScene: '',
              unresolvedHooks: [],
              immediateConflict: '',
          ...
          styleProfile: mapStyleProfile(
            parsedRecord.styleProfile ?? {
              pov: '',
              tense: '',
              pacing: '',
              dialogueDensity: '',
          ...
          contradictions: asArray(parsedRecord.contradictions).map((c, i: number) =>
            mapContradiction(c, packId, i)
          ),
          sourceMap: mapSourceMap(parsedRecord.sourceMap ?? { sections: [], keyConflicts: [] }),
          readingQuestions: asArray(parsedRecord.readingQuestions).map((q, i: number) =>
            mapReadingQuestion(q, packId, i)
          ),
          continuationGaps: asArray(parsedRecord.continuationGaps).map((g, i: number) =>
            mapContinuationGap(g, packId, i)
          ),
      ...
      if (/质量校验|quality_rejected/i.test(message)) {
        return res
          .status(502)
          .json({ error: 'AI 输出未通过质量校验，请重试；若持续失败请减少资料文件数量。' });
      }
  ...
  app.post('/api/continuation-packs/approve-import', (req, res) => {
    ...
    if (!canApproveContinuationImportPack(resolvedPack)) {
      const error =
        resolvedPack.canonFacts.length === 0
          ? '资料包缺少可确认的事实，无法批准'
          : '资料包仍有未解决的高风险冲突，无法批准';
    ...
    try {
      ...
      db.runInTransaction(() => {
        ...
        if (pending) {
          ...
          !db.updateContinuationPack(sourcePack.id, {
  ...
  app.post('/api/continuation-packs/resolve-conflicts', async (req, res) => {
    ...
    try {
      const writeResult = await runInSerializedWriteForGeneration(input.databaseGeneration, () => {
        ...
        const resolvedPack = applyContinuationConflictResolutions(
          sourcePack,
      ...
      const message = error instanceof Error ? error.message : String(error);
      if (message === 'CONTINUATION_PACK_NOT_FOUND')
        return res.status(404).json({ error: '续写资料包不存在或已删除' });
      if (message === 'CONTINUATION_PACK_NOT_APPROVED')
        return res.status(409).json({ error: '仅已确认的续写资料包可以修改冲突裁决' });
      if (message === 'CONTINUATION_PACK_NOVEL_MISMATCH')
        return res.status(409).json({ error: '资料包不属于指定作品' });
  ...
  app.get('/api/continuation-packs/jobs/:jobId', async (req, res) => {
    ...
    if (!job)
      return res.status(404).json({ error: '提取任务不存在', code: 'EXTRACTION_JOB_NOT_FOUND' });
    if (job.databaseGeneration !== getDatabaseGeneration()) {
  ...
  app.post('/api/continuation-packs/jobs/:jobId/cancel', async (req, res) => {
    ...
    if (job.status === 'queued' || job.status === 'running') {
      extractionJobs.abortAndDetach(req.params.jobId, 'EXTRACTION_CANCELLED', (id) =>
        entityExtractionResumeContexts.delete(id)
      );
      job.status = 'failed';
  ...
  app.post('/api/continuation-packs/jobs/:jobId/resume', async (req, res) => {
    ...
    if (job.status === 'queued')
      return res
        .status(202)
        .json({ jobId: job.id, databaseGeneration: job.databaseGeneration, traceId: job.traceId });
    ...
    if (
      ...
      (job.status === 'failed' && !job.failedChunk) ||
      [
        'GENERATION_MISMATCH',
        'EXTRACTION_CANCELLED',
        'EXTRACTION_PROVIDER_PARAMETER',
        'EXTRACTION_PROTOCOL_ERROR',
        'EXTRACTION_CHECKPOINT_PERSIST_FAILED',
      ].includes(failedCode || '')
    ) {
      return res.status(409).json({
        error: '该任务没有可续跑的失败批次，请从头重新提取',
        code: 'EXTRACTION_RESUME_UNAVAILABLE',
        traceId: job.traceId,
      });
    }
    if (job.databaseGeneration !== getDatabaseGeneration()) {
    ...
    if (!pack)
      return res.status(404).json({ error: '资料包不存在', code: 'EXTRACTION_PACK_NOT_FOUND' });
    ...
    try {
      ...
      if (error instanceof SyncExtractionChunkLimitError)
        return res
          .status(413)
          .json({ error: error.message, code: error.code, chunkCount: error.chunkCount });
      return res.status(400).json({ error: '资料包无法重新分块' });
    }
    const expectedMeta = job.chunkMeta || [];
    const metaMatches =
      persistedChunks.length === job.totalChunks &&
      expectedMeta.length === persistedChunks.length &&
      expectedMeta.every((meta) => {
        const chunk = persistedChunks.find((item) => item.index === meta.index);
        return (
          chunk &&
          chunk.filename === meta.filename &&
          chunk.text.length === meta.charCount &&
    ...
    if (!metaMatches)
      return res
        .status(409)
        .json({ error: '资料包内容已变更，请重新提取', code: 'EXTRACTION_CHECKPOINT_MISMATCH' });
    const resumeContext = {
      chunkIndex: job.failedChunk?.index ?? Math.max(0, job.currentChunk - 1),
      schemaIssues: job.schemaIssues,
      repairKind:
        failedCode === 'EXTRACTION_SCHEMA_MISMATCH'
          ? 'schema'
          : ['EXTRACTION_INVALID_JSON', 'EXTRACTION_OUTPUT_TRUNCATED'].includes(failedCode || '')
            ? 'json_syntax'
            : undefined,
    ...
    if (!extractionJobs.rerun(req.params.jobId)) {
      ...
      extractionJobs.registerRerunner(job.id, () => {
        void runPersistedExtractionJob(
          job,
          persistedChunks!,
          context || { chunkIndex: job.failedChunk?.index ?? Math.max(0, job.currentChunk - 1) }
        );
      });
  ...
  app.post(
    '/api/continuation-packs/recommend-relationship-repairs',
    validate(relationshipRepairSchema),
    async (req, res) => {
      if (!rateLimit('continuation-packs-recommend-relationship-repairs')) {
        return res
          .status(429)
          .json({ error: '关系修复推荐请求过于频繁，请稍后再试。', retryAfter: 5 });
      }
      const { packId, novelId, databaseGeneration, relationships, candidates } =
        req.body as z.infer<typeof relationshipRepairSchema>;
      const currentGeneration = getDatabaseGeneration();
      if (databaseGeneration !== currentGeneration) {
        return res
          .status(409)
          .json({ error: '数据已变更，请刷新后重试', code: 'GENERATION_MISMATCH' });
      }
      const pack = db.getContinuationPack(packId);
      if (!pack) return res.status(404).json({ error: '资料包不存在' });
      if (pack.status !== 'approved')
        return res.status(400).json({ error: '仅已批准的资料包可以修复关系' });
      ...
      for (const relationship of typedRelationships) {
        evidenceByIndex[relationship.index] = extractRelationshipEvidence(
          relationship,
          pack.sourceDocuments
        );
      }
      const prompt = buildRelationshipRepairPrompt(
        typedRelationships,
        candidates as Record<RelationshipEntityType, string[]>,
        evidenceByIndex
      );
      const controller = new AbortController();
      const disposeDisconnect = bindClientDisconnect(req, res, () => controller.abort());
      try {
        ...
        const modelOutput = await execution.run(({ signal, traceId }) =>
        ...
        return res.json({
          recommendations: normalizeRelationshipRecommendations(
            typedRelationships,
            candidates as Record<RelationshipEntityType, string[]>,
            evidenceByIndex,
            modelOutput
  ...
  app.post('/api/continuation-packs/extract-entities', async (req, res) => {
    ...
    if (!rateLimit('continuation-packs-extract')) {
      return res
        .status(429)
        .json({ error: '资料设定提取请求过于频繁，请稍后再试。', retryAfter: 5 });
    }
    ...
    const job: EntityExtractionJob = {
      ...
      chunkMeta: chunks.map((chunk) => ({
        ...
        sha256: createHash('sha256').update(chunk.text).digest('hex'),
      })),
    };
      ...
      try {
        ...
        const result = await execution.run(async ({ signal }) => {
          ...
          const extractChunk = async (
            ...
            allowSplit: boolean,
            initialRepairIssues?: Array<{ path: string; code: string; message: string }>,
            initialRepairKind?: 'json_syntax' | 'schema'
          ): Promise<z.infer<typeof extractionResultSchema>[]> => {
            let lastError: unknown;
            let firstError: unknown;
            let repairIssues = initialRepairIssues;
            let repairKind: 'json_syntax' | 'schema' | undefined =
              initialRepairKind || (initialRepairIssues?.length ? 'schema' : undefined);
            let attemptCount = 0;
            ...
            for (let attempt = 0; attempt < attemptLimit; attempt += 1) {
              try {
                ...
                if (isEntityPayloadTooDense(normalized.value))
                  throw new Error('EXTRACTION_OUTPUT_TRUNCATED');
                const validated = extractionResultSchema.safeParse(normalized.value);
                if (!validated.success) {
                  const error = new Error('EXTRACTION_SCHEMA_MISMATCH') as Error & {
                    issues?: unknown;
                  };
                  error.issues = validated.error.issues;
                  repairIssues = validated.error.issues.slice(0, 20).map((issue) => ({
                ...
                job.warnings = [...(job.warnings || []), ...normalized.warnings].slice(0, 50);
                if (
                  validated.data.characters.length +
                    validated.data.locations.length +
                    validated.data.items.length +
                    validated.data.factions.length +
                    validated.data.powerLevels.length +
                    validated.data.timelineEvents.length +
                    validated.data.relationships.length ===
                    0 &&
                  !validated.data.globalOutline.trim() &&
                  !validated.data.worldRules.trim()
                ) {
                ...
                if (error instanceof Error)
                  (error as Error & { attempt?: number }).attempt = attemptCount;
                lastError = error;
                if (error instanceof ModelJsonSyntaxError) repairKind = 'json_syntax';
                if (signal.aborted) throw error;
                if (error instanceof ProviderError) {
                  (error as ProviderError & { previousError?: unknown }).previousError =
                    firstError === error ? undefined : firstError;
                  throw error;
                }
            ...
            const code =
              lastError instanceof ModelJsonTruncatedError
                ? 'EXTRACTION_OUTPUT_TRUNCATED'
                : lastError instanceof ModelJsonSyntaxError
            ...
            if (
              ...
              job.splitCheckpoint = {
                chunkIndex: job.currentChunk - 1,
                splitAt: midpoint,
                leftResults: left,
              };
              try {
                const right = await extractChunk(
                  text.slice(midpoint),
                  filename,
                  sourceDocumentId,
                  false
            ...
            if (lastError instanceof Error)
              (lastError as Error & { attempt?: number }).attempt = attemptCount;
            throw lastError instanceof Error ? lastError : new Error('EXTRACTION_INVALID_JSON');
          };
          for (const chunk of chunks) {
            if (job.completedChunkIndexes?.includes(chunk.index)) continue;
            if (signal.aborted || databaseGeneration !== getDatabaseGeneration())
              throw new Error(
                databaseGeneration !== getDatabaseGeneration()
                  ? 'GENERATION_MISMATCH'
                  : 'EXTRACTION_CANCELLED'
            ...
            const resumedText =
              isResumedChunk && resumeContext.splitAt !== undefined
                ? chunk.text.slice(resumeContext.splitAt)
                : chunk.text;
            partials.push(
              ...(await extractChunk(
                ...
                !isResumedChunk,
                isResumedChunk ? resumeContext.schemaIssues : undefined,
                isResumedChunk ? resumeContext.repairKind : undefined
              ))
            );
            job.completedResults = partials.slice();
            job.completedChunkIndexes = [...(job.completedChunkIndexes || []), chunk.index];
        ...
        const message = error instanceof Error ? error.message : 'EXTRACTION_FAILED';
        const code =
          error instanceof LlmExecutionRejectedError
            ? error.quota.code === 'RATE_LIMITED'
              ? 'EXTRACTION_RATE_LIMIT'
              : error.quota.code === 'DATABASE_CHANGED'
                ? 'GENERATION_MISMATCH'
                : error.quota.code === 'QUOTA_EXCEEDED'
                  ? 'EXTRACTION_QUOTA'
                  : 'EXTRACTION_FAILED'
            : generationChanged || message === 'GENERATION_MISMATCH'
              ? 'GENERATION_MISMATCH'
              : message === 'EXTRACTION_CANCELLED' || controller.signal.aborted
                ? 'EXTRACTION_CANCELLED'
                : message === 'EXTRACTION_CHECKPOINT_PERSIST_FAILED'
                  ? 'EXTRACTION_CHECKPOINT_PERSIST_FAILED'
                  : error instanceof ModelJsonTruncatedError ||
                      message === 'EXTRACTION_OUTPUT_TRUNCATED'
                    ? 'EXTRACTION_OUTPUT_TRUNCATED'
                    : error instanceof ModelJsonSyntaxError ||
                        message === '模型返回的内容不是有效 JSON。' ||
                        message === '模型未返回可解析的 JSON。'
                      ? 'EXTRACTION_INVALID_JSON'
                      : message === 'EXTRACTION_SCHEMA_MISMATCH'
                        ? 'EXTRACTION_SCHEMA_MISMATCH'
                        : message === 'EXTRACTION_EMPTY_SEMANTIC_RESULT'
                          ? 'EXTRACTION_EMPTY_SEMANTIC_RESULT'
                          : error instanceof ProviderError
                            ? (
                                {
                                  configuration: 'EXTRACTION_CONFIG',
                                  ...
                                  empty_response: 'EXTRACTION_EMPTY_RESPONSE',
                                  quality_rejected: 'EXTRACTION_QUALITY_REJECTED',
                                } as Record<ProviderErrorCode, string>
                              )[error.code]
                            : /api key|config/i.test(message)
                              ? 'EXTRACTION_CONFIG'
        ...
        const parserDiagnostic =
          error instanceof ModelJsonSyntaxError || error instanceof ModelJsonTruncatedError
            ? error.diagnostic
            : undefined;
        if (error instanceof ProviderError) {
          job.outputDiagnostic = buildProviderExtractionDiagnostic(error);
        ...
        job.failedChunk = {
          ...
          attempt:
            ...
            typeof (error as Error & { attempt?: number }).attempt === 'number'
        ...
        if (!job.schemaIssues?.length && previousError?.issues) {
          job.schemaIssues = previousError.issues.slice(0, 20).map((issue) => ({
            path: issue.path.join('.'),
            code: issue.code,
            message: issue.message,
          }));
                      ...
                      : code === 'EXTRACTION_RATE_LIMIT'
                        ...
                        : code === 'EXTRACTION_SERVICE_UNAVAILABLE'
                          ...
                          : code === 'EXTRACTION_NETWORK'
                            ...
                            : code === 'EXTRACTION_EMPTY_RESPONSE'
                              ? '模型返回空结果，请稍后续跑'
                              : code === 'EXTRACTION_QUALITY_REJECTED'
                                ? '模型输出未通过质量校验，请重试'
                                : code === 'EXTRACTION_INVALID_JSON'
                                  ? '模型返回结果无法解析，请重试'
                                  : code === 'EXTRACTION_SCHEMA_MISMATCH'
        ...
        extractionJobs.clearRuntimeHandles(jobId, job.status, (id) =>
          entityExtractionResumeContexts.delete(id)
        );
      }
    };
    extractionJobs.registerRerunner(jobId, () => {
  ...
  app.post('/api/continuation-packs/sync-to-world', async (req, res) => {
    if (!rateLimit('continuation-packs-sync')) {
      return res
        .status(429)
        .json({ error: '同步到世界观请求过于频繁，请稍后再试。', retryAfter: 5 });
    }
    ...
    if (!parsed.success) {
      return res
        .status(400)
        .json({ error: '参数校验失败', details: parsed.error.flatten().fieldErrors });
    }
    ...
    try {
      const syncResult = await runInSerializedWriteForGeneration(databaseGeneration, () => {
        return sqliteDb.transaction(() => {
          ...
          const existingPowerLevelNames = new Set(
            existingPowerLevels.map((p) => normalizeName(p.name))
          );
          const existingTimelineTitles = new Set(
            existingTimelineEvents.map((t) => normalizeName(t.title))
          );
          ...
          if (globalOutline && !novel.globalOutline) {
            sqliteDb
              .prepare('UPDATE novels SET global_outline = ? WHERE id = ?')
              .run(globalOutline, novelId);
          }
          if (worldRules && !novel.worldRules) {
            sqliteDb
              .prepare('UPDATE novels SET world_rules = ? WHERE id = ?')
              .run(worldRules, novelId);
          }

          for (const c of characters || []) {
            const key = normalizeName(c.name);
            if (existingCharNames.has(key) || seenCharNames.has(key)) {
              skipped.characters++;
              continue;
            }
          ...
          for (const l of locations || []) {
            ...
            if (existingLocNames.has(key) || seenLocNames.has(key)) {
              skipped.locations++;
              continue;
            }
            ...
            sqliteDb
              .prepare(
                'INSERT INTO locations (id, novel_id, name, region, description, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)'
              )
              .run(id, novelId, l.name, l.region || '', l.description || '', now, now);
          ...
          for (const i of items || []) {
            ...
            if (existingItemNames.has(key) || seenItemNames.has(key)) {
              skipped.items++;
              continue;
            }
            ...
            sqliteDb
              .prepare(
                'INSERT INTO items (id, novel_id, name, type, description, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)'
              )
              .run(id, novelId, i.name, i.type || 'other', i.description || '', now, now);
          ...
          for (const f of factions || []) {
            ...
            if (existingFactionNames.has(key) || seenFactionNames.has(key)) {
              skipped.factions++;
              continue;
            }
            ...
            sqliteDb
              .prepare(
                'INSERT INTO factions (id, novel_id, name, leader, territory, description, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
              )
              .run(
                id,
                novelId,
          ...
          for (const p of powerLevels || []) {
            ...
            sqliteDb
              .prepare(
                'INSERT INTO power_levels (id, novel_id, name, tier, characteristics, description, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
              )
              .run(
                id,
                novelId,
          ...
          for (const t of timelineEvents || []) {
            ...
            sqliteDb
              .prepare(
                'INSERT INTO timeline_events (id, novel_id, title, description, timestamp, status_tag, "order", created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)'
              )
              .run(
                id,
                novelId,
          ...
          const existingRels = sqliteDb
            .prepare(
              'SELECT sourceType, sourceId, targetType, targetId, relationshipType FROM entity_relationships WHERE novelId = ?'
            )
            .all(novelId) as {
            sourceType: string;
            sourceId: string;
          ...
          for (const rel of existingRels) {
            seenRelKeys.add(
              `${rel.sourceType}:${rel.sourceId}:${rel.targetType}:${rel.targetId}:${rel.relationshipType || ''}`
            );
          }
          ...
          sqliteDb
            .prepare('UPDATE continuation_packs SET sync_state = ?, updated_at = ? WHERE id = ?')
            .run(JSON.stringify(syncState), now, packId);
          const sourceDocumentIds = new Set(pack.sourceDocuments.map((document) => document.id));
          const pendingCharacterFacts = characters
            .map((character) => {
              const requestedSourceIds = [
                ...new Set(character.sourceDocumentIds.filter((id) => sourceDocumentIds.has(id))),
              ];
                ...
                .filter(
                  (fact) =>
                    ...
                    (fact.text.includes(character.name) ||
                      (fact.sourceDocumentId !== undefined &&
                        requestedSourceIds.includes(fact.sourceDocumentId)))
                )
                .map((fact) => ({
                  id: fact.id,
                  text: fact.text,
              ...
              const fields = [
                character.summary
                  ? {
                      ...
                      sourceDocumentIds: requestedSourceIds,
                    }
                  : undefined,
                character.bio
                  ? { path: 'bio', value: character.bio, sourceDocumentIds: requestedSourceIds }
                  : undefined,
      ...
      if (error instanceof Error) {
        if (error.message === 'PACK_NOT_FOUND')
          return res.status(404).json({ error: '资料包不存在' });
        if (error.message === 'PACK_NOT_APPROVED')
          return res.status(400).json({ error: '仅已批准的资料包可以同步' });
        if (error.message === 'PACK_NOVEL_MISMATCH')
          return res.status(403).json({ error: '资料包不属于当前作品' });

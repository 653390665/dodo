...
const startBody = z
  .object({ definition, databaseGeneration: z.number().int().nonnegative() })
  .strict();
const outputBody = z
  .object({
...
function errorResponse(res: Response, error: unknown) {
  if (error instanceof CreationFlowError) {
    const status =
      error.code.endsWith('NOT_FOUND') || error.code === 'CREATION_FLOW_NOVEL_NOT_FOUND'
        ? 404
        : error.code.includes('STALE') ||
            error.code.includes('ACTIVE_EXISTS') ||
            error.code.includes('TERMINAL') ||
            error.code.includes('PREREQUISITES') ||
            error.code.includes('MISMATCH') ||
            error.code.includes('NOT_ACCEPTED')
          ? 409
          : 400;
    return res.status(status).json({
      code: error.code,
      error: error.message,
      ...(error.missingArtifactKinds ? { missingArtifactKinds: error.missingArtifactKinds } : {}),
    });
  }
  return res
    .status(500)
    .json({ code: 'CREATION_FLOW_INTERNAL_ERROR', error: '创作流程请求处理失败，请稍后重试。' });
}
...
export function registerCreationFlowRoutes(app: Express): void {
  app.post('/api/novels/:novelId/creation-flows/start', async (req, res) => {
    ...
    if (!novelId.success || !parsed.success)
      return res
        .status(400)
        .json({ code: 'CREATION_FLOW_INVALID_INPUT', error: '创作流程参数无效。' });
    try {
      ...
      if (!guarded.executed)
        return res
          .status(409)
          .json({ code: 'CREATION_FLOW_GENERATION_STALE', error: '数据库已变化，请刷新后重试。' });
  ...
  app.get('/api/novels/:novelId/creation-flows/active', (req, res) => {
    ...
    if (!novelId.success || !generation.success)
      return res
        .status(400)
        .json({ code: 'CREATION_FLOW_INVALID_INPUT', error: '创作流程参数无效。' });
    if (generation.data !== getDatabaseGeneration())
      return res
        .status(409)
        .json({ code: 'CREATION_FLOW_GENERATION_STALE', error: '数据库已变化，请刷新后重试。' });
    try {
      ...
      return session
        ? res.json({ ...session, readiness: getCreationFlowReadiness(session) })
        : res
            .status(404)
            .json({ code: 'CREATION_FLOW_NOT_FOUND', error: '没有活动中的创作流程。' });
    } catch (error) {
      return errorResponse(res, error);
    }
  });
  ...
  app.get('/api/novels/:novelId/creation-flows/:sessionId', (req, res) => {
    ...
    if (!novelId.success || !sessionId.success)
      return res
        .status(400)
        .json({ code: 'CREATION_FLOW_INVALID_INPUT', error: '创作流程参数无效。' });
    try {
      ...
      return session
        ? res.json({ ...session, readiness: getCreationFlowReadiness(session) })
        : res.status(404).json({ code: 'CREATION_FLOW_NOT_FOUND', error: '创作流程不存在。' });
    } catch (error) {
      return errorResponse(res, error);
    }
  });
  ...
  app.post('/api/novels/:novelId/creation-flows/:sessionId/outputs/accept', async (req, res) => {
    ...
    if (!novelId.success || !sessionId.success || !parsed.success)
      return res
        .status(400)
        .json({ code: 'CREATION_FLOW_INVALID_INPUT', error: '创作流程参数无效。' });
    try {
      const guarded = await runInSerializedWriteForGeneration(parsed.data.databaseGeneration, () =>
        recordAcceptedFlowOutput({
          novelId: novelId.data,
  ...
  app.post('/api/novels/:novelId/creation-flows/:sessionId/migration/preview', (req, res) => {
    ...
    try {
      const session = getCreationFlowSession(novelId.data, sessionId.data);
      if (!session)
        return res.status(404).json({ code: 'CREATION_FLOW_NOT_FOUND', error: '创作流程不存在。' });
      const proposed = freezeCreationFlowDefinition(
        parsed.data.definition as CreationFlowDefinitionDraft
      );

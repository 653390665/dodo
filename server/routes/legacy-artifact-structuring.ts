...
function invalid(res: Response) {
  return res
    .status(400)
    .json({ code: 'LEGACY_ARTIFACT_INVALID_INPUT', error: '旧产物整理请求参数无效。' });
}
...
export function registerLegacyArtifactStructuringRoutes(app: Express) {
  app.get('/api/novels/:novelId/legacy-artifacts', (req, res) => {
    ...
    if (!db.getNovel(novelId.data)) {
      return res
        .status(404)
        .json({ code: 'LEGACY_ARTIFACT_NOVEL_NOT_FOUND', error: '作品不存在。' });
    }
    return res.json({
  ...
  app.post('/api/novels/:novelId/legacy-artifacts/preview', async (req, res) => {
    ...
    const source = listLegacyArtifactSources(novelId.data).find(
      (candidate) =>
    ...
    if (!source) {
      return res
        .status(404)
        .json({ code: 'LEGACY_ARTIFACT_SOURCE_NOT_FOUND', error: '旧产物不存在或已经结构化。' });
    }
    ...
    try {
      ...
      try {
        ...
        return res
          .status(422)
          .json({ code: 'LEGACY_ARTIFACT_PREVIEW_INVALID', error: '模型未返回有效的结构化结果。' });
      }
      ...
      if (error instanceof Error && error.message.includes('数据库')) {
        return conflict(
          res,
          'LEGACY_ARTIFACT_GENERATION_STALE',
          '数据库已切换，请重新检查旧产物。'
        );
      }
      return res.status(500).json({
        code: 'LEGACY_ARTIFACT_PREVIEW_FAILED',
        error: '结构化预览生成失败，请稍后重试。',
      });
    }
  });
  ...
  app.post('/api/novels/:novelId/legacy-artifacts/confirm', async (req, res) => {
    ...
    if (!preview || preview.source.novelId !== novelId.data) {
      return conflict(
        res,
        'LEGACY_ARTIFACT_PREVIEW_UNAVAILABLE',
        '预览已过期或不属于当前作品，请重新生成。'
      );
    }
    try {
      ...
      if (
        ...
        return conflict(
          res,
          'LEGACY_ARTIFACT_CONFIRM_STALE',
          '原产物或数据库已变化，请重新生成预览。'
        );
      }
      return conflict(
        res,
        'LEGACY_ARTIFACT_CONFIRM_FAILED',
        '结构化版本未能确认，原产物保持不变。'
      );
    }
  });

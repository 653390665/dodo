...
const bodySchema = z
  ...
  .extend({
    ...
    source: z.enum(['user', 'continuation-pack', 'ai-proposal']).optional(),
  })
  .strict();
...
function error(res: import('express').Response, e: unknown) {
  ...
  return res
    .status(500)
    .json({ code: 'OUTLINE_INTERNAL_ERROR', error: '大纲请求处理失败，请稍后重试。' });
}
  ...
  app.get('/api/novels/:novelId/outlines', (req, res) => {
    ...
    try {
      if (query.data.generation !== undefined && query.data.generation !== getDatabaseGeneration())
        return res
          .status(409)
          .json({ code: 'OUTLINE_GENERATION_STALE', error: '数据库已变化，请刷新大纲后重试。' });
      if (!db.getNovel(parsed.data))
        return res
          .status(404)
          .json({ code: 'OUTLINE_NOT_FOUND', error: '作品不存在，请刷新项目后重试。' });
  ...
  app.post('/api/novels/:novelId/outlines', async (req, res) => {
    ...
    try {
      if (!db.getNovel(novel.data))
        return res
          .status(404)
          .json({ code: 'OUTLINE_NOVEL_NOT_FOUND', error: '作品不存在，请刷新项目后重试。' });
      ...
      const guarded = await runInSerializedWriteForGeneration(databaseGeneration, () =>
        db.createOutlineArtifact({ ...input, novelId: novel.data, source: input.source || 'user' })
      );
      if (!guarded.executed)
        return res
          .status(409)

...
export function registerProductEventRoutes(app: Express) {
  ...
  app.post('/api/product-events', validate(productEventSchema), (req, res) => {
    try {
      return res.status(201).json(createProductEvent(req.body));
    } catch {
  ...
  app.get('/api/product-events/export', (req, res) => {
    ...
    try {
      res
        .attachment('inkflow-product-events.json')
        .type('application/json')
        .send(JSON.stringify(listProductEvents(parsed.data ?? 90)));
    } catch {
      res.status(500).json({ error: 'Failed to export product events' });
    }
  });
  app.delete('/api/product-events', (_req, res) => {
    try {
      ...
      return res.status(500).json({ error: 'Failed to clear product events' });
    }
  });
}

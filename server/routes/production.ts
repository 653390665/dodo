...
import { buildStoryStateLedger } from '../../shared/lib/story-state-ledger';
...
import {
  ...
  DraftQualityRejectionError,
} from '../helpers/ai-production-pipeline';
...
import {
  evaluateDraftAcceptance,
  resolveEffectiveMinDraftChars,
  semanticReviewFromContinuityReport,
...
import type {
  ...
  ChapterProductionRunVersionSource,
} from '../../shared/types';
import {
...
function productionVersionHash(
  snapshot: Pick<
    ChapterProductionRunVersion,
    'sceneBeats' | 'draftContent' | 'styleAudit' | 'continuityReport'
  >
): string {
...
function createProductionVersion(
  ...
  const version: ChapterProductionRunVersion = {
    ...
    draftContent: run.draftContent,
    styleAudit: run.styleAudit,
    continuityReport: run.continuityReport,
    contentHash: productionVersionHash(run),
    createdAt: Date.now(),
  };
...
function attachReviewVersion(
  run: ChapterProductionRun | undefined
): ChapterProductionRun | undefined {
  if (!run) return run;
  ...
  return version
    ? {
        ...
        reviewVersionHash: version.contentHash,
        reviewVersionSource: version.source,
      }
    : run;
}
...
function findProductionTargetChapterOwnershipIssue(
  ...
  if (chapter.novelId !== novelId)
    return { status: 409, message: '目标章节不属于当前作品，请返回项目后重试。' };
  return undefined;
}
...
function assertApplyOwnership(run: ReturnType<typeof db.getChapterProductionRun>): void {
  ...
  for (const entry of patch.foreshadowingsToCreate || []) {
    if (entry.plantedChapterId) {
      assertOwnedEntity(
        db.getChapter(entry.plantedChapterId),
        run.novelId,
        'Foreshadowing planted chapter'
      );
    }
  }
}

function buildProductionContinuityReport(
...
function deriveProductionSemanticReview(
  run: Pick<ChapterProductionRun, 'styleAudit' | 'continuityReport'>
) {
  ...
  if (parsed.fiveDim)
    return semanticReviewFromStructuredAudit(convertFiveDimToStructured(parsed.fiveDim));
...
function initializeProductionRun(
  ...
  const ownershipIssue = findProductionTargetChapterOwnershipIssue(novelId, targetChapterId);
  ...
  const characters = db
    ...
    .filter(
      (c) => !activeEntityNames || activeEntityNames.includes(c.name) || c.role === 'protagonist'
    );
  const locations = db
    .listLocations(novelId)
    .filter((l) => !activeEntityNames || activeEntityNames.includes(l.name));
  const items = db
    .listItems(novelId)
    .filter((i) => !activeEntityNames || activeEntityNames.includes(i.name));
  const factions = db
    .listFactions(novelId)
    .filter((f) => !activeEntityNames || activeEntityNames.includes(f.name));
  const powerLevels = db
    .listPowerLevels(novelId)
    .filter((p) => !activeEntityNames || activeEntityNames.includes(p.name));
  ...
  const targetChapterBaselineHash = targetChapterId
    ? (() => {
        ...
        return targetChapter
          ? computeChapterWorkflowHash(targetChapter.content, targetChapter.sceneBeats)
          : undefined;
      })()
    : undefined;
  ...
  db.createChapterProductionRun({
    ...
    styleAudit: '',
    continuityReport: {
      ...buildProductionContinuityReport(continuationPackId, targetChapterBaselineHash),
      databaseGeneration,
      contextReceipt,
      executionReceipt,
    },
...
export function registerProductionRoutes(app: Express) {
  app.post(
    ...
    validate(chapterProductionSchema),
    async (req, res) => {
      ...
      try {
        ...
        runEvolutionReflexion(
          novelId,
          requestDatabaseGeneration,
          writingStyle.executionSnapshot
        ).catch((err) => logger.error('Reflexion background task error:', err));
        ...
        const fallbackDraft = buildFallbackDraft(
          fallbackBeats,
          writerContext,
          resolveEffectiveMinDraftChars(intent)
        );
        const fallbackQuality = validateCompleteChapterDraftQuality(fallbackDraft, undefined, {
          minChars: resolveEffectiveMinDraftChars(intent),
        });
        ...
        const qualityFailure =
          e instanceof Error && message.startsWith('DRAFT_QUALITY_GATE_FAILED:')
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      try {
        ...
        const isTestEnv =
          process.env.NODE_ENV === 'test' || process.env.PLAYWRIGHT_TEST || !isLlmConfigured();
        if (!fallbackQuality.ok) {
        ...
        const stageContexts = {
          ...
          critic: [initialization.result.criticContext, characterStateStr, contractStr]
            .filter(Boolean)
            .join('\n\n'),
        };
        const learnedContext =
          learnedPreferences.length > 0
            ? '\n\n【学习到的偏好 — 基于你之前的修改习惯】\n' +
              learnedPreferences
                .map((item) => `- ${item.pattern}（可信度：${Math.round(item.confidence * 100)}%）`)
                .join('\n')
            : '';
        ...
        const stagePromptSources = (['planner', 'writer', 'critic'] as const).map((stage) => {
          ...
          const overlayCount = executionSnapshot.overlays.filter((overlay) =>
            overlay.stages.includes(stage)
          ).length;
          const guardrailCount = executionSnapshot.guardrails.filter(
        ...
        const finalContextReceipt = finalizeContextReceipt(contextReceipt, receiptActualText, [
          {
            ...
            label: '资料包上下文',
            text: packContext,
            itemCount: contextReceipt?.itemCount || 0,
            version: contextReceipt?.packUpdatedAt
              ? String(contextReceipt.packUpdatedAt)
              : undefined,
          },
          {
            id: 'planner',
            label: '规划上下文',
            text: plannerContext,
            itemCount: plannerContext ? 1 : 0,
            version: 'planner-v1',
          },
          {
            id: 'character-state',
            label: '角色当前状态',
            text: characterStateStr,
            itemCount: characterStates ? characterStates.split('\n').length : 0,
            version: 'character-state-v1',
          },
          {
            id: 'contract',
            label: '作品写作契约',
          ...
          ...stagePromptSources,
          {
            ...
            text: learnedPreferences
              .map((item) => `- ${item.pattern}（可信度：${Math.round(item.confidence * 100)}%）`)
              .join('\n'),
            itemCount: learnedPreferences.length,
        ...
        runProductionPipeline({
          ...
          progress: {
            ...
            onWriterToken: (chunk) => {
              if (isResponseWritable(res))
                sseWrite(res, { type: 'model_draft_token', content: chunk });
            },
            onWriterDone: () => {
                ...
                if (currentRun && currentRun.status !== 'applied') {
                  db.updateChapterProductionRun(runId!, {
                    ...
                    continuityReport: {
                      ...
                      degradation: { beatsSource: result.beatsSource, draftSource: result.source },
                    },
                    status: 'review_required',
                  });
          ...
          .catch(async (err) => {
            ...
            if (!fallbackPersisted) {
              ...
              const violations =
                qualityFailure && err instanceof Error
                  ? err.message
        ...
        if (runId) {
          try {
            ...
            logger.error('Decision record failed:', e);
          }
        }
        if (!contentDelivered) {
          await refundQuota(reservationId);
  ...
  app.post('/api/chapter-production-runs/:runId/fact-candidate/preview', (req, res) => {
    try {
      ...
      if (typeof novelId !== 'string' || !Number.isInteger(databaseGeneration)) {
        return res
          .status(400)
          .json({ code: 'CHAPTER_FACT_INVALID_INPUT', error: '候选预览参数无效' });
      }
      return res.json(
        previewChapterFactCandidate({
          novelId,
  ...
  app.post('/api/chapter-production-runs/:runId/fact-candidate/apply', async (req, res) => {
    try {
      ...
      if (
        ...
        !Number.isInteger(body.databaseGeneration) ||
        typeof body.candidateId !== 'string' ||
        typeof body.manuscriptContentHash !== 'string' ||
        ...
        (body.factDecisions !== undefined &&
          (!body.factDecisions ||
            typeof body.factDecisions !== 'object' ||
            Array.isArray(body.factDecisions)))
      ) {
        return res
          .status(400)
          .json({ code: 'CHAPTER_FACT_INVALID_INPUT', error: '事实确认参数无效' });
      }
      const result = await applyChapterFactCandidate({
        ...
        manuscriptContentHash: body.manuscriptContentHash,
  ...
  app.post(
    '/api/chapter-production-runs/:runId/apply',
    validate(chapterProductionApplySchema),
    async (req, res) => {
      try {
        ...
        if (run.novelId !== novelId || run.targetChapterId !== requestedChapterId) {
          return res.status(409).json({
            ...
            error: '生成任务与当前章节不匹配，请刷新后重试。',
          });
        }
        if (run.continuityReport.databaseGeneration !== databaseGeneration) {
          return res.status(409).json({
            code: 'PRODUCTION_RUN_GENERATION_MISMATCH',
            error: '生成任务来自旧版本数据库，请刷新后重试。',
          });
        }
        ...
        const canApplyRun =
          run.status === 'review_required' || (run.status === 'failed' && hasReviewableDraft);
        if (!canApplyRun) {
        ...
        if (requestedVersionId !== undefined && requestedVersionHash !== undefined) {
          ...
          if (
            ...
            version.novelId !== run.novelId ||
            version.targetChapterId !== run.targetChapterId
          ) {
            return res.status(409).json({ error: '所选正文版本不属于当前生成任务，请重新选择。' });
          }
          if (version.source === 'fallback') {
            return res.status(409).json({
              code: FALLBACK_REVIEW_REQUIRED_CODE,
              error: FALLBACK_REVIEW_REQUIRED_ERROR,
              retriable: true,
            });
          }
          ...
          if (
            version.contentHash !== String(requestedVersionHash) ||
            recomputedHash !== version.contentHash
          ) {
            return res.status(409).json({ error: '所选正文版本已变化，请重新确认。' });
          }
          applyRun = {
            ...run,
            sceneBeats: version.sceneBeats,
            draftContent: version.draftContent,
            styleAudit: version.styleAudit,
            continuityReport: version.continuityReport,
          };
        }
        if (applyRun.continuityReport.databaseGeneration !== databaseGeneration) {
          return res.status(409).json({
            code: 'PRODUCTION_RUN_GENERATION_MISMATCH',
            error: '正文版本来自旧版本数据库，请刷新后重试。',
          });
        }
        ...
        const guarded = await runInSerializedWriteForGeneration(databaseGeneration, () => {
          ...
          if (existingChapter && baselineHash) {
            const currentHash = computeChapterWorkflowHash(
              existingChapter.content,
              existingChapter.sceneBeats
            );
            if (currentHash !== baselineHash) {
              throw new ProductionDomainConflict(
                '目标章节在生成期间已被修改，预览已失效，请重新生成。'
              );
            }
          }
          const reviewContentHash = computeChapterWorkflowHash(
            applyRun.draftContent,
            applyRun.sceneBeats
          );
          const reviewIssues = continuityToReviewIssues(
            applyRun.continuityReport,
            reviewContentHash
          );
          assertApplyOwnership(applyRun);
          ...
          const workflowMeta = {
            ...
            reviewState: {
              ...
              issues: reviewIssues,
              gate:
                acceptance.status === 'risk-accepted'
                  ? 'accepted-risk'
                  : deriveReviewGate(reviewIssues, resolvedAuditStatus),
              lastReviewedAt: now,
              semanticReview,
            },
          ...
          db.runInTransaction(() => {
            ...
            if (chapterId && db.getChapter(chapterId)) {
              db.updateChapter(chapterId, {
                ...
                critique:
                  auditStatus === 'unknown' || auditStatus === 'not_run'
                    ? db.getChapter(chapterId)?.critique || ''
                    : applyRun.styleAudit,
              ...
              db.createChapter({
                ...
                sceneBeats: applyRun.sceneBeats,
                critique:
                  auditStatus === 'unknown' || auditStatus === 'not_run' ? '' : applyRun.styleAudit,
                workflowMeta,
                createdAt: now,
                updatedAt: now,
              });
        ...
        res.json({
          ...
          narrativePromiseCandidates:
            applyRun.continuityReport.proposedPatch.narrativePromiseCandidates || [],
        });
      } catch (e) {
...
async function runEvolutionReflexion(
  ...
  try {
    ...
    const appliedRuns = db
      .listChapterProductionRuns(novelId)
...
只输出 JSON，不要有任何解释。`;
    ...
    if (parsed && (Array.isArray(parsed.bannedWords) || Array.isArray(parsed.rules))) {
      const rules = Array.isArray(parsed.rules)
        ? parsed.rules.filter(
            (rule: unknown): rule is string => typeof rule === 'string' && Boolean(rule.trim())
          )
        : [];

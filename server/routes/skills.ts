...
const SKILL_EXTRACTION_LLM_OPTIONS = {
  timeoutMs: 35_000,
  maxAttempts: 1,
  // Structured extraction payloads must parse as JSON; headroom + thinking off
  // keeps reasoning-heavy providers from truncating the payload mid-card.
  maxTokens: 4000,
...
async function processModelSkillExtraction(
  ...
  for (const segment of modelSegments) {
    try {
      if (signal?.aborted) {
        throw signal.reason || new Error('拆书任务已中断，请重新提交。');
      }
      ...
      if (deconstructSkillsInfo) {
        prompt = `${prompt}\n\n===== 🚨 黄金拆书规约：强制结合以下用户当前指定的【拆书解构指导卡】进行针对性萃取与分析 =====\n${deconstructSkillsInfo}\n你必须根据以上拆书卡所强调的重点（如特定的节奏、悬念、高潮点）来观察当前 analysis 素材，并在拆出的能力卡组中强力落实以上拆书卡的解构要求！`;
      }
  ...
  const skills = [deck.mainCard, ...deck.supportCards].filter(Boolean).map((skill, index) => {
    const candidate = skill as { id?: string; sourceCardId?: string };
    const s = {
      ...skill,
      id: candidate.id || candidate.sourceCardId || `deck-skill-${index + 1}`,
    };
    return finalizeExtractedCard(s, `deck-skill-${index + 1}`);
  });
...
export function registerSkillsRoutes(app: Express) {
  app.post('/api/extract-skill', validate(extractSkillSchema), async (req, res) => {
    ...
    try {
      ...
      logger.error('Skill extraction request failed', {
      ...
      if (/JSON|可解析的 JSON|不完整的 JSON/.test(message)) {
        return res.status(502).json({
          error: '拆书失败：模型返回格式不稳定，暂时未能解析为能力卡。',
        });
      }
      return res.status(500).json(SKILL_EXTRACTION_FAILURE);
    }
  });
  ...
  app.get('/api/extract-skill/jobs/:jobId', (req, res) => {
    ...
    if (!job) {
      return res.status(404).json({ error: '拆书任务不存在或已过期，请重新提交。' });
    }

...
import {
  validate,
  writingStyleConfirmSchema,
  writingStyleResolveSchema,
  capabilityConfigurationPreviewSchema,
  capabilityConfigurationApplySchema,
} from '../validation.js';
...
const configurationPreviews = new Map<
  string,
  {
    novelId: string;
    generation: number;
  ...
  try {
    ...
    if (error instanceof WritingStyleRequestError) {
      return res.status(error.status).json({
        error: error.message,
        code: error.code,
        ...(error.sessionCardId ? { sessionCardId: error.sessionCardId } : {}),
      });
    }
...
export function registerWritingStyleRoutes(app: Express): void {
  app.post(
    '/api/novels/:novelId/capabilities/configuration/preview',
    validate(capabilityConfigurationPreviewSchema),
    (req, res) => {
      const novel = db.getNovel(req.params.novelId as string);
      if (!novel) return res.status(404).json({ code: 'NOVEL_NOT_FOUND', error: '作品不存在' });
      if (req.body.databaseGeneration !== getDatabaseGeneration())
        return res
          .status(409)
          .json({ code: 'DATABASE_GENERATION_MISMATCH', error: '数据库已变化，请刷新后重试' });
      try {
  ...
  app.post(
    '/api/novels/:novelId/capabilities/configuration/apply',
    validate(capabilityConfigurationApplySchema),
    async (req, res) => {
      const novel = db.getNovel(req.params.novelId as string);
      if (!novel) return res.status(404).json({ code: 'NOVEL_NOT_FOUND', error: '作品不存在' });
      ...
      if (
        ...
        preview.novelId !== (req.params.novelId as string) ||
        preview.generation !== req.body.databaseGeneration ||
        req.body.databaseGeneration !== getDatabaseGeneration()
      ) {
        return res.status(409).json({
          code: 'CAPABILITY_CONFIGURATION_STALE',
          error: '能力配置预览已过期，请重新预览',
        });
      }
      if (preview.claimed) {
        return res.status(409).json({
          code: 'CAPABILITY_CONFIGURATION_APPLY_IN_PROGRESS',
          error: '能力配置正在应用，请勿重复提交',
        });
      }
      ...
      try {
        ...
        for (const step of packageSteps) {
          if (stepIds.has(step.stepId))
            return res
              .status(400)
              .json({ code: 'CAPABILITY_PACKAGE_STEP_DUPLICATE', error: '能力包步骤重复' });
          stepIds.add(step.stepId);
        }
        for (const step of packageSteps) {
          if ((step.dependsOn || []).some((dependencyId: string) => !stepIds.has(dependencyId))) {
            return res.status(400).json({
              code: 'CAPABILITY_PACKAGE_DEPENDENCY_MISSING',
              error: '能力包前置组件未选择',
            });
          }
        }
        if (JSON.stringify(preview.profile) !== JSON.stringify(requestedProfile))
          return res.status(409).json({
            code: 'CAPABILITY_CONFIGURATION_CHANGED',
            error: '能力配置已变化，请重新预览',
          });
        ...
        const guarded = await runInSerializedWriteForGeneration(req.body.databaseGeneration, () => {
          return runInTransaction(() => {
            ...
            const scheduledTechniques = packageSteps.filter(
              (step: CapabilityPackageStep) => step.mode === 'schedule' && step.scope === 'chapter'
            );
            const validScheduledTechniques = scheduledTechniques.map(
              (step: CapabilityPackageStep) => {
            ...
            const targetChapter = req.body.targetChapterId
              ? db.getChapter(req.body.targetChapterId)
              : undefined;
            if (
              req.body.targetChapterId &&
              (!targetChapter || targetChapter.novelId !== (req.params.novelId as string))
            ) {
              throw new WritingStyleRequestError(
                400,
                'CAPABILITY_TARGET_CHAPTER_INVALID',
                '目标章节不属于当前作品'
              );
            }
            if (targetChapter && validScheduledTechniques.length > 0) {
              ...
              const techniqueIds = [
                ...new Set([
                  ...
                  ...validScheduledTechniques.map(
                    ({ step }: { step: CapabilityPackageStep }) => step.assetId
                  ),
                ]),
              ];
              const techniqueVersions = { ...(existing?.techniqueVersions || {}) };
              for (const { step, manifest } of validScheduledTechniques)
                techniqueVersions[step.assetId] = manifest.version;
              db.updateChapter(targetChapter.id, {
                workflowMeta: {
                  ...
                  capabilityState: {
                    ...
                    ...(existing?.overlayVersions
                      ? { overlayVersions: existing.overlayVersions }
                      : {}),
                    updatedAt: Date.now(),
                  },
                },
            ...
            return targetChapter
              ? new Set(
                  validScheduledTechniques.map(
                    ({ step }: { step: CapabilityPackageStep }) => step.stepId
                  )
                )
              : new Set<string>();
          });
        ...
        const items: CapabilityApplicationItemResult[] = (req.body.packageSteps || []).map(
          (step: CapabilityPackageStep) => ({
            ...
            status:
              step.mode === 'configure'
                ? 'configured'
                : scheduledStepIds.has(step.stepId)
                  ? 'scheduled'
                  : step.mode === 'schedule' && step.scope === 'chapter'
                    ? 'skipped'
                    : 'recommended',
            ...(step.mode === 'configure' || (step.mode === 'schedule' && step.scope === 'chapter')
              ? {}
              : { reason: '当前版本不会自动安排或运行该能力，请在写作流程中手动触发。' }),
          })
        );
        return res.json({
          applied: true,
        ...
        if (!consumed && configurationPreviews.get(req.body.previewToken) === preview)
          preview.claimed = false;
      }
    }
  );
  app.post(
    '/api/novels/:novelId/writing-style/resolve',
    validate(writingStyleResolveSchema),
    (req, res) => handle(req, res, false)
  );
  app.post(
    '/api/novels/:novelId/writing-style/confirm',
    validate(writingStyleConfirmSchema),

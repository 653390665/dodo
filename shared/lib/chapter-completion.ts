...
export type ChapterCompletionPhase =
...
export interface ChapterCompletionInput {
  ...
  aiStatus?: 'pass' | 'fail' | 'unknown' | 'unavailable';
  planHash?: string;
}
...
export interface CompleteChapterInput extends Partial<ChapterCompletionInput> {
  novelId: string;
  chapterId: string;
  databaseGeneration: number;
  retryUnavailable?: boolean;
}
export interface AcceptChapterRiskInput {
  unresolvedIssueIds: string[];
  unknownChecks: string[];
  contentHash: string;
  planHash: string;
  authorDecisionAt?: number;
}
export interface ChapterCompletionResult {
  quality: ChapterCompletionQuality;
  gate: ChapterCompletionGate;
...
export function deriveChapterCompletionGate(input: ChapterCompletionInput): ChapterCompletionGate {
  ...
  const review =
    input.reviewState && input.reviewState.contentHash === contentHash
      ? input.reviewState
      : undefined;
  const reviewIssues =
    review?.issues
      .filter(
        (issue) =>
          ['open', 'previewed', 'applied'].includes(issue.status) &&
          ['critical', 'major', 'high'].includes(issue.severity)
      )
      .map((issue) => issue.id) || [];
  ...
  if (input.aiStatus === 'pass' || review?.gate === 'pass')
    quality = allIssues.length ? 'needs-action' : 'pass';
  if (input.aiStatus === 'fail' || review?.gate === 'needs-action') quality = 'needs-action';
  const aiUnknown =
    input.aiStatus === 'unknown' ||
    input.aiStatus === 'unavailable' ||
    (!input.aiStatus && !review);
  if (aiUnknown && allIssues.length === 0) quality = 'unknown';
  const reviewIssuesHaveEvidence = Boolean(
    review?.issues.some(
      (issue) =>
        allIssues.includes(issue.id) &&
        Boolean(issue.snippet?.trim()) &&
        Boolean(issue.suggestedFix?.trim())
    )
  );
  const completionGate: CompletionGate =
    quality === 'pass' ? 'ready' : quality === 'needs-action' ? 'needs-action' : 'review-required';
  return {
    contentHash,
    planHash,
    quality,
...
export function completeChapter(input: CompleteChapterInput): ChapterCompletionResult {
  const gate = deriveChapterCompletionGate({
...
export function acceptChapterRisk(input: AcceptChapterRiskInput): ChapterCompletionResult {
  const gate: ChapterCompletionGate = {
    ...
    planHash: input.planHash,
    quality: 'unknown',
    completionGate: 'accepted-risk',
    deterministicIssues: input.unresolvedIssueIds,
...
export function chapterCompletionHashes(
  chapter: Pick<Chapter, 'content' | 'sceneBeats'>,
  planHash?: string
): { contentHash: string; planHash: string } {
  return {
    contentHash: computeChapterWorkflowHash(chapter.content, chapter.sceneBeats),
    planHash: planHash || computeChapterWorkflowHash(chapter.sceneBeats || ''),
  };
}

...
function digest(input: string): string {
  ...
  new DataView(data.buffer).setUint32(total - 4, bitLen, false);
  let a = 0x6a09e667,
    b = 0xbb67ae85,
    c = 0x3c6ef372,
    d = 0xa54ff53a,
  ...
  for (let off = 0; off < total; off += 64) {
    for (let i = 0; i < 16; i++) words[i] = new DataView(data.buffer).getUint32(off + i * 4, false);
    for (let i = 16; i < 64; i++) {
      const s0 = ro(words[i - 15], 7) ^ ro(words[i - 15], 18) ^ (words[i - 15] >>> 3),
        s1 = ro(words[i - 2], 17) ^ ro(words[i - 2], 19) ^ (words[i - 2] >>> 10);
    ...
    let A = a,
      ...
      H = h;
    for (let i = 0; i < 64; i++) {
      const S1 = ro(E, 6) ^ ro(E, 11) ^ ro(E, 25),
        ch = (E & F) ^ (~E & G),
        t1 = (H + S1 + ch + k[i] + words[i]) >>> 0,
        S0 = ro(A, 2) ^ ro(A, 13) ^ ro(A, 22),
        maj = (A & B) ^ (A & C) ^ (B & C),
        t2 = (S0 + maj) >>> 0;
      H = G;
      G = F;
...
export function buildSourceMapContext(pack: ContinuationPack): string {
  ...
  const sections = map.sections
    ...
    .join('\n');
  const conflicts = map.keyConflicts
    .slice(0, 5)
    .map((c) => `- ${c}`)
    .join('\n');
  return [
...
export function buildContinuationContext(
  ...
  const characters = (pack.characterStates || [])
    ...
    .map(
      (item) =>
        `- ${item.name}：目标=${item.currentGoal}；情绪=${item.emotionalState}；关系=${(item.relationshipNotes || []).join('、')}`
    )
    .join('\n');
  const hooks = (pack.plotState?.unresolvedHooks || [])
    .slice(0, 10)
    .map((hook) => `- ${hook}`)
    .join('\n');
...
export function buildContinuationContextBundle(
  pack: ContinuationPack,
  options:
    | number
    | {
  ...
  const packFull = buildContinuationContext(pack, {
    includeStyle: typeof options === 'number' ? true : options.includeStyle,
  });
  const uniqueRuntimeSources = runtimeSources.filter(
    (source, index, list) => list.findIndex((candidate) => candidate.id === source.id) === index
  );
  const runtimeSegments = uniqueRuntimeSources.map(
    (source) => `【${source.label}】\n${source.text}`
  );
  ...
  const sources = [
    {
      ...
      label: '资料包',
      sha256: digest(text.slice(0, Math.min(packFull.length, text.length))),
      chars: Math.min(packFull.length, text.length),
      itemCount: text.length >= packFull.length ? sourceIds.length : 0,
      truncated: packFull.length > text.length,
      version: String(pack.updatedAt),
    },
    ...uniqueRuntimeSources.map((source, index) => {
      const start =
        packFull.length +
        (runtimeText ? 2 : 0) +
        runtimeSegments.slice(0, index).reduce((sum, value) => sum + value.length + 2, 0);
      const segmentLength = runtimeSegments[index].length;
      const chars = Math.max(0, Math.min(segmentLength, text.length - start));
      return {
        id: source.id,
        label: source.label,
        sha256: digest(text.slice(start, start + chars)),
        chars,
        itemCount: chars === segmentLength ? source.itemCount || 0 : 0,
        truncated: chars < segmentLength,
        ...(source.version ? { version: source.version } : {}),
      };
    }),
  ].filter(
    (source, index, list) =>
      source.chars > 0 && list.findIndex((candidate) => candidate.id === source.id) === index
  );
  return {
    text,
    receipt: {
      ...
      sourceIds: text.length >= packFull.length ? sourceIds : [],
      sources,
      runtimeSha256: digest(text),
      injectedChars: text.length,
      itemCount: sources.reduce((sum, source) => sum + source.itemCount, 0),
      truncated: text.length < full.length,
      packTruncatedCount: Math.max(0, packFull.length - Math.min(packFull.length, text.length)),
      packCharLimit: maxChars,
    },
  };
}

export function finalizeContextReceipt(
  ...
  const sources = [...existing, ...additions].filter(
    (source, index, list) => list.findIndex((candidate) => candidate.id === source.id) === index
  );
  return {
    ...receipt,
    actual: receipt.actual === true,
    sources,
    runtimeSha256: digest(actualText),
    injectedChars: actualText.length,
    itemCount: sources.reduce((sum, source) => sum + source.itemCount, 0),
    truncated: receipt.truncated || sources.some((source) => source.truncated),
  };
}

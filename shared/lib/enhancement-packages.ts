...
export const ENHANCEMENT_PACKAGES: EnhancementPackage[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        order: 2,
        required: false,
        dependsOn: ['free-basic-humanization-before'],
      },
    ],
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        order: 2,
        required: true,
        dependsOn: ['free-onboarding-world'],
      },
    ],
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        order: 2,
        required: false,
        dependsOn: ['free-first-chapter-outline'],
      },
    ],
  },
  {
    ...
    whyUpgrade:
      '您的创作已经初具规模，此时升级名家流程包可以像金牌主编一样进行多轮、系统化的步骤推荐，彻底打通人设到黄金正文的连贯叙事脉络！',
    assets: [
    ...
    steps: [
      ...
      id: `paid-author-flows-${index + 1}`,
      assetId,
      mode: 'recommend' as const,
      trigger: 'project-setup' as const,
  ...
  },
  {
    ...
    whyUpgrade:
      '多平台发书面临完全不同的审核红线与爆款节奏。升级爆款平台诊断包可以对您的前十章大纲进行深度“X光”式爆点质检，大幅提高内投签约成功率！',
  ...
  },
  {
    ...
    whyUpgrade:
      '大纲虽好，生成时却极其容易“提前泄底”或节奏透支。跨章连贯性增强包能在后台为您守死真相锁、锁定伏笔并在后续章节匀速释放冲突！',
  ...
  },
  {
    ...
    whyUpgrade:
      '想学习顶流神作的遣词造句？文风融合包会辅助提炼主笔文风、节奏与钩子候选卡；保存后由作者选择卡组位置并确认融合。',
    assets: [
    ...
    steps: [
      ...
      id: `paid-deconstruction-fusion-${index + 1}`,
      assetId,
      mode: 'recommend' as const,
      trigger: 'project-setup' as const,
      scope: 'project' as const,
  ...
  },
  {
    ...
    whyUpgrade:
      '基础的禁词替换已经无法满足细节质感。升级局部手术包，您可以针对指定段落进行站桩说话净化，动态追加肢体表情微动作，让场景像电影般生动！',
    assets: ['de-ai-slop-shield', 'de-ai-rhythm-restorer', 'prose-action-booster'],
    version: '1',
    intendedOutcome: '在审查后生成局部精修预览，由作者确认应用',
    steps: ['de-ai-slop-shield', 'de-ai-rhythm-restorer', 'prose-action-booster'].map(
      (assetId, index) => ({
        id: `paid-advanced-audit-patch-${index + 1}`,
        assetId,
        mode: 'run-now' as const,
        trigger: 'after-draft' as const,
...
export function getEnhancementPackageSteps(
  pkg: EnhancementPackage
): readonly EnhancementPackageStep[] {
  if (pkg.steps?.length) return pkg.steps;
  return (pkg.assets || []).map((assetId, index) => ({
    id: `${pkg.id}-step-${index + 1}`,
    assetId,
    mode: 'recommend' as const,

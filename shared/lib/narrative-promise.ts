...
export type NarrativePromisePlanIssueCode =
  'INTENT_REQUIRED' | 'RANGE_INVALID' | 'SOURCE_OUTLINE_NODE_INVALID' | 'WINDOW_ORDER_INVALID';
...
export function validateNarrativePromisePlan(
  plan: NarrativePromisePlan
): NarrativePromisePlanIssue[] {
  ...
  if (typeof source.intent !== 'string' || !source.intent.trim())
    issues.push({ code: 'INTENT_REQUIRED', path: 'intent' });
  const hintRanges = Array.isArray(source.plannedHintRanges) ? source.plannedHintRanges : undefined;
  const ranges = [
    ...(source.plannedPlantRange !== undefined
      ? [{ path: 'plannedPlantRange', value: source.plannedPlantRange }]
      : []),
    ...(hintRanges
      ? hintRanges.map((value, index) => ({ path: `plannedHintRanges.${index}`, value }))
      : []),
    ...(source.plannedPayoffRange !== undefined
      ? [{ path: 'plannedPayoffRange', value: source.plannedPayoffRange }]
      : []),
  ];
...
export function buildNarrativePromiseImpacts(
  ...
  const entries = [
    ...(core.plan.plannedPlantRange
      ? [{ action: 'plant' as const, range: core.plan.plannedPlantRange }]
      : []),
    ...core.plan.plannedHintRanges.map((range) => ({ action: 'hint' as const, range })),
    ...(core.plan.plannedPayoffRange
      ? [{ action: 'payoff' as const, range: core.plan.plannedPayoffRange }]
      : []),
  ];

...
function isOutlineScope(
  level: OutlineArtifactLevel,
  scope: unknown
): scope is OutlineArtifactScope {
  if (
    !isRecord(scope) ||
    Object.keys(scope).some((key) => !['volumeName', 'chapterStart', 'chapterEnd'].includes(key))
  )
    return false;
  if (level === 'master') return Object.keys(scope).length === 0;
  if (level === 'volume')
    return (
      isNonEmptyString(scope.volumeName) &&
      scope.chapterStart === undefined &&
      scope.chapterEnd === undefined
    );
  return (
    scope.volumeName === undefined &&
    isChapterRange({ from: scope.chapterStart, to: scope.chapterEnd })
  );
}
...
function isOutlineNode(value: unknown): boolean {
  if (
    ...
    !value.foreshadowingIds.every((foreshadowingId) => typeof foreshadowingId === 'string')
  )
    return false;
  return true;
}
...
function isPromiseAction(value: unknown): boolean {
  return (
    ...
    (value.chapterRange === undefined ||
      (isRecord(value.chapterRange) &&
...
export function isStructuredOutlineCore(value: unknown): value is StructuredOutlineCore {
  return (
    ...
    (value.promiseActions === undefined ||
      (Array.isArray(value.promiseActions) && value.promiseActions.every(isPromiseAction)))
  );
}
...
function scopeIssue(
  ...
  for (const sibling of siblings) {
    if (
      ...
      sibling.level !== artifact.level ||
      !isOutlineScope(sibling.level, sibling.scope)
    )
      continue;
    ...
    if (
      artifact.scope.chapterStart === sibling.scope.chapterStart &&
      artifact.scope.chapterEnd === sibling.scope.chapterEnd
    )
      continue;
    if (
      (artifact.scope.chapterStart as number) <= (sibling.scope.chapterEnd as number) &&
      (sibling.scope.chapterStart as number) <= (artifact.scope.chapterEnd as number)
    ) {
      return {
        code: 'OUTLINE_SCOPE_OVERLAP',
        detail: `chapter range overlaps active sibling ${sibling.id}`,
      };
    }
  }
  return undefined;
...
export function validateOutlineHierarchy(
  input: OutlineHierarchyValidationInput
): OutlineHierarchyValidationResult {
  ...
  for (const node of core.nodes) {
    if (!isNonEmptyString(node.id))
      issues.push({ code: 'OUTLINE_NODE_ID_REQUIRED', detail: 'outline node id is required' });
    else if (nodeIds.has(node.id))
      issues.push({
        code: 'OUTLINE_NODE_ID_DUPLICATE',
        detail: `duplicate outline node id ${node.id}`,
      });
    else nodeIds.add(node.id);
    if (!Number.isInteger(node.order) || node.order < 0)
      issues.push({
        code: 'OUTLINE_NODE_ORDER_INVALID',
        detail: `node ${node.id} has an invalid order`,
      });
    else if (orders.has(node.order))
      issues.push({
        code: 'OUTLINE_NODE_ORDER_DUPLICATE',
        detail: `duplicate outline node order ${node.order}`,
      });
  ...
  for (const node of core.nodes) {
    if (input.artifact.level === 'master') {
      if (node.parentNodeId !== undefined) {
        issues.push({
          code: 'OUTLINE_MASTER_PARENT_FORBIDDEN',
          detail: `master node ${node.id} cannot reference an upstream parent`,
        });
      }
    } else if (!node.parentNodeId) {
      issues.push({
        code: 'OUTLINE_UPSTREAM_PARENT_REQUIRED',
        detail: `node ${node.id} must reference an upstream parent`,
      });
    } else if (!upstream.has(node.parentNodeId)) {
      issues.push({
        code: 'OUTLINE_UPSTREAM_NODE_MISSING',
        detail: `node ${node.id} references missing upstream node ${node.parentNodeId}`,
      });
    }
    for (const characterId of node.characterIds) {
      if (!characters.has(characterId))
        issues.push({
          code: 'OUTLINE_CHARACTER_MISSING',
          detail: `node ${node.id} references missing character ${characterId}`,
        });
    }
    for (const foreshadowingId of node.foreshadowingIds) {
      if (!promises.has(foreshadowingId))
        issues.push({
          code: 'OUTLINE_FORESHADOWING_MISSING',
          detail: `node ${node.id} references missing foreshadowing ${foreshadowingId}`,
        });
    }
  }
  ...
  for (const action of core.promiseActions || []) {
    ...
    if (!promise) {
      issues.push({
        code: 'OUTLINE_PROMISE_NOT_FOUND',
        detail: `promise action references missing foreshadowing ${action.foreshadowingId}`,
      });
    }
    if (action.chapterRange !== undefined && !isChapterRange(action.chapterRange)) {
      issues.push({
        code: 'OUTLINE_PROMISE_RANGE_INVALID',
        detail: `promise action ${action.foreshadowingId} has an invalid chapter range`,
      });
      continue;
    }
    if (
      ...
      action.chapterRange &&
      promise?.plannedPayoffRange &&
      action.chapterRange.to < promise.plannedPayoffRange.from
    ) {
      issues.push({
        code: 'OUTLINE_PROMISE_PAYOFF_PREMATURE',
        detail: `payoff for ${action.foreshadowingId} precedes its planned payoff range`,
      });
    }
  }

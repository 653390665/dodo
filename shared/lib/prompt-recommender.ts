...
export function inferNovelGovernanceProfile(novel: Novel): InferenceOutput {
  ...
  if (explicitActiveFlowId) {
    activeSeriesId = explicitActiveFlowId;
  } else if (hasPremiumLongformFlow && longformFlowId) {
...
/**
 * 根据作品画像、基本信息以及字数等自适应判断应该启用哪些维度的信号，并返回映射的具体行为
 */

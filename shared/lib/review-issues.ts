...
export function stableIssueId(
  issue: Pick<
    ReviewIssue,
    'source' | 'category' | 'issueType' | 'issueSubtype' | 'snippet' | 'explanation' | 'contentHash'
  >
): string {
  const input = [
    issue.source,
...
function fromStructuredIssue(issue: StructuredAuditIssue, contentHash = ''): ReviewIssue {
  ...
  const base = {
    ...
    recommendedCapabilityIds: recommendCapabilities(
      issue.dimension || issue.issueType,
      issue.issueType
    ),
    contentHash,
  } as const;
...
function fromStructuredEvidence(
  audit: NonNullable<StructuredAudit['evidence']>[number],
  contentHash = ''
): ReviewIssue {
  const now = Date.now();
  const base = {
...
export function structuredAuditToReviewIssues(
  ...
  for (const evidence of audit?.evidence || []) {
    const duplicateIndex = issues.findIndex(
      (issue) =>
...
export function continuityToReviewIssues(
  report: ContinuityReport | null | undefined,
  contentHash = ''
): ReviewIssue[] {
  return (report?.issues || []).map((issue) => {
    const now = Date.now();
    const base = {
      source: 'production-audit' as const,
      category: issue.category,
      issueType: `continuity-${issue.category}`,
...
export function deriveReviewGate(
  ...
  fallback: 'pass' | 'fail' | 'unknown' | 'not_run' = 'not_run'
): ChapterReviewGate {
  ...
  const blocking = issues.some(
    (issue) =>
      ...
      (issue.severity === 'critical' || issue.severity === 'major' || issue.severity === 'high')
  );
...
function recommendCapabilities(category = '', issueType = ''): string[] {
  ...
  if (/style|slop|syntax|ai-cliche|pacing/.test(key))
    return ['de-ai-slop-shield', 'de-ai-rhythm-restorer'];
  ...
  if (/canon|setting|timeline|logic|power|location|item/.test(key))
    return ['bible-world-builder', 'opening-gold-three'];
  return [];
}
...
export function resolveReviewRemediation(
  issue: ReviewIssue,
  context: ReviewRemediationContext = {}
): ReviewRemediationResolution {
  const candidates = issue.recommendedCapabilityIds.length
    ? issue.recommendedCapabilityIds
    : recommendCapabilities(issue.category, issue.issueType);
  const available = new Set(context.availableCapabilityIds || candidates);
  const active = new Set(context.activeCapabilityIds || []);
  const capabilityId =
    candidates.find((id) => active.has(id) && available.has(id)) ||
    candidates.find((id) => available.has(id));
  const planner = /canon|setting|timeline|logic|foreshadow|platform/.test(
    `${issue.category || ''} ${issue.issueType || ''}`.toLowerCase()
  );
  ...
  return {
    ...
    stage: planner ? 'planner' : 'writer',
    scope,
    usesSystemDefault: !capabilityId,
  };
}
...
export function isReviewStateStale(
  state: Pick<ChapterReviewState, 'contentHash'> | undefined,

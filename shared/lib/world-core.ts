...
function statements(value: unknown, kind: 'rule' | 'power' | 'faction') {
  ...
  return value.flatMap((item, index) => {
    ...
    if (kind === 'power')
      return [{ id, statement, ...(text(source.cost) ? { cost: text(source.cost) } : {}) }];
...
export function normalizeWorldCore(value: unknown): StructuredWorldCore {
  ...
  return {
    ...
    powerConstraints: statements(
      source.powerConstraints,
      'power'
    ) as StructuredWorldCore['powerConstraints'],
    prohibitions: statements(source.prohibitions, 'rule') as StructuredWorldCore['prohibitions'],
    factionConstraints: statements(
      source.factionConstraints,
      'faction'
    ) as StructuredWorldCore['factionConstraints'],
  };
}

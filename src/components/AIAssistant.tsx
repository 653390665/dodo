        ...
        {msg.role === 'assistant' ? (
          <div className="flex flex-col gap-3">
            {msg.id !== 'welcome' && launchContext
              ? (() => {
                  const suggestionKind: AssistantSuggestionKind =
                    msg.actionPlan?.intent === 'plan-scene'
                      ? 'scene-beat'
                      : msg.actionPlan?.intent === 'build-setting'
                        ...
                        : msg.actionPlan?.intent === 'save-fragment'
                          ...
                          : msg.actionPlan
                            ? 'prose'
                            : classifyAssistantSuggestion(msg.content, launchContext);
                  return (
                    ...
                    return (
                      <>
                        {actionPlan?.intent === 'draft-prose' && launchContext.selectedText && (
                          <ActionButton
                            primary
                            action={() => onPreviewCandidate(actionPlan, msg.content)}
                            label="预览选区候选"
                            icon={Sparkles}
                          />
                        )}
                        ...
                        {actionPlan?.intent === 'build-setting' && (
                          <ActionButton
                            primary
                            action={() => onLaunchSettingCandidate?.(actionPlan, msg.content)}
                            label="使用推荐能力生成设定候选"
                            icon={Globe}
                          />
                        )}
                        {actionPlan?.intent === 'plan-structure' && (
                          <ActionButton
                            primary
                            action={() => onStartCreation?.(actionPlan, msg.content)}
                            label="进入完整创作流程"
                            icon={BrainCircuit}
                          />
                        )}
                        {!actionPlan && primaryAction === 'extract-setting' && (
                          <ActionButton
                            primary
                            action={() => onExtractToCurrentNovel(msg.content)}
                            label="提设定"
                            icon={Globe}
                          />
                        )}
                        {(actionPlan?.intent === 'save-fragment' ||
                          (!actionPlan && primaryAction === 'save-fragment')) && (
                          <ActionButton
                            primary
                            action={() => onSaveAsIdeaFragment(msg.content)}
                            label="存碎片"
                            icon={FolderOpen}
                          />
                        )}
                        {!actionPlan &&
                          primaryAction === 'replace-selection' &&
                          launchContext.selectedText && (
                            <ActionButton
                              primary
                              action={() => onReplaceSelection?.(msg.content)}
                              label="替换选区"
                              icon={Sparkles}
                            />
                          )}
                        {!actionPlan && primaryAction === 'append-content' && (
                          <ActionButton
                            primary
                            action={() => onApplyToContent?.(msg.content)}
                            label="插到末尾"
                            icon={ArrowRight}
                          />
                        )}
                        {!actionPlan && primaryAction === 'append-scene-beat' && (
                          <ActionButton
                            primary
                            action={() => onApplyToSceneBeats?.(msg.content)}
                            label="补分镜"
                            icon={Globe}
                          />
                        )}
                        ...
                        <details className="group relative">
                          ...
                          <div className="absolute bottom-full left-0 mb-2 w-48 bg-theme-sidebar rounded-xl shadow-xl border border-theme-border p-2 flex flex-col gap-1 z-30">
                            <button
                              onClick={() => onSaveToOtherNovel(msg.id)}
                              className="flex items-center gap-2 px-3 py-2 text-[10px] font-bold text-theme-muted hover:bg-theme-sidebar/50 rounded-lg"
                            >
                              <FolderOpen size={12} aria-hidden="true" /> 保存到其他作品
                            </button>
                            <button
                              onClick={() => onExtractToOtherNovel(msg.id)}
                              className="flex items-center gap-2 px-3 py-2 text-[10px] font-bold text-theme-muted hover:bg-theme-sidebar/50 rounded-lg"
                            >
                            ...
                            {primaryAction !== 'save-fragment' && (
                              <button
                                onClick={() => onSaveAsIdeaFragment(msg.content)}
                                className="flex items-center gap-2 px-3 py-2 text-[10px] font-bold text-theme-muted hover:bg-theme-sidebar/50 rounded-lg"
                              >
          ...
          <p className="text-theme-accent-contrast m-0 text-xs leading-relaxed font-sans">
            {msg.content}
          </p>
        )}
      </div>
...
export function AIAssistant({
  ...
  useEffect(() => {
    ...
    const dialog = Array.from(document.querySelectorAll('[role="dialog"][aria-modal="true"]')).at(
      -1
    ) as HTMLElement | null;
  ...
  const handleSubmit = async (
    ...
    customPrompt?: string,
    isRetry = false,
    explicitPlan?: AssistantActionPlan
  ) => {
    e?.preventDefault();
    ...
    void recordProductEvent({
      ...
      chapterId: launchContext?.chapterId,
      objectId: requestId,
    });
    if (failedRequest)
      void recordProductEvent({
    ...
    if (!isRetry) {
      sessionStore.appendMessage(sessionKey, 'general', {
        id: userMsg.id,
        sender: 'user',
    ...
    try {
      ...
      if (
        ...
        useAssistantSessionStore.getState().getSession(sessionKey, 'general').activeRequestId !==
          requestId
      )
        return;
      if (!result.trim()) {
      ...
      void recordProductEvent({
        eventName: code === 'empty_response' ? 'assistant_empty_response' : 'assistant_failure',
        stage: 'assistant',
        result: 'failure',
        novelId: activeNovel?.id,
        chapterId: launchContext?.chapterId,
  ...
  const handleExtractToWorldBible = async (novel: Novel, content: string) => {
    ...
    const isCurrentExtraction = () =>
      ...
      !controller.signal.aborted;
    try {
      const { result: extracted, databaseGeneration } = await extractWorldSetupPhase(
        content,
        novel.id,
        (progress, status) => {
  ...
  const handleSaveToOtherNovel = useCallback(
    (messageId: string) => {
      setShowSaveModal(messageId);
    },
    [setShowSaveModal]
  );
  const handleExtractToOtherNovel = useCallback(
    (messageId: string) => {
      setShowExtractModal(messageId);
    },
    [setShowExtractModal]
  );
  const handleRowLaunchSettingCandidate = useCallback(
    (plan: AssistantActionPlan, seedText: string) => {
    ...
    <div className="h-full flex flex-col bg-theme-sidebar">
      ...
      <div className="shrink-0 p-4 border-b border-theme-border flex items-center justify-between bg-theme-sidebar sticky top-0 z-20">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-lg font-serif font-bold text-theme-text leading-none">
              {assistantTitle}
            </h2>
          ...
          <div className="mt-2 text-[11px] text-theme-muted flex flex-col gap-1">
            {launchContext ? (
              <>
                ...
                {launchContext.chapterTitle ? (
                  <p className="truncate">章节：{launchContext.chapterTitle}</p>
                ) : null}
                {launchContext.intent ? (
                  <p className="line-clamp-1">目标：{launchContext.intent}</p>
                ) : null}
              </>
            ) : activeNovel ? (
              <>
        ...
        <div className="grid grid-cols-2 gap-2">
          {suggestions.map((s) => (
            <button
              ...
              onClick={() =>
                handleSubmit(
                  ...
                  s.prompt,
                  false,
                  buildAssistantActionPlan(s.intent, s.prompt, {
                    novelId: activeNovel?.id,
                    chapterId: launchContext?.chapterId,
                  })
                )
            ...
            >
              <s.icon
                size={14}
                className="text-theme-muted group-hover:text-theme-accent shrink-0"
                aria-hidden="true"
              />
              <span className="text-[11px] font-bold text-theme-muted group-hover:text-theme-text truncate">
                {s.label}
              </span>
            </button>
          ))}
        ...
        <div className="flex flex-col gap-4 pb-4">
          ...
          {session.failure && (
            <div
              role="alert"
              aria-label="助手请求失败"
              className="flex flex-col gap-2 rounded-2xl alert-warning p-3 text-xs"
            >
              <div>{failureReasonText || session.failure.message}</div>
              {session.failure.reason && (
                <div className="text-[10px]">
                  原因：{FAILURE_REASON_COPY[session.failure.reason] ?? '未知原因'}
                </div>
              )}
              ...
              {session.failure.traceId && (
                <div className="text-[10px]">诊断编号：{session.failure.traceId}</div>
              )}
              <div className="flex flex-wrap gap-3">
                {(session.failure.retriable || session.failure.code === 'empty_response') && (
                  <button
                    type="button"
                    onClick={() => void handleSubmit(undefined, session.failure?.prompt, true)}
        ...
        >
          ...
          >
            ...
            <div className="mt-2 text-[11px] text-theme-muted">
              {pendingCandidate.plan.scope === 'chapter' ? '本章使用' : '作品范围'} · 推荐能力{' '}
              {pendingCandidate.plan.recommendedCapabilityId}
            </div>
            <div className="mt-3 max-h-48 overflow-y-auto whitespace-pre-wrap text-xs leading-6 text-theme-text">
            ...
            <div className="mt-4 flex justify-end gap-2 border-t border-theme-border pt-3">
              <button
                type="button"
                onClick={() => setPendingCandidate(null)}
                className="rounded-md border border-theme-border px-3 py-2 text-xs"
              >
                取消
              </button>
              <button
                type="button"
                onClick={() => {
                  if (pendingCandidate.plan.intent === 'plan-scene')
                    void onApplyToSceneBeats?.(pendingCandidate.content);
                  else if (launchContext?.selectedText)
                    void onReplaceSelection?.(pendingCandidate.content);
              ...
              >
                {pendingCandidate.plan.intent === 'plan-scene'
                  ? '确认写入分镜'
                  : launchContext?.selectedText
                    ? '确认替换选区'
                    : '确认写入正文'}
        ...
        >
          ...
          >
            ...
            <p className="text-xs text-theme-muted mb-4">
              选择一个作品，该灵感将作为「💡 灵感备忘录」新增至对应作品的灵感碎片库中。
            </p>
            ...
            <div className="flex-1 overflow-y-auto space-y-2 mb-4">
              {userNovels.length === 0 ? (
                ...
                userNovels.map((novel) => (
                  ...
                  >
                    <span className="font-bold text-sm text-theme-text group-hover:text-theme-accent transition-colors">
                      {novel.title}
                    </span>
                    <span className="flex items-center gap-2">
                      ...
                      <ArrowRight
                        size={14}
                        className="text-transparent group-hover:text-theme-accent transition-colors"
                        aria-hidden="true"
                      />
                    </span>
                  </button>
      ...
      {showExtractModal && (
        ...
        >
          ...
          >
            {isExtracting && (
              <div className="absolute inset-0 bg-theme-sidebar/90 backdrop-blur-md z-10 flex flex-col items-center justify-center p-6">
                <div className="w-16 h-16 bg-theme-accent/10 rounded-full flex items-center justify-center mb-4 relative">
                  <Loader2
                    size={32}
                    className="text-theme-accent animate-spin"
                    aria-hidden="true"
                  />
                  <span className="absolute text-[10px] font-bold text-theme-accent">
                    {extractProgress}%
                  </span>
                </div>
                <p className="font-bold text-theme-text font-serif text-center">
                  {extractStageText || 'AI 正在结构化提取设定...'}
                </p>
            ...
            <p className="text-xs text-theme-muted mb-4">
              选择一个作品，AI
              会先提取当前卡片中的角色、地点、物品等结构化知识；确认提取结果后再写入该作品的「设定集」。
            </p>
              ...
              {userNovels.length === 0 ? (
                ...
                userNovels.map((novel) => (
                  <button
                    key={novel.id}
                    onClick={() =>
                      handleExtractToWorldBible(novel, getMessageContent(showExtractModal))
                    }

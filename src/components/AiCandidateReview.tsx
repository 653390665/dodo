...
export function AiCandidateReview({
  ...
  return (
    <section
      ...
      className={cn(
        'flex flex-wrap items-center gap-2 border border-theme-accent/40 bg-theme-accent/5 text-xs',
        className
      )}
    >
      ...
      <span
        className={cn(
          ...
          qualityState.status === 'eligible'
            ? 'rounded border-emerald-200 bg-emerald-50 text-emerald-700'
            : qualityState.status === 'fallback'
              ? 'alert-warning'
              : qualityState.status === 'blocked'
                ? 'alert-danger'
                : 'border'
        )}
        role="status"
      ...
      {candidate.quality &&
      candidate.quality.findings.some((finding) => finding.severity === 'P2') ? (
        <span className="basis-full text-[10px] text-amber-700" role="status">
          还有 {candidate.quality.findings.filter((finding) => finding.severity === 'P2').length}{' '}
          项文风建议，可在审稿后精修。
        </span>
      ) : null}
      {candidate.quality?.mechanicalReview?.status === 'needs-action' ? (
        <span className="basis-full text-[10px] text-red-700" role="alert">
          机械审查 {candidate.quality.mechanicalReview.score.toFixed(1)}/
          {candidate.quality.mechanicalReview.threshold}：
          {candidate.quality.mechanicalReview.summary}，需精修后才能写入。
        </span>
      ) : null}
      {candidate.quality?.semanticReview ? (
        <details className="basis-full rounded border border-theme-border/70 bg-theme-sidebar/50 px-2 py-1">
          <summary className="cursor-pointer text-[10px] font-semibold text-theme-text">
            语义审阅：
            {candidate.quality.semanticReview.status === 'pass'
              ? '已通过'
              : candidate.quality.semanticReview.status === 'needs-action'
                ? '需要处理'
                : '尚未运行'}
          </summary>
          <ul className="mt-1 grid gap-1 text-[10px] text-theme-muted sm:grid-cols-2">
            {candidate.quality.semanticReview.checks.map((check) => (
              <li
                key={check.id}
                className={check.status === 'needs-action' ? 'text-amber-700' : undefined}
              >
                <div>
      ...
      {candidate.quality?.findings.length ? (
        <details className="basis-full rounded border border-theme-border/70 bg-theme-sidebar/50 px-2 py-1">
          <summary className="cursor-pointer text-[10px] font-semibold text-theme-text">
            硬性检查证据（{candidate.quality.findings.length}）
          </summary>
              ...
              <li key={finding.code}>
                ...
                {finding.evidence?.map((evidence, index) => (
                  <div
                    ...
                    className="mt-1 border-l-2 border-theme-border pl-2 text-theme-muted"
                  >
      ...
      <details className="basis-full rounded-lg border border-theme-border/70 bg-theme-sidebar/60">
        <summary className="cursor-pointer px-2 py-1.5 text-[11px] font-semibold text-theme-text">
          查看候选正文预览
        </summary>
        <div
          role="region"
          aria-label={variant === 'workbench' ? '智能管家正文候选预览' : 'AI 正文候选预览'}
          className="grid max-h-64 gap-2 overflow-y-auto border-t border-theme-border/60 p-2 text-[11px] leading-5 md:grid-cols-2"
        >
          <div className="min-w-0">
            <div className="mb-1 font-bold text-theme-muted">当前正文（未修改）</div>
            <pre className="whitespace-pre-wrap break-words font-sans text-theme-muted">
              {candidate.baselineContent}
            </pre>
          </div>
          <div className="min-w-0">
            <div className="mb-1 font-bold text-theme-text">候选正文</div>
            <pre className="whitespace-pre-wrap break-words font-sans text-theme-text">
              {candidate.content}
            </pre>
          </div>
        </div>

...
export function ChapterCompletionReview({
  result,
  onReturnToEditing,
  onRetryUnavailable,
  onAcceptRisk,
  ...
  return (
    <section
      aria-label="章节完成审阅"
      className="mx-3 mb-3 rounded-xl border border-theme-border bg-theme-sidebar/20 p-4 text-sm text-theme-text sm:mx-5"
    >
      <div className="flex flex-wrap items-center gap-2">
        <h2 className="font-bold">章节完成审阅</h2>
        <span
          className={
            passed ? 'text-emerald-700' : issues.length ? 'text-amber-700' : 'text-amber-700'
          }
        >
          {passed ? '检查通过' : issues.length ? '发现问题' : '格式检查通过，语义审阅未完成'}
        </span>
        {incomplete && issues.length > 0 ? (
          <span className="text-amber-700">AI 检查未完成</span>
        ) : null}
      </div>
      {issues.length > 0 ? (
        <ul className="mt-3 space-y-2 text-xs text-theme-muted">
          {issues.map((issueId) => (
            <li key={issueId} className="flex items-center justify-between gap-2">
              ...
              {onPreviewRevision &&
              result.gate.canAcceptLocalRevision &&
              reviewIssueById.has(issueId) ? (
                <button
                  type="button"
                  className="text-theme-accent"
                  onClick={() => onPreviewRevision(issueId)}
                >
                  预览局部修订
                </button>
              ) : null}
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button
          type="button"
          className="rounded-lg border border-theme-border px-3 py-2 text-xs"
          onClick={onReturnToEditing}
        >
          返回编辑
        </button>
        {onRetryUnavailable && incomplete ? (
          <button
            type="button"
            className="rounded-lg border border-theme-border px-3 py-2 text-xs"

...
export interface ChapterFactCandidateReviewProps {
  ...
  submitting: boolean;
  onConfirm: (selection: {
    factDecisions: Record<string, 'accepted' | 'pending' | 'rejected'>;
  }) => void;
}
...
const KIND_LABELS: Record<string, string> = {
  ...
  location: '地点',
  power: '力量体系',
  'narrative-promise': '叙事承诺',
};

export function ChapterFactCandidateReview({
  candidate,
  canConfirm,
  submitting,
  onConfirm,
}: ChapterFactCandidateReviewProps) {
  ...
  }>({ candidateKey: '', decisions: {} });
  const decisions =
    selection.candidateKey === candidateKey ? selection.decisions : defaultDecisions;
  ...
  return (
    <section
      aria-label="章节事实确认"
      className="mt-4 rounded-xl border border-theme-border bg-theme-sidebar/20 p-3"
    >
            ...
            <div className="mt-1 space-y-2">
              {facts.map((fact) => {
                ...
                return (
                  <label
                    key={fact.id}
                    className="block rounded-lg border border-theme-border px-2 py-2 text-xs text-theme-text"
                  >
                    <div className="flex items-start gap-2">
                      ...
                      {fact.ambiguous || fact.destructive || !fact.selectable ? (
                        <span className="text-amber-700">
                          {fact.selectable ? '需确认' : '待关联'}
                        </span>
                      ) : null}
                    </div>
                    <div className="mt-1 text-theme-muted">目标记录：{fact.target.label}</div>
                    <div className="mt-1 whitespace-pre-wrap text-theme-muted">
                      正文证据：{fact.evidence}
                    </div>
                  </label>
                );

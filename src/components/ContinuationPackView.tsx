...
import {
  ...
  resumePackEntityExtraction,
  requeryPackEntityExtraction,
  resolveContinuationPackConflicts,
} from '../lib/continuation-client';
...
export function ContinuationPackView({
  novel,
  initialActivePackId = null,
  initialAutoSyncPackId = null,
  onAutoSyncConsumed,
  onSyncComplete,
  onOpenGapAssistant,
  onOpenGapAssistantBatch,
}: ContinuationPackViewProps) {
  ...
  const handleDeletePack = async (packId: string) => {
    if (
      !(await appConfirm(
        '删除该资料包？',
        '将永久删除资料包及其全部提取实体与矛盾清单；删除后需重新上传并解析。',
        { confirmLabel: '删除' }
      ))
    )
      return;
  ...
  const handleSaveConflict = async (contradictionId: string) => {
    ...
    const resolution = (
      conflictDrafts[contradictionId] ??
      contradiction?.acceptedResolution ??
      contradiction?.suggestedResolution ??
      ''
    ).trim();
    if (!contradiction || !resolution || resolution === contradiction.acceptedResolution?.trim())
      return;
  ...
  const canApprove =
    activePack &&
    !activePack.contradictions.some(
      (item) =>
        item.severity === 'high' &&
  ...
  const loadExistingEntities = useCallback(
    async (seq: number): Promise<boolean> => {
      ...
      if (failed) {
        ...
        const nextError = getClientErrorCode(reason)
          ? {
              code: getClientErrorCode(reason),
              message: reason instanceof Error ? reason.message : '本地设定读取失败，请重试。',
            }
          : {
              code: 'LOCAL_ENTITIES_LOAD_FAILED',
              message: reason instanceof Error ? reason.message : '本地设定读取失败，请重试。',
            };
  ...
  const consumeAutoSyncIntent = useCallback(
    (packId: string) => {
  ...
  const handleSyncEntities = useCallback(
    async (
      pack: ContinuationPack,
      resumeJob?: { jobId: string; databaseGeneration: number },
      requeryJob?: { jobId: string; databaseGeneration: number }
    ) => {
      ...
      try {
        ...
        const snapshot = requeryJob
          ? await requeryPackEntityExtraction(
              requeryJob.jobId,
              requeryJob.databaseGeneration,
              controller.signal,
              onProgress
            )
          : resumeJob
            ? await resumePackEntityExtraction(
                resumeJob.jobId,
                resumeJob.databaseGeneration,
                controller.signal,
                onProgress
              )
        ...
        const nextError = getClientErrorCode(e)
          ? {
              ...
              detailMessage:
                clientError.detailMessage || (e instanceof Error ? e.message : undefined),
              message:
                clientError.code === 'EXTRACTION_INVALID_JSON'
                  ...
                  : clientError.code === 'EXTRACTION_OUTPUT_TRUNCATED'
                    ...
                    : clientError.code === 'EXTRACTION_SCHEMA_MISMATCH'
                      ...
                      : clientError.code === 'EXTRACTION_EMPTY_SEMANTIC_RESULT'
                        ? '模型未提取到可用设定，请检查资料内容。'
                        : clientError.code === 'EXTRACTION_CONFIG'
                          ? '模型配置不可用，请检查设置后重试。'
                          : clientError.code === 'GENERATION_MISMATCH'
                            ? '数据已变更，请刷新后重试。'
                            : clientError.code === 'EXTRACTION_QUOTA'
                              ? '额度不足，请查看额度说明后重试。'
                              : clientError.code === 'EXTRACTION_RATE_LIMIT'
                                ? '模型服务暂时限流，可稍后续跑。'
                                : clientError.code === 'EXTRACTION_NETWORK' ||
                                    clientError.code === 'EXTRACTION_SERVICE_UNAVAILABLE'
                                  ? '模型服务暂时不可用，可稍后续跑。'
                                  : clientError.code === 'EXTRACTION_EMPTY_RESPONSE'
                                    ? '模型返回空结果，可稍后续跑。'
                                    : clientError.code === 'EXTRACTION_POLLING_UNAVAILABLE'
                                      ? '暂时无法读取进度，任务仍保留，可稍后续跑。'
                                      : `提取失败：${e instanceof Error ? e.message : '未知错误'}`,
            }
          : {
              code: 'EXTRACTION_FAILED',
              message: `提取失败：${e instanceof Error ? e.message : '未知错误'}`,
            };
        setError(nextError);
      } finally {
  ...
  useEffect(() => {
    ...
    const matchedPack = packs.find(
      (pack) =>
        pack.id === initialAutoSyncPackId && pack.novelId === novel.id && pack.status === 'approved'
    );
  ...
  useEffect(() => {
    ...
    if (
      !Number.isFinite(databaseGeneration) ||
      !Number.isInteger(databaseGeneration) ||
    ...
    try {
      ...
      if (result.syncState) {
        setActivePack((current) =>
          current && current.id === syncSnapshot.packId
            ? { ...current, syncState: result.syncState, updatedAt: Date.now() }
            : current
        );
        setPacks((packs) =>
          packs.map((item) =>
            item.id === syncSnapshot.packId
      ...
      if (seq !== syncSeqRef.current || syncPackSnapshotRef.current !== syncSnapshot.packId)
        return false;
      setError({
        code: getClientErrorCode(e) || 'SYNC_FAILED',
  ...
  const errorAlert = error ? (
    <div
      role="alert"
      className="min-w-0 text-xs text-red-500 font-medium bg-red-500/10 border border-red-500/20 px-3.5 py-2.5 rounded-xl animate-fade-in flex flex-col items-stretch gap-2"
    >
      <span>⚠️</span>
      ...
      {(error.code ||
        ...
        <details className="min-w-0 text-[10px] text-red-700">
          ...
          <div className="block max-w-[22rem] whitespace-normal break-words space-y-1">
            <div>
              类别：{error.code || '未知'}
              {error.batch ? `；第 ${error.batch}/${error.totalBatches || '?'} 批` : ''}
              {error.attempt ? `；第 ${error.attempt} 次尝试` : ''}
            ...
            {error.issues?.slice(0, 3).map((issue, index) => (
              <div key={`${issue.path}-${index}`}>
                字段 {issue.path || '(根)'}：{issue.code}；{issue.message}
              </div>
            ))}
            {error.outputDiagnostic && (
              <div>
                输出：{error.outputDiagnostic.provider || '未知 Provider'}；格式{' '}
                {error.outputDiagnostic.responseFormatMode || '未知'}；兼容模式{' '}
                {error.outputDiagnostic.compatibilityMode || 'none'}；结束原因{' '}
                {error.outputDiagnostic.finishReason || '未提供'}；解析阶段{' '}
                {error.outputDiagnostic.parserStage || '未提供'}；上游请求{' '}
                {error.outputDiagnostic.providerRequestCount ?? '未提供'} 次
                {error.outputDiagnostic.providerHttpStatus
                  ? `；HTTP ${error.outputDiagnostic.providerHttpStatus}`
                  : ''}
                {error.outputDiagnostic.rejectedParameter
                  ? `；拒绝参数 ${error.outputDiagnostic.rejectedParameter}`
                  : ''}
                {error.outputDiagnostic.providerErrorCode
                  ? `；Provider code ${error.outputDiagnostic.providerErrorCode}`
                  : ''}
              </div>
            )}
          </div>
        ...
        <div className="flex flex-wrap gap-2 min-w-0">
          {error.code === 'EXTRACTION_POLLING_UNAVAILABLE' && error.jobId && (
            <button
              type="button"
              onClick={() =>
                handleSyncEntities(activePack, undefined, {
                  jobId: error.jobId!,
                  databaseGeneration: error.databaseGeneration || 0,
                })
              }
              className="rounded-lg border border-red-500/30 px-2.5 py-1 font-bold"
            >
          ...
          {[
            ...
            error.batch && (
              ...
              >
                <RefreshCw size={12} className="inline mr-1" />
                {error.code === 'EXTRACTION_SCHEMA_MISMATCH' ? '修复并重试本批' : '从失败批次续跑'}
              </button>
            )}
          {[
            'EXTRACTION_SCHEMA_MISMATCH',
    ...
    <div className="h-full overflow-y-auto p-8 max-w-4xl mx-auto space-y-8">
      ...
      <div className="rounded-2xl border border-theme-border bg-theme-sidebar p-6 space-y-4">
        <div className="flex items-center gap-2">
          <Upload size={18} />
          <span className="font-bold text-theme-text">上传资料文件</span>
        </div>
        <p className="text-xs text-theme-muted">
          支持 .txt / .md / .json / .docx，可一次选多个文件。
        </p>
        <input
          type="file"
          multiple
        ...
        {files.length > 0 && (
          <div className="text-xs text-theme-muted space-y-1">
            {files.map((f) => (
              <div key={`${f.name}-${f.lastModified}`} className="flex items-center gap-2">
                <FileText size={12} />
                {f.name}
              </div>
            ))}
      ...
      {activePack && !syncExtraction && (
        <div
          aria-label={`当前资料包：${activePack.title}`}
          className="rounded-2xl border border-theme-border bg-theme-sidebar p-6 space-y-4"
        >
            ...
            <div>
              ...
              <div className="text-xs text-theme-muted">
                ...
                {activePack.status === 'approved' ? (
                  <span className="text-emerald-600">资料包已确认</span>
                ) : (
                ...
                {activePack.status === 'approved' && (
                  <div className="mt-2 text-xs text-theme-muted">
                    同步：
                    {
                      ...
                      )[activePack.syncState?.status || 'not_started']
                    }
                    {(activePack.syncState?.pendingRelationshipCount || 0) > 0 &&
                      ` · 待处理关系 ${activePack.syncState?.pendingRelationshipCount} 条`}
                    {activePack.syncState?.summary && (
                      <div className="mt-1">
                        摘要：人物 {activePack.syncState.summary.characters} · 地点{' '}
                        {activePack.syncState.summary.locations} · 物品{' '}
                        {activePack.syncState.summary.items} · 势力{' '}
                        {activePack.syncState.summary.factions} · 战力{' '}
                        {activePack.syncState.summary.powerLevels} · 时间线{' '}
                        {activePack.syncState.summary.timelineEvents} · 关系{' '}
                        {activePack.syncState.summary.relationships}
                      </div>
                    )}
                  </div>
                )}
              ...
              <div className="flex items-center gap-2">
                ...
                {isExtracting && (
                  <button
                    ...
                    className="px-3 py-2 rounded-xl border border-theme-border text-theme-muted text-xs font-bold"
                  >
                    取消
                  </button>
                )}
          ...
          {activePack.contradictions.length > 0 && (
            <div className="rounded-xl alert-warning p-3 text-xs">
              ...
              {activePack.contradictions.map((c) => {
                ...
                const isSuggestedDraft =
                  !c.acceptedResolution &&
                  !conflictDrafts[c.id] &&
                  Boolean(c.suggestedResolution?.trim());
                return (
                  <div
                    key={`${c.severity}-${c.id}`}
                    className="mt-3 rounded-lg border border-amber-200/80 bg-theme-sidebar/70 p-3 text-theme-text"
                  >
                    <div className="flex items-center justify-between gap-2">
                      <div className="font-bold">
                        [{c.severity}] {c.summary}
                      </div>
                      {isContinuationContradictionResolved(c) ? (
                        <span className="text-emerald-700">已处理</span>
                      ) : null}
                    </div>
                    <div className="mt-2 text-[11px] text-theme-muted">
                      证据：{c.conflictingEvidence.join('；') || '未提供'}
                    </div>
                    {activePack.status === 'approved' || activePack.status === 'draft' ? (
                      <>
                        <div className="mt-2 text-[11px] text-theme-muted">
                          Agent 初始建议：{c.suggestedResolution || '暂无建议'}
                        </div>
                        <textarea
                        ...
                        <button
                          ...
                          disabled={
                            activePack.status === 'approved'
                              ? savingConflictId !== null ||
                                !resolutionDraft.trim() ||
                                resolutionDraft.trim() === (c.acceptedResolution || '').trim()
                        ...
                        >
                          {savingConflictId === c.id
                            ? '保存中...'
                            : acceptedConflictIds.has(c.id)
                              ? '已确认此方案'
                              : isSuggestedDraft
                                ? '采用 Agent 建议'
                                : activePack.status === 'draft'
              ...
              {activePack.sourceMap.keyConflicts.length > 0 && (
                <div className="mt-2 pt-2 border-t border-theme-border">
                  ...
                  {activePack.sourceMap.keyConflicts.map((c) => (
                    <div key={c} className="text-[10px] text-amber-700">
                      - {c}
                    </div>
                  ))}
                </div>
            ...
            <div className="rounded-xl border border-theme-border bg-theme-sidebar/20 p-4 space-y-2">
              ...
              {activePack.readingQuestions.slice(0, 6).map((q, i) => (
                <div key={q.id || i} className="text-xs">
                  ...
                  <div className="text-[10px] text-theme-muted mt-0.5 ml-4">
                    上下文：{q.context}
                  </div>
                  <div className="text-[10px] text-theme-muted mt-0.5 ml-4">
                    创作决策，不阻塞续写。
          ...
          {activePack.continuationGaps && activePack.continuationGaps.length > 0 && (
            <div className="rounded-xl border border-theme-border bg-theme-sidebar/20 p-4 space-y-2">
              ...
              {activePack.continuationGaps.slice(0, 5).map((g, i) => (
                <div
                  key={g.id || i}
                  className="rounded-lg border border-theme-border bg-theme-sidebar p-3 text-xs"
                >
                  <div className="flex items-center gap-2">
                    <span
                  ...
                  <div className="text-[10px] text-theme-muted mt-1.5">
          ...
          <div className="rounded-xl border border-theme-border bg-theme-sidebar p-4 space-y-2">
            ...
            {editingTask ? (
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg bg-theme-accent text-theme-accent-contrast text-[10px] font-bold"
                  >
                    保存
                  </button>
                  <button
                    onClick={() => setEditingTask(false)}
                    className="px-3 py-1.5 rounded-lg bg-theme-sidebar text-theme-text text-[10px] font-bold border border-theme-border"
                  >
                    取消
                  </button>
                </div>
              ...
              <div className="text-xs text-theme-text">
                {activePack?.continuationTask || (
                  <span className="text-theme-muted italic">未指定 — 点击编辑添加续写方向</span>
                )}
              </div>
            )}
          </div>
          ...
          <details className="text-xs text-theme-muted">
            ...
            <div className="mt-2 space-y-3 ml-4">
              <div>
                <span className="font-bold">硬设定：</span>
                {activePack.canonFacts.map((f) => f.text).join('；') || '无'}
              </div>
              <div>
                <span className="font-bold">人物状态：</span>
                {activePack.characterStates.map((c) => `${c.name}(${c.currentGoal})`).join('；') ||
                  '无'}
              </div>
              <div>
                <span className="font-bold">剧情位置：</span>
                {activePack.plotState.currentTimeline} | {activePack.plotState.latestScene}
              </div>
            </div>
          </details>
        </div>
      ...
      {syncExtraction && (
        <div className="space-y-3">
          ...
          {existingLoadError && (
            <div
              role="alert"
              className="min-w-0 rounded-xl border border-red-500/20 bg-red-500/10 p-3 text-xs text-red-500 flex flex-col items-stretch gap-2"
            >
              <span className="min-w-0 whitespace-normal break-words">
                [{existingLoadError.code}] {existingLoadError.message}
              </span>
              <span className="flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={handleRetryExistingEntities}
                  disabled={isLoadingExisting}
                  className="rounded-lg border border-red-500/30 px-2.5 py-1 font-bold disabled:opacity-50"
                >
                  <RefreshCw size={12} className="inline mr-1" />
                  只重试列表
                </button>
                <button
                  type="button"
                  onClick={() => setSyncExtraction(null)}
                  disabled={isLoadingExisting}
                  className="rounded-lg border border-theme-border px-2.5 py-1 font-bold disabled:opacity-50"
                >
                  暂不同步设定
                </button>
              </span>
      ...
      {syncSuccess && (
        <div
          role="status"
          className="rounded-xl border border-emerald-200 bg-emerald-50 px-3.5 py-2.5 text-xs font-medium text-emerald-700"
        >
            ...
            <div
              ...
              className={`rounded-xl border px-4 py-3 text-xs ${
                activePack?.id === pack.id
                  ...
                  : 'border-theme-border hover:bg-theme-sidebar/20'
              }`}
            >
              <button
                type="button"
              ...
              >
                ...
                <span className="block text-theme-muted mt-1">
                  {pack.canonFacts.length} 条设定 · {pack.characterStates.length} 个人物 ·{' '}
              ...
              <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-theme-border/60 pt-2">
                <span
                  className={pack.status === 'approved' ? 'text-emerald-600' : 'text-amber-600'}
                >
                ...
                {pack.status === 'approved' && (
                  <span className="text-xs text-theme-muted">
                    同步：
                    {
                      ...
                      )[pack.syncState?.status || 'not_started']
                    }
                    {(pack.syncState?.pendingRelationshipCount || 0) > 0 &&
                      ` · 待处理关系 ${pack.syncState?.pendingRelationshipCount} 条`}
                  </span>
                )}
                <div className="flex items-center gap-2">
                  {pack.status === 'approved' && (

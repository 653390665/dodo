...
import { LLM_AVAILABILITY_COPY } from '../lib/llm-availability';
...
interface EditorHeaderProps {
  ...
  syncSuccess: boolean;
  syncFailed: boolean;
  connectionState?: 'missing' | 'unknown' | 'connected';
  worldBibleState?: 'missing' | 'unknown' | 'ready';
  mountedSkills: Skill[];
  effectiveCapabilitySummary?: {
    summaryText: string;
    names: string[];
...
export function EditorHeader({
  ...
  worldBibleState = 'unknown',
  mountedSkills,
  effectiveCapabilitySummary,
  onVolumeNameChange,
  onTitleChange,
}: EditorHeaderProps) {
  const connectionCopy = LLM_AVAILABILITY_COPY[connectionState];
  const capabilityNames = effectiveCapabilitySummary
    ? effectiveCapabilitySummary.names
    : mountedSkills.map((skill) => skill.name);
  const capabilityNameText = capabilityNames.length > 0
    ? `${capabilityNames.join(' / ')}${effectiveCapabilitySummary?.overflowCount ? ` / +${effectiveCapabilitySummary.overflowCount}` : ''}`
    : '系统默认';
  const capabilitySummaryText = effectiveCapabilitySummary?.summaryText;
  return (
    <div className={cn(
      ...
      <div className="flex items-center gap-2 min-w-0 shrink-0">
        {!isAgentSidebarOpen && <div className="hidden xl:flex items-center gap-2 min-w-0">
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-theme-sidebar/70 border border-theme-border text-[10px] text-theme-muted">
            ...
            <span className="font-bold text-theme-text">
              {worldBibleState === 'ready' ? '世界观已就绪' : worldBibleState === 'missing' ? '尚未建立世界观' : '世界观状态未知'}
            </span>
          </div>
          <div
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-theme-sidebar/70 border border-theme-border text-[10px] text-theme-muted max-w-[360px] min-w-0"
            title={capabilityNameText}
          >
            <Sparkles size={11} className="text-theme-accent" />
            <span className="shrink-0">本次写法来源</span>
            {capabilitySummaryText && <span className="shrink-0 font-bold text-theme-text">{capabilitySummaryText}</span>}
            <span className="truncate">{capabilityNameText}</span>
          </div>
          <div
            role="status"
            data-connection-state={connectionState}
            title={connectionCopy.helper}
            className={cn(
              "flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-theme-sidebar/70 border border-theme-border text-[10px]",
              connectionState === 'connected' ? 'text-emerald-600' : connectionState === 'missing' ? 'text-red-600' : 'text-amber-600',
            )}
          >
            <div className={cn(
              "size-1.5 rounded-full",
              connectionState === 'connected' ? 'bg-emerald-500' : connectionState === 'missing' ? 'bg-red-500' : 'bg-amber-500',
            )} />
            <span>AI {connectionCopy.label}</span>
          </div>
        </div>}
        {isAnyGenerating ? (
            <div

...
export class ErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    logger.warn('ErrorBoundary caught:', error, errorInfo);
        ...
        <div className="h-full flex flex-col items-center justify-center p-12 text-gray-500 bg-theme-bg/30">
          ...
          <div className="flex gap-3 mt-4">
            <button
              ...
              className="px-6 py-2 bg-theme-accent text-theme-accent-contrast font-bold rounded-xl hover:bg-theme-accent/90 transition-colors"
            >
              重试
            </button>
            <button
              onClick={() => window.location.reload()}
              className="px-6 py-2 border border-theme-border text-theme-text font-bold rounded-xl hover:bg-theme-sidebar/40 transition-colors"
            >
              刷新页面
            </button>
          </div>

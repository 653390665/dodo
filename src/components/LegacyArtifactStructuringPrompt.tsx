...
export function LegacyArtifactStructuringPrompt({ novelId }: { novelId: string }) {
  ...
  const selectedSource =
    visibleSources.find((source) => sourceKey(source) === selectedKey) || visibleSources[0];
  ...
  const discover = async () => {
    ...
    try {
      ...
      const visible = result.sources.filter(
        (source) => !storedDismissals.includes(source.sourceFingerprint)
      );
  ...
  const confirm = async () => {
    ...
    try {
      ...
      setSources((current) =>
        current.filter((source) => sourceKey(source) !== sourceKey(preview.source))
      );
      setPreview(null);
      setSelectedKey('');
      setMessage('结构化版本已确认');
  ...
  const dismiss = () => {
    ...
    setDismissed(next);
    setPreview(null);
    const nextSource = visibleSources.find(
      (source) => sourceKey(source) !== sourceKey(selectedSource)
    );
  ...
  return (
    <section
      className="mx-auto mb-8 max-w-6xl border-y border-theme-border/60 py-4"
      aria-labelledby="legacy-artifact-maintenance-title"
    >
      <details>
        <summary
          id="legacy-artifact-maintenance-title"
          className="cursor-pointer text-sm font-bold text-theme-text"
        >
          高级维护
        </summary>
        <div className="mt-4 space-y-4">
          <button
            ...
            disabled={busy !== null}
            className="inline-flex min-h-9 items-center gap-2 rounded-md border border-theme-border px-3 py-2 text-xs font-bold text-theme-text hover:border-theme-accent disabled:cursor-wait disabled:opacity-60"
          >
          ...
          {hasScanned && visibleSources.length === 0 && (
            <p className="text-xs text-theme-muted">未发现需要整理的旧产物</p>
          )}
            ...
            <div className="space-y-3">
              <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto_auto] sm:items-end">
                <label className="grid gap-1 text-xs font-bold text-theme-text">
                  ...
                  <select
                    aria-label="旧产物"
                    value={sourceKey(selectedSource)}
                    onChange={(event) => {
                      setSelectedKey(event.target.value);
                      setPreview(null);
                      setError(null);
                    }}
                    className="min-h-9 rounded-md border border-theme-border bg-theme-bg px-3 text-sm text-theme-text"
                  >
                ...
                <button
                  ...
                  disabled={busy !== null}
                  className="inline-flex min-h-9 items-center justify-center gap-2 rounded-md bg-theme-text px-3 py-2 text-xs font-bold text-theme-bg disabled:cursor-wait disabled:opacity-60"
                >
                ...
                <button
                  ...
                  onClick={dismiss}
                  disabled={busy !== null}
                  className="inline-flex min-h-9 items-center justify-center gap-2 rounded-md border border-theme-border px-3 py-2 text-xs font-bold text-theme-muted hover:text-theme-text disabled:opacity-60"
                >
                ...
                <div className="space-y-3">
                  <div className="grid gap-0 border border-theme-border md:grid-cols-2">
                    <div className="min-w-0 border-b border-theme-border p-3 md:border-b-0 md:border-r">
                      ...
                      <pre className="mt-2 max-h-72 overflow-auto whitespace-pre-wrap break-words text-xs leading-5 text-theme-muted">
                        {preview.source.originalContent}
                      </pre>
                    </div>
                    <div className="min-w-0 p-3">
                      <h3 className="text-xs font-bold text-theme-text">结构化结果</h3>
                      <pre className="mt-2 max-h-72 overflow-auto whitespace-pre-wrap break-all text-xs leading-5 text-theme-muted">
                        {JSON.stringify(preview.proposedCore, null, 2)}
                      </pre>
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      disabled={busy !== null}
                      className="inline-flex min-h-9 items-center gap-2 rounded-md bg-theme-accent px-3 py-2 text-xs font-bold text-theme-accent-contrast disabled:cursor-wait disabled:opacity-60"
                    >
                      <Check size={14} aria-hidden="true" />
                      {busy === 'confirm' ? '确认中' : '确认结构化版本'}
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      disabled={busy !== null}
                      className="inline-flex min-h-9 items-center gap-2 rounded-md border border-theme-border px-3 py-2 text-xs font-bold text-theme-text disabled:opacity-60"
                    >

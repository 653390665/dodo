...
interface ProjectCockpitViewProps {
  novel: Novel;
  onNavigate: (
    view: 'welcome' | 'library' | 'editor' | 'world' | 'skills' | 'factory' | 'continuation-import'
  ) => void;
...
function getStageRecommendationLabel(
  ...
  if (capability === 'technique')
    return getCapabilityManifest(asset).output === 'transform-preview' ? '精修卡' : '写作技法';
  return '能力卡';
}
...
export function ProjectCockpitView({
  ...
  const configuredCapabilityCount = new Set(
    [
      ...
      ...(capabilityProfile?.capabilityMemberships || []).map(
        (entry) => entry.persistedSkillId || entry.sourceId
      ),
    ].filter(Boolean)
  ).size;
  ...
  const approvedContinuationPack =
    [...approvedPacks].sort((a, b) => b.updatedAt - a.updatedAt)[0] || null;
  ...
  const activeFlowId =
    novel.projectPreferenceProfile?.capabilityProfile?.activeFlowId ||
    novel.projectPreferenceProfile?.activeSeriesId ||
    'generic-novel-flow';
  const activeFlow =
    SKILL_SERIES_FLOWS.find((flow) => flow.id === activeFlowId) ||
    SKILL_SERIES_FLOWS.find((flow) => flow.id === 'generic-novel-flow');

  const primaryActionLabel = workflowState.primaryAction
    ? getWorkflowDisplay(workflowState.primaryAction).primaryAction
    : '继续编辑';
  const primaryActionDescription =
    workflowState.primaryAction === 'sync'
      ? '接入本章上下文是推荐准备动作；也可以先打开编辑器手写正文。'
      : workflowState.primaryAction === 'generate-plan'
        ? '先建立本章分镜，或先打开编辑器手写正文。'
        : workflowState.primaryAction === 'generate-prose'
          ? '分镜已就绪，可以生成正文，也可以继续手写。'
          : workflowState.primaryAction === 'complete-chapter'
            ? '完成本章会编排保存、审阅与事实确认；内容变化后会重新检查。'
            : workflowState.primaryAction === 'resolve-issues'
              ? '审阅发现问题，可回到正文或按证据预览局部修订。'
              : workflowState.primaryAction === 'confirm-facts'
                ? '确认正文产生的事实后再进入下一章。'
                : workflowState.primaryAction === 'create-next-chapter'
                  ? '本章审阅和事实决定均已完成，可以进入下一章。'
                  : '继续当前创作步骤。';
      ...
      <div className="shrink-0 border-b border-theme-border/60 bg-theme-bg/30 px-4 py-4 sm:px-6 sm:py-6 space-y-3">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="w-full sm:w-auto min-w-0 space-y-1">
            <div className="flex items-center gap-2">
              <span className="px-2 py-0.5 text-[10px] font-bold rounded bg-theme-accent/15 text-theme-accent border border-theme-accent/25 uppercase tracking-wide">
                作品总览
              </span>
              <h1 className="text-2xl font-serif font-black text-theme-text leading-tight">
                {novel.title}
              </h1>
            </div>
          ...
          <div className="flex w-full sm:w-auto flex-wrap items-center gap-2 shrink-0">
            <button
              ...
              onClick={() =>
                onOpenAssistant?.('general', { surface: 'workspace', novelId: novel.id })
              }
        ...
        {novel.globalOutline && (
          <div className="rounded-xl border border-theme-border bg-theme-sidebar/20 p-3 text-xs leading-relaxed text-theme-muted/80 line-clamp-3 sm:line-clamp-none">
            ...
            {novel.globalOutline.length > 150
              ? `${novel.globalOutline.slice(0, 150)}...`
              : novel.globalOutline}
          </div>
        )}
      </div>
              ...
              {chapters.length > 0 &&
                ...
                workflowState.primaryAction && (
                  <div className="rounded-xl border border-theme-accent/30 bg-theme-sidebar/10 p-4 sm:p-5 space-y-4 transition-colors hover:border-theme-accent/50">
                    ...
                    <div
                      data-testid="cockpit-llm-availability"
                      className={`rounded-lg border px-3 py-2 text-[11px] leading-relaxed ${llmStatusClass}`}
                    >
              ...
              {chapters.length === 0 && (
                <div className="rounded-xl border border-theme-accent/30 bg-theme-sidebar/10 p-4 sm:p-5 space-y-4 transition-colors hover:border-theme-accent/50">
                  <div className="space-y-2">
                    ...
                    <p className="text-[11px] text-theme-muted/90 leading-relaxed">
                      还没有章节，创建第一章后即可进入正文编辑。
                    </p>
                  </div>
                  <button
            ...
            <div className="space-y-6">
              <div className="border border-theme-border/40 bg-theme-sidebar/10 rounded-xl p-5 space-y-4">
                <div className="border-b border-theme-border/40 pb-3">
                  <h3 className="text-xs font-bold text-theme-text uppercase tracking-wider font-mono">
                    作品资料概览 / 已配置
                  </h3>
                  <p className="mt-1 text-[10px] text-theme-muted leading-relaxed">
                    仅展示当前作品已保存的资料数量，不代表运行时注入、同步或模型读取证明。
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-3 text-xs font-mono">
                  <div>
                    <span className="block text-[10px] text-theme-muted">章节</span>
                    <span className="font-bold text-theme-text">{chapters.length}</span>
                  </div>
                  <div>
                    <span className="block text-[10px] text-theme-muted">设定条目</span>
                    <span className="font-bold text-theme-text">{worldEntitiesCount}</span>
                  </div>
                  <div>
                    <span className="block text-[10px] text-theme-muted">能力卡</span>
                    <span className="font-bold text-theme-text">
                      {configuredCapabilityCount} 项
                    </span>
                  </div>
                  <div>
                    <span className="block text-[10px] text-theme-muted">审核状态</span>
                    <span className="font-bold text-theme-text">
                      已确认 {approvedPacks.length} · 待审核 {draftPackCount}
                    </span>
                  </div>
                  <div>
                    <span className="block text-[10px] text-theme-muted">本章接入状态</span>
                    <span className="font-bold text-theme-text">
                      {approvedContinuationPack?.syncState?.status === 'synced'
                        ? '已接入'
                  ...
                  <div>
                    <span className="block text-[10px] text-theme-muted">最新章节分镜</span>
                    <span className="font-bold text-theme-text">
                      {latestHasBeats ? '已配置' : '未配置'}
                    </span>
                  </div>
                  <div>
                    <span className="block text-[10px] text-theme-muted">全局大纲</span>
                    <span className="font-bold text-theme-text">
                      {novel.globalOutline?.trim() ? '已配置' : '未配置'}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="border border-theme-border/40 bg-theme-sidebar/10 rounded-xl p-4 space-y-2.5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  {stageRecommendations
                    .filter(({ capability }) => capability !== 'role-skill')
                    .map(({ capability, asset }) => (
                      <span
                        key={`${capability}:${asset.id}`}
                        className="rounded border border-theme-border/40 bg-theme-bg/40 px-2 py-1 text-[10px] text-theme-muted"
                      >
                        {getStageRecommendationLabel(capability, asset)} ·{' '}
                        {sanitizeWhiteLabelText(asset.title)}
                      </span>
                    ))}
                </div>
                <div className="border-t border-theme-border/25 pt-2.5 flex items-center justify-between gap-3">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      onOpenCapabilities?.({ novelId: novel.id, stage: capabilityStage })
                    }
              ...
              <div className="border border-theme-border/40 bg-theme-sidebar/10 rounded-xl p-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-mono text-theme-muted">
                    审核状态：已确认 {approvedPacks.length} · 待审核 {draftPackCount}
                  </span>
                </div>
                <div className="space-y-1.5">
                  {packs.map((p) => (
                    ...
                    >
                      <div className="flex flex-col min-w-0">
                        <span className="truncate font-medium text-theme-text text-[11px]">
                          {p.title}
                        </span>
                        <span className="text-[9px] font-mono text-theme-muted mt-0.5">
                          {p.status === 'approved' ? '审核通过' : '待审核'} · 本章接入：
                          {p.syncState?.status === 'synced' ? '已接入' : '未接入'}
                        </span>
                      </div>
                  ...
                  {packs.length === 0 && (
                    <div className="text-[10px] text-theme-muted/80 italic py-0.5">
                      暂无资料包。
                    </div>
                  )}
                </div>
              ...
              <div className="border border-theme-border/40 bg-theme-sidebar/10 rounded-xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="p-2.5 rounded border border-theme-border/30 bg-theme-bg/20 space-y-0.5">
                    <div className="text-[9px] font-mono font-bold text-theme-muted uppercase tracking-wider">
                      Characters / 核心人物
                    </div>
                    <div className="text-sm font-mono font-bold text-theme-text">
                      {characters.length}{' '}
                      <span className="text-[9px] font-sans font-normal text-theme-muted">人</span>
                    </div>
                  </div>
                  <div className="p-2.5 rounded border border-theme-border/30 bg-theme-bg/20 space-y-0.5">
                    <div className="text-[9px] font-mono font-bold text-theme-muted uppercase tracking-wider">
                    ...
                    <div className="text-sm font-mono font-bold text-theme-text">
                      {locations.length + factions.length}{' '}
                      <span className="text-[9px] font-sans font-normal text-theme-muted">个</span>
                    </div>
                  </div>
                </div>
              </div>

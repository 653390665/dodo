...
export function Sidebar({ currentView, onNavigate, user: _user, isAIAssistantOpen }: SidebarProps) {
  ...
  const continuationItem: NavItem = { id: 'continuation-import', label: '资料续写', icon: Upload };
  const advancedItems: NavItem[] = [
    { id: 'factory', label: '拆书工厂', icon: BookTemplate },
    { id: 'skills', label: '作品能力中心', icon: Wand2 },
  ];
          ...
          <div className="flex flex-col gap-1 pr-1.5 pb-4">
            ...
            <div className="pt-3 mt-3 border-t border-theme-border/50" role="group" aria-label="创作工具">
              ...
              <button
                ...
                className="mt-1 flex w-full items-center gap-3 rounded-lg px-3 py-2 text-theme-muted transition-colors hover:bg-theme-border/30 hover:text-theme-text"
              >
                <MoreHorizontal size={16} aria-hidden="true" />
                {!isCollapsed && <span className="flex-1 text-left text-sm">高级工具</span>}
                {!isCollapsed && (
                  <ChevronDown
                    size={14}
                    aria-hidden="true"
                    className={cn('transition-transform', isAdvancedVisible && 'rotate-180')}
                  />
                )}
              </button>
              {isAdvancedVisible && (

...
export function WelcomeView({
  ...
  const buildConfirmedCard = (card: StoryIdeaCard) => ({
    ...card,
    protagonist: confirmedItems.character ? card.protagonist : '',
    coreConflict: confirmedItems.conflict ? card.coreConflict : '',
    starterSeeds: {
      ...card.starterSeeds,
      worldSeed: confirmedItems.world ? card.starterSeeds.worldSeed : '',
      chapterOneSeed: confirmedItems.conflict ? card.starterSeeds.chapterOneSeed : '',
    },
  });
  const handlePersistStory = async (targetView: ViewType, openBibleAssistant = false) => {
    if (!bubbleData || isPersistingStory) return;
    ...
    try {
      ...
      setPersistStoryError(error instanceof Error ? error.message : '立项保存失败，请重试');
    } finally {
            ...
            <div className="border-b border-theme-border/30 pb-3">
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <h1 className="text-lg font-bold text-theme-text tracking-tight font-serif">
                  InkFlow 智能写作终端：一句话开书、长篇设定记忆、拆书卡、深度审稿精修
                </h1>
              </div>
              <p className="text-xs text-theme-muted leading-relaxed">
                从模糊灵感整理立项候选，或深度导入已有资产大纲，确认后进入高品质正文精写。
              </p>
            </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-theme-bg/85 backdrop-blur-md p-4 animate-fade-in">
            <div className="bg-theme-sidebar border border-theme-border/50 max-w-md w-full rounded-2xl p-6 shadow-2xl relative overflow-hidden flex flex-col gap-5 animate-scale-in text-left">
              ...
              <div className="text-center space-y-2">
                ...
                <h3 className="text-base font-bold text-theme-text">确认设定并选择写入位置</h3>
                <p className="text-xs text-theme-muted max-w-sm mx-auto leading-relaxed">
                  立项摘要已确认。选择首发创作起点后，系统会保存作品与第一章。
                </p>
              </div>

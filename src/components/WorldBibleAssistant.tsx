...
请响应作者：${boundedInput}。给出150字内点评；若作者要求创建实体，必须用 [JSON_DATA] 和 [/JSON_DATA] 包裹 JSON，且标记内只能是 JSON。type 可为 character/location/item/faction/powerLevel/timeline，data 遵循：character(name,role,summary,traits[],bio)，location(name,region,description)，item(name,type,description)，faction(name,leader,territory,description)，powerLevel(name,tier,characteristics,description)，timeline(title,description,timestamp,statusTag,order)。`;
  ...
  return <div role="complementary" aria-label="AI 协作助手设定模式" className="bg-theme-sidebar/95 flex flex-col h-full min-h-0">
    <div className="p-4 border-b border-theme-border/50 flex items-center justify-between"><div className="flex items-center gap-2"><Sparkles size={16} className="text-theme-accent" /><h3 className="text-xs font-bold text-theme-text">设定记忆</h3></div><div className="flex gap-1"><button aria-label="清空对话历史" title="清空对话历史" onClick={() => { void (async () => { if (!(await appConfirm('清空对话历史？', '当前设定助手的全部对话将被清空，不可撤销。', { confirmLabel: '清空' }))) return; cancelActiveRequest(); store.clearSession(novel.id, 'bible'); store.setMessages(novel.id, 'bible', [welcome]); })(); }} className="size-7"><Trash2 size={14} /></button><button aria-label="关闭 AI 协作" title="关闭 AI 协作" onClick={onClose} className="size-7"><X size={14} /></button></div></div>

...
async function describeInspirationError(response: Response): Promise<SseError> {
  ...
  return new SseError(`${message}${code}`, {
    ...
    retriable:
      payload && typeof payload.retriable === 'boolean'
        ? payload.retriable
    ...
    reason:
      ...
      payload?.reason === 'reasoning_only' ||
      payload?.reason === 'length_exhausted'
        ? payload.reason
        : undefined,
  });
}
  ...
  return JSON.stringify({
    characters: {
      ...
      details: snapshot.characters.slice(0, MAX_CONTEXT_DETAIL_ROWS).map((item) => ({
        name: item.name,
        role: item.role,
        summary: clipContext(item.summary),
        traits: item.traits.slice(0, 8),
        bio: clipContext(item.bio),
        current_state: clipContext(item.current_state),
      })),
    },
    locations: {
      names: snapshot.locations.map((item) => item.name),
      details: snapshot.locations.slice(0, MAX_CONTEXT_DETAIL_ROWS).map((item) => ({
        name: item.name,
        region: item.region,
        description: clipContext(item.description),
      })),
    },
    items: {
      names: snapshot.items.map((item) => item.name),
      details: snapshot.items.slice(0, MAX_CONTEXT_DETAIL_ROWS).map((item) => ({
        name: item.name,
        type: item.type,
        description: clipContext(item.description),
      })),
    ...
    factions: {
      ...
      details: snapshot.factions.slice(0, MAX_CONTEXT_DETAIL_ROWS).map((item) => ({
        name: item.name,
        leader: item.leader,
    ...
    powerLevels: {
      ...
      details: snapshot.powerLevels.slice(0, MAX_CONTEXT_DETAIL_ROWS).map((item) => ({
        name: item.name,
        tier: item.tier,
        characteristics: clipContext(item.characteristics),
        description: clipContext(item.description),
      })),
    },
    timelineEvents: {
      names: snapshot.timelineEvents.map((item) => item.title),
      details: snapshot.timelineEvents.slice(0, MAX_CONTEXT_DETAIL_ROWS).map((item) => ({
        title: item.title,
...
function normalizeSyncExtraction(value: unknown): SyncExtractionResult {
  ...
  const rows = (key: string) =>
    Array.isArray(root[key]) ? (root[key] as unknown[]).map(asRecord) : [];
  const entityTypes = new Set(['character', 'location', 'item', 'faction']);
  return {
    characters: rows('characters')
      .map((row) => ({
        name: asText(row.name),
        role: asText(row.role) || 'supporting',
        summary: asText(row.summary),
        bio: asText(row.bio),
        traits: Array.isArray(row.traits) ? row.traits.map(asText).filter(Boolean) : [],
        sourceDocumentIds: Array.isArray(row.sourceDocumentIds)
          ? row.sourceDocumentIds.map(asText).filter(Boolean)
          : [],
      }))
      .filter((row) => row.name),
    ...
    factions: rows('factions')
      .map((row) => ({
        name: asText(row.name),
        leader: asText(row.leader),
        territory: asText(row.territory),
        description: asText(row.description),
      }))
      .filter((row) => row.name),
    powerLevels: rows('powerLevels')
      .map((row) => ({
        name: asText(row.name),
        tier: asInteger(row.tier),
        characteristics: asText(row.characteristics),
        description: asText(row.description),
      }))
      .filter((row) => row.name),
    timelineEvents: rows('timelineEvents')
      .map((row) => ({
        title: asText(row.title),
        timestamp: asText(row.timestamp),
        description: asText(row.description),
        order: asInteger(row.order),
      }))
    ...
    relationships: rows('relationships')
      .map((row) => ({
        ...
        targetName: asText(row.targetName),
        targetType: asText(row.targetType),
        relationshipType: asText(row.relationshipType),
        description: asText(row.description),
      }))
      .filter(
        (row) =>
...
async function readInspirationStream(
  ...
  if (!result.done && !sawDoneEvent)
    throw new SseError('生成连接提前结束，未完成冲突检查。', { code: 'SSE_EOF', retriable: true });
  if (!succeeded)
    throw new SseError('模型未返回可用内容，请重试。', {
      code: 'empty_response',
      reason: 'no_content',
      retriable: true,
    });
...
export function WorldBibleAssistant({
  novel,
  onClose,
  continuationPackId,
}: {
  ...
  const requestRef = useRef<{ id: string; novelId: string; controller: AbortController } | null>(
    null
  );
  ...
  const [inferredContinuationPack, setInferredContinuationPack] = useState<{
    novelId: string;
    packId: string;
  } | null>(null);
  const resolvedContinuationPackId =
    continuationPackId ||
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void listContinuationPacks(novel.id)
      .then((packs) => {
        if (cancelled) return;
        const matches = packs.filter(
          (pack) => pack.status === 'approved' && pack.title.trim() === title
        );
        setInferredContinuationPack(
          matches.length === 1 ? { novelId: novel.id, packId: matches[0].id } : null
        );
      })
      .catch(() => undefined);
    return () => {
  ...
  const submit = async (isRetry = false) => {
    ...
    if (failedRequest)
      void recordProductEvent({
        eventName: 'assistant_retry',
        stage: 'assistant',
        result: 'success',
        novelId: novel.id,
        objectId: failedRequest.requestId,
      });
    ...
    const isCurrent = () =>
      ...
      requestRef.current.novelId === novel.id &&
    ...
    try {
      const [characters, locations, items, factions, powerLevels, timelineEvents] =
        await Promise.all([
      ...
      const context = buildCompactWorldContext({
        characters,
        locations,
        items,
        factions,
...
请响应作者：${boundedInput}。给出150字内点评；若作者要求创建实体，必须用 [JSON_DATA] 和 [/JSON_DATA] 包裹 JSON，且标记内只能是 JSON。type 可为 character/location/item/faction/powerLevel/timeline，data 遵循：character(name,role,summary,traits[],bio)，location(name,region,description)，item(name,type,description)，faction(name,leader,territory,description)，powerLevel(name,tier,characteristics,description)，timeline(title,description,timestamp,statusTag,order)。`;
      ...
      const streamed = await readInspirationStream(response, (accumulated) => {
        if (isCurrent())
          store.updateMessage(novel.id, 'bible', assistantId, {
            text: cleanStreamingText(accumulated) || '✍️ 正在设计专属新设定中...',
          });
      });
      if (!isCurrent()) return;
      ...
      const text =
        cleanedText ||
        (parsed
          ? `✨ 已设计好「${String(parsed.data.name || parsed.data.title || '新设定')}」，请确认写入。`
      ...
      if (isCurrent() && store.getSession(novel.id, 'bible').activeRequestId === requestId) {
        ...
        if (failedRequest)
          void recordProductEvent({
            eventName: 'assistant_recovered',
            stage: 'assistant',
            result: 'success',
            novelId: novel.id,
            objectId: failedRequest.requestId,
            durationMs: Math.max(0, Date.now() - failedRequest.failedAt),
          });
        store.clearFailure(novel.id, 'bible');
        setLastReplyFailed(false);
      }
      ...
      if (
        ...
        void recordProductEvent({
          eventName: empty ? 'assistant_empty_response' : 'assistant_failure',
          stage: 'assistant',
          result: 'failure',
          novelId: novel.id,
          objectId: requestId,
        ...
        const finishReason =
          typeof (error as { finishReason?: unknown })?.finishReason === 'string'
            ? String((error as { finishReason: string }).finishReason)
            : undefined;
        const traceId =
        ...
        store.setFailure(novel.id, 'bible', {
          ...
          retriable,
          reason:
            reason === 'no_content' || reason === 'reasoning_only' || reason === 'length_exhausted'
              ? reason
              : undefined,
          finishReason,
          traceId,
      ...
      if (requestRef.current?.id === requestId && requestRef.current.novelId === novel.id) {
        requestRef.current = null;
        store.finishRequest(novel.id, 'bible', requestId);
        store.setLoading(novel.id, 'bible', false);
      }
    }
  };
    ...
    try {
      ...
      for (let attempt = 0; attempt < 2; attempt += 1) {
        try {
          ...
          const { text, databaseGeneration: responseDatabaseGeneration } =
            await readInspirationStream(response);
          const extraction = normalizeSyncExtraction(
            parseModelJsonPayloadStrict(text, { expectedRoot: 'object' })
          );
          const extractedCount =
            extraction.characters.length +
            extraction.locations.length +
            extraction.items.length +
            extraction.factions.length +
            extraction.powerLevels.length +
          ...
          setPreparedSync({
            ...
            textConflicts: [
              ...(novel.globalOutline?.trim() && extraction.globalOutline
                ? ['全局大纲已有内容，新草稿不会自动覆盖']
                : []),
              ...(novel.worldRules?.trim() && extraction.worldRules
                ? ['世界规则已有内容，新草稿不会自动覆盖']
                : []),
            ],
          });
          ...
          const retryableCode =
            ...
            [
              ...
              'INSPIRATION_STREAM_FAILED',
              'INSPIRATION_STREAM_INTERRUPTED',
              'SSE_EOF',
              'SSE_INTERRUPTED',
              'SSE_NO_BODY',
            ].includes(code);
  ...
  const handleConfirmPreparedSync = async (
    ...
    if (
      preparedSync.packId !== resolvedContinuationPackId ||
      preparedSync.sourceReplyId !== latestAssistantMessage?.id
    ) {
      setPreparedSync(null);
    ...
    try {
      ...
      setSyncError(
        `写入失败，未完成本次同步：${error instanceof Error ? error.message : '请重试。'}`
      );
      return false;
    } finally {
      setIsSyncing(false);
  ...
  const commit = async () => {
    ...
    const rejectCommit = (text: string) =>
      store.appendMessage(novel.id, 'bible', {
        id: `${Date.now()}-save-rejected`,
        sender: 'system',
        text,
      });
    ...
    try {
      if (draft.type === 'character')
        await createCharacter(
          {
            id,
            novelId: novel.id,
            name: String(data.name || '未命名人物'),
            role: (data.role || 'supporting') as 'protagonist' | 'supporting' | 'antagonist',
            summary: String(data.summary || ''),
            traits: Array.isArray(data.traits) ? (data.traits as string[]) : [],
            bio: String(data.bio || ''),
            createdAt: now,
            updatedAt: now,
          },
        ...
        await createLocation(
          {
            id,
            novelId: novel.id,
            name: String(data.name || '未命名地点'),
            region: String(data.region || '未知区域'),
        ...
        await createItem(
          {
            id,
            novelId: novel.id,
            name: String(data.name || '未命名道具'),
            type: String(data.type || '普通道具'),
            description: String(data.description || ''),
            createdAt: now,
            updatedAt: now,
          },
        ...
        await createFaction(
          {
            id,
            novelId: novel.id,
            name: String(data.name || '未命名势力'),
            leader: String(data.leader || '未知'),
            territory: String(data.territory || '未知'),
            description: String(data.description || ''),
            createdAt: now,
            updatedAt: now,
          },
      ...
      else if (draft.type === 'powerLevel') {
        const levels = await listPowerLevels(novel.id);
        await createPowerLevel(
          {
            id,
            novelId: novel.id,
            name: String(data.name || '未命名境界'),
            tier:
              Number(data.tier) ||
              (levels.length ? Math.max(...levels.map((level) => level.tier)) + 1 : 1),
        ...
        const events = await listTimelineEvents(novel.id);
        await createTimelineEvent(
          {
            ...
            description: String(data.description || ''),
            timestamp: String(data.timestamp || '未知时间'),
            statusTag: String(data.statusTag || '发生中'),
            order:
              Number(data.order) ||
              (events.length ? Math.max(...events.map((event) => event.order)) + 1 : 1),
            createdAt: now,
            updatedAt: now,
          },
          databaseGeneration
      ...
      store.appendMessage(novel.id, 'bible', {
        id: `${Date.now()}-saved`,
        sender: 'assistant',
        text: `🎉 已将「${String(data.name || data.title || '新设定')}」写入《${novel.title}》。`,
      });
    } catch {
      store.appendMessage(novel.id, 'bible', {
  ...
  const updateDraft = (field: string, value: unknown) =>
    setDraft(
      session.draft ? { ...session.draft, data: { ...session.draft.data, [field]: value } } : null
    );
  ...
  > = {
    character: [
      { key: 'name', label: '姓名' },
      {
        ...
        label: '角色定位',
        kind: 'select',
        options: ['protagonist', 'supporting', 'antagonist'],
      },
      { key: 'summary', label: '一句话简介' },
      { key: 'traits', label: '性格特征' },
      { key: 'bio', label: '背景小传', kind: 'textarea' },
    ],
    location: [
      { key: 'name', label: '地点名称' },
      { key: 'region', label: '所属区域' },
      { key: 'description', label: '环境与危机描述', kind: 'textarea' },
    ],
    item: [
      { key: 'name', label: '道具名称' },
      { key: 'type', label: '道具类型' },
      { key: 'description', label: '异能与来历描述', kind: 'textarea' },
    ],
    faction: [
      { key: 'name', label: '势力名称' },
      { key: 'leader', label: '掌门/领袖' },
      { key: 'territory', label: '总部地盘' },
      { key: 'description', label: '宗门传袭与背景', kind: 'textarea' },
    ],
    powerLevel: [
      { key: 'name', label: '境界名称' },
      { key: 'tier', label: '等阶级别', kind: 'number' },
      { key: 'characteristics', label: '外显异象' },
      { key: 'description', label: '修行奥义', kind: 'textarea' },
    ],
    timeline: [
      { key: 'title', label: '事件标题' },
      { key: 'timestamp', label: '时间纪元' },
      {
        key: 'statusTag',
        label: '事件状态',
        kind: 'select',
      ...
      { key: 'order', label: '排序权重', kind: 'number' },
      { key: 'description', label: '影响与过程描述', kind: 'textarea' },
    ],
  };
  const renderDraftFields = (draft: Draft) =>
    fieldSpecs[draft.type].map((spec) => {
      ...
      return (
        <label key={spec.key} className="block text-[10px] text-theme-muted">
          {spec.label}
          {spec.kind === 'textarea' ? (
            <textarea
              rows={2}
              ...
              className="w-full mt-1 bg-theme-sidebar border border-theme-border rounded px-2 py-1 text-xs text-theme-text resize-none"
            />
          ) : spec.kind === 'select' ? (
            <select
              value={displayValue || spec.options?.[0]}
              onChange={(event) => change(event.target.value)}
              className="w-full mt-1 bg-theme-sidebar border border-theme-border rounded px-2 py-1 text-xs text-theme-text"
            >
    ...
    const collect = (label: string, proposed: string[], existing: string[]) => {
      ...
      proposed.forEach((name) => {
        if (existingNames.has(normalize(name)))
          conflicts.push(`${label}“${name}”已存在，将保留原设定并跳过新增`);
      });
    };
    collect(
      '人物',
      preparedSync.extraction.characters.map((item) => item.name),
      preparedSync.existing.characters.map((item) => item.name)
    );
    collect(
      '地点',
      preparedSync.extraction.locations.map((item) => item.name),
      preparedSync.existing.locations.map((item) => item.name)
    );
    collect(
      '道具',
      preparedSync.extraction.items.map((item) => item.name),
      preparedSync.existing.items.map((item) => item.name)
    );
    collect(
      '势力',
      preparedSync.extraction.factions.map((item) => item.name),
      preparedSync.existing.factions.map((item) => item.name)
    );
    collect(
      '力量体系',
      preparedSync.extraction.powerLevels.map((item) => item.name),
      preparedSync.existing.powerLevels.map((item) => item.name)
    );
    collect(
      '时间线',
      preparedSync.extraction.timelineEvents.map((item) => item.title),
      preparedSync.existing.timelineEvents.map((item) => item.title)
    );
  ...
  return (
    <div
      role="complementary"
      aria-label="AI 协作助手设定模式"
      className="bg-theme-sidebar/95 flex flex-col h-full min-h-0"
    >
      <div className="p-4 border-b border-theme-border/50 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Sparkles size={16} className="text-theme-accent" />
          <h3 className="text-xs font-bold text-theme-text">设定记忆</h3>
        </div>
            ...
            onClick={() => {
              void (async () => {
                if (
                  !(await appConfirm(
                    '清空对话历史？',
                    '当前设定助手的全部对话将被清空，不可撤销。',
                    { confirmLabel: '清空' }
                  ))
                )
                  return;
                cancelActiveRequest();
                store.clearSession(novel.id, 'bible');
                store.setMessages(novel.id, 'bible', [welcome]);
              })();
            }}
            className="size-7"
          >
      ...
      <div role="log" aria-live="polite" className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            className={
              message.sender === 'user'
                ...
                : message.sender === 'system'
                  ? 'text-center text-[10px] text-theme-muted'
                  : ''
            }
          >
            <div
              className={
                message.sender === 'user'
                  ? 'bg-theme-accent text-theme-accent-contrast px-3 py-2 rounded-2xl text-xs max-w-[85%] whitespace-pre-wrap'
                  : 'bg-theme-bg/60 border border-theme-border/30 text-theme-text px-3 py-2 rounded-2xl text-xs max-w-[85%] whitespace-pre-wrap'
              }
            >
              {message.text}
            </div>
        ...
        {isLoading && (
          <div className="text-theme-muted text-[10px]">
            <Loader2 size={12} className="inline animate-spin" /> 助手正在思考并设计中...
          </div>
        )}
        <div ref={endRef} />
      </div>
      {session.failure && (
        <div
          role="alert"
          aria-label="助手请求失败"
          className="mx-4 mb-3 rounded-xl alert-warning p-3 text-[10px]"
        >
          <div>
            {session.failure.reason === 'no_content'
              ? '模型未返回内容'
          ...
          {session.failure.finishReason && (
            <div>
              结束方式：
              {FINISH_REASON_COPY[session.failure.finishReason] ?? session.failure.finishReason}
            </div>
          )}
          {session.failure.traceId && <div>诊断编号：{session.failure.traceId}</div>}
      ...
      {syncError && (
        <div className="mx-4 mb-3 rounded-xl alert-warning p-3 text-[10px]">
          <div>{syncError}</div>
          {(syncErrorCode === 'configuration' ||
            syncErrorCode === 'authentication' ||
            syncErrorCode === 'billing') && (
            <button
              type="button"
          ...
          <button
            type="button"
            className="ml-3 mt-2 font-bold underline"
            onClick={() => void prepareSyncFromLatestReply()}
            disabled={isPreparingSync}
          >
            重试冲突检查
      ...
      {preparedSync && (
        <div className="mx-4 mb-3 min-h-0 max-h-[52vh] overflow-y-auto rounded-xl border border-theme-border bg-theme-sidebar p-3 space-y-3">
          ...
          <div className="rounded-lg alert-warning p-2 text-[10px]">
            ...
            <div className="mt-1">
              系统只自动识别同名重复、已有全局文本和关系端点；无法证明语义完全一致，重复项默认保留旧设定。
            </div>
            {directConflicts.length > 0 && (
              <ul className="mt-1 list-disc pl-4 space-y-0.5">
                {directConflicts.map((item) => (
                  <li key={item}>{item}</li>
                ))}
              </ul>
            )}
      ...
      {session.draft && (
        <div className="mx-4 mb-4 p-4 border border-theme-border rounded-2xl space-y-3">
          <div className="text-xs font-bold">新设定确认单</div>
          <div className="space-y-2 max-h-56 overflow-y-auto">
            {renderDraftFields(session.draft)}
          </div>
          <div className="flex gap-2">
            <button onClick={() => setDraft(null)} className="flex-1 text-xs border rounded py-2">
              放弃
            </button>
            <button
              aria-label="确认写入设定"
              onClick={commit}
              className="flex-1 text-xs bg-theme-accent text-theme-accent-contrast rounded py-2"
            >
              确认写入设定 <ChevronRight size={12} className="inline" />
            </button>
          </div>
        </div>
      )}
      <div className="p-4 border-t border-theme-border/50">
        <textarea
          ...
          className="w-full bg-theme-bg border border-theme-border rounded-xl px-3 py-2 text-xs text-theme-text resize-none"
        />
        <button
          aria-label="发送设定灵感"
          onClick={() => void submit()}
          disabled={isLoading}
          className="mt-2 w-full bg-theme-accent text-theme-accent-contrast rounded-xl py-2 text-xs"
        >

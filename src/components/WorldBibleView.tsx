...
import type {
  ...
  CharacterCore,
  StructuredWorldCore,
} from '../../shared/types/creative-artifacts';
import { parseDocAsync } from '../lib/prompt-client';
import { startWorldJob } from '../lib/world-job-client';
import {
  decideArtifactCandidate,
  dismissCapabilityRecommendation,
  fetchArtifactGovernance,
  fetchCapabilityRecommendationDismissed,
} from '../lib/world-governance-client';
...
import {
  readContinuationSyncIntent,
  clearContinuationSyncIntent,
} from '../lib/continuation-sync-intent';
import { diagnoseCharacterCore } from '../../shared/lib/character-core';
import {
  buildCapabilityRecommendationDismissal,
  buildCapabilityRecommendations,
} from '../../shared/lib/capability-recommendation';
...
type EditableWorldEntityType =
  'character' | 'location' | 'item' | 'timeline' | 'faction' | 'powerLevel';
...
function mergeEntityDrafts<T extends { id: string }>(
  ...
  return entities.map((entity) => ({
    ...entity,
    ...(drafts.get(entityDraftKey(type, entity.id)) || {}),
  }));
}
...
export function WorldBibleView({
  ...
  const [characterCandidatesById, setCharacterCandidatesById] = useState<
    Record<string, ArtifactCandidate<CharacterCore>>
  >({});
  const [worldCandidate, setWorldCandidate] =
    useState<ArtifactCandidate<StructuredWorldCore> | null>(null);
  const [worldCore, setWorldCore] = useState<{ core: StructuredWorldCore; version: number } | null>(
    null
  );
  ...
  const [pendingCharacterLaunch, setPendingCharacterLaunch] = useState<{
    launchToken: number;
    capabilityId: string;
    selectedCharacterId: string;
  } | null>(null);
  const [dismissedCharacterRecommendations, setDismissedCharacterRecommendations] = useState<
    Set<string>
  >(() => new Set());
  ...
  useEffect(
    () => () => {
      worldCandidateAbortControllerRef.current?.abort(
        new Error('作品上下文已变化，世界观候选任务已取消')
      );
    ...
    return Object.fromEntries(
      characters.flatMap((character) => {
        ...
        const result = buildCapabilityRecommendations({
          ...
          availableArtifacts: [
            { kind: 'character', id: character.id, version: character.coreVersion || 0 },
          ],
  ...
  const characterRecommendations = useMemo(
    () =>
      Object.fromEntries(
        Object.entries(rawCharacterRecommendations).filter(
          ([, result]) => !dismissedCharacterRecommendations.has(result.fingerprint)
        )
      ),
  ...
  useEffect(() => {
    let cancelled = false;
    const pending = Object.values(rawCharacterRecommendations).filter(
      (result) => !checkedCharacterRecommendationFingerprintsRef.current.has(result.fingerprint)
    );
    pending.forEach((result) =>
      checkedCharacterRecommendationFingerprintsRef.current.add(result.fingerprint)
    );
    if (pending.length === 0)
      return () => {
        cancelled = true;
      };
    void getDatabaseGenerationSnapshot()
      .then(async (databaseGeneration) => {
        const dismissed = await Promise.all(
          pending.map(async (result) => {
            const isDismissed = await fetchCapabilityRecommendationDismissed(
              buildCapabilityRecommendationDismissal(result, novel.id, databaseGeneration)
            );
            return isDismissed ? result.fingerprint : undefined;
          })
        );
        if (!cancelled)
          setDismissedCharacterRecommendations(
            (current) =>
              new Set([...current, ...dismissed.filter((value): value is string => Boolean(value))])
          );
      })
      .catch(() => {});
    return () => {
  ...
  const dismissCharacterRecommendation = async (
    result: ReturnType<typeof buildCapabilityRecommendations>
  ) => {
    const databaseGeneration = await getDatabaseGenerationSnapshot();
    await dismissCapabilityRecommendation(
      buildCapabilityRecommendationDismissal(result, novel.id, databaseGeneration)
    );
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      if (!Number.isInteger(beforeGeneration))
        throw new Error('设定读取缺少有效数据库代次，请刷新后重试。');
      const characterGovernancePromise = fetchArtifactGovernance<{
        ...
        `/api/novels/${encodeURIComponent(novel.id)}/artifacts?kind=character&status=pending`,
        '角色治理'
      );
      const worldGovernancePromise = fetchArtifactGovernance<{
        ...
        `/api/novels/${encodeURIComponent(novel.id)}/artifacts?kind=world&status=pending`,
        '世界治理'
      );
      const [
        characters,
      ...
      const characterGovernance =
        characterGovernanceResult.status === 'fulfilled'
          ? characterGovernanceResult.value
          : { cores: [], candidates: [] };
      const worldGovernance =
        worldGovernanceResult.status === 'fulfilled'
      ...
      const governedCharacters = characters.map((character) => {
        ...
        return stored
          ? { ...character, core: stored.core, coreVersion: stored.version }
          : character;
      });
      ...
      setCharacterCandidatesById(
        Object.fromEntries(
          characterGovernance.candidates.map((candidate) => [candidate.target.id, candidate])
        )
      );
      const storedWorldCore = worldGovernance.cores.find((entry) => entry.artifactId === novel.id);
      setWorldCore(
        storedWorldCore ? { core: storedWorldCore.core, version: storedWorldCore.version } : null
      );
      setWorldCandidate(
        worldGovernance.candidates.find((candidate) => candidate.target.id === novel.id) || null
      );
      ...
      setWorldDataError(
        governanceReady ? null : '设定治理状态读取失败，普通设定仍可查看；请刷新后重试。'
      );
    } catch (error) {
      if (requestSeq !== fetchRequestSeqRef.current) return;
  ...
  const handleSaveGlobalInfo = async (outline: string, rules: string) => {
    ...
    try {
      ...
      const updated = await updateNovel(
        novel.id,
        { globalOutline: outline, worldRules: rules },
        generation
      );
      ...
      setWorldDataError(
        isDatabaseGenerationConflict(error)
  ...
  const applyLocalEntityPatch = (
    ...
    if (type === 'character')
      setCharacters((current) =>
        current.map((entity) => (entity.id === id ? ({ ...entity, ...data } as Character) : entity))
      );
    else if (type === 'location')
      setLocations((current) =>
        current.map((entity) => (entity.id === id ? ({ ...entity, ...data } as Location) : entity))
      );
    else if (type === 'item')
      setItems((current) =>
        current.map((entity) => (entity.id === id ? ({ ...entity, ...data } as Item) : entity))
      );
    else if (type === 'timeline')
      setTimelineEvents((current) =>
        current.map((entity) =>
          entity.id === id ? ({ ...entity, ...data } as TimelineEvent) : entity
        )
      );
    else if (type === 'faction')
      setFactions((current) =>
        current.map((entity) => (entity.id === id ? ({ ...entity, ...data } as Faction) : entity))
      );
    else
      setPowerLevels((current) =>
        current.map((entity) =>
          entity.id === id ? ({ ...entity, ...data } as PowerLevel) : entity
        )
      );
  };

  const clearCommittedEntityDraft = (
    type: EditableWorldEntityType,
    id: string,
    data: Record<string, unknown>
  ) => {
  ...
  const addEntity = async (
    type: 'character' | 'location' | 'item' | 'timeline' | 'faction' | 'powerLevel'
  ) => {
    try {
      ...
      if (type === 'character') {
        ...
        const highestOrder =
          timelineEvents.length > 0 ? Math.max(...timelineEvents.map((e) => e.order)) : 0;
        ...
        const highestTier =
          powerLevels.length > 0 ? Math.max(...powerLevels.map((e) => e.tier)) : 0;
        const entity: PowerLevel = {
          id,
          novelId: novel.id,
          name: '新境界',
          tier: highestTier + 1,
  ...
  const deleteEntity = async (
    ...
    try {
      ...
      if (type === 'character')
        setCharacters((current) => current.filter((entity) => entity.id !== id));
      else if (type === 'location')
        setLocations((current) => current.filter((entity) => entity.id !== id));
      else if (type === 'item') setItems((current) => current.filter((entity) => entity.id !== id));
      else if (type === 'timeline')
        setTimelineEvents((current) => current.filter((entity) => entity.id !== id));
      else if (type === 'faction')
        setFactions((current) => current.filter((entity) => entity.id !== id));
  ...
  const handleGenerateBio = async (char: Character) => {
    ...
    try {
      ...
      await streamCharacterBio({
        ...
        onPreview: (bio) => {
          setCharacters((prev) =>
            prev.map((item) => (item.id === char.id ? { ...item, bio } : item))
          );
        },
        onCommit: async (bio) => {
  ...
  const handleGenerateCharacterCandidate = async (
    char: Character,
    capabilityId = 'bible-character-arc'
  ) => {
    setGeneratingCharacterCandidateIds((current) =>
      current.includes(char.id) ? current : [...current, char.id]
    );
    try {
  ...
  const handleGenerateWorldCandidate = async (
    ...
    setWorldCandidateError(null);
    try {
      const { result } = await startWorldJob<{
        candidate?: ArtifactCandidate<StructuredWorldCore>;
      }>(
        '/api/generate-outline',
        {
          novelId: novel.id,
  ...
  const decideCharacterCandidate = async (
    candidate: ArtifactCandidate<CharacterCore>,
    action: 'accept' | 'reject'
  ) => {
    try {
      const databaseGeneration = await getDatabaseGenerationSnapshot();
      const result = await decideArtifactCandidate<CharacterCore>(
        novel.id,
        candidate.id,
        action,
        databaseGeneration
      ...
      if (action === 'accept' && result.core) {
        setCharacters((current) =>
          current.map((character) =>
            character.id === candidate.target.id
  ...
  const decideWorldCandidate = async (
    candidate: ArtifactCandidate<StructuredWorldCore>,
    action: 'accept' | 'reject'
  ) => {
    try {
      setWorldCandidateError(null);
      const databaseGeneration = await getDatabaseGenerationSnapshot();
      const result = await decideArtifactCandidate<StructuredWorldCore>(
        novel.id,
        candidate.id,
        action,
        databaseGeneration
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      if (intent.artifactKind === 'world') {
        ...
        setPendingCharacterLaunch(null);
        if (!worldCandidate)
          void handleGenerateWorldCandidate(intent.capabilityId, intent.seedText);
        return;
      }
      ...
      if (target) {
        setPendingCharacterLaunch(null);
        if (!characterCandidatesById[target.id])
          void handleGenerateCharacterCandidate(target, intent.capabilityId);
        return;
      }
      setPendingCharacterLaunch({
        launchToken: intent.launchToken,
    ...
    artifactGovernanceLoaded,
    capabilityLaunchIntent,
    characterCandidatesById,
    characters,
    novel.id,
    onCapabilityLaunchConsumed,
    worldCandidate,
    ...
    return (
      <div className="max-w-3xl mx-auto py-16 px-6 flex flex-col items-center justify-center text-center space-y-8 bg-transparent">
        ...
        <div className="space-y-3">
          <h2 className="text-3xl font-serif font-black text-theme-text">
            初始化您的《{novel.title}》设定集
          </h2>
          <p className="text-sm text-theme-muted max-w-lg leading-relaxed mx-auto">
            当前设定集内空空如也。AI
            无法在此嗅探到您笔下世界的人物和规则。推荐通过以下动作快速冷启动，让您的作品拥有丰满的底蕴：
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full pt-4">
          ...
          >
            ...
            <span className="text-[11px] text-theme-muted leading-relaxed">
              设定主角姓名、身份和背景小传
            </span>
          </button>
  ...
  if (onboarding) {
    return (
      <WorldBibleOnboarding onboarding={onboarding} isGlobalAssistantOpen={isGlobalAssistantOpen} />
    );
  }
      ...
      <div className="flex-1 flex overflow-hidden">
          ...
          {tabs.map((tab) => (
            ...
            >
              ...
              <span className="ml-auto text-xs opacity-60">
                {tab.id === 'pack-management' &&
                  continuationPacks.length > 0 &&
                  continuationPacks.length}
        ...
        <div className="flex-1 overflow-y-auto p-8 relative">
          {worldDataError ? (
            <div
              role="alert"
              className="mb-5 flex flex-wrap items-center justify-between gap-3 border-y alert-warning px-4 py-3 text-sm"
            >
          ...
          activeTab !== 'graph' ? (
            ...
            <>
                ...
                <div key="overview">
                  <ContinuationOverviewPanel
                    ...
                    onStartStoryboard={(packId, prefillIntent) => {
                      ...
                      onEnterStoryboard?.(
                        packId,
                        prefillIntent || (pack ? buildCreationIntentDraft(pack) : undefined)
                      );
                    }}
                  />
                </div>
                ...
                <div className="space-y-4">
                  {pendingCharacterLaunch ? (
                    <section
                      role="region"
                      aria-label="选择角色候选目标"
                      className="border-y border-theme-border bg-theme-sidebar/40 px-5 py-4"
                    >
                      <div className="flex flex-wrap items-end gap-3">
                        <label className="min-w-56 flex-1 text-xs font-bold text-theme-text">
                          ...
                          <select
                            ...
                            onChange={(event) =>
                              setPendingCharacterLaunch((current) =>
                                current
                        ...
                        <button
                          ...
                          onClick={() => {
                            const target = characters.find(
                              (character) =>
                                character.id === pendingCharacterLaunch.selectedCharacterId
                            );
                            if (!target) return;
                            const capabilityId = pendingCharacterLaunch.capabilityId;
                            setPendingCharacterLaunch(null);
                            if (!characterCandidatesById[target.id])
                              void handleGenerateCharacterCandidate(target, capabilityId);
                          }}
                  ...
                  <CharactersTab
                    ...
                    onGenerateCandidate={(character) =>
                      void handleGenerateCharacterCandidate(character)
                    }
                    onAcceptCandidate={(candidate) =>
                      void decideCharacterCandidate(candidate, 'accept')
                    }
                    onRejectCandidate={(candidate) =>
                      void decideCharacterCandidate(candidate, 'reject')
                    }
                    recommendationsByCharacterId={characterRecommendations}
                    onDismissRecommendation={(_character, result) => {
                      void dismissCharacterRecommendation(result).catch((error) =>
                        toast(
                          error instanceof Error ? error.message : '暂时无法忽略该推荐',
                          'error'
                        )
                      );
                    }}
                ...
                <div key="pack-management">
                  <ContinuationPackView
                    ...
                    onAutoSyncConsumed={(packId) => {
                      setRequestedAutoSyncPackId((current) =>
                        current === packId ? null : current
                      );
                    }}
              ...
              {activeTab === 'contract' && (
                <div
                  key="contract"
                  className="max-w-3xl mx-auto bg-theme-sidebar rounded-2xl border border-theme-border/50 shadow-md"
                >
                  <StoryContractPanel
                    ...
                    onSave={async (newContract) => {
                      const updatedProfile: ProjectPreferenceProfile = {
                        ...
                        acceptedDimensions:
                          novel.projectPreferenceProfile?.acceptedDimensions || [],
                        rejectedDimensions:
                          novel.projectPreferenceProfile?.rejectedDimensions || [],
                      ...
                      <div className="divide-y divide-theme-border/20">
                        {relationships.map((rel) => {
                          ...
                          return (
                            <div
                              key={rel.id}
                              className="flex items-center justify-between px-4 py-2.5 gap-3 hover:bg-theme-sidebar/30 transition-colors"
                            >
                              <div className="flex items-center gap-2 text-xs min-w-0">
                                ...
                                <span className="text-theme-accent font-bold shrink-0">
                                ...
                                {rel.description && (
                                  <span className="text-theme-muted truncate hidden sm:inline">
                                    （{rel.description}）
                                  </span>
                                )}
                              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-md animate-fade-in">
            <div className="w-[420px] bg-theme-sidebar/85 border border-theme-border/60 p-7 rounded-2xl shadow-2xl backdrop-blur-xl relative overflow-hidden flex flex-col gap-5 text-center">
              ...
              <div className="flex flex-col items-center justify-center gap-3">
                ...
                <h3 className="text-base font-serif font-bold text-theme-text mt-1">
                  智能设定导入与深度解析
                </h3>
                <p className="text-xs text-theme-muted px-2">
                  我们正在使用 AI
                  深入阅读和解构您上传的设定文档，并在本地持久化建立各实体索引与逻辑关联。
                </p>
              </div>

              <div className="space-y-2 text-left">
                <div className="flex justify-between items-center text-xs px-0.5">
                  <span className="text-theme-muted font-medium animate-pulse">
                    {importStageText || '正在读取文档内容...'}
                  </span>

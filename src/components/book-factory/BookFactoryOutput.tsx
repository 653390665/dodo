...
interface BookFactoryOutputProps {
  ...
  extractionQuality?: ExtractSkillResponse['quality'] | null;
  ...
  userNovels: Novel[];
  /** Legacy UI state kept for callers while saved-card feedback is derived elsewhere. */
  lastSavedSkillId?: string;
  selectedSavedSkillId: string;
  savedDeckIds: string[];
  onTestDrive: () => void;
  ...
  onDeckSelectionChange: (selection: ProjectSkillDeckSelection) => void;
}
...
export function BookFactoryOutput({
  ...
  const persistenceBlocked = isModelPending || extractionQuality?.passed === false;
  const persistenceHint = isModelPending
    ? 'AI 深度分析完成前不能保存或提交卡片。'
    : extractionQuality?.passed === false

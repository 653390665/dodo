...
import type {
  Chapter,
  ContinuationPack,
  ContextReceipt as ContextReceiptData,
} from '../../../shared/types';
    ...
    <details className="bg-theme-sidebar/40 rounded-xl border border-theme-border/60 p-3.5 mb-4">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 text-[10px] font-bold text-theme-muted uppercase tracking-wider focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-theme-accent/50 focus-visible:ring-offset-2">
        <span className="flex min-w-0 items-center gap-1.5 truncate">
          <span
            aria-hidden="true"
            className={`h-1.5 w-1.5 shrink-0 rounded-full ${isActual ? 'bg-emerald-500' : 'bg-amber-500'}`}
          />
      ...
      <div className="mt-3 grid grid-cols-1 gap-2 text-xs sm:grid-cols-2">
        {receipt ? (
          <div className="col-span-full rounded-lg border border-theme-border/60 px-2 py-1.5 text-theme-muted">
            {receipt.actual ? '本次运行上下文凭证' : '选择摘要（非实际运行凭证）'} ·{' '}
            {receipt.injectedChars} 字符 · {receipt.itemCount} 条
            {receipt.truncated ? ' · 已截断' : ''}
            {!receipt.actual && ' · 来源版本未知'}
            {receipt.sources?.map((source) => (
              <div key={source.id}>
                {source.label} · {source.sha256 ? source.sha256.slice(0, 8) : 'legacy/unknown'} ·{' '}
                {source.chars} 字符 · {source.itemCount} 条{source.truncated ? ' · 已截断' : ''}
              </div>
            ))}
          </div>
        ) : (
        ...
        <div className="flex min-w-0 items-center gap-1.5 text-theme-muted">
          ...
          <span className="min-w-0 truncate">
            目标章节:{' '}
            <strong className="text-theme-text">{currentChapter?.title || '未选择'}</strong>
          </span>
        </div>
        ...
        <div className="flex min-w-0 items-center gap-1.5 text-theme-muted">
          ...
          <span className="min-w-0 truncate">
            ...
            <strong className="text-theme-text">
              {selectedContinuationPack?.title || '未绑定'}
            </strong>
          </span>
        </div>

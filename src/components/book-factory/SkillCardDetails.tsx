...
export function SkillCardDetails({
  ...
  const cardTypeInfo = selectedSkill.deconstructionCardType
    ? DECONSTRUCTION_CARD_TYPES[selectedSkill.deconstructionCardType]
    : null;
  ...
  return (
    <div className="space-y-6 mt-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
        <div className="rounded-xl border border-theme-border bg-theme-sidebar/25 px-3 py-2 text-theme-muted md:col-span-2">
          ...
          {governance.reasons.length > 0 && (
            <div className="mt-1">{governance.reasons.join('；')}</div>
          )}
        </div>
        <div className="rounded-xl border border-theme-border bg-theme-sidebar/25 px-3 py-2 text-theme-muted">
          ...
          <div>
            分值{' '}
            <span className="font-bold text-theme-text">{scoreChannels.coldStartScore ?? '—'}</span>
          </div>
        </div>
        <div className="rounded-xl border border-theme-border bg-theme-sidebar/25 px-3 py-2 text-theme-muted">
          <div className="font-bold text-theme-text">真实使用反馈</div>
          <div>
            {scoreChannels.observedPerformance ? (
              <>
                使用反馈{' '}
                <span className="font-bold text-theme-text">
                  {scoreChannels.observedPerformance.score}
                </span>
                （{scoreChannels.observedPerformance.sampleSize} 次）
              </>
            ) : (
              <>
                <div>暂无使用反馈</div>
      ...
      {/* 拆解卡健康打分与去污染面板 */}
      <div
        className={`p-5 rounded-2xl border transition-all duration-300 ${gradeInfo.border} ${gradeInfo.bg} ${gradeInfo.glow}`}
      >
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex flex-col items-center shrink-0">
            <div
              className={`w-14 h-14 rounded-2xl border ${gradeInfo.border} flex items-center justify-center font-bold text-3xl font-serif ${gradeInfo.text} bg-theme-sidebar shadow-inner`}
            >
              {report.grade}
            </div>
            <span className="text-[10px] font-bold text-theme-muted mt-1.5">
              {report.score} / 100
            </span>
          </div>
        </div>
            ...
            {!hasDeductions ? (
              <>
                ...
                <span className="text-xs text-emerald-600 font-medium">
                  白璧无瑕！该卡未检测到任何命名实体泄露或 AI 腔套话，纯净度完美。
                </span>
              </>
            ) : (
              <>
              ...
              <div className="space-y-1">
                ...
                <div className="space-y-1">
                  {report.details.evidenceDeductions.map((deduction, idx) => (
                    <div
                      key={deduction + idx}
                      className="flex items-center gap-2 text-xs text-theme-text/80 bg-theme-sidebar/50 px-3 py-1.5 rounded-lg border border-theme-border"
                    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div
          className={`rounded-xl border px-4 py-3 ${isDeconstructionCard && cardTypeInfo ? `${cardTypeInfo.border} ${cardTypeInfo.bg}` : 'border-theme-border bg-theme-sidebar/25'}`}
        >
          <div className="text-[10px] font-bold text-theme-muted uppercase tracking-wider flex items-center gap-1.5">
            {IconComponent && (
              <IconComponent
                className={`w-3.5 h-3.5 ${cardTypeInfo?.text || ''}`}
                aria-hidden="true"
              />
            )}
            <span>卡片类型</span>
          ...
          <div
            className={`text-sm font-bold mt-2 ${isDeconstructionCard && cardTypeInfo ? cardTypeInfo.text : 'text-theme-text'}`}
          >
            {isDeconstructionCard && cardTypeInfo
              ? cardTypeInfo.label
              : selectedSkillIndex === 0 && deck
                ? '主笔卡（主导叙事基调）'
                : `辅卡 · ${rec.cardType}`}
          </div>
      ...
      <div className="bg-theme-sidebar/20 p-4 rounded-xl border border-theme-border/50 space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {(selectedSkill.evidenceMoments || []).map((moment) => (
            <span
              key={moment}
              className="px-3 py-1.5 rounded-full border border-theme-border bg-theme-sidebar text-[11px] text-theme-muted"
            >
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 inline-flex">
                {selectedSkill.corePatterns?.map((v) => (
                  <span
                    key={v}
                    className="px-2 py-0.5 bg-theme-accent/10 border border-theme-accent/20 text-theme-accent rounded text-[10px] font-bold"
                  >
                    {v}
                  </span>
                ))}
      ...
      <div className="bg-theme-sidebar p-4 rounded-xl border border-theme-border shadow-sm">
        <h4 className="text-[10px] font-bold text-red-500 uppercase mb-3">
          绝对禁止红线 (OOC / 毒点)
        </h4>
        <div className="flex flex-wrap gap-2">
          {(selectedSkill.bannedElements || selectedSkill.bannedWords || []).map((w) => (
            <span
              key={w}
              className="px-2 py-0.5 bg-red-50 border border-red-100 text-red-600 rounded text-[10px] line-through"
            >
              {w}
            </span>
          ))}
      ...
      <div className="bg-theme-sidebar p-4 rounded-xl border border-theme-border shadow-sm">
        ...
        <div className="space-y-2">
          {(selectedSkill.fewShots || []).map((s, idx) => (
            <div
              key={s + idx}
              className="text-xs text-theme-muted italic p-2 bg-theme-sidebar/10 rounded-lg border-l-2 border-theme-accent/30 font-serif"
            >
              "{s}"
            </div>
          ))}
      ...
      {deck?.methodChain && (
        <div className="bg-theme-sidebar p-4 rounded-xl border border-theme-border shadow-sm">
          ...
          <p className="text-[10px] text-theme-muted mb-3 leading-relaxed">
            {deck.methodChain.summary}
          </p>
          <div className="space-y-3">
            {deck.methodChain.items.map((qa, idx) => (
              <div
                key={qa.question + idx}
                className="rounded-lg border border-theme-border bg-theme-sidebar/10 p-3"
              >
      ...
      {selectedSkill.whyThisSkillWorks && (
        <div className="bg-theme-sidebar/20 p-4 rounded-xl border border-theme-border/50">
          <h4 className="text-[10px] font-bold text-theme-text uppercase mb-1">
            为什么这张拆书卡成立
          </h4>
          <p className="text-xs text-theme-text leading-relaxed">
            {selectedSkill.whyThisSkillWorks}

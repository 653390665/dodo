  ...
  return (
    <article
      className={`rounded-3xl border p-5 shadow-sm ${
        selected
          ? 'border-theme-accent bg-theme-sidebar/20'
          : 'border-theme-border bg-theme-sidebar'
      }`}
    >
      <div className="mb-3 text-xs font-bold uppercase tracking-[0.16em] text-theme-muted">
        故事方案
      </div>
      ...
      <div className="space-y-2 text-sm text-theme-text">
        ...
        <div className="rounded-2xl bg-theme-sidebar/20 px-3 py-3 text-xs text-theme-muted">
          ...
          <div className="mt-1">
            {card.planningFit.recommendedFocus} · {card.planningFit.recommendedPacing}
          </div>
      ...
      <div className="mt-4 flex gap-2">
        <button
          onClick={onSelect}
          className="rounded-full bg-theme-accent px-4 py-2 text-xs font-bold text-theme-accent-contrast"
        >
          选这个
        </button>
        <button

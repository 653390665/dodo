...
export function GuardrailPolicyPanel({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="guardrail-policy-title"
    >
      ...
      <div className="relative w-full max-w-xl max-h-[85vh] overflow-y-auto rounded-xl border border-theme-border bg-theme-sidebar p-5 shadow-xl">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 id="guardrail-policy-title" className="text-base font-bold text-theme-text">
              质量标准
            </h2>
        ...
        <section className="mt-4" aria-label="默认护栏">
          ...
          <div className="mt-2 space-y-1.5">
            {coreGuardrails.map((asset) => (
              <div
                key={asset.id}
                className="flex items-start justify-between gap-3 rounded-lg border border-theme-border/60 bg-theme-bg/50 px-3 py-2"
              >
                <div className="min-w-0">
                  <p className="text-xs font-bold text-theme-text truncate">{asset.title}</p>
                  {asset.goal && (
                    <p className="mt-0.5 text-[10px] leading-4 text-theme-muted line-clamp-1">
                      {asset.goal}
                    </p>
                  )}
                </div>
                  ...
                  <div
                    ...
                    className={cn(
                      ...
                      enabled
                        ...
                        : 'border-theme-border/60 bg-theme-bg/50'
                    )}
                  >
                    <div className="min-w-0">
                      <p className="text-xs font-bold text-theme-text truncate">{asset.title}</p>

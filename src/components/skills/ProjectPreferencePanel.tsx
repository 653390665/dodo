...
export function ProjectPreferencePanel({ profile }: ProjectPreferencePanelProps) {
  ...
  const hasCompleteProfile = Boolean(
    ...
    ['styleWeight', 'characterWeight', 'worldWeight', 'plotWeight', 'pacingWeight'].every((key) =>
      Number.isFinite(source.weights?.[key as keyof typeof source.weights])
    )
  );
          ...
          {normalizedProfile.acceptedDimensions.length > 0 && (
            <div className="rounded-xl border border-emerald-200/50 bg-emerald-50/50 px-3 py-2">
              ...
              <div className="text-emerald-600">
                {normalizedProfile.acceptedDimensions
                  .map((dim) => getSkillRoleLabel(dim))
                  .join('、')}
              </div>
            </div>
          )}
          {normalizedProfile.rejectedDimensions.length > 0 && (
            <div className="rounded-xl border border-red-200/50 bg-red-50/50 px-3 py-2">
              ...
              <div className="text-red-600">
                {normalizedProfile.rejectedDimensions
                  .map((dim) => getSkillRoleLabel(dim))
                  .join('、')}
              </div>
            </div>
        ...
        <div className="space-y-1.5">
          ...
          <ul className="space-y-1">
            {normalizedProfile.notes.slice(-3).map((note) => (
              <li
                key={note}
                className="text-[11px] text-theme-muted leading-relaxed pl-3 border-l-2 border-theme-border"
              >
                {note}
              </li>
            ))}

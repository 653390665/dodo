    ...
    return (
      <div className="rounded-3xl border border-theme-border bg-white p-5 shadow-sm">
        ...
        <p className="text-xs text-theme-muted leading-relaxed">
          画像形成中。继续在创作舞台装配 Skill 并给出反馈，系统会逐渐学习这个项目的写法偏好。
        </p>
      </div>
    );
  }

  const weightEntries = (Object.entries(profile.weights) as Array<[string, number]>)
    ...
    <div className="rounded-3xl border border-theme-border bg-white p-5 shadow-sm space-y-4">
      ...
      {weightEntries.length > 0 && (
        <div className="space-y-2">
          <div className="text-[10px] font-bold text-theme-muted uppercase tracking-wider">职责权重</div>
      ...
      {(profile.acceptedDimensions.length > 0 || profile.rejectedDimensions.length > 0) && (
        <div className="grid grid-cols-2 gap-3 text-[10px]">
          {profile.acceptedDimensions.length > 0 && (
            <div className="rounded-xl border border-emerald-200/50 bg-emerald-50/50 px-3 py-2">
              ...
              <div className="text-emerald-600">
                {profile.acceptedDimensions.map((dim) => getSkillRoleLabel(dim)).join('、')}
              </div>
            </div>
          )}
          {profile.rejectedDimensions.length > 0 && (
            <div className="rounded-xl border border-red-200/50 bg-red-50/50 px-3 py-2">
              ...
              <div className="text-red-600">
                {profile.rejectedDimensions.map((dim) => getSkillRoleLabel(dim)).join('、')}
              </div>
            </div>
          )}
        </div>

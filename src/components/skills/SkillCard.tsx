...
export function SkillCard({ skill, selected, onOpen, onDelete, userNovels, onEquip }: SkillCardProps) {
  ...
  return (
    ...
    >
      <button
        type="button"
        aria-label={`打开能力卡 ${skill.name}`}
        onClick={(event) => {
          event.stopPropagation();
          onOpen();
        }}
        className="absolute inset-0 z-0 rounded-2xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-theme-accent"
      />
      <div className="flex justify-between items-start mb-4 gap-3">
        ...
        <div className="relative z-10 flex items-center gap-1">
              ...
              <button
                ...
                onClick={(event) => {
                  ...
                  if (userNovels.length === 1) {
                    onEquip(userNovels[0].id);
                    return;
                  }
                  setShowEquipMenu(!showEquipMenu);
                }}
                className="p-2 text-theme-muted hover:text-theme-accent rounded-lg hover:bg-theme-accent/10 transition-colors opacity-0 group-hover:opacity-100 focus-visible:opacity-100"
              >
                <Zap size={16} />
              </button>
              {showEquipMenu && (
          ...
          <button
            type="button"
            aria-label={`删除能力卡 ${skill.name}`}
            onClick={(event) => {
              event.stopPropagation();
              onDelete();
            }}
            className="p-2 text-theme-muted hover:text-red-500 rounded-lg hover:bg-red-50 transition-colors opacity-0 group-hover:opacity-100 focus-visible:opacity-100"
          >
            <Trash2 size={16} />
          </button>
        </div>

...
import { createProductEventId, recordProductEvent } from '../../lib/product-events-client';
    ...
    return (
      <div className="hidden xl:flex w-[380px] shrink-0 border-l border-theme-border bg-theme-sidebar/80 backdrop-blur-sm flex-col p-6 overflow-y-auto">
        <div className="text-center py-6 border-b border-theme-border/50 shrink-0">
          ...
          <h3 className="text-sm font-bold text-theme-text">选择一张能力卡</h3>
          <p className="text-xs text-theme-muted mt-1">查看详细内容、版本谱系与可复用规则</p>
        </div>
        ...
        <div className="flex-1 py-6 space-y-5 text-left">
          ...
          <div className="flex gap-3 items-start">
            ...
            <div>
              ...
              <p className="text-[11px] text-theme-muted leading-relaxed mt-1">
                保存历次修改的演进记录，防丢防崩；支持通过“能力卡融合”合成高级文风卡。
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-3 items-start">
            ...
            <div>
              <h4 className="text-xs font-bold text-theme-text">使用范围与来源 (Usage & Source)</h4>
              <p className="text-[11px] text-theme-muted leading-relaxed mt-1">
                在能力中心收藏或加入当前作品的配置候选；本章使用需从对应章节入口发起。
              </p>
            </div>
          </div>
        </div>
          ...
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <label className="space-y-2">
                <span className="text-[11px] font-bold text-theme-muted">主能力</span>
                <select
            ...
            <div className="rounded-2xl border border-theme-border p-4 bg-theme-sidebar/20">
              {usageStats ? (
                ...
                <div className="text-xs text-theme-muted">
                  暂无使用反馈数据。后续在创作舞台使用并生成正文后，这里会自动累积统计。
                </div>
              )}
            </div>
          </section>
          ...
          <section>
            <SkillFusionWorkbench
              ...
              onPreview={(nextPreview) => {
                setFusionPreview(nextPreview);
                if (nextPreview) {
                  void recordProductEvent({
                    eventName: 'fusion_previewed',
                    stage: 'advanced',
                    result: 'success',
                    novelId: novelId || undefined,

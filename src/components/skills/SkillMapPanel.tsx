  ...
  const stats = useMemo(() => {
    ...
    for (const skill of skills) {
      ...
      for (const tag of tags) {
        ...
        if (!entry) continue;
        entry.count++;
        const score = getSkillScoreChannels(skill).coldStartScore ?? 0;
    ...
    const observedSkills = skills
      .map((skill) => getSkillScoreChannels(skill).observedPerformance)
  ...
  return (
    <div className="rounded-2xl border border-theme-border bg-theme-sidebar p-5 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <span className="font-bold text-theme-text text-sm">能力卡地图</span>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="rounded-xl bg-theme-sidebar/20 px-3 py-2.5 text-center">
          ...
          <div className="text-[10px] text-theme-muted">能力卡总数</div>
        </div>
        <div className="rounded-xl bg-theme-sidebar/20 px-3 py-2.5 text-center">
          <div className="text-lg font-bold text-theme-text">{stats.mountedCount}</div>
          <div className="text-[10px] text-theme-muted">有使用反馈的能力卡</div>
        </div>
        <div className="rounded-xl bg-theme-sidebar/20 px-3 py-2.5 text-center">
          <div className="text-lg font-bold text-theme-text">{stats.avgColdStart ?? '—'}</div>
          <div className="text-[10px] text-theme-muted">冷启动均分</div>
        </div>
      </div>

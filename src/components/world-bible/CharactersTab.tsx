...
import {
  CHARACTER_CORE_GAP_LABELS,
  diagnoseCharacterCore,
} from '../../../shared/lib/character-core';
          ...
          return (
            ...
            >
              ...
              <select
                value={char.role}
                onChange={(e) =>
                  updateEntity('character', char.id, { role: e.target.value as Character['role'] })
                }
              ...
              <section
                className="border-t border-theme-border/40 pt-3 text-xs text-theme-text/75 space-y-2"
                aria-label={`${char.name}结构化设定审阅`}
              >
                <p>
                  <span className="font-semibold text-theme-text">当前小传：</span>
                  {char.bio || '未填写'}
                </p>
                <p>
                  <span className="font-semibold text-theme-text">结构缺口：</span>
                  {gaps.length
                    ? gaps.map((gap) => CHARACTER_CORE_GAP_LABELS[gap]).join('、')
                    : '无'}
                </p>
                {recommendation?.primary && !candidate ? (
                  <ContextualCapabilityRecommendation
                  ...
                  <>
                    <div aria-label="候选字段差异" className="space-y-1">
                      {candidate.diff.fields.length ? (
                        candidate.diff.fields.map((field) => (
                          <p key={`${field.path}-${field.kind}`}>
                            {field.path || '根字段'}：{field.kind}
                          </p>
                        ))
                      ) : (
                        <p>候选未改变结构字段</p>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        onClick={() => {
                          setPreviewedCandidateIds((current) =>
                    ...
                    {previewedCandidateIds.includes(candidate.id) && (
                      <div
                        role="region"
                        className="border-l-2 border-theme-accent/40 pl-2"
                        aria-label="角色候选预览"
                      >
                        {candidate.proposedContent ||
                          candidate.diff.fields
                            .map((field) => `${field.path}: ${String(field.after ?? '')}`)
                            .join('；')}
                      </div>
                    )}
                  </>
                ...
                {!candidate && onGenerateCandidate && !recommendation?.primary && (
                  <button
                    ...
                    aria-label="生成角色结构候选"
                  >
                    {generatingCandidateIds.includes(char.id) ? (
                      <Loader2 size={12} className="animate-spin" />
                    ) : (
                      <Sparkles size={12} />
                    )}

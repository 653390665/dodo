  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === 'create') {
        ...
        const ok = await createEntityRelationshipClient(rel);
        ...
        const ok = await updateEntityRelationshipClient(existingRelationship.id, data);
        if (!ok) throw new Error('更新失败');
        onSaved({ ...existingRelationship, ...data });
      } else if (mode === 'delete' && existingRelationship) {
        const ok = await deleteEntityRelationshipClient(existingRelationship.id);

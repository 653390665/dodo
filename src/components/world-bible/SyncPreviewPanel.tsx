...
function CharacterRow({
  ...
  return (
    <label
      className={`flex items-start gap-3 p-3 rounded-xl border transition-colors ${checked ? 'border-theme-accent/30 bg-theme-accent/5' : 'border-theme-border bg-theme-sidebar/30'}`}
    >
      <input
        type="checkbox"
        checked={checked}
        onChange={(e) => onToggle(e.target.checked)}
        className="mt-1 h-4 w-4 rounded border-theme-border text-theme-accent focus:ring-theme-accent/20"
      />
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-theme-muted mt-1 line-clamp-2">
          <span className="text-theme-accent font-medium">{entity.region}</span> ·{' '}
          {entity.description}
        </div>
      </div>
    </label>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-theme-muted mt-1 line-clamp-2">
          <span className="text-theme-accent font-medium">{entity.type}</span> ·{' '}
          {entity.description}
        </div>
      </div>
    </label>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-theme-muted mt-1 line-clamp-2">
          首领 <span className="text-theme-accent font-medium">{entity.leader}</span> ·{' '}
          {entity.description}
        </div>
      </div>
    </label>
...
export function SyncPreviewPanel({
  ...
  const [selectedChars, setSelectedChars] = useState<Set<number>>(() => {
    ...
    extraction.characters.forEach((c, i) => {
      if (!isDuplicateChar(c.name)) s.add(i);
    });
    return s;
  });
  const [selectedLocs, setSelectedLocs] = useState<Set<number>>(() => {
    ...
    extraction.locations.forEach((l, i) => {
      if (!isDuplicateLoc(l.name)) s.add(i);
    });
    return s;
  });
  const [selectedItems, setSelectedItems] = useState<Set<number>>(() => {
    ...
    extraction.items.forEach((item, i) => {
      if (!isDuplicateItem(item.name)) s.add(i);
    });
    return s;
  });
  const [selectedFactions, setSelectedFactions] = useState<Set<number>>(() => {
    ...
    extraction.factions.forEach((f, i) => {
      if (!isDuplicateFaction(f.name)) s.add(i);
    });
    return s;
  });
  ...
  const allEntityNames = useMemo(() => {
    ...
    extraction.characters.forEach((c, i) => {
      if (selectedChars.has(i)) names.add(`character:${normalizeName(c.name)}`);
    });
    extraction.locations.forEach((l, i) => {
      if (selectedLocs.has(i)) names.add(`location:${normalizeName(l.name)}`);
    });
    extraction.items.forEach((item, i) => {
      if (selectedItems.has(i)) names.add(`item:${normalizeName(item.name)}`);
    });
    extraction.factions.forEach((f, i) => {
      if (selectedFactions.has(i)) names.add(`faction:${normalizeName(f.name)}`);
    });
  ...
  const entityOptionsByType = useMemo(() => {
    ...
    extraction.characters.forEach((c, i) => {
      if (
        selectedChars.has(i) &&
        !opts.character.some((n) => normalizeName(n) === normalizeName(c.name))
      )
        opts.character.push(c.name);
    });
    extraction.locations.forEach((l, i) => {
      if (
        selectedLocs.has(i) &&
        !opts.location.some((n) => normalizeName(n) === normalizeName(l.name))
      )
        opts.location.push(l.name);
    });
    extraction.items.forEach((item, i) => {
      if (
        selectedItems.has(i) &&
        !opts.item.some((n) => normalizeName(n) === normalizeName(item.name))
      )
        opts.item.push(item.name);
    });
    extraction.factions.forEach((f, i) => {
      if (
        selectedFactions.has(i) &&
        !opts.faction.some((n) => normalizeName(n) === normalizeName(f.name))
      )
        opts.faction.push(f.name);
    });
    return opts;
  }, [
    existingCharacters,
  ...
  const isRelationshipConfirmed = useCallback(
    (r: SyncExtractionResult['relationships'][number], idx: number) => {
  ...
  const [repairRelationshipIndexes, setRepairRelationshipIndexes] = useState<Set<number> | null>(
    null
  );
  const [recommendations, setRecommendations] = useState<
    Record<number, RelationshipRecommendation>
  >({});
  ...
  const handleRecommendRepairs = useCallback(async () => {
    ...
    try {
      const relationships: RelationshipRepairInput[] = unresolvedRelationshipIndexes.map(
        (index) => ({ index, ...extraction.relationships[index] })
      ) as RelationshipRepairInput[];
      const candidates = entityOptionsByType as Record<RelationshipEntityType, string[]>;
      const result = await recommendRelationshipRepairs({
        packId,
        novelId,
        databaseGeneration,
        relationships,
      ...
      result.recommendations.forEach((recommendation) => {
        if (unresolvedRelationshipIndexes.includes(recommendation.index))
          next[recommendation.index] = recommendation;
      });
      ...
      if (Object.keys(mappedNames).length > 0)
        setResolvedNames((prev) => ({ ...prev, ...mappedNames }));
    ...
    unresolvedRelationshipIndexes,
  ]);

  const isRelationshipSelected = useCallback(
    (relationship: SyncExtractionResult['relationships'][number], index: number) =>
    ...
    [
      effectiveSkippedRelationships,
      isRelationshipConfirmed,
      repairRelationshipIndexes,
      selectedRelationships,
    ]
  );
  ...
  const counts = useMemo(() => {
    ...
    extraction.factions.forEach((f, i) => {
      if (selectedFactions.has(i) && !completedFactions.has(i) && !isDuplicateFaction(f.name))
        newCount++;
    });
  ...
  const handleConfirm = async () => {
    const submittedChars = extraction.characters
      .map((_, i) => i)
      .filter(
        (i) =>
          selectedChars.has(i) &&
          !completedChars.has(i) &&
          !isDuplicateChar(extraction.characters[i].name)
      );
    const submittedLocs = extraction.locations
      .map((_, i) => i)
      .filter(
        (i) =>
          selectedLocs.has(i) &&
          !completedLocs.has(i) &&
          !isDuplicateLoc(extraction.locations[i].name)
      );
    const submittedItems = extraction.items
      .map((_, i) => i)
      .filter(
        (i) =>
          selectedItems.has(i) &&
          !completedItems.has(i) &&
          !isDuplicateItem(extraction.items[i].name)
      );
    const submittedFactions = extraction.factions
      .map((_, i) => i)
      .filter(
        (i) =>
          selectedFactions.has(i) &&
          !completedFactions.has(i) &&
          !isDuplicateFaction(extraction.factions[i].name)
      );
    const submittedPowerLevels = extraction.powerLevels
      .map((_, i) => i)
      .filter((i) => selectedPowerLevels.has(i) && !completedPowerLevels.has(i));
    const submittedTimeline = extraction.timelineEvents
      .map((_, i) => i)
      .filter((i) => selectedTimeline.has(i) && !completedTimeline.has(i));
    const confirmed = await onConfirm(
      {
        ...
        relationships: extraction.relationships
          .filter(
            (r, i) =>
              (!repairRelationshipIndexes || repairRelationshipIndexes.has(i)) &&
              isRelationshipSelected(r, i) &&
              isRelationshipConfirmed(r, i) &&
              !effectiveSkippedRelationships.has(i)
          )
          .map((r) => {
      ...
      case 'characters':
        return (
          <div className="space-y-2">
            {extraction.characters.map(
              (c, i) =>
                !completedChars.has(i) &&
                !isDuplicateChar(c.name) && (
                  <CharacterRow
                    key={i}
                    entity={c}
                    checked={selectedChars.has(i)}
                    onToggle={(ch) => toggleSet(setSelectedChars, i, ch)}
                    isDuplicate={false}
                  />
                )
            )}
            {extraction.characters.every(
              (c, i) => completedChars.has(i) || isDuplicateChar(c.name)
            ) && <EmptyHint text="未提取到人物" />}
          </div>
        );
      case 'locations':
        return (
          <div className="space-y-2">
            {extraction.locations.map(
              (l, i) =>
                !completedLocs.has(i) &&
                !isDuplicateLoc(l.name) && (
                  <LocationRow
                    key={i}
                    entity={l}
                    checked={selectedLocs.has(i)}
                    onToggle={(ch) => toggleSet(setSelectedLocs, i, ch)}
                    isDuplicate={false}
                  />
                )
            )}
            {extraction.locations.every(
              (l, i) => completedLocs.has(i) || isDuplicateLoc(l.name)
            ) && <EmptyHint text="未提取到地点" />}
          </div>
        );
      case 'items':
        return (
          <div className="space-y-2">
            {extraction.items.map(
              (item, i) =>
                !completedItems.has(i) &&
                !isDuplicateItem(item.name) && (
                  <ItemRow
                    key={i}
                    entity={item}
                    checked={selectedItems.has(i)}
                    onToggle={(ch) => toggleSet(setSelectedItems, i, ch)}
                    isDuplicate={false}
                  />
                )
            )}
            {extraction.items.every(
              (item, i) => completedItems.has(i) || isDuplicateItem(item.name)
            ) && <EmptyHint text="未提取到道具" />}
          </div>
        );
      case 'factions':
        return (
          <div className="space-y-2">
            {extraction.factions.map(
              (f, i) =>
                !completedFactions.has(i) &&
                !isDuplicateFaction(f.name) && (
                  <FactionRow
                    key={i}
                    entity={f}
                    checked={selectedFactions.has(i)}
                    onToggle={(ch) => toggleSet(setSelectedFactions, i, ch)}
                    isDuplicate={false}
                  />
                )
            )}
            {extraction.factions.every(
              (f, i) => completedFactions.has(i) || isDuplicateFaction(f.name)
            ) && <EmptyHint text="未提取到势力" />}
          </div>
        );
      case 'powerLevels':
        return (
          <div className="space-y-2">
            {extraction.powerLevels.map(
              (pl, i) =>
                !completedPowerLevels.has(i) && (
                  <label
                    key={i}
                    className={`flex items-start gap-3 p-3 rounded-xl border transition-colors ${selectedPowerLevels.has(i) ? 'border-theme-accent/30 bg-theme-accent/5' : 'border-theme-border bg-theme-sidebar/30'}`}
                  >
                    <input
                      type="checkbox"
                      checked={selectedPowerLevels.has(i)}
                      onChange={(e) => toggleSet(setSelectedPowerLevels, i, e.target.checked)}
                      className="mt-1 h-4 w-4 rounded border-theme-border text-theme-accent focus:ring-theme-accent/20"
                    />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                      ...
                      <div className="text-xs text-theme-muted mt-1 line-clamp-2">
                        <span className="text-theme-accent font-medium">{pl.characteristics}</span>{' '}
                        · {pl.description}
                      </div>
                    </div>
                  </label>
            ...
            {extraction.powerLevels.every((_, i) => completedPowerLevels.has(i)) && (
              <EmptyHint text="未提取到力量体系" />
            )}
          </div>
        );
      case 'timelineEvents':
        return (
          <div className="space-y-2">
            {extraction.timelineEvents.map(
              (te, i) =>
                !completedTimeline.has(i) && (
                  <label
                    key={i}
                    className={`flex items-start gap-3 p-3 rounded-xl border transition-colors ${selectedTimeline.has(i) ? 'border-theme-accent/30 bg-theme-accent/5' : 'border-theme-border bg-theme-sidebar/30'}`}
                  >
                    <input
                      type="checkbox"
                      checked={selectedTimeline.has(i)}
                      onChange={(e) => toggleSet(setSelectedTimeline, i, e.target.checked)}
                      className="mt-1 h-4 w-4 rounded border-theme-border text-theme-accent focus:ring-theme-accent/20"
                    />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                      ...
                      <div className="text-xs text-theme-muted mt-1 line-clamp-2">
                        <span className="text-theme-accent font-medium">{te.timestamp}</span> ·{' '}
                        {te.description}
                      </div>
                    </div>
                  </label>
            ...
            {extraction.timelineEvents.every((_, i) => completedTimeline.has(i)) && (
              <EmptyHint text="未提取到时间线事件" />
            )}
          </div>
        );
      case 'relationships':
          ...
          <div className="space-y-2">
            {extraction.relationships.map((r, i) => {
              ...
              return (
                <div
                  key={i}
                  data-relationship-repair={
                    repairRelationshipIndexes?.has(i) || !confirmed ? 'true' : undefined
                  }
                  className={`p-3 rounded-xl border transition-colors ${isRelationshipSelected(r, i) ? 'border-theme-accent/30 bg-theme-accent/5' : 'border-theme-border bg-theme-sidebar/30'} ${!confirmed ? 'border-amber-300/50' : ''}`}
                >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 text-sm flex-wrap">
                        {!srcKnown && srcOptions.length > 0 ? (
                          ...
                          <span
                            className={`font-bold ${srcKnown ? 'text-theme-text' : 'text-amber-600'}`}
                          >
                            {srcResolved}
                          </span>
                        )}
                        <span className="text-theme-muted">→</span>
                        <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-theme-accent/10 text-theme-accent">
                          {r.relationshipType}
                        </span>
                        <span className="text-theme-muted">→</span>
                        {!tgtKnown && tgtOptions.length > 0 ? (
                          ...
                          <span
                            className={`font-bold ${tgtKnown ? 'text-theme-text' : 'text-amber-600'}`}
                          >
                            {tgtResolved}
                          </span>
                        )}
                        ...
                        <div className="mt-2 border-l-2 border-theme-accent/30 pl-2 text-xs text-theme-muted space-y-1">
                          <div className="flex flex-wrap items-center gap-2">
                            <span className="font-bold text-theme-accent">
                              {recommendation.action === 'map' ? '推荐映射' : '建议跳过'}
                            </span>
                            <span
                              className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${recommendation.confidence === 'high' ? 'bg-emerald-100 text-emerald-700' : recommendation.confidence === 'medium' ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-600'}`}
                            >
                            ...
                            <button
                              type="button"
                              disabled={effectiveSkippedRelationships.has(i) || confirmed}
                              className="font-bold text-theme-accent hover:underline disabled:cursor-default disabled:no-underline disabled:opacity-60"
                              onClick={() => {
                                if (recommendation.action === 'skip') {
                                ...
                                setResolvedNames((prev) => ({
                                  ...
                                  ...(recommendation.sourceName
                                    ? { [`${i}:source`]: recommendation.sourceName }
                                    : {}),
                                  ...(recommendation.targetName
                                    ? { [`${i}:target`]: recommendation.targetName }
                                    : {}),
                                }));
                                setSkippedRelationships((prev) => {
                                  const next = new Set(prev);
                                ...
                                if (recommendation.sourceName && recommendation.targetName)
                                  setSelectedRelationships((prev) => new Set(prev).add(i));
                              }}
                            >
                              {effectiveSkippedRelationships.has(i)
                                ? '已采用跳过'
                          ...
                          {recommendation.evidence.map((evidence, evidenceIndex) => (
                            ...
                            >
                              来源：{evidence.filename} · “{evidence.quote}”
                            </div>
                          ))}
                        </div>
                      )}
            ...
            {(repairRelationshipIndexes
              ? repairRelationshipIndexes.size === 0
              : extraction.relationships.length === 0) && <EmptyHint text="没有待处理关系" />}
          </div>
        );
      default:
        return null;
    ...
    <div className="rounded-2xl border border-theme-border bg-theme-sidebar p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={onCancel}
          disabled={isSyncing}
          className="text-theme-muted hover:text-theme-text transition-colors disabled:opacity-50"
          aria-label="关闭同步预览"
        >
          <X size={16} />
        </button>
      ...
      {/* Tab content */}
      <div className="max-h-96 overflow-y-auto space-y-2">{renderTabContent()}</div>
      ...
      <div className="space-y-2 pt-2 border-t border-theme-border">
        {extraction.globalOutline && (
          <label className="flex items-center gap-2 text-xs text-theme-text">
            <input
              type="checkbox"
              checked={includeGlobalOutline}
              onChange={(e) => setIncludeGlobalOutline(e.target.checked)}
              className="h-4 w-4 rounded border-theme-border text-theme-accent focus:ring-theme-accent/20"
            />
            同步世界大纲
          </label>
        )}
      ...
      <div className="flex flex-col gap-3 pt-3 border-t border-theme-border xl:flex-row xl:items-center xl:justify-between">
        <div className="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-2 text-xs text-theme-muted">
          <span>
            新增 <strong className="text-emerald-600">{counts.newCount}</strong> 项
          </span>
          {counts.skipCount > 0 && (
            <span>
              跳过 <strong className="text-gray-500">{counts.skipCount}</strong> 项
            </span>
          )}
          {unresolvedCount > 0 && (
            <>
              <span>
                待处理 <strong className="text-amber-600">{unresolvedCount}</strong> 项关系
              </span>
              <button
                type="button"
                onClick={() => {
              ...
              {packId && novelId && databaseGeneration !== undefined && (
                <button
                  type="button"
                  onClick={handleRecommendRepairs}
                  disabled={isRecommending}
                  className="flex items-center gap-1 rounded-lg border border-theme-accent/30 px-2 py-1 font-bold text-theme-accent hover:bg-theme-accent/5 disabled:opacity-50"
                >
                  {isRecommending ? (
              ...
              {recommendError && (
                <span className="flex flex-wrap items-center gap-2 text-red-500">
                  <span className="break-words">{recommendError}</span>
                  <button
                    type="button"
                    onClick={handleRecommendRepairs}
          ...
          {recommendStatus && (
            <span role="status" className="text-emerald-600">
              {recommendStatus}
            </span>
          )}
        </div>
        <div className="flex shrink-0 items-center gap-2 self-end xl:self-auto">
          <button
            onClick={onCancel}
            disabled={isSyncing}
            className="shrink-0 whitespace-nowrap px-4 py-2 rounded-xl border border-theme-border text-theme-text text-xs font-bold hover:bg-theme-sidebar/20 transition-colors"
          >
            取消
          </button>
          <button
            ...
            disabled={isSyncing}
            className="flex shrink-0 items-center gap-2 whitespace-nowrap rounded-xl bg-theme-accent px-4 py-2 text-xs font-bold text-theme-accent-contrast disabled:opacity-50"
            title={
              unresolvedCount > 0 ? `先导入可确认项，再逐条处理 ${unresolvedCount} 条关系` : ''
            }
          >

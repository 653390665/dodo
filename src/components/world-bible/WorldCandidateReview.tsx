        ...
        <div>
          ...
          {candidate ? (
            <p className="mt-1 text-xs text-theme-muted">
              {candidate.goal || '待确认的世界规则变更'}
            </p>
          ) : null}
          {!candidate && activeVersion !== undefined ? (
            <p className="mt-1 text-xs text-theme-muted">当前结构版本 v{activeVersion}</p>
          ) : null}
        </div>
        {!candidate ? (
          <button
        ...
        <div className="mt-4 space-y-4">
          ...
          {candidate.proposedContent ? (
            <div>
              ...
              <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-theme-text">
                {candidate.proposedContent}
              </p>
            </div>
          ) : null}

          {candidate.impactReport.reasons.length > 0 ||
          candidate.impactReport.reviewRequired.length > 0 ? (
            <div>
              <h3 className="text-xs font-bold text-theme-text">影响与复核</h3>
              <ul className="mt-2 space-y-1 text-xs text-theme-muted">
                {candidate.impactReport.reasons.map((reason) => (

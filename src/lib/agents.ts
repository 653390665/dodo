...
const MAX_FORESHADOWING_CHARS = 2000;
...
export function buildContextPrompt(context: AgentContext): string {
  ...
  const openForeshadowings = (context.foreshadowings || [])
    .filter((entry) => entry.status !== 'payoff')
    .sort((left, right) => {
      const leftCurrent = left.plantedChapterId === context.chapterId ? 1 : 0;
      const rightCurrent = right.plantedChapterId === context.chapterId ? 1 : 0;
      return rightCurrent - leftCurrent || left.createdAt - right.createdAt;
    })

...
const ACTIONS: Record<
  ...
  Omit<AssistantActionPlan, 'intent' | 'userRequest' | 'novelId' | 'chapterId'>
> = {
  'draft-prose': {
    label: '创作当前章节',
    scope: 'chapter',
  ...
  'plan-structure': {
    ...
    scope: 'project',
    executionMode: 'single-run',
    outputArtifact: 'outline-candidate',
    recommendedCapabilityId: 'opening-gold-three',
    requiresReview: true,
  },
  'save-fragment': {
...
export function getAssistantQuickActions(context: {
  hasNovel: boolean;
  hasChapter: boolean;

...
export type CapabilitySessionCategory =
  'all' | 'creative-setup' | 'active-drafting' | 'style-polish' | 'commercial-sign';
...
export function isCapabilityConfigurationSessionStale(
  ...
  return (
    session.databaseGeneration !== databaseGeneration || session.baselineToken !== baselineToken
  );
}
...
export function getCapabilityConfigurationBaselineToken(
...
function normalizeSession(value: unknown): CapabilityConfigurationSession | null {
  ...
  if (record.version !== 1 || typeof record.novelId !== 'string' || !record.novelId.trim())
    return null;
  if (!Number.isInteger(record.databaseGeneration) || (record.databaseGeneration as number) < 0)
    return null;
  ...
  const candidateCardIds = Array.isArray(record.candidateCardIds)
    ? [
        ...new Set(
          record.candidateCardIds.filter(
  ...
  const pendingPackageSteps = Array.isArray(record.pendingPackageSteps)
    ? record.pendingPackageSteps.filter((step): step is EnhancementPackageStep =>
        Boolean(
          step &&
          typeof step === 'object' &&
          typeof (step as Record<string, unknown>).id === 'string' &&
          typeof (step as Record<string, unknown>).assetId === 'string'
        )
      )
    : [];
  const activeTab = record.activeTab === 'plaza' ? 'plaza' : 'mySkills';
  const selectedCapability =
    typeof record.selectedCapability === 'string'
      ? (record.selectedCapability as CapabilitySessionStoreTab)
      : 'flow';
  const selectedCategory =
    typeof record.selectedCategory === 'string'
      ? (record.selectedCategory as CapabilitySessionCategory)
      : 'all';
  const scrollTop =
    Number.isFinite(record.scrollTop) && (record.scrollTop as number) >= 0
      ? (record.scrollTop as number)
      : 0;
  return {
    version: 1,
    ...
    configurationDraft:
      record.configurationDraft && typeof record.configurationDraft === 'object'
    ...
    candidateCardIds,
    pendingCandidateId:
      typeof record.pendingCandidateId === 'string' &&
      candidateCardIds.includes(record.pendingCandidateId)
...
export function loadCapabilityConfigurationSession(
  novelId: string,
  databaseGeneration: number,
  baselineToken: string
): CapabilityConfigurationSession | null {
  const storage = getStorage();
  ...
  try {
    ...
    if (
      ...
      parsed.databaseGeneration !== databaseGeneration ||
      parsed.baselineToken !== baselineToken
    ) {
      return null;
    }
    return parsed;
 ...
 */
export function loadLatestCapabilityConfigurationSession(
  novelId: string
): CapabilityConfigurationSession | null {
  const storage = getStorage();
...
export function clearLatestCapabilityConfigurationSession(novelId: string): void {
  ...
  try {
    storage.removeItem(storageKey(novelId));
  } catch {
    /* optional storage */
  }
}

export function clearCapabilityConfigurationSession(
  novelId: string,
  databaseGeneration?: number,
  baselineToken?: string
): void {
  ...
  if (
    ...
    !loadCapabilityConfigurationSession(novelId, databaseGeneration, baselineToken)
  )
    return;
  try {
    storage.removeItem(storageKey(novelId));

...
const DECONSTRUCTION_CARD_CATEGORY: Record<
...
export function getAuthorFacingCapabilityCardCategory(
  capability:
    | Pick<
        CapabilityManifestEntry,
        'kind' | 'output' | 'stages' | 'deconstructionCardType' | 'outputArtifact'
      >
    | DeconstructionCardType
): AuthorFacingCapabilityCardCategory {
  if (typeof capability === 'string') return DECONSTRUCTION_CARD_CATEGORY[capability];
  if (capability.deconstructionCardType)
    return DECONSTRUCTION_CARD_CATEGORY[capability.deconstructionCardType];
  if (capability.kind === 'guardrail') return '护栏卡';
  if (capability.output === 'transform-preview') return '精修卡';
  if (
    capability.kind === 'diagnostic' ||
    capability.kind === 'utility' ||
    capability.output === 'diagnostic'
  )
    return '审稿卡';
  if (
    capability.outputArtifact === 'worldBibleCandidate' ||
    capability.outputArtifact === 'characterCardCandidate'
  )
    return '世界观卡';
...
export function getAuthorFacingCapabilityUseHint(
  category: AuthorFacingCapabilityCardCategory
): string {
...
export function getAuthorFacingCapabilityEntryHint(
  category: AuthorFacingCapabilityCardCategory
): string {
...
export function getAuthorFacingCapabilityActionLabel(
  capability: Pick<CapabilityManifestEntry, 'action' | 'allowedScopes' | 'sideEffect'> &
    Partial<Pick<CapabilityManifestEntry, 'output'>>,
  ...
  if (preferredScope === 'chapter')
    return capability.output === 'transform-preview' ? '应用配置后写入本章规则' : '用于本章';
  if (capability.output === 'transform-preview' && capability.allowedScopes.includes('single-run'))
    return '生成精修预览';
  if (capability.action === 'run-diagnostic') return '运行审稿诊断';
  if (
    capability.action === 'run-utility' ||
    capability.action === 'preview-transform' ||
    capability.action === 'use-this-time'
  )
    return '运行一次，不保存配置';
  if (capability.action === 'automatic' || capability.allowedScopes.includes('system'))
    return '保存为系统检查候选';
  if (capability.sideEffect === 'configuration' && capability.allowedScopes.includes('project'))
    return '应用配置后设为作品默认';
  return '用于本章';
}

export function getAuthorFacingCapabilityActionHint(
  capability: Pick<CapabilityManifestEntry, 'action' | 'allowedScopes' | 'sideEffect' | 'kind'> &
    Partial<Pick<CapabilityManifestEntry, 'output' | 'deconstructionCardType' | 'outputArtifact'>>
): string {
  if (capability.allowedScopes.includes('system') || capability.action === 'automatic')
    return '护栏卡先保存为系统检查候选；应用配置后参与写作与审稿检查，凭证在生成或审稿结果中查看。';
  if (capability.deconstructionCardType || capability.kind === 'skill-card')
    return '卡组位置：先选主卡或辅卡，应用配置后写入作品卡组。';
  if (
    capability.outputArtifact === 'worldBibleCandidate' ||
    capability.outputArtifact === 'characterCardCandidate'
  )
    return '配置到作品：应用配置后写入设定素材，并前往世界观继续整理。';
  if (
    (capability.output === 'outline-candidate' || capability.output === 'artifact-candidate') &&
    capability.allowedScopes.includes('project')
  )
    return '配置到作品：应用配置后写入大纲技法，并前往大纲继续使用。';
  if (capability.output === 'transform-preview')
    return '应用配置后可写入本章规则；运行一次只生成精修预览。';
  if (
    capability.action === 'run-diagnostic' ||
    capability.action === 'run-utility' ||
    capability.sideEffect === 'none'
  )
    return '运行一次：只生成诊断或辅助结果，不改正文。';
  if (capability.allowedScopes.includes('project') && capability.allowedScopes.includes('chapter'))
    return '可设为作品默认统一全文，也可只用于当前章节。';
  if (capability.allowedScopes.includes('chapter')) return '应用配置后只影响当前章节写作。';
  if (capability.allowedScopes.includes('project'))
    return '配置到作品：应用配置后写入作品默认配置。';
...
function resolveTechniqueManifestId(
  id: string,
  profile: ProjectPreferenceProfile['capabilityProfile']
): string {
  const direct = getCatalogCapabilityManifest(id);
  if (direct?.kind === 'technique') return id;
  return (
    profile?.capabilityMemberships?.find((membership) => membership.persistedSkillId === id)
      ?.sourceId || id
  );
}
...
export function buildEffectiveCapabilitySummary(input: {
  ...
  const chapterCardIds = uniqueIds(
    input.currentChapter?.workflowMeta?.capabilityState?.overlayCardIds || []
  ).filter((id) => isWriterCard(id, input.librarySkills, 'chapter'));
  const projectTechniqueIds = uniqueIds(
    capabilityProfile?.projectTechniqueIds ?? capabilityProfile?.favoriteTechniqueIds ?? []
  ).filter((id) => isWriterTechnique(resolveTechniqueManifestId(id, capabilityProfile)));
  const projectTechniqueIdSet = new Set(
    projectTechniqueIds.map((id) => resolveTechniqueManifestId(id, capabilityProfile))
  );
  const chapterTechniqueIds = uniqueIds(
    input.currentChapter?.workflowMeta?.capabilityState?.techniqueIds || []
  )
  ...
  const guardrailIds = getWriterGuardrailIds(
    getOptionalStringIds(
      rawCapabilityProfile?.guardrailIds || rawCapabilityProfile?.systemGuardrailIds
    )
  );
  const names = [
    ...projectCardIds,

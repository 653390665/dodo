...
export async function updateContinuationPack(
  id: string,
  data: { continuationTask?: string }
): Promise<boolean> {
...
export async function approveContinuationImport(payload: {
  ...
  if (!response.ok || !data.novel || !data.pack) {
    void recordProductEvent({
      eventName: 'continuation_confirm',
      stage: 'review',
      result: 'failure',
      durationMs: Date.now() - startedAt,
      errorCode: data.code || `HTTP_${response.status}`,
      novelId: payload.existingNovelId,
      objectId: payload.packId,
    }).catch(() => undefined);
...
export async function syncPackToWorld(data: SyncToWorldRequest): Promise<SyncResult> {
  ...
  const record = (result: 'success' | 'failure' | 'unknown', errorCode?: string) => {
    ...
    recorded = true;
    void recordProductEvent({
      eventName: 'world_sync',
      stage: 'sync',
      result,
      durationMs: Date.now() - startedAt,
      errorCode,
      novelId: data.novelId,
      objectId: data.packId,
    }).catch(() => undefined);
  };
  try {
    ...
    if (error instanceof Error && error.name === 'AbortError')
      record('unknown', 'OPERATION_CANCELLED');
...
async function readJson(
  response: Response,
  context?: { jobId?: string; databaseGeneration?: number }
): Promise<unknown> {
  try {
    return await response.json();
  } catch {
    throw new ContinuationClientError(
      '提取服务返回了无法读取的响应。',
...
async function pollExtractionJob(
  ...
  const cancel = () => {
    ...
    void fetch(
      `/api/continuation-packs/jobs/${encodeURIComponent(jobId)}/cancel?databaseGeneration=${databaseGeneration}`,
      { method: 'POST' }
    ).catch(() => {});
  };
  ...
  try {
    while (true) {
      ...
      try {
        jobRes = await fetch(
          `/api/continuation-packs/jobs/${encodeURIComponent(jobId)}?databaseGeneration=${databaseGeneration}`,
          { signal }
        );
        ...
        if (pollFailures > 4)
          throw new ContinuationClientError(
            '暂时无法读取提取进度，请稍后续跑。',
            'EXTRACTION_POLLING_UNAVAILABLE',
            undefined,
            undefined,
            undefined,
            jobId,
      ...
      if (!jobRes.ok)
        throw new ContinuationClientError(
          job.error || `无法查询提取任务（HTTP ${jobRes.status}）`,
          job.code || `HTTP_${job.status}`,
          job.currentChunk,
          job.totalChunks,
          job.traceId,
          jobId,
      ...
      onProgress?.({
        ...
        stageText: job.stageText || '正在提取设定...',
      ...
      if (job.status === 'failed')
        throw new ContinuationClientError(
          job.error || '提取任务失败，请重试。',
          job.code || 'EXTRACTION_FAILED',
          job.currentChunk,
          job.totalChunks,
          job.traceId,
          jobId,
...
export async function extractPackEntities(
  ...
  if (!res.ok) {
    throw new ContinuationClientError(
      data.error || `提取失败（HTTP ${res.status}）`,
  ...
  const jobGeneration =
    typeof data.databaseGeneration === 'number' && Number.isInteger(data.databaseGeneration)
      ? data.databaseGeneration
      : databaseGeneration;
  return pollExtractionJob(jobId, jobGeneration, signal, onProgress);
}

export async function resumePackEntityExtraction(
  jobId: string,
  databaseGeneration: number,
  signal?: AbortSignal,
  onProgress?: (progress: ExtractionProgress) => void
  ...
  const data = (await readJson(response, { jobId, databaseGeneration })) as {
    error?: string;
    code?: string;
    traceId?: string;
    databaseGeneration?: number;
  ...
  if (
    data.jobId !== jobId ||
    typeof data.databaseGeneration !== 'number' ||
    data.databaseGeneration !== databaseGeneration
  ) {
    throw new ContinuationClientError(
      '续跑响应不完整，任务未被继续执行。',
      'EXTRACTION_PROTOCOL_ERROR',
...
export async function getPackEntityExtractionJob(
  jobId: string,
  databaseGeneration: number,
  signal?: AbortSignal
  ...
  if (!response.ok)
    throw new ContinuationClientError(
      data.error || `无法查询提取任务（HTTP ${response.status}）`,
      data.code || `HTTP_${response.status}`,
      data.currentChunk,
      data.totalChunks,
      data.traceId,
...
export async function requeryPackEntityExtraction(
  jobId: string,
  databaseGeneration: number,
  signal?: AbortSignal,
  onProgress?: (progress: ExtractionProgress) => void

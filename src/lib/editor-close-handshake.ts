...
export function collectPendingEditorSnapshot(target: Window): PendingEditorCloseSnapshot {
  const visibleFields = [
    ...target.document.querySelectorAll<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>(
    ...
    .map((element, index) => ({
      name:
        element.name || element.id || element.getAttribute('aria-label') || `field-${index + 1}`,
      value: element.value,
    }));
...
export function bindEditorCloseSafety(target: Window, bridge: CloseBridge | undefined): () => void {
  ...
  const unsubscribeClose = bridge?.onPrepareClose?.(async (attemptId) => {
    ...
    const flushed = await flushEditorWritesForClose(undefined, (error) => {
      const reason =
        error instanceof Error ? error.message : 'Editor writes could not be persisted';

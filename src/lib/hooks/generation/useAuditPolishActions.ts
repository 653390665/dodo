...
import {
  deriveChapterReviewState,
  deriveReviewGate,
  structuredAuditToReviewIssues,
} from '../../../../shared/lib/review-issues';
...
import {
  semanticReviewFromStructuredAudit,
  validateCandidateDraftQuality,
...
export async function readErrorBodyOnce(
  response: Response
...
export function useAuditPolishActions({
  ...
  const handleStyleConfirmationResponse = async (
    response: Response,
    retry?: (fingerprint: string) => Promise<void>
  ): Promise<{ handled: boolean; data: Record<string, unknown> | null }> => {
    ...
    onStyleConfirmationRequired?.({
      ...(data as { resolution?: WritingStyleResolution; candidates?: WritingStyleCandidate[] }),
      retry,
    });
  ...
  const handleRunAudit = async (options: AuditRequestOptions = {}) => {
    ...
    const candidateForAudit =
      candidateSnapshot && candidateSnapshot.chapterId === currentChapter.id
        ? candidateSnapshot
        : null;
    ...
    const cancelAuditJob = () => {
      ...
      try {
        const cancelResult = fetch(
          `/api/audit/jobs/${encodeURIComponent(auditJobId)}/cancel?databaseGeneration=${auditDatabaseGeneration}`,
          { method: 'POST' }
        );
    ...
    try {
      ...
      const auditLatestContent =
        candidateForAudit?.content ?? contentRef.current?.value ?? currentChapter.content;
      ...
      const response = await fetch('/api/audit', {
        ...
        body: JSON.stringify({
          ...
          styleConfirmationFingerprint:
            options.styleConfirmationFingerprint || writingStyleFingerprint || undefined,
      ...
      const styleOutcome = await handleStyleConfirmationResponse(response, (fingerprint) =>
        handleRunAudit({ ...options, styleConfirmationFingerprint: fingerprint })
      );
      ...
      auditDatabaseGeneration =
        typeof initData?.databaseGeneration === 'number' ? initData.databaseGeneration : null;
      if (!auditJobId || !Number.isInteger(auditDatabaseGeneration))
        throw new Error('Failed to initiate audit job');
      const databaseGeneration = auditDatabaseGeneration as number;
      if (databaseGeneration !== requestDatabaseGeneration)
        throw new Error('数据库已在审稿启动期间切换');
      ...
      while (pollRetries < AUDIT_POLL_MAX_RETRIES) {
        ...
        const jobResponse = await fetch(
          `/api/audit/jobs/${auditJobId}?databaseGeneration=${databaseGeneration}`,
          {
            signal: controller.signal,
          }
        );
        ...
        if (job.status === 'completed') {
          ...
          setAuditStatus(
            `[${percent}%] ${job.stageText || '总编正在逐段扫描机械感、节奏和人设一致性…'}`
          );
        }
      }
      ...
      if (jobResult.status === 'unknown') {
        const category =
          typeof jobResult.errorCategory === 'string' ? jobResult.errorCategory : 'invalid_json';
        const diagnostic =
          typeof jobResult.diagnostic === 'string' ? jobResult.diagnostic : '审稿结果未确认';
      ...
      const numericAuditScore =
        typeof jobResult.score === 'number'
          ? jobResult.score
      ...
      const structuredAudit =
        jobResult.structured && typeof jobResult.structured === 'object'
      ...
      const workflowMeta: ChapterWorkflowMeta = {
        ...(options.baseWorkflowMeta ||
          candidateForAudit?.workflowMeta ||
          currentChapter.workflowMeta || { version: 1 as const }),
        version: 1 as const,
        lastAudit: {
          status: auditStatus as 'pass' | 'fail',
          contentHash,
      ...
      if (
        candidateForAudit?.operation === 'polish' &&
        candidateForAudit.reviewIssueIds?.length &&
        candidateForAudit.workflowMeta?.reviewState
      ) {
      ...
      if (candidateForAudit) {
        const nextQuality = {
          ...(candidateForAudit.quality ||
            validateCandidateDraftQuality(auditLatestContent, auditBaselineContent)),
          semanticReview,
        };
        setCandidate?.({
          ...candidateForAudit,
        ...
        const saved = await updateChapter(
          currentChapter.id,
          { critique: feedbackStr, workflowMeta },
          databaseGeneration
        );
        if (!saved) throw new Error('章节已不存在，审稿结果未保存。');
        if (!isRequestCurrent(startingChapterId, currentSeq)) return;
        setCurrentChapter((prev) =>
          prev?.id === currentChapter.id ? { ...prev, critique: feedbackStr, workflowMeta } : prev
        );
      }
      try {
      ...
      if (error instanceof Error && error.name === 'AuditUnknownError') {
        ...
        setAuditStatus(null);
        setAiActionStateForRequest(startingChapterId, currentSeq, (state) =>
          createAiActionError(state, '审稿结果无法解析，正文未被修改。')
        );
        return;
      }
      void recordProductEvent({
      ...
      if (error instanceof Error && error.message === 'QUOTA_LIMIT_EXCEEDED') {
        setAiActionStateForRequest(startingChapterId, currentSeq, (state) =>
          createAiActionError(state, '审稿暂不可用，请检查当前能力额度后重试。')
        );
        return;
      }
      setAuditStatus(null);
  ...
  const handleRewriteSelectedText = async (retryInput?: {
    start: number;
    end: number;
    instruction: string;
    fingerprint?: string;
    ...
    setRetryContext?.({
      operation: 'rewrite',
      input: { start, end, instruction, fingerprint: retryInput?.fingerprint },
    });
    ...
    try {
      ...
      if (baselineContent.substring(start, end) !== selectedTextAtCapture) {
        setAiActionStateForRequest(startingChapterId, currentSeq, (state) =>
          createAiActionError(state, '选区内容已变化，请重新选择后再改写。')
        );
        setIsGeneratingContent(false);
        return;
      }
      ...
      const response = await fetch('/api/rewrite', {
        ...
        body: JSON.stringify({
          ...
          sessionCardIds: sessionCardIds?.length ? sessionCardIds : undefined,
          styleConfirmationFingerprint:
            retryInput?.fingerprint || writingStyleFingerprint || undefined,
        }),
        signal: controller.signal,
      });
      ...
      const styleOutcome = await handleStyleConfirmationResponse(response, (fingerprint) =>
        handleRewriteSelectedText({ start, end, instruction, fingerprint })
      );
      if (styleOutcome.handled) {
        setAiActionStateForRequest(startingChapterId, currentSeq, idleAiAction());
        return;
      ...
      if (!response.ok) {
        const errorData = (styleOutcome.data ?? (await readErrorBodyOnce(response))) as {
          error?: string;
          quotaExceeded?: boolean;
        } | null;
      ...
      if (!streamResult.done) {
        ...
        setAiActionStateForRequest(startingChapterId, currentSeq, (state) =>
          createAiActionError(state, '改写流未正常结束，原文已恢复。')
        );
        return;
      }
      ...
      const newText =
        baselineContent.substring(0, start) + rewritten + baselineContent.substring(end);

      if (!rewritten.trim()) {
        restorePreviewIfCurrent(baselineContent, startingChapterId, currentSeq);
        setAiActionStateForRequest(startingChapterId, currentSeq, (state) =>
          createAiActionError(state, '改写结果为空，原文已恢复。')
        );
        return;
      }
      ...
      setCandidate?.({
        id: `${currentChapter.id}:${currentSeq}:rewrite`,
        operation: 'rewrite',
        novelId: novel.id,
        chapterId: currentChapter.id,
        databaseGeneration,
      ...
      setAiActionStateForRequest(startingChapterId, currentSeq, (state) =>
        createAiActionSuccess(state, '选中改写候选已生成，接受后写入正文。')
      );
    } catch (error) {
      if (!isRequestCurrent(startingChapterId, currentSeq)) return;
      if (error instanceof Error && error.name === 'AbortError') {
      ...
      if (error instanceof Error && error.message === 'QUOTA_LIMIT_EXCEEDED') {
        setAiActionStateForRequest(startingChapterId, currentSeq, (state) =>
          createAiActionError(state, '改写暂不可用，请检查当前能力额度后重试。')
        );
        return;
      }
      restorePreviewIfCurrent(baselineContent, startingChapterId, currentSeq);
  ...
  const handleContextRewriteCandidate = async (input: ContextRewriteCandidateInput) => {
    const startingChapterId = currentChapter?.id;
    if (!currentChapter || !contentRef.current || input.selectionEnd <= input.selectionStart)
      return;
    const currentSeq = ++requestSeqRef.current;
    const controller = new AbortController();
    abortControllerRef.current?.abort();
    ...
    try {
      ...
      const response = await fetch('/api/rewrite', {
        ...
        body: JSON.stringify({
          ...
          auditIssue: input.auditIssue,
          instruction: '只修复结构证据窗口，保持人物、事实、关系、设定和事件后果不变。',
          contextStr: buildContextPrompt(buildAgentContext()),
          sceneBeats: input.sceneBeats || currentChapter.sceneBeats || '',
          novelId: novel.id,
          chapterId: currentChapter.id,
      ...
      if (!response.ok)
        throw new Error(
          ((await response.json().catch(() => ({}))) as { error?: string }).error ||
            `精修失败：HTTP ${response.status}`
        );
      const responseGeneration = requireResponseDatabaseGeneration(response);
      if (responseGeneration !== input.databaseGeneration)
        throw new Error('数据库已切换，候选已失效。');
      const streamResult = await readSseStream(response, () => undefined);
      if (!streamResult.done || !streamResult.text.trim())
        throw new Error('精修结果为空或未正常结束。');
      if (!isRequestCurrent(startingChapterId, currentSeq)) return;
      ...
      setCandidate?.({
        id: `${currentChapter.id}:${currentSeq}:context-rewrite`,
        operation: 'rewrite',
        novelId: novel.id,
        chapterId: currentChapter.id,
        databaseGeneration: input.databaseGeneration,
        ...
        instruction: input.auditIssue,
        quality,
        source: 'model',
      });
      setAiActionStateForRequest(startingChapterId, currentSeq, (state) =>
        createAiActionSuccess(state, '上下文精修候选已生成，接受后写入正文。')
      );
    } catch (error) {
      if (
        isRequestCurrent(startingChapterId, currentSeq) &&
        !(error instanceof Error && error.name === 'AbortError')
      ) {
        setAiActionStateForRequest(startingChapterId, currentSeq, (state) =>
          createAiActionError(state, formatAiFailure(error, '上下文精修'))
        );
      }
      throw error;
    } finally {
  ...
  const handlePolishChapterFromAudit = async (
    ...
    const startingChapterId = currentChapter?.id;
    const candidateSnapshot = getCandidate?.() ?? aiContentCandidate;
    const candidateForPolish =
      currentChapter && candidateSnapshot && candidateSnapshot.chapterId === currentChapter.id
        ? candidateSnapshot
        : null;
    ...
    try {
      ...
      const actionableTargets = selectRewriteTargetsForPatch(
        candidate,
        rewriteTargets,
        3,
        auditFeedback
      ...
      if (duplicateTargets.length === 0 && actionableTargets.length === 0) {
        void recordProductEvent({
          ...
          result: 'failure',
          durationMs: Date.now() - startedAt,
          errorCode: 'NO_ACTIONABLE_TARGET',
          novelId: novel.id,
          chapterId: currentChapter.id,
        }).catch(() => undefined);
        setGenerationStatus(null);
        setAiActionStateForRequest(startingChapterId, currentSeq, (state) =>
          createAiActionError(state, '本轮审计没有定位到可自动精修的明确片段。')
        );
        return;
      }
      ...
      for (const { snippet } of actionableTargets) {
        ...
        const targetWindow = selectRewriteTargetsForPatch(candidate, [snippet], 1, auditFeedback)[0]
          ?.window;
        ...
        const response = await fetch('/api/rewrite', {
          ...
          body: JSON.stringify({
            ...
            sessionCardIds: sessionCardIds?.length ? sessionCardIds : undefined,
            styleConfirmationFingerprint:
              fingerprintOverride || writingStyleFingerprint || undefined,
          }),
          signal: controller.signal,
        });
        ...
        const styleOutcome = await handleStyleConfirmationResponse(
          response,
          async (fingerprint) => {
      ...
      if (changed) {
        ...
        if (!guard.ok) {
          ...
          setGenerationStatus(null);
          setAiActionStateForRequest(startingChapterId, currentSeq, (state) =>
            createAiActionError(state, `精修结果未通过安全校验：${guard.reason}`)
          );
          return;
        }
        ...
        if (!polishQuality.ok) {
          ...
          setAiActionStateForRequest(startingChapterId, currentSeq, (state) =>
            createAiActionError(
              state,
              `精修候选未通过质量门禁：${polishQuality.violations.join('；')}`
            )
          );
          return;
        }
        ...
        if (reviewOptions?.previewOnly) {
          setGenerationStatus(null);
          setAiActionStateForRequest(startingChapterId, currentSeq, (state) =>
            createAiActionSuccess(state, '修正预览已生成，正文尚未修改。')
          );
          return candidate;
        }
        ...
        const existingReviewState =
          candidateForPolish?.workflowMeta?.reviewState || currentChapter.workflowMeta?.reviewState;
        ...
        const appliedIssueIds = new Set(
          reviewOptions?.issueIds?.length
            ...
            : (reviewState?.issues || [])
                .filter((issue) =>
                  actionableTargets.some((target) => target.snippet === issue.snippet)
                )
                .map((issue) => issue.id)
        );
        const nextWorkflowMeta: ChapterWorkflowMeta = {
          ...(candidateForPolish?.workflowMeta ||
            currentChapter.workflowMeta || { version: 1 as const }),
          version: 1,
          reviewState: reviewState
            ? {
                ...reviewState,
                issues: reviewState.issues.map((issue) =>
                  appliedIssueIds.has(issue.id)
                    ? {
                        ...issue,
        ...
        setCandidate?.({
          id: `${currentChapter.id}:${currentSeq}:polish`,
          operation: 'polish',
          novelId: novel.id,
          chapterId: currentChapter.id,
          databaseGeneration,
          createdAt: Date.now(),
          baselineHash: computeChapterWorkflowHash(baseline, currentChapter.sceneBeats),
          baselineContent: baseline,
          content: candidate,
          reviewIssueIds: [...appliedIssueIds],
          reviewRecheck: reviewOptions?.previewOnly ? false : reviewOptions?.recheck !== false,
          auditFeedback,
          workflowMeta: nextWorkflowMeta,
        ...
        setAiActionStateForRequest(startingChapterId, currentSeq, (state) =>
          createAiActionSuccess(state, '精修候选已生成，接受后写入正文。')
        );
        return candidate;
      } else {
        void recordProductEvent({
        ...
        setGenerationStatus(null);
        setAiActionStateForRequest(startingChapterId, currentSeq, (state) =>
          createAiActionError(state, '精修没有产生有效变化，请调整审稿意见后重试。')
        );
      }
    } catch (error) {
      if (!isRequestCurrent(startingChapterId, currentSeq)) return;
      ...
      if (error instanceof Error && error.message === 'QUOTA_LIMIT_EXCEEDED') {
        setAiActionStateForRequest(startingChapterId, currentSeq, (state) =>
          createAiActionError(state, '精修暂不可用，请检查当前能力额度后重试。')
        );
        return;
      }
      restorePreviewIfCurrent(baseline, startingChapterId, currentSeq);

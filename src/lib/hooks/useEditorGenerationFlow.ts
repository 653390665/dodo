...
export function useEditorGenerationFlow({
  ...
  const setIsGeneratingCritique = useEditorGenerationStore(
    (state) => state.setIsGeneratingCritique
  );
  ...
  const [auditUnknownState, setAuditUnknownState] = useState<{
    chapterId: string;
    feedback: string;
  } | null>(null);
  ...
  const setIsAcceptingAiCandidate = useEditorGenerationStore(
    (state) => state.setIsAcceptingAiCandidate
  );
  ...
  const pendingAuditRecheckRef = useRef<{
    chapterId: string;
    issueIds: string[];
    contentHash: string;
  } | null>(null);
  const auditHandlerRef = useRef<
    | ((options?: {
  ...
  useEffect(() => {
    ...
    setGenerationStatus,
    setIsAcceptingAiCandidate,
  ]);

  useEffect(() => {
    const previousScope = generationScopeRef.current;
    if (
      previousScope.databaseGeneration === databaseGeneration &&
      previousScope.novelId === novel.id
    )
      return;
  ...
  const acceptAiContentCandidate = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        candidate.databaseGeneration !== databaseGeneration
      ) {
        candidateInvalidated = true;
      ...
      if (!evaluation.accepted) {
        candidateInvalidated = true;
        throw new Error(
          `正文候选${evaluation.status === 'review-required' ? '尚未完成审阅' : '未通过质量门禁'}：${evaluation.reasons.join('；')}`
        );
      }
      const acceptedAt = Date.now();
      const acceptedWorkflowMeta =
        candidate.workflowMeta &&
        candidate.operation === 'polish' &&
        candidate.reviewIssueIds?.length
          ? {
              ...candidate.workflowMeta,
              reviewState: candidate.workflowMeta.reviewState
                ? {
                    ...candidate.workflowMeta.reviewState,
                    issues: candidate.workflowMeta.reviewState.issues.map((issue) =>
                      candidate.reviewIssueIds?.includes(issue.id)
                        ? {
                            ...issue,
                    ...
                    gate: deriveReviewGate(
                      candidate.workflowMeta.reviewState.issues.map((issue) =>
                        candidate.reviewIssueIds?.includes(issue.id)
                          ? {
                              ...issue,
                              status: 'applied' as const,
                      ...
                      candidate.workflowMeta.reviewState.gate === 'unknown' ? 'unknown' : 'pass'
                    ),
                  }
                : undefined,
            }
  ...
  const {
    ...
    setGenerationStatus,
    setAuditStatus,
    setAuditUnknownFeedback: (feedback) =>
      setAuditUnknownState(feedback ? { chapterId: currentChapter?.id || '', feedback } : null),
    setAiActionState,
    setCurrentChapter,
    buildAgentContext,
    handleUpdateContent,
    ...
    setRetryContext: (context) => {
      retryContextRef.current = context;
    },
  });
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (
      !pendingAuditRecheck ||
      pendingAuditRecheck.chapterId !== currentChapter?.id ||
      !currentChapter
    )
      return;
  ...
  const retryLastAiAction = useCallback(async () => {
    ...
    else if (aiActionState.operation === 'polish')
      await handlePolishChapterFromAudit(
        retryContextRef.current?.fingerprint,
        retryContextRef.current?.reviewOptions
      );
    else if (aiActionState.operation === 'rewrite')
      await handleRewriteSelectedText(retryContextRef.current?.input);
  ...
  return {
    ...
    auditUnknownFeedback:
      auditUnknownState && auditUnknownState.chapterId === currentChapter?.id

...
function readStoredState(
  key: string,
  initialStackedIds: string[] | undefined,
  limit: number
): { stackedIds: string[]; skippedIds: string[] } {
  ...
  try {
    ...
    return {
      stackedIds: normalizeIds(parsed.stackedIds, limit),
      skippedIds: normalizeIds(parsed.skippedIds),
    };
  } catch {
    return { stackedIds: initial, skippedIds: [] };
  }
...
export function useEditorRecommendationCards({
  ...
  const storageKey = useMemo(
    () =>
      `inkflow:recommendation-cards:v1:${encodeURIComponent(novelId)}:${encodeURIComponent(chapterId || 'none')}`,
    [novelId, chapterId]
  );
  const initialStackedSignature = normalizeIds(initialStackedIds, stackLimit).join('\u0000');
  const [sessionState, setSessionState] = useState<SessionState>(() => ({
    key: storageKey,
    ...readStoredState(storageKey, initialStackedIds, stackLimit),
  }));
  ...
  const persist = useCallback(
    (stacked: string[], skipped: string[]) => {
      try {
        window.sessionStorage.setItem(
          storageKey,
          JSON.stringify({ stackedIds: stacked, skippedIds: skipped })
        );
      } catch {
        /* storage is optional */
      }
  ...
  const handleStackDeconstructionCard = async (assetId: string) => {
    ...
    if (stackedRef.current.length >= stackLimit) {
      toast(`本章最多使用 ${stackLimit} 张拆书卡`, 'info');
      return;
    }
    ...
    if (recordSkillUsageRef.current) {
      void recordSkillUsageRef
        .current('accepted', { notes: `stacked:${id}`, skillIds: [id] })
        .catch(() => {});
    }
  ...
  const handleUnstackDeconstructionCard = async (assetId: string) => {
    ...
    if (recordSkillUsageRef.current) {
      void recordSkillUsageRef
        .current('rejected', { notes: `unstacked:${id}`, skillIds: [id] })
        .catch(() => {});
    }
    void recordProductEvent({
      eventName: 'deconstruction_card_unstack',
      stage: 'drafting',
      result: 'success',
      novelId,
      chapterId,
  ...
  const removeStackedDeconstructionCard = useCallback(
  ...
  const handleSkipAsset = async (assetId: string) => {
    ...
    if (skippedRef.current.length >= MAX_SESSION_CARDS) {
      toast('最多保留 6 张跳过卡', 'info');
      return;
    }
    ...
    if (recordSkillUsageRef.current) {
      void recordSkillUsageRef
        .current('rejected', { notes: `skipped:${id}`, skillIds: [id] })
        .catch(() => {});
    }
    void recordProductEvent({
      eventName: 'deconstruction_card_skip',
      stage: 'drafting',
      result: 'success',
      novelId,
      chapterId,

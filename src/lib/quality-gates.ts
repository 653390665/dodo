...
/**
 * Check if the parsed model output is a refusal instead of valid skill cards.
...
export function scoreSkillOutputAnchoring(
  ...
  // Gather all text fields from all skill cards
...
/**
 * Check field completeness across all skill cards.
...
export function evaluateSkillOutputQuality(
  ...
  if (!passed) {
    ...
    if (genericSkillCount > 1) {
      reasons.push(`${genericSkillCount}/${totalSkillCount} 张技能卡为模板化输出`);
    }
    if (anchoringScore < 0.15) {
      reasons.push('输出未充分引用原文关键词，可能是虚构的 skill');
    }
    if (fieldReport.overall < 0.5) {
      reasons.push('技能卡必要字段缺失严重');
    }
    issue = reasons.join('；');
  }

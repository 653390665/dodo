...
export function getProjectCapabilityProfile(
  novel?: Pick<Novel, 'projectPreferenceProfile'> | null
): ProjectCapabilityProfile | null {
  ...
  return {
    ...
    projectSkillDeck: {
      mainCardId:
        typeof profile.projectSkillDeck.mainCardId === 'string'
          ? profile.projectSkillDeck.mainCardId
          : undefined,
      supportCardIds: Array.isArray(profile.projectSkillDeck.supportCardIds)
        ? [
            ...new Set(
              profile.projectSkillDeck.supportCardIds.filter(
                (id): id is string => typeof id === 'string' && id.trim().length > 0
              )
            ),
          ]
        : [],
      updatedAt: Number.isFinite(profile.projectSkillDeck.updatedAt)
        ? profile.projectSkillDeck.updatedAt
        : 0,
    },
    favoriteTechniqueIds: Array.isArray(profile.favoriteTechniqueIds)
      ? [
          ...new Set(
            profile.favoriteTechniqueIds.filter(
              (id): id is string => typeof id === 'string' && id.trim().length > 0
            )
          ),
        ]
    ...
    projectTechniqueIds: Array.isArray(profile.projectTechniqueIds)
      ? [
          ...new Set(
            profile.projectTechniqueIds.filter(
              (id): id is string => typeof id === 'string' && id.trim().length > 0
            )
          ),
        ]
    ...
    migrationPendingIds: Array.isArray(profile.migrationPendingIds)
      ? [
          ...new Set(
            profile.migrationPendingIds.filter(
              (id): id is string => typeof id === 'string' && id.trim().length > 0
            )
          ),
        ]
  ...
  return {
    ...
    capabilityProfile: {
      ...
      projectSkillDeck: {
        ...
        supportCardIds: [
          ...new Set(
            patch.projectSkillDeck?.supportCardIds || existing.projectSkillDeck.supportCardIds
          ),
        ],
        updatedAt: patch.projectSkillDeck?.updatedAt ?? existing.projectSkillDeck.updatedAt,
      },
      favoriteTechniqueIds: [
        ...new Set(patch.favoriteTechniqueIds ?? existing.favoriteTechniqueIds),
      ],
      projectTechniqueIds: [
        ...new Set(patch.projectTechniqueIds ?? existing.projectTechniqueIds ?? []),
      ],
      guardrailIds: patch.guardrailIds || existing.guardrailIds,
      capabilityMemberships: (
        patch.capabilityMemberships ||
        existing.capabilityMemberships ||
        []
...
export function addCardToProjectDeck(
  ...
  return {
    ...
    profile: {
      ...
      projectSkillDeck: {
        mainCardId,
        supportCardIds: [...new Set(supportCardIds)],
        updatedAt: Date.now(),
      },
    },
  };
...
export function getMountedRoleSlotState(
...
export function getCapabilityDisplayText(
  text: string,
  ...
  return sourceType === 'licensed'
    ...
    : sourceType === 'built-in'
      ? '官方内置能力，具体效果以实际运行结果为准。'
      : '广场共享能力，具体效果以实际运行结果为准。';
}
...
export function getRoleSkillSlots(assetId: string): RoleSkillSlot[] {
  ...
  return manifest.stages.filter(
    (stage): stage is RoleSkillSlot =>
      stage === 'planner' || stage === 'writer' || stage === 'critic'
  );
}

export function buildRoleSkillLoadout(
  current: MountedSkillLoadoutItem[],
  skillId: string,
  slot: RoleSkillSlot
): MountedSkillLoadoutItem[] {
  const slotIndex = ROLE_SLOT_INDEX[slot];
  return [
    ...current.filter((entry) => entry.slot !== slotIndex && entry.skillId !== skillId),
    {
      slot: slotIndex,
      skillId,
      weight: 1,
...
export function getCapabilityManifest(
  asset: Pick<CuratedProductSkill, 'id'>

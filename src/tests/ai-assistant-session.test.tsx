...
const launchContext: AssistantLaunchContext = {
  source: 'workspace',
  novelId: novelA.id,
...
describe('AIAssistant general session', () => {
  ...
  test('按作品隔离并在关闭重挂后保留输入和消息', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('创作困惑？'), {
      target: { value: 'A 待续输入' },
    });
  ...
  test('作品切换或卸载会取消请求，迟到结果不会写入新作品', async () => {
    ...
    mocks.generateInspiration.mockImplementation(
      () =>
  ...
  test('作品切换后迟到的设定提取不会导入或更新新作品', async () => {
    let resolveExtraction:
      | ((value: { result: Record<string, unknown>; databaseGeneration: number }) => void)
      | undefined;
    mocks.extractWorldSetupPhase.mockImplementation(
      (
        _content: string,
        _novelId: string,
  ...
  test('空字符串响应显示结构化的人话诊断信息', async () => {
    ...
    expect(screen.getByRole('alert', { name: '助手请求失败' }).textContent).not.toContain(
      'no_content'
    );
  });

  test('服务端 reason、finishReason、traceId 在告警中可见且不暴露 prompt', async () => {
    mocks.generateInspiration.mockRejectedValueOnce(
      Object.assign(new Error('模型响应为空'), {
        code: 'empty_response',

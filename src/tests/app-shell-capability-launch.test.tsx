...
vi.mock('../lib/api', () => ({
  ...
  getNovel: vi.fn(),
  listChapters: vi.fn(),
  listSkills: vi.fn().mockResolvedValue([recommendedSkill]),
...
vi.mock('../components/WelcomeView', () => ({
  WelcomeView: ({
    ...
    const planning: StoryPlanningInput = {
    ...
    return (
      <>
        <button
          type="button"
          onClick={() =>
            void onSelectStoryCard(card, planning, ['番茄'], 'workspace', 'tomato-platform-flow')
          }
        >
          用番茄流程开书
        </button>
        <button
          type="button"
          onClick={() =>
            void onSelectStoryCard(card, planning, ['番茄'], 'world', 'tomato-platform-flow')
          }
        >
          用番茄流程开书并补设定
        </button>
...
vi.mock('../components/ProjectCockpitView', () => ({
  ProjectCockpitView: () => <div>COCKPIT_OVERVIEW</div>,
}));
...
vi.mock('../components/WorldBibleView', () => ({
  WorldBibleView: ({
    ...
    onboarding?: {
      ...
      recommendedSkills: Array<{ skillName: string }>;
    };
    ...
    <div>
      WORLD_BIBLE
      <div>
        WORLD_CAPABILITY:{capabilityLaunchIntent?.capabilityId || 'none'}:
        {capabilityLaunchIntent?.artifactKind || 'none'}:
        {capabilityLaunchIntent?.launchToken || 'none'}
      </div>
      {capabilityLaunchIntent ? (
        <button
          type="button"
          onClick={() => onCapabilityLaunchConsumed?.(capabilityLaunchIntent.launchToken)}
        >
          确认启动意图已接收
        </button>
      ) : null}
      {onboarding?.recommendedSkills.length ? (
        <>
          <div>
            ...
            {onboarding.recommendedSkills.map((skill) => skill.skillName).join(',')}
          </div>
...
vi.mock('../components/SkillsStudioView', () => ({
  SkillsStudioView: ({
    ...
    <div>
      ...
      <button type="button" onClick={() => onNavigate?.('editor', { capabilityApplied: true })}>
        应用后返回能力来源
      </button>
      <button type="button" onClick={() => onNavigate?.('workspace', { capabilityApplied: true })}>
        应用后返回工作台
      </button>
      <button
        type="button"
...
vi.mock('../components/EditorView', () => ({
  EditorView: ({
    ...
    onNavigate?: (
      view: 'skills' | 'factory',
      context: { targetChapterId: string; stage?: string }
    ) => void;
    ...
    return (
      <div>
        <div>
          ...
          {initialChapterId}:CAP_TARGET:{capabilityLaunchState?.targetChapterId}:CARDS:
          {capabilityLaunchState?.sessionCardIds?.join(',') || 'none'}
        </div>
        <button
          type="button"
          onClick={() => onNavigate?.('skills', { targetChapterId: 'chapter-2' })}
        >
          打开作品能力中心
        </button>
        <button
          type="button"
          onClick={() =>
            onNavigate?.('skills', { targetChapterId: 'chapter-2', stage: 'style-polish' })
          }
        >
          打开精修能力卡
        </button>
        <button
          type="button"
          onClick={() => onNavigate?.('factory', { targetChapterId: 'chapter-2' })}
        >
          从当前章节拆书
        </button>
        <button
          type="button"
          onClick={() => onNavigate?.('factory', { targetChapterId: 'chapter-3' })}
        >
          从其它章节拆书
        </button>
      </div>
...
const novel: Novel = {
  ...
  projectPreferenceProfile: {
    ...
    writingStyleConfirmation: {
      mode: 'skill-deck',
      fingerprint: 'style-fingerprint-2',
      confirmedAt: 1,
    },
  },
};
...
describe('AppShell capability launch', () => {
  ...
  test('creates onboarding novels with v3 active flow configuration', async () => {
    ...
    await waitFor(() =>
      expect(dbCallMock).toHaveBeenCalledWith(
        'createNovelWithChapter',
        expect.any(Object),
        expect.objectContaining({ title: '第一章', order: 0 })
      )
    );
    const createdNovel = dbCallMock.mock.calls.find(
      ([method]) => method === 'createNovelWithChapter'
    )?.[1] as Novel;
    ...
    expect(createdNovel.projectPreferenceProfile?.capabilityProfile?.activeFlowId).toBe(
      'tomato-platform-flow'
    );
    expect(
      createdNovel.projectPreferenceProfile?.capabilityProfile?.projectSkillDeck.supportCardIds
    ).toEqual([]);
  ...
  test('routes a governed capability directly to Editor with independent launch state', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(/EDITOR:use-overlay:overlay-1.*CARDS:persisted-card-1/)).toBeDefined()
    );
  ...
  test('passes the originating chapter execution context into Book Factory', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText('FACTORY:CHAPTER:chapter-2:GEN:17:STYLE:style-fingerprint-2')
      ).toBeDefined()
    );
  });
  ...
  test('does not pass a stale writing style fingerprint into Book Factory for a different chapter', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText('FACTORY:CHAPTER:chapter-3:GEN:17:STYLE:')).toBeDefined()
    );
  });
  ...
  test('runs diagnostics against the chapter that opened the capability center', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText(
          /EDITOR:run-diagnostic:diagnostic-1:.*INITIAL:chapter-2:CAP_TARGET:chapter-2/
        )
      ).toBeDefined()
    );
  });
  ...
  test('uses an overlay card against the chapter that opened the capability center', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText(
          /EDITOR:use-overlay:overlay-1:.*CAP_TARGET:chapter-2:CARDS:persisted-card-1/
        )
      ).toBeDefined()
    );

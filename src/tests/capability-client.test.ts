...
describe('capability client', () => {
  test('executes a governed utility by ids and database generation only', async () => {
    const response = {
      kind: 'diagnostic',
      capabilityId: 'platform-tomato-scoring',
      baselineHash: 'hash-1',
      readOnly: true,
    ...
    await expect(
      executeCapability('novel-1', 'platform-tomato-scoring', {
        chapterId: 'chapter-1',
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/novels/novel-1/capabilities/platform-tomato-scoring/execute',
  ...
  test('preserves a stable server error code', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      Response.json({ code: 'UTILITY_GENERATION_STALE', error: 'changed' }, { status: 409 })
    );
    await expect(
      executeCapability('n', 'u', { chapterId: 'c', databaseGeneration: 1 })
    ).rejects.toMatchObject({
      code: 'UTILITY_GENERATION_STALE',
      status: 409,
    } satisfies Partial<CapabilityRequestError>);
  });
});

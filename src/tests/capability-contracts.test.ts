...
} from '../../shared/lib/enhancement-packages';
...
describe('capability contracts', () => {
  ...
  test('validates itemized application results and idempotency', () => {
    expect(
      isCapabilityApplicationResult({
        applied: true,
        idempotent: true,
        databaseGeneration: 4,
        items: [{ capabilityId: 'u', status: 'configured' }],
      })
    ).toBe(true);
    expect(
      isCapabilityApplicationResult({
        ...
        idempotent: true,
        databaseGeneration: 4,
        items: [{ capabilityId: 'u', status: 'unknown' }],
      })
    ).toBe(false);
  });
});

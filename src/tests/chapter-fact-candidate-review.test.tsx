...
vi.mock('../lib/chapter-production-db-client', () => ({
  listChapterProductionRuns: vi.fn().mockResolvedValue([]),
}));
  ...
  facts: [
    {
      ...
      title: '角色状态',
      evidence: '阿青拔剑',
      evidenceSpan: { start: 0, end: 4 },
      target: { kind: 'character', id: 'char-1', label: '阿青' },
      proposedValue: {},
      destructive: false,
      ambiguous: false,
      selectable: true,
    },
    {
      ...
      target: { kind: 'narrative-promise', id: 'promise-1', label: '戒指' },
      proposedValue: {},
      destructive: true,
      ambiguous: true,
      selectable: true,
...
function run(status: ChapterProductionRun['status'] = 'applied'): ChapterProductionRun {
  return {
    ...
    userIntent: '',
    sceneBeats: '节拍',
    draftContent: completeDraft('正文证据：阿青拔剑'),
    styleAudit: '',
    continuityReport: {
      databaseGeneration: 7,
      issues: [],
...
describe('ChapterFactCandidateReview', () => {
  ...
  beforeEach(() => {
    fetchMock = vi
      ...
      .mockResolvedValueOnce(
        response({ candidate, factStatuses: { safe: 'accepted', payoff: 'pending' } })
      )
      .mockResolvedValueOnce(response(pendingCandidate))
      .mockResolvedValueOnce(
        response({ candidate: pendingCandidate, factStatuses: { payoff: 'accepted' } })
      )
  ...
  test('uses real client/fetch URLs and re-previews pending facts for a second decision', async () => {
    ...
    fireEvent.change(screen.getByRole('combobox', { name: '角色状态 决定' }), {
    ...
    resolveFirstApply(
      response({ candidate, factStatuses: { safe: 'rejected', payoff: 'pending' } })
    );
    await waitFor(() => expect(fetchMock).toHaveBeenCalledTimes(3));
    expect(fetchMock.mock.calls[0]?.[0]).toBe(
      '/api/chapter-production-runs/run-1/fact-candidate/preview'
    );
    ...
    expect(fetchMock.mock.calls[1]?.[0]).toBe(
      '/api/chapter-production-runs/run-1/fact-candidate/apply'
    );
    expect(JSON.parse(fetchMock.mock.calls[1]?.[1].body)).toMatchObject({
      candidateId: 'fact-1',
      factDecisions: { safe: 'rejected', payoff: 'pending' },
    });
    ...
    expect(JSON.parse(fetchMock.mock.calls[3]?.[1].body)).toMatchObject({
      candidateId: 'fact-1',
      factDecisions: { payoff: 'accepted' },
    });
    expect(JSON.parse(fetchMock.mock.calls[3]?.[1].body).factDecisions).toEqual({
      payoff: 'accepted',
    });
  });

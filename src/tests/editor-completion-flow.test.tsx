...
const result = {
  ...
  gate: {
    ...
    planHash: 'plan-hash',
    quality: 'unknown' as const,
    completionGate: 'review-required' as const,
    deterministicIssues: ['issue-1'],
    unknownChecks: ['ai-review'],
    reviewRequired: true,
    canAcceptLocalRevision: true,
  },
};
...
function renderWritingSurface(
  ...
  return render(
    <WritingSurface
      ...
      onRunAudit={actions.audit || noop}
      onCompleteChapter={actions.complete || noop}
      onConfirmFacts={actions.facts || noop}
      onUpdateContent={vi.fn()}
      onQueueContentWrite={vi.fn()}
      onAddFirstChapter={noop}
      onAddChapter={noop}
      setAgentTab={actions.setTab || vi.fn()}
      setIsAgentSidebarOpen={actions.setOpen || vi.fn()}
      packStatus="none"
...
describe('ChapterCompletionReview', () => {
  ...
  test('统一主动作调用完成和事实确认，且正文保持可编辑', () => {
    ...
    renderWritingSurface(
      chapter({
        ...
        completionContentHash: computeChapterWorkflowHash('正文', '分镜'),
  ...
  test('处理审阅问题打开质量面板，不重复运行审稿', () => {
    ...
    renderWritingSurface(
      chapter({
        version: 1,
        reviewState: { schemaVersion: 1, contentHash, issues: [reviewIssue], gate: 'needs-action' },
      }),
      { audit, setTab, setOpen }
    );
  ...
  test('接受请求进行中防双击，失败后保留确认并可重试', async () => {
    ...
    expect(
      (screen.getByRole('checkbox', { name: '确认接受未审阅风险' }) as HTMLInputElement).checked
    ).toBe(true);
    fireEvent.click(accept);
  ...
  test('结果哈希变化后必须重新确认风险', () => {
    ...
    expect(screen.getByRole('button', { name: '接受未审阅风险' }).hasAttribute('disabled')).toBe(
      true
    );

...
const dbTransport = vi.hoisted(() => ({
  getDatabaseGenerationSnapshot: vi.fn(),
}));

vi.mock('../lib/api', () => api);
vi.mock('../lib/db-transport', () => ({
  getDatabaseGenerationSnapshot: dbTransport.getDatabaseGenerationSnapshot,
}));
...
describe('useEditorData full chapter loading', () => {
  beforeEach(() => {
    ...
    dbTransport.getDatabaseGenerationSnapshot.mockReset().mockResolvedValue(7);
  ...
  test('loads the complete persisted chapter instead of metadata placeholders', async () => {
    ...
    expect(result.current.databaseGeneration).toBe(7);
  });
  ...
  test('discards a chapter when the database generation changes during the read', async () => {
    ...
    await act(async () => {
      await result.current.selectChapter('a');
    });

    expect(result.current.currentChapter?.content).toBe('数据库完整正文');
  });
  ...
  test('clears only stale-generation writes after the new database context loads', async () => {
    ...
    const { result } = renderHook(() => useEditorData(novel.id));
    await waitFor(() => expect(result.current.databaseGeneration).toBe(7));
  ...
  test('normalizes a partial project preference profile at the editor data boundary', async () => {
    api.getNovel.mockResolvedValue({ ...novel, projectPreferenceProfile: { tags: [] } });

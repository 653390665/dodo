...
vi.mock('../lib/product-events-client', () => ({ recordProductEvent: client.recordProductEvent }));
...
describe('useEditorPersistence safety boundary', () => {
  beforeEach(() => {
    ...
    client.recordProductEvent.mockResolvedValue(undefined);
  });
  ...
  test('coalesces repeated add chapter requests while creation is in flight', async () => {
    ...
    act(() => {
      first = result.current.handleAddChapter();
      second = result.current.handleAddChapter();
    });
    for (let i = 0; i < 5 && !release; i += 1) await Promise.resolve();
    expect(client.createChapter).toHaveBeenCalledTimes(1);
    await act(async () => {
      release();
  ...
  test('adopts an imported outline through the queue before updating local state', async () => {
    ...
    await act(async () => {
      await expect(result.current.adoptGlobalOutline('导入大纲')).resolves.toBe(true);
    });

    expect(client.updateNovel).toHaveBeenCalledWith(novel.id, { globalOutline: '导入大纲' });
    expect(setGlobalOutline).toHaveBeenCalledWith('导入大纲');
    expect(hasPendingEditorWrites()).toBe(false);
  });

  test('keeps the old local outline when imported outline persistence fails', async () => {
    client.updateNovel.mockResolvedValue(false);
    const setGlobalOutline = vi.fn();
    const { result } = setup(chapter, [chapter], { current: null }, setGlobalOutline);
  ...
  test('records first content input once without content payload', async () => {
    const { result } = setup({ ...chapter, content: '' }, [{ ...chapter, content: '' }]);
  ...
  test('records content save only after a successful persistence', async () => {
    const { result } = setup({ ...chapter, content: '' }, [{ ...chapter, content: '' }]);

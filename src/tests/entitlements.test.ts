...
import {
  ...
  normalizeCapabilityUnavailableDetail,
...
describe('entitlements', () => {
  it('defaults to open beta when monetization is disabled', () => {
    ...
    expect(
      canUseEnhancedCapability({
        commercialMode: 'free',
        env: { VITE_INKFLOW_ENABLE_MONETIZATION: 'false' },
      })
    ).toBe(true);
    expect(
      canUseEnhancedCapability({
        commercialMode: 'strict',
        env: { VITE_INKFLOW_ENABLE_MONETIZATION: 'false' },
      })
    ).toBe(true);
  ...
  it('keeps the licensed Fenghua polish asset open in beta only', () => {
    const input = {
      activeSeriesId: 'fenghua-short-flow' as const,
      currentStage: 'polish' as const,
    };
    const betaFree = recommendPromptAssets({
      ...input,
      commercialMode: getEffectiveCommercialMode('free', {
        VITE_INKFLOW_ENABLE_MONETIZATION: 'false',
      }),
    });
    const commercialFree = recommendPromptAssets({
      ...input,
      commercialMode: getEffectiveCommercialMode('free', {
        VITE_INKFLOW_ENABLE_MONETIZATION: 'true',
      }),
    });
  ...
  it('filters licensed recommendations by the real entitlement axis', () => {
    ...
    const recommend = (mode: string, env: Record<string, string>) =>
      filterLicensedAssetsByEntitlement(
    ...
    expect(recommend('free', commercialEnv).some((asset) => asset.id === 'private-163')).toBe(
      false
    );
    expect(recommend('strict', commercialEnv).some((asset) => asset.id === 'private-163')).toBe(
      false
    );
  });
  ...
  it('preserves strict governance routing in beta for generic flows', () => {
    ...
    const input = {
      activeSeriesId: 'generic-novel-flow' as const,
      currentStage: 'polish' as const,
    };
  ...
  it('keeps user-facing entitlement copy neutral', () => {
    ...
    expect(source('../components/book-factory/QualityTab.tsx')).toContain(
      'filterLicensedAssetsByEntitlement'
    );
    ...
    expect(source('../components/SettingsModal.tsx')).not.toMatch(
      /activateGlobalPremium|deactivateGlobalPremium|立即激活/
    );
  });

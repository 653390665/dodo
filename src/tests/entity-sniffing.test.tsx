...
describe('useEntitySniffing', () => {
  beforeEach(() => {
    ...
    startWorldJob.mockResolvedValue({
      result: { activeExisting: [], newEntities: [] },
  ...
  test('同一章节内容与实体名称重复触发在节流窗口内只发一次请求', async () => {
    ...
    const { result, rerender } = renderHook(
      ({ currentChapter, characters }: { currentChapter: Chapter; characters: Character[] }) =>
        useEntitySniffing({
          novelId: 'novel-1',
          currentChapter,
          characters,
    ...
    await act(async () => {
      await result.current.handleSniffEntities();
    });
    rerender({
      currentChapter: { ...chapter },
      characters: [{ name: '主角', updatedAt: 2 } as Character],
    });
    await act(async () => {
      await result.current.handleSniffEntities();
    });

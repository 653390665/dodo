...
const source: LegacyArtifactSource = {
  ...
  artifactId: 'novel-1',
  label: '世界观',
  originalContent: '世界规则原文',
  artifactVersion: 0,
  sourceFingerprint: 'fingerprint-1',
};
const changedSource = {
  ...source,
  originalContent: '更新后的世界规则',
...
describe('LegacyArtifactStructuringPrompt', () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.endsWith('/legacy-artifacts/preview')) {
          if (previewFails)
            return Response.json({ code: 'INVALID', error: '预览失败' }, { status: 422 });
          return Response.json({
            preview: {
              ...
              proposedCore: {
                schemaVersion: 1,
                hardRules: [{ id: 'rule-1', statement: '不可飞' }],
              },
        ...
        if (url.endsWith('/legacy-artifacts/confirm'))
          return Response.json({ status: 'accepted', version: 1 });
    ...
    await waitFor(() =>
      expect(fetch).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          method: 'POST',
          body: JSON.stringify({ previewId: 'preview-1', databaseGeneration: 7 }),
        })
      )
    );
  ...
  test('dismisses only the current fingerprint and shows a changed source again', async () => {
    ...
    expect(localStorage.getItem('inkflow-legacy-structuring-dismissals:novel-1')).toContain(
      'fingerprint-1'
    );

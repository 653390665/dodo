...
describe('outline client', () => {
  test('list encodes novel ID and filters', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/novels/novel%2F%E4%B8%80/outlines?level=chapter&status=candidate',
      undefined
    );
  });
  ...
  test('non-2xx yields stable error and does not emit', async () => {
    const fetchMock = vi
      .spyOn(globalThis, 'fetch')
      .mockResolvedValue(response({ code: 'OUTLINE_GENERATION_STALE', error: 'stale' }, 409));
    const events: string[] = [];
    const unsubscribe = subscribeToOutlineGovernanceChanges((event) => events.push(event.novelId));
    await expect(activateOutline('n1', 'o1', 3)).rejects.toMatchObject({
      code: 'OUTLINE_GENERATION_STALE',
      status: 409,
    });
    unsubscribe();

...
import {
  buildContinuationContextBundle,
  finalizeContextReceipt,
...
describe('P0 AI trust contracts', () => {
  ...
  test('context receipt hashes the actual runtime fragments and records source metadata', () => {
    ...
    expect(bundle.receipt.sources?.some((source) => source.id === 'planner' && source.sha256)).toBe(
      true
    );
    expect(bundle.receipt.runtimeSha256).toMatch(/^[a-f0-9]{64}$/);
    expect(bundle.receipt.runtimeSha256).not.toBe(changed.receipt.runtimeSha256);
    const finalized = finalizeContextReceipt(
      bundle.receipt,
      `${bundle.text}\n\nintent-a\n\nskill-a\n\npref-a`,
      []
    );
    const changedFinalized = finalizeContextReceipt(
      bundle.receipt,
      `${bundle.text}\n\nintent-b\n\nskill-a\n\npref-a`,
      []
    );

...
describe('useEditorRecommendationCards session state', () => {
  ...
  const key = (novelId: string, chapterId: string) =>
    `inkflow:recommendation-cards:v1:${encodeURIComponent(novelId)}:${encodeURIComponent(chapterId)}`;
  ...
  it('restores per novel and chapter, ignores corrupt storage, and caps normalized ids', async () => {
    sessionStorage.setItem(
      'inkflow:recommendation-cards:v1:n1:c1',
      JSON.stringify({
        stackedIds: [' a ', 'a', '', ...Array.from({ length: 8 }, (_, i) => `s${i}`)],
        skippedIds: ['x', 'x'],
      })
    );
  ...
  it('makes stack, unstack, and skip idempotent and records product events', async () => {
    ...
    await act(async () => {
      await result.current.handleStackDeconstructionCard(' card-1 ');
    });
    await act(async () => {
      await result.current.handleStackDeconstructionCard('card-1');
    });
    await act(async () => {
      await result.current.handleUnstackDeconstructionCard(' card-1 ');
    });
    await act(async () => {
      await result.current.handleUnstackDeconstructionCard('card-1');
    });
    await act(async () => {
      await result.current.handleSkipAsset('card-2');
    });
    await act(async () => {
      await result.current.handleSkipAsset('card-2');
    });
    ...
    expect(
      recordSkillUsageRef.current.mock.calls.map(([action, options]) => [
        action,
        options?.notes,
        options?.skillIds,
      ])
    ...
    expect(payloads).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          eventName: 'deconstruction_card_stack',
          stage: 'drafting',
          novelId: 'n1',
          chapterId: 'c1',
          objectId: 'card-1',
  ...
  it('keeps local state, storage, events, and toast successful when usage recording rejects', async () => {
    const fetchSpy = vi
      .spyOn(globalThis, 'fetch')
      .mockResolvedValue(new Response('{}', { status: 200 }));
    const recordSkillUsageRef = {
      current: vi.fn().mockRejectedValue(new Error('telemetry failed')),
    };
    const { result } = renderHook(() =>
      useEditorRecommendationCards({ novelId: 'n1', chapterId: 'c1', recordSkillUsageRef })
    );
    await act(async () => {
      await expect(
        result.current.handleStackDeconstructionCard('card-usage-fail')
  ...
  it('encodes each storage key segment to prevent colon collisions', async () => {
    ...
    await act(async () => {
      await result.current.handleStackDeconstructionCard('safe-card');
    });
  ...
  it('rejects the seventh stack and skip without changing storage, usage, or events', async () => {
    ...
    for (let i = 0; i < 6; i++)
      await act(async () => {
        await result.current.handleStackDeconstructionCard(`stack-${i}`);
      });
    ...
    await act(async () => {
      await result.current.handleStackDeconstructionCard('stack-6');
    });
    expect(result.current.stackedDeconstructionCardIds).toHaveLength(6);
    ...
    for (let i = 0; i < 6; i++)
      await act(async () => {
        await result.current.handleSkipAsset(`skip-${i}`);
      });
    ...
    await act(async () => {
      await result.current.handleSkipAsset('skip-6');
    });
  ...
  it('restores the original group after switching groups and never exposes raw setters', async () => {
    sessionStorage.setItem(
      'inkflow:recommendation-cards:v1:n1:c1',
      JSON.stringify({ stackedIds: ['one'], skippedIds: [] })
    );
    sessionStorage.setItem(
      'inkflow:recommendation-cards:v1:n2:c1',
      JSON.stringify({ stackedIds: ['two'], skippedIds: [] })
    );
    const { result, rerender } = setup('n1', 'c1');
    expect(result.current.stackedDeconstructionCardIds).toEqual(['one']);
    rerender({ novelId: 'n2', chapterId: 'c1' });
    await act(async () => {
      await result.current.handleStackDeconstructionCard('should-not-cross-write');
    });
  ...
  it('preserves structured session-card errors from the client', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(
        JSON.stringify({
          code: 'UNKNOWN_SESSION_CARD',
          error: 'invalid',
          sessionCardId: 'bad-card',
        }),
    ...
    try {
      await resolveWritingStyle('n1', scopedRequest);
    } catch (error) {
      expect(error).toBeInstanceOf(WritingStyleRequestError);
    }
    vi.restoreAllMocks();
  });
  ...
  it('hydrates chapter capability cards and persists stack changes through the chapter boundary', async () => {
    ...
    await act(async () => {
      await result.current.handleStackDeconstructionCard('second-card');
    });
    ...
    await act(async () => {
      await result.current.handleStackDeconstructionCard('third-card');
    });

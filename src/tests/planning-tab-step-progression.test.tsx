...
describe('PlanningTab Step Progression', () => {
  ...
  test('窄栏向导布局允许推进按钮收缩并换行', () => {
    render(<PlanningTab {...defaultProps} />);

    const advanceBtn = screen.getByRole('button', { name: /完成本步并前往/ });
    const actionArea = advanceBtn.parentElement;
    const wizardLayout = actionArea?.parentElement;
    const contentArea = screen.getByText('脑洞灵感闪耀').parentElement;

    expect(actionArea?.className).toContain('w-full');
    expect(actionArea?.className).toContain('min-w-0');
    expect(actionArea?.className).toContain('items-stretch');
    expect(advanceBtn.className).toContain('w-full');
    expect(advanceBtn.className).toContain('min-w-0');
    expect(advanceBtn.className).toContain('whitespace-normal');
    expect(advanceBtn.className).toContain('text-center');
    expect(advanceBtn.className).toContain('leading-relaxed');
    expect(wizardLayout?.className).toContain('flex-col');
    expect(wizardLayout?.className).not.toContain('md:flex-row');
  ...
  test('v3 activeFlowId overrides legacy activeSeriesId in planning wizard', () => {
    const v3Novel = {
      ...mockNovel,
      projectPreferenceProfile: {
        ...mockNovel.projectPreferenceProfile,
    ...
    render(<PlanningTab {...defaultProps} novel={v3Novel as any} />);

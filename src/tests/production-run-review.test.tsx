...
describe('ProductionRunReview', () => {
  ...
  test('allows applying the completed active review run', async () => {
    const activeRun = createRun(
      'active-run',
      'review_required',
      completeDraft('active completed draft')
    );
    listChapterProductionRunsMock.mockResolvedValue([]);
  ...
  test('blocks applying a failed audit and explains that the draft needs polish', async () => {
    const failedRun = createRun(
      'failed-audit-run',
      'review_required',
      completeDraft('failed audit draft')
    );
  ...
  test('blocks applying a run with unknown provenance and explains that it must be regenerated', async () => {
    const unknownSourceRun = createRun(
      'unknown-source-run',
      'review_required',
      completeDraft('unknown source')
    );
    unknownSourceRun.continuityReport.auditMeta = {
      status: 'pass',
    } as unknown as typeof unknownSourceRun.continuityReport.auditMeta;
    listChapterProductionRunsMock.mockResolvedValue([]);
  ...
  test('blocks applying a fallback draft and preserves the retry path', async () => {
    const fallbackRun = createRun(
      'fallback-run',
      'review_required',
      completeDraft('fallback preview')
    );

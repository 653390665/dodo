...
function mockWelcomeFetch(response: Response) {
  return vi.fn().mockImplementation((url: string) => {
    if (url === '/api/db/generation') {
      return Promise.resolve(Response.json({ databaseGeneration: 7 }));
    }
...
describe('generateInspiration SSE protocol', () => {
  ...
  it('uses a server-issued onboarding session for welcome inspiration', async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(1, '/api/db/generation', expect.anything());
    expect(fetchMock).toHaveBeenNthCalledWith(2, '/api/onboarding/llm-session', {
      method: 'POST',
  ...
  it('passes an optional AbortSignal through to the inspiration fetch', async () => {

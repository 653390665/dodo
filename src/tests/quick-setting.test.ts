...
describe('persistQuickSetting', () => {
  ...
  test.each(['character', 'location', 'item'] as const)(
    'persists %s before returning it to local state',
    async (type) => {
      ...
      expect(expectedClient).toHaveBeenCalledWith(
        expect.objectContaining({ novelId: 'novel-1', name: 'Name' })
      );

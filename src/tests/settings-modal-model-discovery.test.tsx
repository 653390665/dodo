...
describe('SettingsModal Model Discovery', () => {
  ...
  test('config load failure reports an unknown key state and retries without pretending to be configured', async () => {
    let attempts = 0;
    window.fetch = vi.fn().mockImplementation((url: string) => {
      if (url !== '/api/config') return DEFAULT_FETCH(url);
      attempts += 1;
      if (attempts === 1) {
        return Promise.resolve({
          ok: false,
      ...
      return makeConfigResponse({
        hasApiKey: true,
        embeddingStatus: { status: 'unavailable', reason: 'not_initialized' },
      });
    });

    render(<SettingsModal isOpen onClose={() => {}} theme="dark" onThemeChange={() => {}} />);

    expect(await screen.findByText(/配置状态暂时无法确认/)).toBeDefined();
    ...
    await waitFor(() => expect(screen.getByText('已配置')).toBeDefined());
    expect(attempts).toBe(2);
    expect(screen.getByText('尚未初始化')).toBeDefined();
  });

  test('prompt test error keeps preview and offers a retry with stable error guidance', async () => {
    let attempts = 0;
    window.fetch = vi.fn().mockImplementation((url: string) => {
      if (url === '/api/config') return makeConfigResponse();
      if (url === '/api/prompt-template-test') {
        attempts += 1;
        return Promise.resolve({
          ok: false,
          json: () => Promise.resolve({
  ...
  test('switching prompt templates prevents an older test response from replacing the current result', async () => {
    let attempts = 0;

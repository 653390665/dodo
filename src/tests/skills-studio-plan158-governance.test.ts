...
describe('Plan 158 project capability deck', () => {
  test('creates an explicit project technique set and preserves legacy fallback input', () => {
    const legacy = buildV3CapabilityProfile(
      {
    ...
    const explicit = buildV3CapabilityProfile(
      {
        projectPreferenceProfile: {
          tags: [],
          weights: {
  ...
  test('requires an explicit replacement and preserves the other cards', () => {
    const profile = {
      ...
      projectSkillDeck: {
        mainCardId: 'main',
        supportCardIds: ['support-1', 'support-2'],
        updatedAt: 1,
      },
  ...
  test('keeps a stable source to persisted skill mapping across profile rebuilds', () => {
    ...
    const rebuilt = buildV3CapabilityProfile(
      {
        projectPreferenceProfile: {
          tags: [],
          weights: {
...
describe('Plan 158 capability configuration API', () => {
  ...
  test('previews then applies with generation and token', async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ previewToken: 'preview-1', databaseGeneration: 7 }), {
          status: 200,
        })
      )
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ profile: { version: 3 }, databaseGeneration: 8 }), {
          status: 200,
        })
      );
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      '/api/novels/novel-1/capabilities/configuration/preview',
      expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({ databaseGeneration: 7, capabilityProfile: profile }),
      })
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      '/api/novels/novel-1/capabilities/configuration/apply',
      expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({
  ...
  test('preserves server failure as a typed error for draft retry', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(
        JSON.stringify({ code: 'CAPABILITY_GENERATION_CONFLICT', error: 'generation changed' }),
        { status: 409 }
      )
    );
    await expect(
      previewCapabilityConfiguration(
        'novel-1',
        7,
        buildV3CapabilityProfile(null, {}).capabilityProfile!
      )
    ).rejects.toMatchObject({ code: 'CAPABILITY_GENERATION_CONFLICT', status: 409 });
  });
});

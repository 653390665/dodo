...
describe('deriveWorkflowState', () => {
  test('加载中不暴露动作', () => {
    expect(deriveWorkflowState({ loading: true })).toMatchObject({
      phase: 'import',
      primaryAction: null,
    });
  });
  ...
  test('草稿资料包进入资料审阅阶段', () => {
    expect(
      deriveProjectWorkflowState({ loading: false, chapter: null, packStatus: 'draft' })
    ).toMatchObject({
      phase: 'review',
      primaryAction: 'review',
    });
    expect(
      deriveWorkflowState({ loading: false, chapter: { content: '' }, packStatus: 'draft' })
    ).toMatchObject({
      phase: 'review',
      primaryAction: 'review',
    });
  ...
  test('按章节状态派生统一主动作', () => {
    expect(
      deriveWorkflowState({ loading: false, chapter: { content: '', sceneBeats: '' } })
        .primaryAction
    ).toBe('generate-plan');
    expect(
      deriveWorkflowState({ loading: false, chapter: { content: '', sceneBeats: '分镜' } })
        .primaryAction
    ).toBe('generate-prose');
    expect(
      deriveWorkflowState({ loading: false, chapter: { content: '正文', sceneBeats: '分镜' } })
        .primaryAction
    ).toBe('complete-chapter');
    ...
    expect(deriveWorkflowState({ loading: false, chapter: failed }).primaryAction).toBe(
      'resolve-issues'
    );
    ...
    expect(deriveWorkflowState({ loading: false, chapter: passed }).primaryAction).toBe(
      'complete-chapter'
    );
  });
    ...
    expect(
      deriveWorkflowState({
        ...
        chapter: {
          ...
          workflowMeta: {
            version: 1,
            lastAudit: { status: 'pass', contentHash: staleHash, completedAt: 1, source: 'model' },
          },
        },
      }).primaryAction
    ).toBe('complete-chapter');
    ...
    expect(
      deriveWorkflowState({
        ...
        chapter: {
          ...
          workflowMeta: {
            version: 1,
            lastAudit: { status, contentHash, completedAt: 1, source: 'fallback' },
          },
        },
      }).primaryAction
    ).toBe('complete-chapter');
    ...
    expect(
      deriveWorkflowState({
        ...
        chapter: {
          ...
          workflowMeta: {
            version: 1,
            reviewState: { schemaVersion: 1, contentHash, issues: [issue], gate: 'needs-action' },
          },
        },
      }).primaryAction
    ).toBe('resolve-issues');
        ...
        chapter: {
          ...
          workflowMeta: {
            ...
            reviewState: {
              schemaVersion: 1,
              contentHash,
              issues: [{ ...issue, status: 'accepted-risk' }],
              gate: 'accepted-risk',
            },
          },
        },
    ...
    expect(
      deriveWorkflowState({
        ...
        chapter: {
          ...
          workflowMeta: {
            version: 1,
            reviewState: { schemaVersion: 1, contentHash, issues: [issue], gate: 'pass' },
          },
        },
      }).primaryAction
    ).toBe('complete-chapter');
      ...
      deriveWorkflowState({
        ...
        chapter: {
          ...
          workflowMeta: {
            ...
            completionContentHash: contentHash,
            factCandidateId: 'candidate-1',
            factCandidateRunId: 'run-1',
          },
        },
      }).primaryAction
    ).toBe('confirm-facts');
      ...
      deriveWorkflowState({
        ...
        chapter: {
          ...
          workflowMeta: {
            ...
            completionGate: 'ready',
            completionContentHash: contentHash,
            completionDecisionAt: 1,
          },
        },
      }).primaryAction
    ).toBe('create-next-chapter');
    expect(
      deriveWorkflowState({
        loading: false,
        chapter: {
    ...
    expect(
      deriveWorkflowState({
        ...
        chapter: {
          content: '正文已修改',
          sceneBeats: '分镜',
          workflowMeta: { version: 1, completionGate: 'ready', completionContentHash: contentHash },
        },
      }).primaryAction
    ).toBe('complete-chapter');

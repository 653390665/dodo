...
describe('WorldBibleAssistant continuation batch sync', () => {
  ...
  test('批量提取请求保持独立 purpose，不按正文生成额度计费', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole('button', { name: '模拟确认同步预览' })).toBeTruthy()
    );
  ...
  test('大量既有设定会压缩为受限上下文，不触发 inspiration 请求长度校验', async () => {
    const fetchMock = fetch as unknown as ReturnType<typeof vi.fn>;
    mocks.listCharacters.mockResolvedValueOnce(
      Array.from({ length: 100 }, (_, index) => ({
        id: `character-${index}`,
        name: `人物${index}`,
        role: 'supporting',
        summary: '很长的背景'.repeat(500),
        traits: ['谨慎'],
        bio: '很长的小传'.repeat(500),
        current_state: '存活',
        novelId: novel.id,
      }))
    );
  ...
  test('批量回复结构化解析失败时保留可操作草稿状态，不显示无效设定误导文案', async () => {
    (fetch as unknown as ReturnType<typeof vi.fn>).mockResolvedValueOnce(
      streamResponse('[JSON_DATA]{"type":"character","data":[/JSON_DATA]')
    );
    renderBatch();
  ...
  test('未传 continuationPackId 时按用户资料包标题匹配唯一 pack 并显示检查入口', async () => {
    mocks.listContinuationPacks.mockResolvedValueOnce([
      { id: 'pack-from-title', title: '批量资料包', novelId: novel.id, status: 'approved' },
    ]);
    (fetch as unknown as ReturnType<typeof vi.fn>).mockResolvedValueOnce(
      streamResponse('资料包整理完成，等待冲突审查。')
    );
    renderBatch('');
    fireEvent.change(screen.getByRole('textbox', { name: '输入设定灵感' }), {
      target: { value: '处理资料包《批量资料包》中的设定' },
    });
    fireEvent.click(screen.getByRole('button', { name: '发送设定灵感' }));
  ...
  test('提取失败显示未知状态，不调用同步或 create*', async () => {
    ...
    fetchMock.mockResolvedValueOnce(
      new Response(JSON.stringify({ error: '提取服务不可用' }), { status: 503 })
    );
    fireEvent.click(screen.getByRole('button', { name: '检查冲突并准备写入' }));
  ...
  test('最新请求失败后不复用旧回复准备同步', async () => {
    const fetchMock = fetch as unknown as ReturnType<typeof vi.fn>;
    fetchMock.mockResolvedValueOnce(streamResponse('第一条可同步回复')).mockResolvedValueOnce(
      new Response(JSON.stringify({ error: '模型暂不可用', code: 'service_unavailable' }), {
        status: 503,
      })
    );
    renderBatch();
    ...
    fireEvent.change(screen.getByRole('textbox', { name: '输入设定灵感' }), {
      target: { value: '生成第二条回复' },
    });
    fireEvent.click(screen.getByRole('button', { name: '发送设定灵感' }));
    await waitFor(() =>
      expect(screen.getByRole('alert', { name: '助手请求失败' }).textContent).toContain(
        '模型暂不可用'
      )
    );
  ...
  test('配置错误不重复消耗模型请求，并提供打开设置入口', async () => {
    ...
    await submitReply('待检查的批量回复');
    fetchMock.mockResolvedValueOnce(
      new Response(JSON.stringify({ error: '模型配置不可用，请检查设置', code: 'configuration' }), {
        status: 503,
      })
    );
    fireEvent.click(screen.getByRole('button', { name: '检查冲突并准备写入' }));
  ...
  test('SSE 配置错误保留错误码并提供设置入口', async () => {
    ...
    await submitReply('待检查的批量回复');
    fetchMock.mockResolvedValueOnce(
      errorStreamResponse('模型配置不可用，请检查设置', 'configuration')
    );
    fireEvent.click(screen.getByRole('button', { name: '检查冲突并准备写入' }));
    await waitFor(() => expect(screen.getByText(/模型配置不可用/)).toBeTruthy());
    expect(screen.getByRole('button', { name: '打开设置检查模型配置' })).toBeTruthy();
  ...
  test('提取成功后只有预览确认触发 syncPackToWorld，并携带版本上下文', async () => {
    ...
    expect(mocks.syncPackToWorld).toHaveBeenCalledWith(
  ...
  test('服务端同步失败时不回退到逐条 create* 写入', async () => {
    ...
    await submitReply('待检查的批量回复');
    fetchMock.mockResolvedValueOnce(
      streamResponse(JSON.stringify({ characters: [{ name: '批量人物' }] }))
    );
    fireEvent.click(screen.getByRole('button', { name: '检查冲突并准备写入' }));
    await waitFor(() =>
      expect(screen.getByRole('button', { name: '模拟确认同步预览' })).toBeTruthy()
  ...
  test('新回复提交会取消进行中的旧提取，旧结果不得显示为可同步预览', async () => {
    ...
    fetchMock
      ...
      .mockImplementationOnce(
        (_: string, options?: { signal?: AbortSignal }) =>
  ...
  test('切换资料包后清除旧预览，旧预览不能确认同步', async () => {
    ...
    view.rerender(
      <WorldBibleAssistant novel={novel} continuationPackId="pack-two" onClose={vi.fn()} />
    );
    await waitFor(() =>
      expect(screen.queryByRole('button', { name: '模拟确认同步预览' })).toBeNull()
    );
  ...
  test('资料包标题无唯一匹配时清除旧的自动推断，不显示检查入口', async () => {
    ...
    mocks.listContinuationPacks
      .mockResolvedValueOnce([
        { id: 'pack-inferred', title: '唯一资料包', novelId: novel.id, status: 'approved' },
      ])
      .mockResolvedValueOnce([]);
    fetchMock

...
const candidate: ArtifactCandidate<CharacterCore> = {
  id: 'candidate-1',
  novelId: 'n1',
  target: { kind: 'character', id: 'c1', version: 0 },
  operation: 'restructure',
  goal: '',
  baseFingerprint: 'fp',
  sourceCapabilityVersions: [{ capabilityId: 'bible-character-arc', version: '3' }],
  proposedCore: core,
  diff: { changed: true, fields: [{ path: 'desire', kind: 'added', after: '回家' }] },
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: RequestInfo | URL) => {
        ...
        if (url.includes('/artifacts?kind=character'))
          return Response.json({ cores: [], candidates: [candidate] });
        if (url.endsWith('/candidate-1/accept'))
          return Response.json({ core: { core, version: 1 } });
  ...
  test('recommends character restructuring without auto-running and persists dismissal', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: RequestInfo | URL) => {
        ...
        if (url.endsWith('/capability-recommendations/dismiss'))
    ...
    expect(fetch).toHaveBeenCalledWith(
      '/api/capability-recommendations/dismiss',
      expect.objectContaining({ method: 'POST' })
    );

    view.rerender(
      <WorldBibleView novel={{ ...novel, id: 'n2' }} onOpenCapabilityStore={openStore} />
    );
    expect(await screen.findByText(/角色结构缺少/)).toBeTruthy();

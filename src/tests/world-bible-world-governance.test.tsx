...
const worldCandidate: ArtifactCandidate<StructuredWorldCore> = {
  ...
  diff: {
    changed: true,
    fields: [{ path: 'hardRules[0]', kind: 'added', after: '夜间禁止使用火焰术' }],
  },
  impactReport: {
...
vi.mock('../lib/world-client', () => {
  ...
  return {
    listCharacters: vi.fn(async () => mocks.characters),
    listLocations: empty,
    listItems: empty,
    listTimelineEvents: empty,
    listFactions: empty,
    listPowerLevels: empty,
    listEntityRelationshipsClient: empty,
    createCharacter: mocks.createCharacter,
    updateCharacter: mocks.updateCharacter,
    deleteCharacter: mocks.deleteCharacter,
    createLocation: empty,
    ...
    updateFaction: empty,
    deleteFaction: empty,
    createPowerLevel: empty,
    updatePowerLevel: empty,
    deletePowerLevel: empty,
    createTimelineEvent: empty,
    updateTimelineEvent: empty,
    deleteTimelineEvent: empty,
...
vi.mock('../components/ContinuationOverviewPanel', () => ({
  ContinuationOverviewPanel: () => <div>概览</div>,
}));
vi.mock('../components/ContinuationPackView', () => ({
  ContinuationPackView: () => <div>资料包</div>,
}));
vi.mock('../components/WorldBibleOnboarding', () => ({
  WorldBibleOnboarding: () => <div>引导</div>,
}));
...
vi.mock('../components/world-bible/GlobalSetupTab', () => ({
  GlobalSetupTab: ({ onSave }: { onSave: (outline: string, rules: string) => Promise<void> }) => (
    <button type="button" onClick={() => void onSave('新大纲', '新规则')}>
      保存全局设定
    </button>
  ),
}));
...
function worldIntent(
  overrides: Partial<WorldCapabilityLaunchIntent> = {}
...
describe('WorldBibleView world capability governance', () => {
  ...
  beforeEach(() => {
    ...
    mocks.startWorldJob.mockResolvedValue({
      result: { candidate: worldCandidate },
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: RequestInfo | URL) => {
        ...
        if (url.includes('/artifacts?kind=character'))
          return Response.json({ cores: [], candidates: [] });
        if (url.includes('/artifacts?kind=world'))
          return Response.json({ cores: [], candidates: mocks.worldCandidates });
        if (url.endsWith('/world-candidate-1/accept'))
          return Response.json({ core: { core: worldCore, version: 1 } });
        if (url.endsWith('/world-candidate-1/reject'))
          return Response.json({ candidate: { ...worldCandidate, status: 'rejected' } });
  ...
  test('shows a pending world diff and accepts it without writing readable world fields', async () => {
    ...
    expect(
      screen.getByText(
        (_text, element) =>
          element?.tagName === 'LI' && element.textContent?.includes('hardRules[0]：added') === true
      )
    ).toBeTruthy();
  ...
  test('rejects a pending world candidate without touching readable world fields', async () => {
    ...
    await waitFor(() =>
      expect(screen.queryByRole('button', { name: '拒绝世界观候选' })).toBeNull()
    );
    expect(mocks.updateNovel).not.toHaveBeenCalled();
  });
  ...
  test('consumes a world launch once and generates a review candidate', async () => {
    ...
    const view = render(
      <WorldBibleView
        novel={novel}
        capabilityLaunchIntent={launch}
        onCapabilityLaunchConsumed={consume}
      />
    );

    await waitFor(() =>
      expect(mocks.startWorldJob).toHaveBeenCalledWith(
  ...
  test('requires an explicit character target when the protagonist is ambiguous', async () => {
    ...
    fireEvent.change(screen.getByRole('combobox', { name: '角色目标' }), {
      target: { value: 'c2' },
    });
  ...
  test('discards a mixed database-generation snapshot instead of rendering it', async () => {
    ...
    mocks.getDatabaseGenerationSnapshot
      .mockReset()
  ...
  test('keeps a local character edit visible when the frozen generation conflicts', async () => {
    ...
    mocks.updateCharacter.mockRejectedValue(
      Object.assign(new Error('generation conflict'), {
        status: 409,
    ...
    await waitFor(() =>
      expect(mocks.updateCharacter).toHaveBeenCalledWith('c1', { name: '保留的本地角色名' }, 7)
    );
  ...
  test('uses the accepted snapshot generation for character create and delete', async () => {
    ...
    await waitFor(() =>
      expect(mocks.createCharacter).toHaveBeenCalledWith(
        expect.objectContaining({ name: '新人物' }),
        7
      )
    );
  ...
  test('uses the accepted snapshot generation when saving global world fields', async () => {
    ...
    await waitFor(() =>
      expect(mocks.updateNovel).toHaveBeenCalledWith(
        'n1',
        {
          globalOutline: '新大纲',
          worldRules: '新规则',
  ...
  test('uses the accepted generation for the bio request and final character update', async () => {
    ...
    await waitFor(() =>
      expect(mocks.updateCharacter).toHaveBeenCalledWith('c-bio', { bio: '生成小传' }, 7)
    );
    const bioRequest = vi
      .mocked(fetch)
      .mock.calls.find(([input]) => String(input) === '/api/generate-bio');
  ...
  test('uses the accepted page generation when parsed import data reports a newer generation', async () => {
    ...
    vi.stubGlobal(
      ...
      class {
        ...
        readAsDataURL() {
          this.onload?.({
            target: { result: 'data:text/plain;base64,6K6+5a6a' },
    ...
    fireEvent.change(input, {
      target: { files: [new File(['设定'], '设定.txt', { type: 'text/plain' })] },
    });

    await waitFor(() =>
      expect(mocks.importWorldExtraction).toHaveBeenCalledWith(
        expect.objectContaining({
          databaseGeneration: 7,
          globalOutline: '导入大纲',
        })

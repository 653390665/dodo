...
describe('world job client', () => {
  ...
  it('polls the returned job instead of treating the start envelope as model output', async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        Response.json({ status: 'completed', progress: 100, result: { outline: '完整大纲' } })
      );
    vi.stubGlobal('fetch', fetchMock);

    await expect(
      startWorldJob<{ outline: string }>('/api/generate-outline', {}, { intervalMs: 0 })
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(3, '/api/world/jobs/job-1?databaseGeneration=7', {
      signal: undefined,
    });
  });
  ...
  it('calls the server cancellation endpoint when the caller aborts', async () => {
    ...
    const fetchMock = vi.fn().mockImplementation((url: string) => {
      if (url === '/api/db/generation')
        return Promise.resolve(Response.json({ databaseGeneration: 8 }));
      if (url === '/api/generate-outline')
        return Promise.resolve(Response.json({ jobId: 'job-2', databaseGeneration: 8 }));
      if (url === '/api/world/jobs/job-2/cancel?databaseGeneration=8')
    ...
    await expect(
      startWorldJob('/api/generate-outline', {}, {}, controller.signal)
    ).rejects.toThrow();
    await Promise.resolve();
    expect(fetchMock).toHaveBeenCalledWith('/api/world/jobs/job-2/cancel?databaseGeneration=8', {
      method: 'POST',
    });
  });
  ...
  it('cancels the provider job when polling fails before completion', async () => {
    ...
    await expect(startWorldJob('/api/generate-outline', {}, { intervalMs: 0 })).rejects.toThrow(
      'HTTP 502'
    );
    await Promise.resolve();
    expect(fetchMock).toHaveBeenCalledWith('/api/world/jobs/job-3/cancel?databaseGeneration=9', {
      method: 'POST',
    });
  });
});

...
describe('writing style client', () => {
  ...
  test('exposes resolution and candidates when generation requires confirmation', async () => {
    ...
    const candidates = [
      { mode: 'default', fingerprint: 'fp-1', summary: '系统默认笔调', sources: [] },
    ];
    vi.spyOn(globalThis, 'fetch').mockImplementation(
      async () =>
        new Response(
          JSON.stringify({ code: 'STYLE_CONFIRMATION_REQUIRED', resolution, candidates }),
          { status: 409 }
        )
    );

    const scopedRequest = { chapterId: 'chapter-1', databaseGeneration: 7 };
    await expect(resolveWritingStyle('novel-1', scopedRequest)).rejects.toMatchObject({

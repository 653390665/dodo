...
import { createTestWorkspace, type TestWorkspace } from './helpers/test-environment';

let testWorkspace: TestWorkspace;
let dbPath: string;

test('Book Factory uses the Skill Deck planner and does not rebuild legacy role slots', () => {
  const source = fs.readFileSync('src/components/book-factory/useBookFactory.ts', 'utf8');
  assert.match(source, /buildDeckMountPlan\(/);
  assert.match(source, /capabilityModelVersion:\s*3/);
  assert.doesNotMatch(source, /const currentIds = novel\.mountedSkillIds/);
});
...
test.describe('Book Deconstruction Flow (拆书转化工作流) & BookFactory Integration Tests', () => {
  ...
  test.describe('2. Database Persistence & Profile Integration Tests', () => {
    ...
    test.beforeEach(() => {
      try { closeDb(); } catch {}
      testWorkspace = createTestWorkspace('book-deconstruction');
      dbPath = testWorkspace.path('book-deconstruction.test.db');
      initDb(dbPath);
    });

...
import { drainWriteQueue, getDatabaseGeneration } from '../server/lib/db-instance.js';
...
describe('Commercial Boundary & Quota Control Tests', () => {
  ...
  test('Quota Guard logic correctly tracks, increments, and blocks free vs paid tiers', () => {
    ...
    try {
      ...
      createChapter({ id: 'novel-free-chapter', novelId: 'novel-free', title: 'Chapter', content: '', order: 1, wordCount: 0, createdAt: Date.now(), updatedAt: Date.now() });
      createChapter({ id: 'novel-premium-chapter', novelId: 'novel-premium', title: 'Chapter', content: '', order: 1, wordCount: 0, createdAt: Date.now(), updatedAt: Date.now() });
      ...
      assert.ok(proseBlockedCheck.error?.includes('高级生产工作流'));
  ...
  // ==========================================
  test('orchestrate basic BYOK is allowed while production enhanced quota returns 403', async () => {
    ...
    createChapter({ id: 'novel-exhausted-chapter', novelId: 'novel-exhausted', title: 'Chapter', content: '', order: 1, wordCount: 0, createdAt: Date.now(), updatedAt: Date.now() });
    const writingStyleFingerprint = confirmWritingStyleForTest('novel-exhausted');
    ...
    try {
      const resOrchBasic = await fetch(`${baseUrl}/api/orchestrate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          novelId: 'novel-exhausted',
          chapterId: 'novel-exhausted-chapter',
          databaseGeneration: getDatabaseGeneration(),
          sceneBeats: '主角林惊羽在剑冢中得到古剑',
          writingStyleFingerprint,
        }),
      });
      assert.equal(resOrchBasic.status, 200);
      const orchBody = await resOrchBasic.text();
      assert.match(orchBody, /"type":"done"/);
      assert.doesNotMatch(orchBody, /quotaExceeded/);
      assert.equal(getNovel('novel-exhausted')?.projectPreferenceProfile?.quotaLimits?.generateProseCount, 10);

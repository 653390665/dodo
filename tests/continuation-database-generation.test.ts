...
test('continuation imports reject async results from an obsolete database generation', async (t) => {
  ...
  const { advanceDatabaseGeneration, getDatabaseGeneration } = await import('../server/lib/db-instance');
  ...
  try {
    ...
    await t.test('resolves conflicts on approved packs atomically', async () => {
      resetActiveDatabase('novel-a');
      const now = Date.now();
      db.createNovel({
        id: 'novel-b', title: 'novel-b', authorId: 'local-user', summary: '', status: 'ongoing', createdAt: now, updatedAt: now,
      });
      db.createContinuationPack({
        id: 'approved-conflict-pack', novelId: 'novel-a', title: 'approved pack', status: 'approved',
        sourceDocuments: [], canonFacts: [{ id: 'fact-1', priority: 'hard', category: 'plot', text: '事实', evidence: '正文' }],
        ...
        contradictions: [{ id: 'conflict-1', severity: 'medium', summary: '冲突', conflictingEvidence: ['甲', '乙'], suggestedResolution: '以甲为准' }],
        continuationTask: '继续写', createdAt: now, updatedAt: now,
      });
      const generation = getDatabaseGeneration();
      const endpoint = `${baseUrl}/api/continuation-packs/resolve-conflicts`;
      const request = (body: unknown) => fetch(endpoint, {
        method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body),
      });

      const accepted = await request({
        packId: 'approved-conflict-pack', novelId: 'novel-a', databaseGeneration: generation,
        conflictResolutions: [{ contradictionId: 'conflict-1', resolution: '采用甲证据' }],
      });
      assert.equal(accepted.status, 200);
      const acceptedBody = await accepted.json() as { pack: { contradictions: Array<{ acceptedResolution?: string; resolvedAt?: number }> } };
      assert.equal(acceptedBody.pack.contradictions[0]?.acceptedResolution, '采用甲证据');
      assert.equal(typeof acceptedBody.pack.contradictions[0]?.resolvedAt, 'number');
      const updatedAt = db.getContinuationPack('approved-conflict-pack')?.updatedAt;
      ...
      assert.equal(db.getContinuationPack('approved-conflict-pack')?.updatedAt, updatedAt);

      const forged = await request({
        packId: 'approved-conflict-pack', novelId: 'novel-a', databaseGeneration: generation,
        conflictResolutions: [{ contradictionId: 'forged-id', resolution: '不应写入' }],
      });
      assert.equal(forged.status, 400);
      assert.equal(db.getContinuationPack('approved-conflict-pack')?.updatedAt, updatedAt);
      assert.equal(db.getContinuationPack('approved-conflict-pack')?.contradictions[0]?.acceptedResolution, '采用甲证据');

      const crossNovel = await request({
        packId: 'approved-conflict-pack', novelId: 'novel-b', databaseGeneration: generation,
        conflictResolutions: [{ contradictionId: 'conflict-1', resolution: '跨作品不应写入' }],
      });
      assert.equal(crossNovel.status, 409);
      assert.equal(db.getContinuationPack('approved-conflict-pack')?.contradictions[0]?.acceptedResolution, '采用甲证据');

      advanceDatabaseGeneration();
      const stale = await request({
        packId: 'approved-conflict-pack', novelId: 'novel-a', databaseGeneration: generation,
        conflictResolutions: [{ contradictionId: 'conflict-1', resolution: '过时代际不应写入' }],
      });

...
test('overview excludes resolved contradictions from pending risk counts', () => {
  const pack = buildPack({
    status: 'approved',
    contradictions: [
      { id: 'resolved', severity: 'high', summary: '已裁决', conflictingEvidence: [], suggestedResolution: '采用 A', acceptedResolution: '采用 A', resolvedAt: 3 },
      { id: 'pending', severity: 'medium', summary: '待确认', conflictingEvidence: [], suggestedResolution: '确认 B' },
    ],
  });
  const state = buildContinuationOverviewState([pack]);

  assert.equal(state.kind, 'ready');
  assert.equal(state.contradictionCount, 1);
  assert.deepEqual(state.highlightWarnings, []);
});

...
test('buildCopilotSuggestion returns one primary action and reason summary', () => {
  ...
  assert.equal(suggestion.reasons.missing.includes('本章分镜'), true);
});

test('buildCopilotSuggestion uses author-facing capability card wording', () => {
  const readyToDraft = buildCopilotSuggestion(baseInput());

  assert.equal(
    readyToDraft.secondaryActions.some((action) => action.label === '调整能力配置'),
    true,
  );
  assert.equal(readyToDraft.reasons.ready.includes('作品默认能力卡 2 张'), true);
  assert.equal(
    [...readyToDraft.reasons.ready, ...readyToDraft.reasons.risks, ...readyToDraft.secondaryActions.map((action) => action.label)]
      .some((text) => /mounted skills|no mounted skills|调整技能|scene beats|chapter draft|audit critique|low fit score|new entities not synced/.test(text)),
    false,
  );

  const withoutCards = buildCopilotSuggestion(baseInput({ mountedSkillCount: 0 }));
  assert.equal(withoutCards.reasons.risks.includes('尚未配置作品默认能力卡'), true);
  assert.equal(withoutCards.reasons.ready.includes('本章分镜'), true);
});

...
describe('deleteNovel cascades and integrity tests', () => {
  ...
  test('should delete novel and cascade delete all related child table entries', async () => {
    ...
    db.prepare(`INSERT INTO outline_artifacts (id, novel_id, level, scope, content, source, status, created_at, updated_at) VALUES (?, ?, 'master', '{}', 'outline', 'user', 'active', ?, ?)`).run('outline-1', novelId, now, now);
    db.prepare(`INSERT INTO canon_patches (id, novel_id, base_fingerprint, operations, status, created_at, updated_at) VALUES (?, ?, 'fp', '[]', 'pending', ?, ?)`).run('patch-1', novelId, now, now);
    ...
    assert.equal(getCount('SELECT COUNT(*) as count FROM outline_artifacts WHERE novel_id = ?', novelId), 1);
    assert.equal(getCount('SELECT COUNT(*) as count FROM canon_patches WHERE novel_id = ?', novelId), 1);
    ...
    assert.equal(getCount('SELECT COUNT(*) as count FROM outline_artifacts WHERE novel_id = ?', novelId), 0);
    assert.equal(getCount('SELECT COUNT(*) as count FROM canon_patches WHERE novel_id = ?', novelId), 0);

 ...
 * （keyless → 确定性保底事件流；意图声明 800 字使保底草稿通过流内质量门，
 * 否则 run 直接 failed）→ 断言 run 进入 review_required 且 plan165 严格门
 * 禁用「接受并写入」。无 stub。
 */
...
test('agent workspace：面板挂载并真实生产一章到 review_required', async ({ page }) => {
  ...
  // 声明 800 字目标：保底草稿可通过整章质量门（默认 4000 字目标下保底模板必被门禁拒绝）
  await reviewRegion.getByLabel('生产意图').fill('本章写800字：主角在废墟边缘完成第一次反杀。');

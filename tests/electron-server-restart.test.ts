...
test('packaged server can start on an explicit smoke-test port', () => {
  const source = require('node:fs').readFileSync('electron.cjs', 'utf8');
  assert.match(source, /function getInitialServerPort\(\)/);
  assert.match(source, /process\.env\.INKFLOW_PACKAGED_PORT/);
  assert.match(source, /Number\.isSafeInteger\(parsed\) && parsed > 0 && parsed <= 65535/);
  assert.match(source, /let serverPort = getInitialServerPort\(\)/);
});

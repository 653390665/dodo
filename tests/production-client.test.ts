...
function scopedPayload<T extends { novelId: string; userIntent: string }>(payload: T) {
  return { chapterId: `${payload.novelId}-chapter`, databaseGeneration: 7, ...payload };
}
...
test('applyChapterProductionRun returns chapter id from apply endpoint', async () => {
  ...
  try {
    const result = await applyChapterProductionRun('run-42', {
      novelId: 'novel-1', chapterId: 'chapter-9', databaseGeneration: 7,
    });
...
test('applyChapterProductionRun sends the selected version id and hash', async () => {
  const originalFetch = globalThis.fetch;
  globalThis.fetch = async (input, init) => {
    assert.equal(input, '/api/chapter-production-runs/run-version/apply');
    assert.deepEqual(JSON.parse(String(init?.body)), {
      novelId: 'novel-1', chapterId: 'chapter-version', databaseGeneration: 7,
      versionId: 'version-1', versionHash: 'hash-1',
    });
    return { ok: true, json: async () => ({ chapterId: 'chapter-version' }) } as Response;
  };

  try {
    await applyChapterProductionRun('run-version', {
      novelId: 'novel-1', chapterId: 'chapter-version', databaseGeneration: 7,
    }, { versionId: 'version-1', versionHash: 'hash-1' });
  } finally {
    globalThis.fetch = originalFetch;
...
test('startChapterProductionRunStream rejects missing chapter context before fetch', async () => {
  const originalFetch = globalThis.fetch;
  let called = false;
  ...
  try {
    await assert.rejects(
      () => startChapterProductionRunStream({ novelId: 'novel-1', userIntent: '继续写' } as never, () => undefined),
      /chapter.*generation|章节.*数据库版本/i,
    );
...
test('startChapterProductionRunStream rejects EOF without done', async () => {
  ...
  try {
    await assert.rejects(
      () => startChapterProductionRunStream(
        scopedPayload({ novelId: 'novel-5', userIntent: '继续写' }),
        () => undefined,
      ),
      /完成事件前中断/,
    );
...
test('startChapterProductionRunStream rejects raw DONE without a typed run', async () => {
  ...
  try {
    await assert.rejects(
      () => startChapterProductionRunStream(
        scopedPayload({ novelId: 'novel-7', userIntent: '继续写' }),
        (event) => events.push(event),
      ),
      /完成事件前中断/,
    );
...
for (const [label, invalidRun] of [
  ...
  ['a review version with a missing hash', { ...makeProductionRun('run-invalid-version', 'novel-invalid', 'running'), reviewVersionId: 'version-1', reviewVersionSource: 'fallback' }],
  ['a review version with an invalid source', { ...makeProductionRun('run-invalid-source', 'novel-invalid', 'running'), reviewVersionId: 'version-1', reviewVersionHash: 'hash-1', reviewVersionSource: 'remote' }],
] as const) {
  test(`startChapterProductionRunStream rejects ${label}`, async () => {
    ...
    try {
      await assert.rejects(
        () => startChapterProductionRunStream(
          scopedPayload({ novelId: 'novel-invalid-run', userIntent: '继续写' }),
          () => undefined,
        ),
        /invalid run/i,
      );
...
test('startChapterProductionRunStream rejects a business error event', async () => {
  ...
  try {
    await assert.rejects(
      () => startChapterProductionRunStream(
        scopedPayload({ novelId: 'novel-6', userIntent: '继续写' }),
        () => undefined,
      ),
      /writer failed/,
    );

...
test('chapter production enforces novel ownership and preserves concurrent preferences', async (t) => {
  ...
  const { getDb, getDatabaseGeneration, runInSerializedWrite, drainWriteQueue } =
  ...
  const { __productionTestHooks, registerProductionRoutes } =
  ...
  const reflexionArgs = (novelId: string, chapterId: string) => {
    ...
    return [
      novelId,
      databaseGeneration,
      resolveWritingStyleRequest(novelId, { chapterId, databaseGeneration }).executionSnapshot,
    ] as const;
  };
  const emptyProfile = {
    tags: [],
  ...
  try {
    await t.test(
      'rejects a foreign target chapter before creating start and stream runs',
      async () => {
    ...
    await t.test('preserves continuation-pack provenance when the fallback persists', async () => {
      __rateLimitTestHooks.reset();
      const response = await fetch(`${baseUrl}/api/chapter-production-runs/start`, {
        ...
        body: JSON.stringify({
          ...
          userIntent: 'continue',
          styleConfirmationFingerprint: packAFingerprint,
        }),
      });
      const run = db
        .listChapterProductionRuns('novel-a')
        .find((candidate) => candidate.userIntent === 'continue');
      // plan 198：保底过门 → start 返回 200，run 进入评审态，溯源信息原样保留
      assert.equal(response.status, 200);
      assert.ok(run, 'fallback should preserve a diagnostic run');
    ...
    await t.test('rejects a forged cross-novel target at apply time with zero writes', async () => {
      ...
      const response = await fetch(
        `${baseUrl}/api/chapter-production-runs/foreign-target-run/apply`,
        {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({
    ...
    await t.test(
      'atomically rejects a production run whose source pack belongs to another novel',
      async () => {
        createReviewRun(
          'foreign-pack-run',
        ...
        const response = await fetch(
          `${baseUrl}/api/chapter-production-runs/foreign-pack-run/apply`,
          {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({
    ...
    await t.test(
      'generic DB proxy cannot mutate production run ownership or generated content',
      async () => {
    ...
    const foreignPatchCases: Array<{ name: string; patch: ProposedLedgerPatch }> = [
      {
        ...
        patch: {
          ...emptyContinuityReport().proposedPatch,
          characterUpdates: [{ characterId: 'character-b', summaryAppend: 'polluted' }],
        },
      },
      {
        name: 'item update',
        patch: {
          ...emptyContinuityReport().proposedPatch,
          itemUpdates: [{ itemId: 'item-b', descriptionAppend: 'polluted' }],
        },
      },
      {
        ...
        patch: {
          ...
          foreshadowingUpdates: [
            {
              foreshadowingId: 'foreshadowing-b',
              status: 'payoff' as const,
              notesAppend: 'polluted',
            },
          ],
        },
      },
    ...
    await t.test(
      'Reflexion ignores an applied run whose chapter belongs to another novel',
      async () => {
        db.createChapterProductionRun({
        ...
        try {
          ...
          await __productionTestHooks.runEvolutionReflexion(
            ...reflexionArgs('novel-a', 'chapter-a')
          );
          assert.equal(llmCalls, 0);
        } finally {
          globalThis.fetch = originalFetch;
          getDb()
            .prepare('DELETE FROM chapter_production_runs WHERE id = ?')
            .run('foreign-reflexion-run');
        }
      }
    );

    await t.test(
      'Reflexion merges only notes into the latest profile and is idempotent by content',
      async () => {
        ...
        globalThis.fetch = async () => {
          ...
          return new Response(
            JSON.stringify({
              choices: [
                { message: { content: JSON.stringify({ rules: ['new reflexion rule'] }) } },
              ],
            }),
        ...
        try {
          __productionTestHooks.resetReflexionKeys();
          const reflexion = __productionTestHooks.runEvolutionReflexion(
            ...reflexionArgs('novel-a', 'chapter-a')
          );
          await started;
          const latestProfile = {
          ...
          assert.equal(
            llmCalls,
            1,
            'the same chapter content must not be analyzed twice in one process'
          );
        } finally {
          globalThis.fetch = originalFetch;
    ...
    await t.test(
      'serializes post-reservation validation and run creation against database import',
      async () => {
        ...
        assert.equal(
          db.listChapterProductionRuns('novel-a').some((run) => run.userIntent === 'continue'),
          false
        );

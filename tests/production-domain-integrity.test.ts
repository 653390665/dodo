...
test('chapter production enforces novel ownership and preserves concurrent preferences', async (t) => {
  ...
  try {
    await t.test('rejects a foreign target chapter before creating start and stream runs', async () => {
      ...
      const requestBody = { novelId: 'novel-a', targetChapterId: 'chapter-b', userIntent: 'continue' };
    ...
    await t.test('rejects a continuation pack owned by another novel', async () => {
      __rateLimitTestHooks.reset();
      const requestBody = { novelId: 'novel-a', continuationPackId: 'pack-b', userIntent: 'continue' };
    ...
    await t.test('preserves continuation-pack provenance through fallback persistence', async () => {
      __rateLimitTestHooks.reset();
      const response = await fetch(`${baseUrl}/api/chapter-production-runs/start`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ novelId: 'novel-a', continuationPackId: 'pack-a', userIntent: 'continue' }),
      });
      const payload = await response.json() as {
        run: { id: string; continuityReport: { continuationPackId?: string } };
      };
      if (payload.run?.id) {
        getDb().prepare('DELETE FROM chapter_production_runs WHERE id = ?').run(payload.run.id);
      } else {
        getDb().prepare('DELETE FROM chapter_production_runs WHERE user_intent = ?').run('continue');
      }
      assert.equal(response.status, 200);
      assert.equal(payload.run.continuityReport.continuationPackId, 'pack-a');
    });
    ...
    await t.test('links a newly created chapter back to the applied production run', async () => {
      ...
      const payload = await response.json() as { chapterId: string };
      const persistedTargetChapterId = db.getChapterProductionRun('new-chapter-run')?.targetChapterId;
      const versionCount = db.listChapterVersions(payload.chapterId).length;
      getDb().prepare('DELETE FROM chapter_versions WHERE chapter_id = ?').run(payload.chapterId);
      getDb().prepare('DELETE FROM vector_chunks WHERE chapter_id = ?').run(payload.chapterId);
      getDb().prepare('DELETE FROM chapters WHERE id = ?').run(payload.chapterId);
      getDb().prepare('DELETE FROM chapter_production_runs WHERE id = ?').run('new-chapter-run');

      assert.equal(response.status, 200);
      assert.equal(persistedTargetChapterId, payload.chapterId);
    ...
    await t.test('Reflexion ignores an applied run whose chapter belongs to another novel', async () => {
      ...
      try {
        ...
        await __productionTestHooks.runEvolutionReflexion('novel-a');
    ...
    await t.test('Reflexion merges only notes into the latest profile and is idempotent by content', async () => {
      ...
      try {
        __productionTestHooks.resetReflexionKeys();
        const reflexion = __productionTestHooks.runEvolutionReflexion('novel-a');
        await started;
        const latestProfile = {
          ...emptyProfile,

...
test('public catalog normalizes one-click asset copy into confirmable tool names', () => {
  ...
  for (const forbidden of [
    ...
    assert.equal(serializedPublicCatalog.includes(forbidden), false, `public catalog should not expose ${forbidden}`);
  }
  ...
  for (const expected of [
    ...
    assert.equal(serializedPublicCatalog.includes(expected), true, `public catalog should expose ${expected}`);
  }

  assert.equal(
    SOURCE_PROMPT_GOVERNANCE_CATALOG.some((asset) => asset.title.includes('一键')),
    true,
    'source governance catalog keeps original names for traceability',
  );
});

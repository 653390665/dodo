...
const waitForJob = async (baseUrl: string, jobId: string, generation: number) => {
  for (let i = 0; i < 100; i += 1) {
    const response = await fetch(
      `${baseUrl}/api/world/jobs/${jobId}?databaseGeneration=${generation}`
    );
    const job = (await response.json()) as {
      status: string;
      result?: { kind?: string; candidate?: { target?: { kind?: string }; proposedCore?: { immutableFacts?: string[] } } };
      error?: string;
    };
...
test('refine-character-rebuild returns a governed candidate and never touches the original character', async () => {
  closeDb();
  initDb(':memory:');
  ...
  config.baseUrl = 'https://candidate.test/v1';
          ...
          choices: [
            {
              message: {
                content: JSON.stringify({
                  core: {
                    desire: '守住北门',
                    immutableFacts: ['守门人', '原始小传'],
                  },
                  proposedContent: '重构后的人设候选',
                }),
              },
            },
          ],
  ...
  try {
    ...
    const response = await fetch(`${baseUrl}/api/generate-outline`, {
      ...
      body: JSON.stringify({
        ...
        techniqueId: 'refine-character-rebuild',
      }),
    });
    ...
    assert.deepEqual(job.result?.candidate?.proposedCore?.immutableFacts, ['守门人', '原始小传']);
    // 原资料零改动：候选未接受前，角色本体与 artifact 版本都保持原样。
    assert.equal(getArtifactCore('n1', 'character', 'c1'), undefined);
    assert.equal(getCharacter('c1')?.bio, '原始小传');
  } finally {

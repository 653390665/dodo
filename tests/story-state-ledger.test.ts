...
test('projects the current hint action and future payoff range from a narrative core', () => {
  const narrativeCore: NarrativePromiseCore = {
    schemaVersion: 1,
    plan: {
      intent: '逐步揭开玄铁令来历',
      revealConstraint: '本章不得揭示幕后身份',
      plannedHintRanges: [{ from: 3, to: 4 }],
      plannedPayoffRange: { from: 6, to: 7 },
      sourceOutlineNodeIds: ['outline-node-1'],
    },
    evidence: [],
  };
  const ledger = buildStoryStateLedger({
    ...
    foreshadowings: [{
      id: 'promise-1', novelId: novel.id, title: '玄铁令来历', description: '来历未明', status: 'planted',
      relatedCharacterIds: [], narrativeCore, createdAt: now, updatedAt: now,
    }],
  });

  assert.equal(ledger.openForeshadowings[0]?.plannedAction, 'hint');
  assert.deepEqual(ledger.openForeshadowings[0]?.plannedPayoffRange, { from: 6, to: 7 });
});

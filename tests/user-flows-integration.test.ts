...
test.describe('InkFlow End-to-End User Flows Integration Tests', () => {
  test.beforeEach(() => {
    ...
    testWorkspace = createTestWorkspace('user-flows');
    dbPath = testWorkspace.path('user-flows.test.db');
    // 1. Database setup
    try { closeDb(); } catch {}
    initDb(dbPath);
  ...
  test('Flow 6: Prompt Governance (项目画像识别 -> 根据商业模式过滤推荐受限包 -> free/paid 服务端作品模式边界)', () => {
